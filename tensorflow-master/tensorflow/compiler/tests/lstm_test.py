...
class LSTMTest(test.TestCase):
    ...
    def _RunLSTMCell(
        self, basename, init_weights, m_prev_scalar, c_prev_scalar, pad_scalar
    ):
    ...
    def testLSTMCell(self):
        ...
        for m_prev in [0.0, 1.0]:
            for c_prev in [0.0, 1.0]:
                m, c = self._RunLSTMCell(
                    "ones", init_ops.ones_initializer(), m_prev, c_prev, 0.0
                )
        ...
        for weight in np.random.rand(3):
            ...
            for m_prev in [0.0, 1.0]:
                for c_prev in [0.0, 1.0]:
                    m, c = self._RunLSTMCell(
                        "random", random_weight, m_prev, c_prev, 0.0
                    )
                    self.assertAllClose(
                        m, self._NextM(self._inputs, weight, m_prev, c_prev)
                    )
                    self.assertAllClose(
                        c, self._NextC(self._inputs, weight, m_prev, c_prev)
                    )
            ...
            for m_prev in [0.0, 1.0]:
                for c_prev in [0.0, 1.0]:
                    m, c = self._RunLSTMCell(
                        "random", random_weight, m_prev, c_prev, 1.0
                    )
    ...
    def _RunLSTMLayer(
        self, basename, init_weights, m_init_scalar, c_init_scalar, pad_scalar
    ):
        with self.session() as sess:
            num_inputs = 1
            num_nodes = 1
    ...
    def testLSTMLayer(self):
        ...
        self.assertAllClose(
            o,
            [
                [[0.25]] * self._batch_size,
                [[0.125]] * self._batch_size,
                [[0.0625]] * self._batch_size,
            ],
        )
        ...
        for m_init in [0.0, 1.0]:
            for c_init in [0.0, 1.0]:
                o = self._RunLSTMLayer(
                    "ones", init_ops.ones_initializer(), m_init, c_init, 0.0
                )
...
class LSTMBenchmark(test.Benchmark):
    ...
    def _LayerBuilder(self, do_training):
        out_seq, weights = lstm.BuildLSTMLayer(
            FLAGS.batch_size, FLAGS.seq_length, FLAGS.num_inputs, FLAGS.num_nodes
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_nodes", type=int, default=1024, help="Number of nodes in each LSTM cell."
    )
    parser.add_argument(
        "--device",
        type=str,
