...
class XlaSortOpTest(xla_test.XLATestCase, parameterized.TestCase):
        ...
        for key_type in self._supported_key_types():
            ...
            def wrap_sort(x):
                return xla.variadic_sort(
                    [x], dimension=dimension, is_stable=False, comparator=compare_lt
                )
    ...
    def testVariadicSortReverse(self):
        ...
        for key_type in self._supported_key_types():
            ...
            def wrap_sort(x):
                return xla.variadic_sort(
                    [x], dimension=0, is_stable=False, comparator=compare_gt
                )
        ...
        for key_type_1 in set([np.int16, np.uint16, np.int32, np.uint32]):
            for key_type_2 in self._supported_key_types():
                for value_type in self._supported_key_types():
                    ...
                    expected = [
                        np.take_along_axis(inp, sorted_indices, axis=0)
                        for inp in inputs
                    ]
                    ...
                    def compare_lexicographic(x1, x2, y1, y2, z1, z2):
                        ...
                        return math_ops.logical_or(
                            x1 < x2,
                            math_ops.logical_and(math_ops.equal(x1, x2), y1 < y2),
                        )
        ...
        for dtype in supported_types.intersection(self.numeric_types):
            with self.session() as sess:
                ...
                results = sess.run(
                    ...
                    {
                        p: np.array(
                            [0.0, -0.0, 0.0, 3.0, -0.0, -4.0, 0.0, -0.0], dtype=dtype
                        )
                    },
                )
                self.assertAllEqual(
                    np.array([3.0, 0.0, 0.0, 0.0], dtype=dtype), results[0]
                )
        ...
        for dtype in supported_types.intersection(self.numeric_types):
            with self.session() as sess:
                ...
                results = sess.run(
                    ...
                    {
                        p: np.array(
                            [1, 2, float("inf"), -float("inf"), -1, -2], dtype=dtype
                        )
                    },
                )
                self.assertAllEqual(
                    np.array(
                        [float("inf"), 2.0, 1.0, -1.0, -2.0, -float("inf")], dtype=dtype
                    ),
                    results[0],
                )
