...
class ResourceApplyFtrlTest(xla_test.XLATestCase):
    ...
    def setUp(self):
        super().setUp()
        self.rewrite_ops_for_tpu = (
            "TPU" in self.device and test_util.is_mlir_bridge_enabled()
        )
    ...
    def _eval(
        ...
        with self.session() as session:
            with self.test_scope():
                ...
                if use_v2:
                    session.run(
                        training_ops.resource_apply_ftrl_v2(
                            v_var.handle,
                            v_accum.handle,
                            v_linear.handle,
                            grad,
                    ...
                    session.run(
                        training_ops.resource_apply_ftrl(
                            v_var.handle,
                            v_accum.handle,
                            v_linear.handle,
                            grad,
                ...
                return (
                    v_var.read_value().eval().reshape(var.shape),
