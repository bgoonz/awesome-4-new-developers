...
class Log1pTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def _test_range(self, low, high, dtype, rtol, atol, is_negative=False):
        ...
        x = np.exp(np.random.uniform(low=low, high=high, size=[NUM_SAMPLES])).astype(
            dtype
        )
        if is_negative:
            x = -x
    ...
    @parameterized.parameters((np.float32, 1e-7, 0.0), (np.float64, 1e-15, 0.0))
    ...
    @parameterized.parameters((np.float32, 2e-7, 0.0), (np.float64, 1e-15, 0.0))
    ...
    @parameterized.parameters((np.float32, 4e-7, 0.0), (np.float64, 3e-14, 0.0))
    ...
    @parameterized.parameters((np.float32, 2e-7, 0.0), (np.float64, 2e-15, 0.0))
...
class ZetaTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def testMediumValues(self, dtype, rtol, atol):
        ...
        q = np.random.uniform(low=1.0, high=1e1, size=[NUM_SAMPLES]).astype(dtype)
    ...
    def testLargeValues(self, dtype, rtol, atol):
        x = np.random.uniform(low=100.0, high=int(1e3), size=[NUM_SAMPLES]).astype(
            dtype
        )
        q = np.random.uniform(low=1.0, high=int(1e1), size=[NUM_SAMPLES]).astype(dtype)
...
class PolygammaTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def testMediumLargeN(self, dtype, rtol, atol):
        ...
        x = np.random.uniform(low=1.0, high=1e1, size=[NUM_SAMPLES]).astype(dtype)
