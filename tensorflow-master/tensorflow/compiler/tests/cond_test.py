...
class CondTest(xla_test.XLATestCase):
        ...
        with self.session(), self.test_scope():
            ...
            def f():
                ...
                output = control_flow_ops.cond(
                    ...
                    lambda: ta.write(0, 5.0),
                    lambda: ta.write(0, 10.0),
                )
    ...
    def testCondConstPropagation(self):
        with self.session() as sess, self.test_scope():
            ...
            self.assertAllEqual(
                1.0, sess.run(output, feed_dict={x: [0.0, 1.0, 2.0], p: 1})
            )
    ...
    def testCondConstPropagation_errorMsg(self):
        ...
        with self.session() as sess, self.test_scope():
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "must be a compile-time constant"
            ):
        ...
        with self.session(), self.test_scope():
            ...
            def f():
                ...
                output = control_flow_ops.switch_case(
                    ...
                    {
                        0: lambda: ta.write(0, 5.0),
                        1: lambda: ta.write(0, 10.0),
                        2: lambda: ta.write(0, 15.0),
                    },
                )
    ...
    def testSwitchCaseConstPropagation(self):
        ...
        with self.session() as sess, self.test_scope():
            ...
            output = control_flow_ops.switch_case(
                constant_op.constant(2), {0: branch0, 1: branch1, 2: branch2}
            )

            self.assertAllEqual(
                7.0, sess.run(output, feed_dict={x: [0.0, 1.0, 7.0], p: 2})
            )
