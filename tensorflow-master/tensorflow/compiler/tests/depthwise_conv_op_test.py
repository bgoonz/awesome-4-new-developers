...
def ReferenceDepthwiseConv2D(
    input_tensor, filter_tensor, strides, padding, data_format=None
):
...
def ConfigsWithDilationsToTest():
  ...
  """
    ...
    for i, f, o, s, d, p in zip(
        input_sizes, filter_sizes, out_sizes, strides, dilations, paddings
    ):
        yield i, f, o, s, d, p
...
class DepthwiseConv2DTest(xla_test.XLATestCase):
    ...
    """
        ...
        x1 = np.array(
            [f * 1.0 for f in range(1, total_size_1 + 1)], dtype=data_type
        ).reshape(tensor_in_sizes)
        x2 = np.array(
            [f * 1.0 for f in range(1, total_size_2 + 1)], dtype=data_type
    ...
    def testDepthwiseConv2D(self):
        for index, (input_size, filter_size, _, stride, padding) in enumerate(
            ConfigsToTest()
        ):
            print(
                "Testing DepthwiseConv2D,",
                index,
                "th config:",
                input_size,
                "*",
    ...
    """
        ...
        # numbers from 1.
        x1 = np.array(
            [f * 1.0 for f in range(1, total_size_1 + 1)], dtype=np.float32
        ).reshape(tensor_in_sizes)
        x2 = np.array(
            [f * 1.0 for f in range(1, total_size_2 + 1)], dtype=np.float32
        ).reshape(filter_in_sizes)
        with self.session() as sess:
    ...
    def testDilationDepthwiseConv2DWith(self):
        for index, (input_size, filter_size, _, stride, dilation, padding) in enumerate(
            ConfigsWithDilationsToTest()
        ):
            print(
                "Testing DilationDepthwiseConv2D,",
                index,
                "th config:",
                input_size,
                "*",
            ...
            for data_type in self.float_types:
                ...
                if data_type == np.float32:
                    self._VerifyValuesWithDilation(
                        input_size, filter_size, stride, dilation, padding, data_type
                    )
    ...
    def testDilationDepthwiseConv2DWithFormat(self):
        for index, (input_size, filter_size, _, stride, dilation, padding) in enumerate(
            ...
            print(
                "Testing DilationDepthwiseConv2DFormat,",
                index,
                "th config:",
                input_size,
                "*",
    ...
    def testDepthwiseConv2DInputGradCompare(self):
        for index, (input_size, filter_size, output_size, stride, padding) in enumerate(
            ConfigsToTest()
        ):
            print(
                "Testing DepthwiseConv2DInputGradCompare,",
                index,
                "th config:",
                input_size,
                "*",
    ...
    def testDepthwiseConv2DFilterGradCompare(self):
        for index, (input_size, filter_size, output_size, stride, padding) in enumerate(
            ...
            print(
                "Testing DepthwiseConv2DFilterGradCompare,",
                index,
                "th config:",
                input_size,
                "*",
    ...
    def testDepthwiseConv2DFilterGradFormatNCHWCompare(self):
        for index, (input_size, filter_size, output_size, stride, padding) in enumerate(
            ...
            print(
                "Testing DepthwiseConv2DFilterGradFormatNCHWCompare,",
                index,
                "th config:",
                input_size,
                "*",
    ...
    def _CompareBackpropInputWithDilation(
        self, input_sizes, filter_sizes, output_sizes, stride, dilation, padding
    ):
        x1 = np.random.rand(*filter_sizes).astype(np.float32)
        x2 = np.random.rand(*output_sizes).astype(np.float32)
            ...
            with self.session():
                ...
                if use_xla:
                    ...
                    input_sizes_transform = [
                        input_sizes[0] * dilation * dilation,
                        input_sizes[1] // dilation,
                        input_sizes[2] // dilation,
                        input_sizes[3],
                    ]
    ...
    def testDilationDepthwiseConv2DInputGradWithCompare(self):
        for (
            index,
            (input_size, filter_size, output_size, stride, dilation, padding),
        ) in enumerate(ConfigsWithDilationsToTest()):
            print(
                "Testing DilationDepthwiseConv2DInputGradWithDilationCompare,",
                index,
                "th config:",
                input_size,
                "*",
            ...
            if stride == 1:
                self._CompareBackpropInputWithDilation(
                    input_size, filter_size, output_size, stride, dilation, padding
                )

    def _CompareBackpropFilterWithDilation(
        self,
        input_sizes,
        filter_sizes,
        output_sizes,
    ...
    def testDilationDepthwiseConv2DFilterGradCompare(self):
        for (
            ...
            print(
                "Testing DilationDepthwiseConv2DFilterGradCompare,",
                index,
                "th config:",
                input_size,
                "*",
            ...
            if stride == 1:
                # TODO(wangtao): implement CPU grad computation with stride > 1.
                self._CompareBackpropFilterWithDilation(
                    input_size, filter_size, output_size, stride, dilation, padding
                )
