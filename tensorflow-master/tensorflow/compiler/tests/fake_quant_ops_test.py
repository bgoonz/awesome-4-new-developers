...
class FakeQuantWithMinMaxArgsGradientTest(xla_test.XLATestCase):
    ...
    def _TestOp(
        ...
        with self.session() as session:
            ...
            backprops = session.run(
                outputs, {gradient_placeholder: gradients, input_placeholder: inputs}
            )
            self.assertAllCloseAccordingToType(
                backprops, expected_backprops, rtol=1e-3, atol=1e-5, bfloat16_rtol=0.03
            )
