...
class DynamicStitchTest(xla_test.XLATestCase):
    def _AssertDynamicStitchResultIs(self, indices, data, expected):
        with self.session() as session:
            ...
            with self.test_scope():
                output = data_flow_ops.dynamic_stitch(
                    index_placeholders, data_placeholders
                )
    ...
    def testSimpleEmpty(self):
        ...
        self._AssertDynamicStitchResultIs(
            [idx1, idx2], [val1, val2], expected=np.array([[], [], [], []], np.int32)
        )
    ...
    def testEmptyIndex(self):
        ...
        self._AssertDynamicStitchResultIs(
            [idx1, idx2],
            [val1, val2],
            expected=np.ndarray(shape=(0, 9), dtype=np.int32),
        )
