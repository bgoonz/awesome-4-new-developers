...
class BinaryOpsTest(xla_test.XLATestCase):
    ...
    def testFloatOps(self):
        for dtype in self.float_types:
            ...
            self._testBinary(
                ...
                np.array([-0.6, -0.4, -0.2, 0, 0.2, 0.4], dtype=dtype),
            ...
            self._testBinary(
                ...
                np.array([1, 2, 3, 4, 5, 6], dtype=dtype),
                np.array([-0.6, -0.4, -0.2, 0.2, 0.4, 0.6], dtype=dtype),
                expected=np.array(
                    [
                        1.158099340847,
                        2.7161986816948,
            ...
            self._testBinary(
                ...
                np.array([[-np.inf, 0.0]], dtype=dtype),
            ...
            if dtype != dtypes.bfloat16.as_numpy_dtype:
                ...
                self._testBinary(
                    ...
                    np.array([[0, 5, 6, 7, 8, float("NaN")]], dtype=dtype),
                    expected=np.array(
                        [[0, 6.437752, 5.375278, 3.89182, 2.079442, 0]], dtype=dtype
                    ),
                    rtol=1e-4,
                    atol=1e-6,
                )
                ...
                self._testBinary(
                    ...
                    expected=np.array(
                        [[0, 7.167038, 5.837730, 4.158883, 2.197225, 0]], dtype=dtype
                    ),
                    rtol=1e-4,
                    atol=1e-6,
                )
        ...
        for dtype in self.numeric_types:
            ...
            if dtype not in self.int_types:
                self._testBinary(
                    ...
                    expected=np.array(
                        [3.0269620882574744, 3.3149631512242195], dtype=dtype
                    ),
                )
        ...
        for dtype in self.numeric_types:
            ...
            if dtype not in self.int_types:
                self._testBinary(
                    ...
                    np.array([1.1137667913355869, 1.7186636469261405], dtype=dtype),
                    expected=np.array(
                        [2.130853387051026, 2.743501927643327], dtype=dtype
                    ),
                    rtol=1e-14,
                )
    ...
    def testComplexOps(self):
        for dtype in self.complex_types:
            ...
            self._testBinary(
                lambda x, y: math_ops.approximate_equal(x, y, tolerance=0.0001),
                np.array(
                    [[[[-1 + 2j, 2.00009999 - 3j], [2 - 3j, 3 + 4.01j]]]], dtype=dtype
                ),
                np.array(
        ...
        for dtype in self.float_types | {np.float64}:
            ...
            if dtype != np.float16 and hasattr(np, "matmul"):
                ...
                if (
                    ...
                    and dtype == dtypes.bfloat16.as_numpy_dtype
                ):
                    return
    ...
    def testBatchMatMulBroadcast(self):
        ...
        self._testBinary(
            ...
            expected=np.array(
                [[[140, 280, 420, 560], [146, 292, 438, 584]]], dtype=np.float32
            ),
        )
        # [1, 2, 3] @ [3, 4] -> [1, 2, 4]
        self._testBinary(
        ...
        self._testBinary(
            math_ops.matmul,
            np.full([4, 5, 1, 2, 3], 2.0, dtype=np.float32),
            np.full([1, 1, 3, 5], 3.0, dtype=np.float32),
            expected=np.full([4, 5, 1, 2, 5], 18.0, dtype=np.float32),
        )
    ...
    def testSymmetricMirrorPadGrad(self):
        ...
        for dtype in self.numeric_types:
            self._testBinary(
                ...
                np.array([[1, 1], [0, 0], [0, 0], [2, 2]], dtype=np.int32),
    ...
    def testReflectMirrorPadGrad(self):
        ...
        for dtype in self.numeric_types:
            self._testBinary(
                ...
                np.array([[0, 0], [1, 1], [2, 2], [0, 0]], dtype=np.int32),
                expected=np.broadcast_to(
                    np.reshape(np.array([16, 18, 8], dtype=dtype), (3, 1)), (1, 2, 3, 1)
                ),
            )
    ...
    def testBroadcastTo(self):
        for dtype in self.all_types:
            ...
            self._testBinary(
                array_ops.broadcast_to, x, np.array([2, 3], dtype=np.int32), expected=x
            )
            self._testBinary(
