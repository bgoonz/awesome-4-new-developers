...
class DequantizedOpsTest(xla_test.XLATestCase):
    def pack_uint8_r2_to_uint32(self, test_input):
        ...
        padding_input = array_ops.pad(
            ...
            constant_op.constant([[0, 0], [0, num_output_columns * 4 - num_columns]]),
        )
    ...
    def testDequantizeQuint8(self):
        ...
        with self.session() as session:
            with ops.device("CPU"):
                ...
                quantized_input = array_ops.quantize(
                    transposed_input, 0.0, 255.0, dtypes.quint8
                )
                packed_input = self.pack_uint8_r2_to_uint32(quantized_input.output)
            with self.test_scope():
                transposed_quantized_output = xla.dequantize(
                    packed_input, 0.0, 255.0, "MIN_COMBINED", True
                )
                quantized_output = array_ops.slice(
                    transposed_quantized_output, [0, 0], [num_rows, num_columns]
                )
