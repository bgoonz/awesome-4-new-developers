...
class XlaDataFormatDimMapTest(xla_test.XLATestCase):
            ...
            with self.session() as session:
                with self.test_scope():
                    placeholder = array_ops.placeholder(
                        dtypes.as_dtype(x.dtype), x.shape
                    )
                    param = {placeholder: x}
    ...
    def test(self):
        ...
        self._test([1, -3, -2], "NHWC", "NCHW", [2, 2, 3])
        self._test(
            [-4, -3, -2, -1, 0, 1, 2, 3], "NHWC", "HWNC", [2, 0, 1, 3, 2, 0, 1, 3]
        )
        self._test(
            [-4, -3, -2, -1, 0, 1, 2, 3], "NHWC", "WHCN", [3, 1, 0, 2, 3, 1, 0, 2]
        )
        self._test(
            [-4, -3, -2, -1, 0, 1, 2, 3], "qwer", "rewq", [3, 2, 1, 0, 3, 2, 1, 0]
        )
...
class XlaPermuteOpTest(xla_test.XLATestCase):
    ...
    def testNHWCToNCHW2D(self):
        for dtype in {np.int32, np.int64}:
            ...
            self._runPermuteAndCompare(
                x, "NHWC", "NCHW", [[7, 4], [5, 1], [9, 3], [4, 5]]
            )
