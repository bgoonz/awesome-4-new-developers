...
class TernaryOpsTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def testBetaincSanity(self):
        ...
        for dtype in self.numeric_types & {np.float32, np.float64}:
            ...
            self._testTernary(math_ops.betainc, a, b, x, expected, rtol=5e-6, atol=6e-6)

    @parameterized.parameters(
        {"sigma": 1e15, "rtol": 1e-6, "atol": 1e-4},
        {"sigma": 30, "rtol": 1e-6, "atol": 2e-3},
        {"sigma": 1e-8, "rtol": 5e-4, "atol": 3e-4},
        {"sigma": 1e-16, "rtol": 1e-6, "atol": 2e-4},
    )
