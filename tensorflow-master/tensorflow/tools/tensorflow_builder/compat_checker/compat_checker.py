            ...
            for config in all_configs:
                ...
                if section == "Dependency":
                    ...
                    if section == "Required":
                        ...
                        msg = "[Error] Section name `%s` is not accepted." % str(
                            section
                        )
                    ...
                    add_to[config] = self._Reqs(
                        req_list, config=config, section=section
                    )
            ...
            if not (in_required or in_optional or in_unsupported or in_dependency):
                ...
                if in_unsupported:
                    if self.in_range(spec, self.unsupported[config_name]):
                        ...
                        err_msg += (
                            "defined under 'Unsupported' section in the config file."
                        )
                        err_msg += " (config = %s, spec = %s)" % (
                            config_name,
                            str(spec),
                        )
                        logging.error(err_msg)
                ...
                if in_required:
                    if not self.in_range(spec, self.required[config_name]):
                        err_msg = (
                            "[Error] User config cannot be supported. It is not in "
                        )
                ...
                if in_dependency:
                    ...
                    if dep_list:
                        for rule in dep_list:
                            ...
                            dep_req = rule[
                                3
                            ]  # _Reqs() instance for dependency requirement
                            ...
                            try:
                                ...
                                if cfg_status:
                                    if not dep_status:
                                        ...
                                        err_msg += "'%s' has a dependency on " % str(
                                            config_name
                                        )
                                        err_msg += "'%s'." % str(dep)
                                        logging.error(err_msg)
                                ...
                                err_msg = (
                                    "[Error] Dependency is missing from `Required`. "
                                )
