...
def is_python(cell):
    ...
    return (
        cell["cell_type"] == "code"  # code cells only
    ...
    with tempfile.NamedTemporaryFile("w", delete=False) as temp_file:
        ...
        if temp_file and processed_file:
            new_notebook = _update_notebook(
                notebook, raw_code, six.ensure_str(new_file_content).split("\n")
            )
    ...
    for cell in notebook["cells"]:
        if is_python(cell):
            ...
            for line_idx, code_line in enumerate(cell_lines):
                ...
                if (line_idx == len(cell_lines) - 1) and six.ensure_str(
                    code_line
                ).endswith("\n"):
...
def _update_notebook(original_notebook, original_raw_lines, updated_code_lines):
    ...
    for cell in new_notebook["cells"]:
        ...
        applicable_lines = [
            idx
            for idx, code_line in enumerate(original_raw_lines)
