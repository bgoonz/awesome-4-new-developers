...
class TestUpgrade(test_util.TensorFlowTestCase, parameterized.TestCase):
  ...
  """
    ...
    def _upgrade(
        self, old_file_text, import_rename=False, upgrade_compat_v1_import=False
    ):
    ...
    def testV2KeywordArgNames(self):
        ...
        v1_name_exceptions = {"tf.print"}  # requires print_function import
            ...
            for child in children:
                ...
                for name in names_v1:
                    ...
                    if new_function_name.startswith("tf.compat.v2"):
                        self.assertIn(
                            new_function_name.replace("tf.compat.v2.", "tf."),
                            self.v2_symbols,
                        )
                        continue
...
"""
    ...
    def testDropout(self):
        ...
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(
            new_text, 'tf.nn.dropout(x, rate=1 - (keep_prob), name="foo")\n'
        )
    ...
    def testDropoutExpr(self):
        ...
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(
            new_text, 'tf.nn.dropout(x, rate=1 - (1 - func(3 + 4.)), name="foo")\n'
        )
    ...
    def test_substr(self):
        ...
        _, unused_report, errors, new_text = self._upgrade(text)
        self.assertEqual(
            "tf.strings.substr(input=input, pos=pos, len=len, "
            "name=name, unit=unit)\n",
            new_text,
        )
        self.assertEqual(errors, [])
...
def _log_prob(self, x):
  return tf.reduce_logsumexp(
    ...
    @parameterized.parameters(
        # Add to_sparse unless result_type is RaggedTensor:
        ["tf.strings.split(x, sep)", "tf.strings.split(x, sep).to_sparse()"],
        [
