...
def total_function_cache(defined):
    ...
    return set(defined._function_cache.primary) | set(
        defined._function_cache.arg_relaxed
    )
...
class FunctionTest(test.TestCase, parameterized.TestCase):
    ...
    def testPackedVariable(self):
        ...
        self.assertEqual(
            arg_attrs[0].attr["_composite_device"].s,
            compat.as_bytes(packed_var_0.device),
        )
        self.assertEqual(
            arg_attrs[1].attr["_composite_device"].s,
    ...
    def testImplementsAttributeBasic(self):
        v = def_function.function(experimental_implements="func")(lambda x, y: x + y)
        with context.graph_mode(), self.cached_session():
            ...
            for f in fdefs:
                ...
                if "forward" in name or "backward" in name:
                    ...
                    present += 1
                    self.assertEqual(
                        f.attr[function.IMPLEMENTS_ATTRIBUTE_NAME].s,
                        "func".encode("ascii"),
                        f,
                    )
    ...
    def testImplementsAttributeAssertsOnSideInput(self):
        with context.graph_mode(), self.cached_session():
            z = array_ops.zeros(0)
            v = def_function.function(experimental_implements="func")(
                lambda x, y: x + y + z
            )
            a = array_ops.ones((1.0,))
            b = array_ops.ones((1.0,))
            with self.assertRaisesRegex(
                AssertionError, "variables are always captured"
            ):
                v(a, b)
    ...
    def testImplementsAttributeSpecializes(self):
        with context.graph_mode(), self.cached_session():
            ...
            # Verify that eval() reads the current values.
            a.initializer.run()
            numpy.testing.assert_equal(r1.eval(), [3.0])
            numpy.testing.assert_equal(r2.eval(), [3.0, 3.0])
    ...
    def testImplementsAttributeAsNameAttrList(self):
        ...
        v = def_function.function(experimental_implements=implements_attr)(
            lambda x, y: x + y
        )
        with context.graph_mode(), self.cached_session():
            a = array_ops.placeholder(dtypes.float32, ())
    ...
    def testInputShapeFunctionRelaxationWithStructuredTensors(self):
        ...
        check_trace(  # use relaxed graph
            structured_tensor.StructuredTensor.from_pyval({"a": [1, 2, 3, 4]}), None
        )
    ...
    def testInputShapeFunctionRelaxationWithDatasetIterators(self):
        ...
        check_trace(  # shape=[1, 2]: retrace
            ...
            iterator_ops.IteratorSpec(tensor_spec.TensorSpec([1, 2], dtypes.float32)),
        )
        ...
        check_trace(  # shape=[2, 2]: retrace
            ...
            iterator_ops.IteratorSpec(tensor_spec.TensorSpec([2, 2], dtypes.float32)),
        )
            ...
            [
                (
                    tensor_spec.TensorSpec(
                        (None, None), dtypes.float32, name="first_mat"
                    ),
                    tensor_spec.TensorSpec(
                        (None, None), dtypes.float32, name="second_mat"
                    ),
                )
            ]
        )
    ...
    def testShareRendezvous(self):
        ...
        @def_function.function
        def fn(n):
            functional_ops.While(
                [n], cond.get_concrete_function(), send_body.get_concrete_function()
            )
            return functional_ops.While(
                [n], cond.get_concrete_function(), recv_body.get_concrete_function()
            )
    ...
    def testNestedOutputGraphFunction(self):
        ...
        self.assertEqual(
            sq_op.output_shapes,
            (tensor_shape.TensorShape([2, 2]), {"b": tensor_shape.TensorShape([])}),
        )
    ...
    def testFuncGraphCaptureByValueNested(self):
        ...
        def trivial_function():
            return control_flow_ops.cond(
                array_ops.placeholder_with_default(True, ()), v.read_value, v.read_value
            )
    ...
    def testDefunShapeInferenceWithCapturedTensorListInGraphMode(self):
        with context.graph_mode():
            ...
            tensor_list = list_ops.tensor_list_push_back(
                tensor_list, constant_op.constant(1.0)
            )
            tensor_list = list_ops.tensor_list_push_back(
                tensor_list, constant_op.constant(2.0)
            )
    ...
    @parameterized.named_parameters(
        [
            ("IndexedSlicesWithDenseShape", _example_indexed_slices_with_dense_shape),
            (
    ...
    def testReturnCompositeTensorWithDefun(
        self, factory_fn, factory_kwargs={}, input_signature=None
    ):
    ...
    def testCompositeAsArgumentTensorWithDefun(
        self, factory_fn, factory_kwargs={}, input_signature=None
    ):
        input_ct = factory_fn(**factory_kwargs)
    ...
    def testSequenceInputs(self):
        ...
        clipped_list, global_norm = clip_by_global_norm(
            t_list, constant_op.constant(0.2)
        )
    ...
    def testCacheTensorUnknownShapesCollisionRelaxedShapes(self):
        ...
        with context.graph_mode(), self.cached_session():
            ...
            relaxed_specs = list(defined._function_cache.arg_relaxed_specs.values())[0]
    ...
    def testInputSignatureWithCompatibleInputs(self):
        ...
        rank2_spec = tensor_spec.TensorSpec(shape=(None, None), dtype=dtypes.float32)
    ...
    def testInputsIncompatibleWithSignatureRaisesError(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Structure of Python function inputs.*"
        ):
            defined()

        with self.assertRaisesRegex(
            ValueError, "inputs incompatible with input_signature"
        ):
            defined.get_concrete_function(
    ...
    def testUnderspecifiedInputSignature(self):
        @function.defun(input_signature=[tensor_spec.TensorSpec([], dtypes.float32)])
    ...
    def testFunctionWithExtraAttributes(self):
        @function.defun_with_attributes(
            attributes={"experimental_1": "value1", "experimental_2": 2}
        )
        def matmul(x, y):
    ...
    def testFunctionWithInvalidAttribute(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Attribute experimental_1 must be .* Got .*"
        ):
            with context.graph_mode(), self.cached_session():
                with ops.get_default_graph().as_default():
                    t = constant_op.constant([[1.0, 2.0], [3.0, 4.0]])
    ...
    def testSwapImplementationInEager(self):
        ...
        @function.defun_with_attributes(
            attributes={"api_implements": "foo", "api_preferred_device": "CPU"}
        )
        def on_cpu(x):
            return x + 2

        @function.defun_with_attributes(
            attributes={"api_implements": "foo", "api_preferred_device": "GPU"}
        )
        def on_gpu(x):
            return x + 4
    ...
    def testVariableRetracing(self):
        v1 = variables.Variable(1.0)
        v2 = variables.Variable(1.0)
        v3 = copy.deepcopy(variables.Variable(1.0))

        var_dict = {
            id(v1): constant_op.constant(1),
    ...
    def testFunctionModifiesInputNest(self):
        with self.assertRaisesRegex(ValueError, "modify.* should not modify"):
            ...
            nested_input = [
                {"t1": [constant_op.constant(0.0), constant_op.constant(1.0)]},
    ...
    def testReferenceCycles(self):
        ...
        fn = function.defun(lambda x: 2.0 * x)
    ...
    def testFunctionStackInErrorMessage(self):
        if context.executing_eagerly():
            ...
            self.skipTest(
                "Error interpolation is not working when function is "
    ...
    def test_concrete_function_shape_mismatch(self):
        ...
        self.assertAllEqual(
            [2.0, 3.0], f_concrete(constant_op.constant([1.0, 2.0])).numpy()
        )
    ...
    def test_shape_inference_with_symbolic_shapes(self):
        ...
        def _call_concrete():
            ...
            output1 = conc(
                array_ops.ones([2]), array_ops.ones([5, 4, 2]), array_ops.ones([20, 2])
            )
            self.assertEqual([5, 4, 2], output1.shape)
            output2 = conc(
                array_ops.ones([3]), array_ops.ones([5, 4, 3]), array_ops.ones([40, 3])
            )
        ...
        def g(**kwargs):
            return string_ops.reduce_join(
                string_ops.reduce_join(
                    ops.convert_to_tensor(sorted(kwargs.items())), axis=1, separator="="
                ),
                axis=0,
                separator=", ",
            )
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                ...
                call_args=lambda: ({"a": constant_op.constant([1, 2, 3])},),
    ...
    @test_util.run_in_graph_and_eager_modes
    def testConcreteFunctionStructuredSignatureError(
        self,
        conc_args=(),
        conc_kwargs=None,
        call_args=(),
    ...
    # pylint: enable=g-long-lambda
    @test_util.run_in_graph_and_eager_modes
    def testConcreteFunctionFlatSignatureError(
        self,
        conc_args=(),
        conc_kwargs=None,
        call_args=(),
    ...
    def testPrettyPrintedSignature(self):
        ...
        self.assertRegex(
            c1.pretty_printed_signature(verbose=True), c1_summary + "\n" + c1_details
        )
        self.assertRegex(
        ...
        self.assertRegex(c2.pretty_printed_signature(), c2_summary + "\n" + c2_details)
        ...
        if sys.version_info >= (3, 6):
            self.assertRegex(
                c3.pretty_printed_signature(), c3_summary + "\n" + c3_details
            )
    ...
    def testFollowTypeHintsTraceWithOnlyKwargsNamed(self):
        ...
        enabled(
            1, 2, 3, 4, 5, 6, a=1.5, b=2.5, c=3.5
        )  # No retrace - change in **kwargs
    ...
    def testFollowTypeHintsWithTensorSpec(self):
        ...
        x = v(constant_op.constant(1.0), 3)
        self.assertEqual(x.numpy(), 4.0)
    ...
    @parameterized.parameters(
        [
            (def_function.function, dummy_tf_decorator),
    ...
    def testMissingArgsTfFunctionedMethod(self):
        ...
        tf_func_decorated_method = def_function.function(a_instance.decorated_method)
    ...
    def testMissingArgsTfFunctionedFunctions(self):
        ...
        with self.assertRaisesRegex(
            TypeError, ".* missing 1 required argument: position_arg"
        ):
            tf_func_with_default(named_arg="foo")

        tf_func_pos_3args = def_function.function(func_pos_3args)
        with self.assertRaisesRegex(
            TypeError, ".* missing required arguments: position_arg1, position_arg3"
        ):
    ...
    def testShapeInferencePropagateConstNestedStack(self):
        ...
        @def_function.function(
            input_signature=[tensor_spec.TensorSpec(shape=(3, 6), dtype=dtypes.int32)]
        )
        def g(x):
            y = f(x, s=5)
...
class MultiDeviceTest(test.TestCase, parameterized.TestCase):
    ...
    """
        ...
        @function.defun_with_attributes(
            attributes={"_noinline": True, "_nospecialize": True}
        )
        def inner(resource1):
        ...
        def lazy_capture():
            y = ops.get_default_graph().capture_call_time_value(
                lambda: {"i": i_s, "t": (r_t, s_t)},
                {
                    ...
                    "t": (
                        ragged_tensor.RaggedTensorSpec([2, None, None], dtypes.int32),
                        sparse_tensor.SparseTensorSpec([None], dtypes.int32),
                    ),
                },
            )
