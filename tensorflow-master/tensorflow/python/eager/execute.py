...
def quick_execute(op_name, num_outputs, inputs, attrs, ctx, name=None):
  ...
  """
    ...
    try:
        ...
        keras_symbolic_tensors = [x for x in inputs if ops._is_keras_symbolic_tensor(x)]
...
def execute_with_cancellation(
    op_name, num_outputs, inputs, attrs, ctx, cancellation_manager, name=None
):
    """Execute a TensorFlow operation.
  ...
  """
    ...
    try:
        ctx.ensure_initialized()
        tensors = pywrap_tfe.TFE_Py_ExecuteCancelable(
            ctx._handle,
            device_name,
            op_name,
            inputs,
...
def record_gradient(unused_op_name, unused_inputs, unused_attrs, unused_outputs):
    """Import backprop if you want gradients recorded."""
    pass
