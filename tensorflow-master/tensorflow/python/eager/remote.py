  ...
  """
    ...
    if cluster_device_filters:
        if isinstance(cluster_device_filters, server_lib.ClusterDeviceFilters):
            ...
            raise ValueError(
                "`cluster_device_filters` must be an instance of "
    ...
    if (
        current_server_def is None
        or current_server_def.cluster != cluster_def
        or current_server_def.job_name != job_name
        or current_server_def.task_index != task_index
    ):
    ...
    if (
        ...
        and isinstance(cluster_spec_or_resolver, cluster_resolver.ClusterResolver)
