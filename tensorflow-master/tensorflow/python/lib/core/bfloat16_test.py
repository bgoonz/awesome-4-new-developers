...
class Bfloat16Test(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        (
            {"testcase_name": "_" + dtype.__name__, "dtype": dtype}
    ...
    def testAdd(self):
        ...
        self.assertEqual(np.float32, type(bfloat16(3.5) + np.array(2.25, np.float32)))
        self.assertEqual(np.float32, type(np.array(3.5, np.float32) + bfloat16(2.25)))
    ...
    def testNan(self):
        ...
        a = np.array(
            [bfloat16(1.34375), bfloat16(1.4375), bfloat16(float("nan"))],
            dtype=bfloat16,
        )
        b = np.array(
            [bfloat16(1.3359375), bfloat16(1.4375), bfloat16(float("nan"))],
            dtype=bfloat16,
        )
    ...
    def testSort(self):
        ...
        sorted_bf16 = np.sort(
            values_to_sort.astype(bfloat16)
...
class Bfloat16NumPyTest(parameterized.TestCase):
    ...
    def testArange(self):
        ...
        np.testing.assert_equal(
            np.arange(-0.0, -7.0, -0.25, dtype=np.float32).astype(bfloat16),
            np.arange(-0.0, -7.0, -0.25, dtype=bfloat16),
        )
        np.testing.assert_equal(
            np.arange(-16384.0, 16384.0, 64.0, dtype=np.float32).astype(bfloat16),
            np.arange(-16384.0, 16384.0, 64.0, dtype=bfloat16),
        )

    # pylint: disable=g-complex-comprehension
    @parameterized.named_parameters(
        ({"testcase_name": "_" + op.__name__, "op": op} for op in UNARY_UFUNCS)
    )
    ...
    @parameterized.named_parameters(
        ({"testcase_name": "_" + op.__name__, "op": op} for op in BINARY_UFUNCS)
    )
    ...
    @parameterized.named_parameters(
        (
            {"testcase_name": "_" + op.__name__, "op": op}
    ...
    def testNextAfter(self):
        one = np.array(1.0, dtype=bfloat16)
        two = np.array(2.0, dtype=bfloat16)
        zero = np.array(0.0, dtype=bfloat16)
        ...
        np.testing.assert_equal(np.nextafter(zero, -one), -smallest_denormal)
        for a, b in itertools.permutations([0.0, -0.0, nan], 2):
            np.testing.assert_equal(
                np.nextafter(
