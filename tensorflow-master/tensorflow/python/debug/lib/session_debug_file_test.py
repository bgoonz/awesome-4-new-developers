        ...
        with session.Session(
            ...
            for i in range(2):
                ...
                if i == 0:
                    self.assertAllClose(
                        [u_init_val],
                        dump.get_tensors("%s/read" % u_name, 0, "DebugIdentity"),
                    )
                    self.assertGreaterEqual(
                        dump.get_rel_timestamps("%s/read" % u_name, 0, "DebugIdentity")[
                            0
                        ],
                        0,
                    )
                    ...
                    self.assertAllClose(
                        [v_init_val],
                        dump.get_tensors("%s/read" % v_name, 0, "DebugIdentity"),
                    )
                    self.assertGreaterEqual(
                        dump.get_rel_timestamps("%s/read" % v_name, 0, "DebugIdentity")[
                            0
                        ],
                        0,
                    )
