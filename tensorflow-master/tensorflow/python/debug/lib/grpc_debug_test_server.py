...
class EventListenerTestStreamHandler(grpc_debug_server.EventListenerBaseStreamHandler):
    ...
    """
        if self._dump_dir:
            ...
            if (
                ...
                output_slot,
                debug_op,
            ) in self._event_listener_servicer.breakpoints:
    ...
    def _write_graph_def(self, graph_def, device_name, wall_time):
        ...
        graph_file_path = os.path.join(
            ...
            debug_data.METADATA_FILE_PREFIX
            + debug_data.GRAPH_FILE_TAG
            + debug_data.HASH_TAG
            + "%d_%d" % (graph_hash, wall_time),
        )
    ...
    def _write_value_event(self, event):
        ...
        try:
            ...
            raise ValueError(
                "Could not parse content into JSON: %r, %r" % (content, err)
            )
...
class EventListenerTestServicer(grpc_debug_server.EventListenerBaseServicer):
    ...
    def toggle_watch(self):
        for watch_key in self._toggle_watch_state:
            ...
            self._toggle_watch_state[watch_key] = not self._toggle_watch_state[
                watch_key
            ]
        ...
        for op_log_proto in self._graph_tracebacks:
            for log_entry in op_log_proto.log_entries:
                if log_entry.name == op_name:
                    return self._code_def_to_traceback(
                        log_entry.code_def, op_log_proto.id_to_string
                    )
        raise ValueError(
    ...
    def _code_def_to_traceback(self, code_def, id_to_string):
        return [
            (id_to_string[trace.file_id], trace.lineno, id_to_string[trace.function_id])
            for trace in code_def.traces
        ]


def start_server_on_separate_thread(
    dump_to_filesystem=True,
  ...
  """
    ...
    if poll_server:
        if not _poll_server_till_success(
            50, 0.2, debug_server_url, server_dump_dir, server, gpu_memory_fraction=0.1
        ):
            raise ValueError(
        ...
        for poll_count in range(max_attempts):
            ...
            if dump_dir:
                if (
                    ...
                    and debug_data.DebugDumpDir(dump_dir).size > 0
                ):
