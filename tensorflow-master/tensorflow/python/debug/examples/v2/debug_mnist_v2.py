...
def main(_):
    if FLAGS.check_numerics and FLAGS.dump_dir:
        raise ValueError(
            "The --check_numerics and --dump_dir flags are mutually " "exclusive."
        )
    ...
    train_ds = (
        tf.data.Dataset.from_tensor_slices(mnist_train)
        .shuffle(FLAGS.train_batch_size * FLAGS.max_steps, seed=RAND_SEED)
        .batch(FLAGS.train_batch_size)
    )
    train_ds = train_ds.map(format_example)

    test_ds = (
        tf.data.Dataset.from_tensor_slices(mnist_test)
        .repeat()
        .batch(len(mnist_test[0]))
    )
