...
class ProfileDataTableView(object):
    ...
    """
        ...
        self.formatted_op_time = [
            cli_shared.time_to_readable_str(datum.op_time, force_time_unit=time_unit)
            for datum in profile_datum_list
        ]
        self.formatted_exec_time = [
            cli_shared.time_to_readable_str(
                datum.node_exec_stats.all_end_rel_micros, force_time_unit=time_unit
            )
            for datum in profile_datum_list
        ]
...
def _list_profile_filter(
  ...
  """
    if node_name_regex and not node_name_regex.match(
        ...
        return False
    if file_path_regex:
        if not profile_datum.file_path or not file_path_regex.match(
            profile_datum.file_path
        ):
            return False
    if (
        min_lineno > 0
        and profile_datum.line_number
        and profile_datum.line_number < min_lineno
    ):
        return False
    if (
        max_lineno > 0
        and profile_datum.line_number
        and profile_datum.line_number >= max_lineno
    ):
        return False
    if (
        profile_datum.op_type is not None
        and op_type_regex
        and not op_type_regex.match(profile_datum.op_type)
    ):
        return False
...
class ProfileAnalyzer(object):
    ...
    """
        ...
        ap = argparse.ArgumentParser(
            description="List nodes profile information.", usage=argparse.SUPPRESS
        )
        ap.add_argument(
            "-d",
        ...
        ap.add_argument(
            ...
            help=(
                "the field to sort the data by: (%s)"
                % " | ".join(
                    [
                        SORT_OPS_BY_OP_NAME,
                        SORT_OPS_BY_OP_TYPE,
        ...
        ap.add_argument(
            "source_file_path", type=str, help="Path to the source_file_path"
        )
        ap.add_argument(
            "--cost_type",
            type=str,
    ...
    """
        ...
        op_time_interval = (
            command_parser.parse_time_interval(parsed.op_time)
            if parsed.op_time
            else None
        )
        exec_time_interval = (
        ...
        node_name_regex = (
            re.compile(parsed.node_name_filter) if parsed.node_name_filter else None
        )
        file_path_regex = (
            re.compile(parsed.file_path_filter) if parsed.file_path_filter else None
        )
        op_type_regex = (
            re.compile(parsed.op_type_filter) if parsed.op_type_filter else None
        )

        output = debugger_cli_common.RichTextLines([""])
        device_name_regex = (
            re.compile(parsed.device_name_filter) if parsed.device_name_filter else None
        )
        ...
        for index in range(device_count):
            ...
            if not device_name_regex or device_name_regex.match(device_stats.device):
                profile_data = [
                    datum
                    for datum in data_generator(device_stats)
    ...
    """
        ...
        total_exec_time = sum(
            datum.node_exec_stats.all_end_rel_micros for datum in profile_datum_list
        )
        device_total_row = [
            "Device Total",
            "",
            cli_shared.time_to_readable_str(total_op_time, force_time_unit=time_unit),
            cli_shared.time_to_readable_str(total_exec_time, force_time_unit=time_unit),
        ]
    ...
    """
        ...
        for line_index in source_annotation:
            total_cost = self._get_total_cost(
                source_annotation[line_index], parsed.cost_type
            )
        ...
        for i, line in enumerate(source_lines):
            ...
            if lineno in source_annotation:
                ...
                if parsed.device_name_filter:
                    command += " --%s %s" % (
                        _DEVICE_NAME_FILTER_FLAG,
                        parsed.device_name_filter,
                    )
                if parsed.node_name_filter:
                    command += " --%s %s" % (
                        _NODE_NAME_FILTER_FLAG,
                        parsed.node_name_filter,
                    )
                if parsed.op_type_filter:
                    command += " --%s %s" % (
                        _OP_TYPE_FILTER_FLAG,
                        parsed.op_type_filter,
                    )
                menu_item = debugger_cli_common.MenuItem(None, command)
                num_nodes_execs = RL(
                    "%d(%d)" % (annotation.node_count, annotation.node_exec_count),
            ...
            if parsed.init_line == lineno:
                output_annotations[debugger_cli_common.INIT_SCROLL_POS_KEY] = (
                    len(lines) - 1
                )
...
def create_profiler_ui(
    graph, run_metadata, ui_type="curses", on_ui_exit=None, config=None
):
