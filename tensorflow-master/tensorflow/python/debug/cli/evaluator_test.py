...
class ParseDebugTensorNameTest(test_util.TensorFlowTestCase):
    def testParseNamesWithoutPrefixOrSuffix(self):
        device_name, node_name, output_slot, debug_op, exec_index = evaluator._parse_debug_tensor_name(
            "foo:1"
        )
...
class EvaluatorTest(test_util.TensorFlowTestCase):
    def testEvaluateSingleTensor(self):
        ...
        with test.mock.patch.object(dump, "get_tensors", side_effect=fake_get_tensors):
    ...
    def testEvaluateTwoTensors(self):
        ...
        with test.mock.patch.object(dump, "get_tensors", side_effect=fake_get_tensors):
            ev = evaluator.ExpressionEvaluator(dump)
            self.assertAllClose([[-3.0], [1.0]], ev.evaluate("np.matmul(`a:0`, `b:0`)"))
            self.assertAllClose(
    ...
    def testEvaluateWithMultipleDevicesContainingTheSameTensorName(self):
        ...
        def fake_get_tensors(node_name, output_slot, debug_op, device_name=None):
            ...
            if node_name == "a" and device_name is None:
                ...
                node_name == "a" and device_name == "/job:worker/replica:0/task:0/cpu:0"
            ):
                return [np.array(10.0)]
            elif (
                node_name == "a" and device_name == "/job:worker/replica:0/task:1/cpu:0"
            ):
        ...
        with test.mock.patch.object(dump, "get_tensors", side_effect=fake_get_tensors):
            ...
            self.assertAllClose(
                30.0,
                ev.evaluate(
                    "`/job:worker/replica:0/task:0/cpu:0:a:0` + "
    ...
    def testEvaluateWithNonDefaultDebugOp(self):
        ...
        with test.mock.patch.object(dump, "get_tensors", side_effect=fake_get_tensors):
            ev = evaluator.ExpressionEvaluator(dump)
            self.assertAllClose(
                [[4.0]], ev.evaluate("np.matmul(`a:0:DebugFoo`, `a:0:DebugIdentity`)")
            )
    ...
    def testEvaluateWithMultipleExecIndexes(self):
        ...
        with test.mock.patch.object(dump, "get_tensors", side_effect=fake_get_tensors):
            ev = evaluator.ExpressionEvaluator(dump)
            self.assertAllClose([[4.0]], ev.evaluate("np.matmul(`a:0[1]`.T, `a:0[0]`)"))
