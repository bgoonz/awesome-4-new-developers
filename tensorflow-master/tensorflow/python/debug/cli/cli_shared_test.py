...
class BytesToReadableStrTest(test_util.TensorFlowTestCase):
    ...
    def testSizesBetweenOneMegaByteandOneGigaByteWorks(self):
        ...
        self.assertEqual(
            "2.40M", cli_shared.bytes_to_readable_str(int(1024 ** 2 * 2.4))
        )
    ...
    def testReadableStrIncludesBAtTheEndOnRequest(self):
        ...
        self.assertEqual(
            "1.00MB", cli_shared.bytes_to_readable_str(1024 ** 2, include_b=True)
        )
        self.assertEqual(
            "1.00GB", cli_shared.bytes_to_readable_str(1024 ** 3, include_b=True)
        )
...
class TimeToReadableStrTest(test_util.TensorFlowTestCase):
    ...
    def testForceTimeUnit(self):
        ...
        self.assertEqual(
            "4e-05s",
            cli_shared.time_to_readable_str(40, force_time_unit=cli_shared.TIME_UNIT_S),
        )
        self.assertEqual(
            "0",
            cli_shared.time_to_readable_str(0, force_time_unit=cli_shared.TIME_UNIT_S),
        )
...
class GetErrorIntroTest(test_util.TensorFlowTestCase):
    ...
    def testShapeError(self):
        ...
        self.assertEqual(
            "!!! An error occurred during the run !!!", error_intro.lines[1]
        )
        self.assertEqual(
            [(0, len(error_intro.lines[1]), "blink")], error_intro.font_attr_segs[1]
        )
        ...
        self.assertEqual(
            "ni -a -d -t a/Assign", error_intro.font_attr_segs[4][0][2][0].content
        )
        ...
        self.assertEqual(
            "li -r a/Assign", error_intro.font_attr_segs[6][0][2][0].content
        )
