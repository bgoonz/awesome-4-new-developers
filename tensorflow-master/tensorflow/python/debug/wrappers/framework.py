...
class OnRunEndRequest(object):
  ...
  """
    ...
    def __init__(
        self, performed_action, run_metadata=None, client_graph_def=None, tf_error=None
    ):
...
class BaseDebugWrapperSession(session.SessionInterface):
  ...
  """
    ...
    """
        ...
        self._sess = sess
        self._thread_name_filter_pattern = (
            re.compile(thread_name_filter) if thread_name_filter else None
        )
        ...
        if response.action == OnSessionInitAction.PROCEED:
            ...
            raise NotImplementedError(
                "OnSessionInitAction REMOTE_INSTR_LOOP has not been " "implemented."
            )
        else:
    ...
    """
        ...
        if self._is_disabled_thread() or empty_fetches:
            if callable_runner:
                ...
                return self._sess._make_callable_from_options(callable_options)(
    ...
    def _is_disabled_thread(self):
        ...
        return (
            self._thread_name_filter_pattern
            and not self._thread_name_filter_pattern.match(thread_name)
        )
...
class WatchOptions(object):
    ...
    def __repr__(self):
        return (
            "WatchOptions(debug_ops=%r, node_name_regex_allowlist=%r, "
            "op_type_regex_allowlist=%r, tensor_dtype_regex_allowlist=%r, "
            "tolerate_debug_op_creation_failures=%r)"
            % (
                self.debug_ops,
                self.node_name_regex_allowlist,
                self.op_type_regex_allowlist,
                self.tensor_dtype_regex_allowlist,
