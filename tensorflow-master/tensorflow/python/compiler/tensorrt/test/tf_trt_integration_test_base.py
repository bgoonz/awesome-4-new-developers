...
class TfTrtIntegrationTestBase(test_util.TensorFlowTestCase):
    ...
    def ShouldRunTest(self, run_params):
        ...
        return (
            (
                ...
                or not IsQuantizationMode(run_params.precision_mode)
            ),
            "test either calibration or non-INT8",
        )
    ...
    def ExpectedAbsoluteTolerance(self, run_params):
        ...
        return 1.0e-05 if run_params.precision_mode == "FP32" else 1.0e-02
        ...
        for expected_shapes, current_input_data in zip(
            ...
            for _ in range(num_runs):
                ...
                if val is not None:
                    # Some ops may have nondeterministic output. E.g. Conv2D may use
                    # winograd algorithm. So we set atol/rtol be larger than 1.e-06.
                    self.assertAllClose(val, new_val, atol=1.0e-05, rtol=1.0e-05)
                val = new_val
            results.append(val)
    ...
    def _RunGraph(
        self, run_params, saved_model_dir, inputs_data, graph_state, num_runs=2
    ):
        params = self._GetParamsCached()
        ...
        if run_params.is_v2:
            results = self._RunGraphV2(
                saved_model_dir, inputs_data, graph_state, num_runs
            )
    ...
    def _CreateConverter(self, run_params, saved_model_dir, conversion_params):
        ...
        if run_params.is_v2:
            ...
            if self._disable_non_trt_optimizers:
                converter_v2._test_only_disable_non_trt_optimizers = (
                    True
        ...
        if self._disable_non_trt_optimizers:
            converter_v1._test_only_disable_non_trt_optimizers = (
                True
            )  # pylint: disable=protected-access
        return converter_v1
    ...
    def _GetCalibratedInferGraph(self, run_params, saved_model_dir, inputs_data):
        ...
        converter = self._CreateConverter(
            run_params, saved_model_dir, conversion_params
        )
        int8_gdef = converter.convert()
        self._VerifyGraphDef(
            run_params, saved_model_dir, int8_gdef, GraphState.CALIBRATE
        )
    ...
    def _WriteGraph(self, run_params, gdef, graph_state):
        ...
        graph_name = (
            ...
            + "_"
            + run_params.test_name
            + "_"
            + self._GetGraphStateLabel(graph_state)
            + ".pbtxt"
        )
    ...
    def _VerifyConnections(self, expected_engines, original_gdef, converted_gdef):
        ...
        for node in original_gdef.node:
            ...
            for inp in node.input:
                ...
                if not is_engine_op or (
                    mapped_input != target_node_name
                    and name_to_node_map[inp_name].op != "Const"
                ):
        ...
        expected_input_map = {
            k: v for k, v in expected_input_map.items() if k not in removed_const_nodes
        }
    ...
    """
        ...
        old_name_to_node_map = {
            self._ToString(node.name): node for node in _ChainAllNodes(original_gdef)
        }
      ...
      """
        ...
        for node in _ChainAllNodes(converted_gdef):
            if node.op == "TRTEngineOp":
                ...
                logging.info(
                    "Segment function name: %s. Including %d nodes.",
                    seg_func.name,
                    len(seg_func_def.node_def),
                )
                ...
                for alternative_node in seg_func_def.node_def:
                    node_name = self._Canonicalize(
                        self._ToString(alternative_node.name)
                    )
                    ...
                    if "_tftrt_op_max_batch_size" in original_node.attr:
                        ...
                        original_node.op != "Const"
                        and alternative_node.op != "Const"
                        and "_output_shapes" in original_node.attr
                    ):
                        node_max_batch_size = _DetectStaticBatchSize(original_node)
                    logging.info(
                        "'{%s}(%s)'s max batch size annotation is %s. "
                        "'{%s}'s max batch size is %s.",
                        node_name,
                        original_node.op,
                        str(node_max_batch_size),
                        engine_name,
                        str(engine_max_batch_size),
                    )
                    node_max_batch_size_all_none &= node_max_batch_size is None
                    self.assertTrue(
                        engine_max_batch_size == node_max_batch_size
                        or node_max_batch_size is None
                    )
                logging.info(
                    "'{%s}'s max batch size is %d.", engine_name, engine_max_batch_size
                )
                self.assertTrue(
                    default_max_batch_size is None
                    or engine_max_batch_size == default_max_batch_size
                    or not node_max_batch_size_all_none
                )
    ...
    def _GetGraphDef(self, run_params, gdef_or_saved_model_dir):
        if isinstance(gdef_or_saved_model_dir, str):
            ...
            return saved_model_utils.get_meta_graph_def(
                gdef_or_saved_model_dir, tag_constants.SERVING
            ).graph_def
    ...
    def _VerifyGraphDefV2(self, run_params, original_gdef, gdef_to_verify, graph_state):
        ...
        for func in gdef_to_verify.library.function:
            if not re.search(
                r"TRTEngineOp_\d+_\d+_native_segment", func.signature.name
            ):
    ...
    def _VerifyGraphDef(
        ...
        gdef_to_verify = self._GetGraphDef(
            run_params, gdef_or_saved_model_dir_to_verify
        )
        self._WriteGraph(run_params, gdef_to_verify, graph_state)
        if run_params.is_v2:
            self._VerifyGraphDefV2(
                run_params, original_gdef, gdef_to_verify, graph_state
            )
        else:
            self._VerifyGraphDefV1(
                run_params, original_gdef, gdef_to_verify, graph_state
            )
    ...
    def _MakeSavedModelV1(self, run_params):
        ...
        with self.session(
            graph=g, config=self._GetConfigProto(run_params, GraphState.ORIGINAL)
        ) as sess:
    ...
    def RunTest(self, run_params):
        with trace.Trace(run_params.test_name):
            ...
            self._VerifyGraphDef(
                run_params, saved_model_dir, saved_model_dir, GraphState.ORIGINAL
            )
            ...
            result = self._RunGraph(
                run_params, infer_saved_model_dir, inputs_data, GraphState.INFERENCE
            )
            self.assertAllClose(
                ref_result,
                result,
...
def _GetTestConfigsV2():
    ...
    opts = list(
        itertools.product(
            [FP32, FP16, INT8], [convert_offline], [dynamic_engine], [no_calibration]
        )
    )
    # We always run calibration with offline tool.
...
def _AddTestsFor(test_class, is_v2):
    ...
    for (precision_mode, convert_online, dynamic_engine, use_calibration) in opts:
        ...
        test_name = "%s_%s_%s_%s_%s" % (
            "testTfTrtV2" if is_v2 else "testTfTrt",
            conversion,
            engine_type,
            precision_mode,
            calibration_type,
        ...
        if is_v2:
            ...
            setattr(
                test_class, test_name, test_util.run_v1_only("", _GetTest(run_params))
            )
