...
class XLACompileContextTest(test.TestCase, parameterized.TestCase):
    ...
    def test_non_resource_variable_error(self):
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            "Non-resource Variables are not supported inside "
    ...
    def test_nested_xla_compile_error(self):
        ...
        context2.Enter()
        with self.assertRaisesRegex(
            ValueError, "XLA compiled computations cannot be nested"
        ):
...
class CheckFunctionArgumentCountTest(test.TestCase):
    ...
    """
        ...
        self.assertEqual(
            "exactly 3 arguments", xla.check_function_argument_count(func, 2, None)
        )
    ...
    def test_default_args(self):
        ...
        self.assertEqual(
            "at least 2 arguments", xla.check_function_argument_count(func, 1, None)
        )
        self.assertEqual(
            "at most 3 arguments", xla.check_function_argument_count(func, 4, None)
        )
        ...
        self.assertEqual(
            "at most 3 arguments", xla.check_function_argument_count(func, 4, queue)
        )
