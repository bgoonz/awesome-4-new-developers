...
class ResourceVariableSaveable(saveable_object.SaveableObject):
    ...
    def __init__(self, var, slice_spec, name):
        ...
        spec = saveable_object.SaveSpec(
            tensor, slice_spec, name, dtype=var.dtype, device=var.device
        )
    ...
    if isinstance(op, saveable_object.SaveableObject):
        ...
        if isinstance(op, resource_variable_ops.BaseResourceVariable):
            ...
            if context.executing_eagerly():
                raise ValueError(
                    "Can only save/restore ResourceVariables when "
            ...
            if not _tensor_comes_from_variable(variable):
                raise TypeError(
                    ...
                    "names to Tensors/Variables. Not a variable: %s" % variable
                )
            if variable.op.type in ["Variable", "VariableV2", "AutoReloadVariable"]:
...
def op_list_to_dict(op_list, convert_variable_to_tensor=True):
  ...
  """
    if not isinstance(op_list, (list, tuple, set)):
        raise TypeError(
            "Variables to save should be passed in a dict or a " "list: %s" % op_list
        )
    ...
    # pylint: disable=protected-access
    for var in op_list:
        resource_or_ref_variable = isinstance(
            var, resource_variable_ops.BaseResourceVariable
        ) or isinstance(var, variables.RefVariable)
        ...
        if isinstance(var, saveable_object.SaveableObject):
            ...
            if var.name in names_to_saveables:
                raise ValueError(
                    "At least two variables have the same name: %s" % var.name
                )
            names_to_saveables[var.name] = var
            ...
            if name in names_to_saveables:
                if not isinstance(names_to_saveables[name], list):
                    raise ValueError(
                        "Mixing slices and non-slices with the same name: " "%s" % name
                    )
            ...
            if not getattr(var, "_in_graph_mode", True):
                ...
                if name in names_to_saveables:
                    raise ValueError(
                        "At least two variables have the same name: %s" % name
                    )
                names_to_saveables[name] = var
...
def _add_saveable(saveables, seen_ops, saveable):
  ...
  """
    if saveable.op is not None and saveable.op in seen_ops:
        raise ValueError(
            "The same saveable will be restored with two names: %s" % saveable.name
        )
...
def trace_save_restore_functions(object_to_save):
    ...
    for (
        ...
        object_to_save._gather_saveables_for_checkpoint().items()
    ):  # pylint: disable=protected-access
        if not callable(saveable_factory):
            if isinstance(saveable_factory, saveable_object.SaveableObject):
                logging.debug(
                    ...
                    "problems loading the SavedModel back into Python.".format(
                        object_to_save
                    )
                )
            continue
...
def _trace_save_and_restore_function(saveable_factory, object_to_save):
    ...
    if any(
        isinstance(saveable, trackable.PythonStateSaveable) for saveable in saveables
    ):
        logging.warn(
...
class RestoredSaveableObject(saveable_object.SaveableObject):
    ...
    def __init__(self, save_function, restore_function, name):
        ...
        specs = [
            saveable_object.SaveSpec(x["tensor"], x["slice_spec"], x["name"])
            for x in tensors
        ]
...
def create_saveable_object(factory, name, call_with_mapped_captures):
  ...
  """
    if (
        call_with_mapped_captures is None
        or not is_factory_for_restored_saveable_object(factory)
    ):
...
def is_factory_for_restored_saveable_object(factory):
    return (
        isinstance(factory, functools.partial)
        and factory.func is RestoredSaveableObject
    )
