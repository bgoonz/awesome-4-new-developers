...
def _init_from_checkpoint(ckpt_dir_or_file, assignment_map):
    ...
    for tensor_name_in_ckpt, current_var_or_name in sorted(
        ...
        if _is_variable(current_var_or_name) or (
            ...
            store_vars = (
                vs._get_default_variable_store()._vars
...
def _set_checkpoint_initializer(
    variable, ckpt_file, tensor_name, slice_spec, name="checkpoint_initializer"
):
  ...
  """
    ...
    with ops.device(variable.device), ops.device("/cpu:0"):
        restore_op = io_ops.restore_v2(
            ckpt_file, [tensor_name], [slice_spec], [base_type], name=name
        )[0]
