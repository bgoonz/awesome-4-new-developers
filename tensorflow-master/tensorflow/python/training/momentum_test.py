...
class MomentumOptimizerTest(test.TestCase):
    ...
    def doTestBasic(self, use_resource=False, use_callable_params=False):
        for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
            ...
            # Check that the momentum accumulators have been updated.
            self.assertAllCloseAccordingToType(
                np.array([(0.9 * 0.1 + 0.1), (0.9 * 0.1 + 0.1)]), self.evaluate(slot0)
            )
            self.assertAllCloseAccordingToType(
        ...
        for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
            ...
            with ops.Graph().as_default(), self.cached_session():
                ...
                grads1 = ops.IndexedSlices(
                    constant_op.constant([[0.01, 0.01], [0.01, 0.01]], dtype=dtype),
                ...
                self.assertAllCloseAccordingToType(
                    np.array([0.1, 0.1]), self.evaluate(slot0)[1]
                )
                self.assertAllCloseAccordingToType(
                    np.array([0.01, 0.01]), self.evaluate(slot1)[2]
                )
                # Check that the parameters have been updated.
                self.assertAllCloseAccordingToType(
                ...
                self.assertAllCloseAccordingToType(
                    np.array([-(0.1 * 2.0), -(0.1 * 2.0)]), self.evaluate(var0)[1]
                )
                self.assertAllCloseAccordingToType(
