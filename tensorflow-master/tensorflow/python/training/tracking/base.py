...
class TrackableReference(collections.namedtuple("TrackableReference", ["name", "ref"])):
  ...
  """
    ...
    def __init__(self, checkpoint_position, shape=None, shard_info=None):
        ...
        self.wrapped_value = checkpoint_position.value_tensors(
            {VARIABLE_VALUE_KEY: shape_and_slice}
        )[VARIABLE_VALUE_KEY]
        self._checkpoint_position = checkpoint_position
...
class PythonStringStateSaveable(PythonStateSaveable):
    ...
    """
        ...
        super(PythonStringStateSaveable, self).__init__(self._save_string, [spec], name)
    ...
    def freeze(self):
        ...
        return NoRestoreSaveable(
            tensor=_constant_state, dtype=dtypes.string, name=self.name, device="cpu:0"
        )
    ...
    def restore(self, trackable):
        ...
        with ops.init_scope():
            if self.bind_object(trackable):
                ...
                restore_ops = trackable._restore_from_checkpoint_position(
                    self
...
_DeferredSlotVariableRestoration = collections.namedtuple(
    "_DeferredSlotVariableRestoration",
    ["original_variable", "slot_variable_id", "slot_name"],
)
  ...
  """
    ...
    """
        ...
        with ops.init_scope():
            ...
            if checkpoint_initializer is not None and not (
                isinstance(initializer, CheckpointInitialValueCallable)
    ...
    """
        ...
        if not isinstance(trackable, Trackable):
            raise TypeError(
                ("Trackable._track_trackable() passed type %s, not a " "Trackable.")
                % (type(trackable),)
            )
    ...
    """
        ...
        for name_based_restore in sorted(
            ...
            trackable._name_based_attribute_restore(
                name_based_restore
            )  # pylint: disable=protected-access
    ...
    def _restore_from_checkpoint_position(self, checkpoint_position):
        ...
        while visit_queue:
            ...
            new_restore_ops, new_tensor_saveables, new_python_saveables = current_position.trackable._single_restoration_from_checkpoint_position(  # pylint: disable=protected-access
                checkpoint_position=current_position, visit_queue=visit_queue
            )
