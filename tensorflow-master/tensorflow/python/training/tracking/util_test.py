...
class InterfaceTests(test.TestCase):
    ...
    def testAddVariable(self):
        ...
        other_duplicate = resource_variable_ops.ResourceVariable(
            name="duplicate", initial_value=1.0
        )
        duplicate = trackable_utils.add_variable(obj, name="duplicate", shape=[])
        ...
        self.assertEqual(
            "some_variable_scope/ones_initializer:0", ones_initializer.name
        )
        self.assertAllEqual([1, 1], self.evaluate(ones_initializer))
        self.assertAllEqual([[0.0, 0.0], [0.0, 0.0]], self.evaluate(bare_initializer))
        ...
        named_variables, _, _ = graph_view.ObjectGraphView(obj).serialize_object_graph()
    ...
    def testShapeDtype(self):
        ...
        v1 = trackable_utils.add_variable(
            root, name="v1", initializer=3.0, dtype=dtypes.float64
        )
        ...
        self.assertAllEqual([1.0, 1.0, 1.0], self.evaluate(v2))
...
class _MirroringSaveable(saver_lib.BaseSaverBuilder.SaveableObject):
    def __init__(self, primary_variable, mirrored_variable, name):
        ...
        spec = saver_lib.BaseSaverBuilder.SaveSpec(
            tensor=tensor, slice_spec="", name=name
        )
        super(_MirroringSaveable, self).__init__(tensor, [spec], name)
...
class _OwnsMirroredVariables(base.Trackable):
    ...
    def __init__(self):
        self.non_dep_variable = variable_scope.get_variable(
            name="non_dep_variable", initializer=6.0, use_resource=True
        )
        self.mirrored = variable_scope.get_variable(
            name="mirrored", initializer=15.0, use_resource=True
        )
...
class CheckpointingTests(parameterized.TestCase, test.TestCase):
    ...
    def testMoreComplexSaveableReturned(self):
        ...
        self.evaluate(v.non_dep_variable.assign(44.0))
        save_path = checkpoint.save(prefix)
        self.evaluate(v.non_dep_variable.assign(45.0))
        checkpoint.restore(save_path).assert_consumed().initialize_or_restore()
        self.assertEqual(44.0, self.evaluate(v.non_dep_variable))
        self.assertEqual(44.0, self.evaluate(v.mirrored))
    ...
    def testAssertConsumedNoCheckpoint(self):
        ...
        v = variable_scope.get_variable(name="v", initializer=0.0)
    ...
    def testPassingCheckpointOptions(self):
        ...
        options = checkpoint_options.CheckpointOptions(experimental_io_device=localhost)
        prefix = os.path.join(self.get_temp_dir(), "ckpt")
        v = variable_scope.get_variable(name="v", initializer=0.0)
        self.evaluate(v.initializer)
        ckpt = trackable_utils.Checkpoint(v=v)
    ...
    def testPartialRestoreWarningAttribute(self):
        with context.eager_mode():
            original_root = trackable_utils.Checkpoint(
                v1=variables_lib.Variable(2.0), v2=variables_lib.Variable(3.0)
            )
            prefix = os.path.join(self.get_temp_dir(), "ckpt")
            save_path = original_root.save(prefix)
            partial_root = trackable_utils.Checkpoint(
                v1=base.Trackable(), v2=variables_lib.Variable(0.0)
            )
            ...
            with test.mock.patch.object(logging, "warning") as mock_log:
                ...
                self.assertEqual(3.0, partial_root.v2.numpy())
                del partial_root
    ...
    def testSilencePartialWarning(self):
        with context.eager_mode():
            ...
            prefix = os.path.join(self.get_temp_dir(), "ckpt")
            save_path = original_root.save(prefix)
            partial_root = trackable_utils.Checkpoint(v1=variables_lib.Variable(0.0))
            weak_partial_root = weakref.ref(partial_root)
    ...
    def testLocalNameValidation(self):
        ...
        self.assertEqual(
            "..ATTRIBUTES/a/.ATTRIBUTES/VARIABLE_VALUE", named_variable.name
        )
    ...
    def testLateDependencyTracking(self):
        class Dependency(tracking.AutoTrackable):
            def build(self):
                self.var = trackable_utils.add_variable(self, "var", initializer=0.0)
        ...
        original.add_dep()
        self.evaluate(state_ops.assign(original.dep.var, 123.0))
        ...
        self.assertEqual(123.0, self.evaluate(load_into.dep.var))
    ...
    def testDepAfterVar(self):
        ...
        self.evaluate(state_ops.assign(dep_after_var.dep.var, -14.0))
        checkpoint_directory = self.get_temp_dir()
        checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
        ...
        self.assertEqual(-14.0, self.evaluate(loaded_dep_after_var.dep.var))
    ...
    def testOverlappingRestores(self):
        ...
        save_root.dep.var = trackable_utils.add_variable(
            save_root.dep, name="var", initializer=0.0
        )
        self.evaluate(state_ops.assign(save_root.dep.var, 12.0))
        first_path = save_root.save(os.path.join(checkpoint_directory, "first"))
        self.evaluate(state_ops.assign(save_root.dep.var, 13.0))
        ...
        load_dep.var = trackable_utils.add_variable(load_dep, name="var", shape=[])
        first_root.dep = load_dep
        first_status.assert_consumed()
        first_status.run_restore_ops()
        self.assertEqual(12.0, self.evaluate(load_dep.var))
        second_root.dep = load_dep
        second_status.assert_consumed()
        second_status.run_restore_ops()
        self.assertEqual(13.0, self.evaluate(load_dep.var))
    ...
    def testAmbiguousLoad(self):
        ...
        trackable_utils.add_variable(dep_three, name="var", initializer=0.0)
        ...
        trackable_utils.add_variable(
            load_root.dep_one.dep_three, name="var", initializer=0.0
        )
        trackable_utils.add_variable(
            load_root.dep_two.dep_three, name="var", initializer=0.0
        )
        with self.assertRaises(AssertionError):
            status.assert_consumed()
        with self.assertRaises(AssertionError):
    ...
    def testObjectsCombined(self):
        ...
        save_root.dep_two = tracking.AutoTrackable()
        trackable_utils.add_variable(
            save_root.dep_one, name="var1", initializer=32.0, dtype=dtypes.float64
        )
        trackable_utils.add_variable(
            save_root.dep_two, name="var2", initializer=64.0, dtype=dtypes.float64
        )
        self.evaluate(trackable_utils.gather_initializers(save_root))
        save_path = save_root.save(os.path.join(checkpoint_directory, "ckpt"))
        load_root = trackable_utils.Checkpoint()
        ...
        status = (
            load_root.restore(save_path)
            .assert_consumed()
            .assert_existing_objects_matched()
        )
        status.run_restore_ops()
        self.assertEqual(32.0, self.evaluate(v1))
        self.assertEqual(64.0, self.evaluate(v2))
    ...
    def testDependencyLoop(self):
        ...
        first.second = second
        second.first = first
        first.v = trackable_utils.add_variable(first, "v1", initializer=[3.0, 1.0, 4.0])
        second.v = trackable_utils.add_variable(
            second, "v2", initializer=[1.0, 1.0, 2.0, 3.0]
        )
        ...
        with self.assertRaises(AssertionError):
            status.assert_consumed()
        first_load.v = trackable_utils.add_variable(first_load, "v1", shape=[3])
        second_load.v = trackable_utils.add_variable(second_load, "v2", shape=[4])
        status.assert_consumed()
        status.run_restore_ops()
        self.assertAllEqual([3.0, 1.0, 4.0], self.evaluate(first_load.v))
        self.assertAllEqual([1.0, 1.0, 2.0, 3.0], self.evaluate(second_load.v))

        # Test loading when variables have already been created
        self.evaluate(first_load.v.assign([2.0, 7.0, 1.0]))
        self.assertAllEqual([2.0, 7.0, 1.0], self.evaluate(first_load.v))
        self.evaluate(second_load.v.assign([2.0, 7.0, 1.0, 8.0]))
        self.assertAllEqual([2.0, 7.0, 1.0, 8.0], self.evaluate(second_load.v))
    ...
    def testRestoreOnAssign(self):
        ...
        checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
        first = trackable_utils.Checkpoint()
        first.var1 = variables_lib.Variable(0.0, name="outside_var")
        first.var2 = variables_lib.Variable(0.0, name="blah")
        self.evaluate(first.var1.assign(4.0))
        self.evaluate(first.var2.assign(8.0))
        save_path = first.save(checkpoint_prefix)

        second = trackable_utils.Checkpoint()
        second.var2 = variables_lib.Variable(0.0, name="blah")
        status = second.restore(save_path)
        recreated_var1 = variables_lib.Variable(0.0, name="outside_var")
        status.run_restore_ops()
        self.assertEqual(8.0, self.evaluate(second.var2))
        self.evaluate(recreated_var1.assign(-2.0))
        self.assertEqual(-2.0, self.evaluate(recreated_var1))
        second.var1 = recreated_var1
        status.run_restore_ops()
        self.assertEqual(4.0, self.evaluate(recreated_var1))
    ...
    def test_write_checkpoint_from_function(self):
        ...
        save_checkpoint = trackable_utils.Checkpoint(v=variables_lib.Variable(1.0))
        ...
        load_checkpoint = trackable_utils.Checkpoint(v=variables_lib.Variable(0.0))
        ...
        status.assert_consumed()
        status.run_restore_ops()
        self.assertEqual(1.0, self.evaluate(load_checkpoint.v))
        self.evaluate(save_checkpoint.v.assign(3.0))
        self.evaluate(_write_checkpoint())
        self.evaluate(save_checkpoint.v.assign(0.0))
        status = load_checkpoint.read(checkpoint_prefix)
        status.assert_existing_objects_matched()
        status.assert_consumed()
        status.run_restore_ops()
        self.assertEqual(3.0, self.evaluate(load_checkpoint.v))

    def test_inititialize_with_data_structures(self):
        checkpoint = trackable_utils.Checkpoint(
            a=[variables_lib.Variable(0.0), variables_lib.Variable(1.0)],
            b={"a": variables_lib.Variable(2.0), "b": variables_lib.Variable(3.0)},
        )
        checkpoint_directory = self.get_temp_dir()
        checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
        save_path = checkpoint.save(checkpoint_prefix)
        load_checkpoint = trackable_utils.Checkpoint(
            a=[variables_lib.Variable(4.0), variables_lib.Variable(5.0)],
            b={"a": variables_lib.Variable(6.0), "b": variables_lib.Variable(7.0)},
        )
    ...
    def _create_trackable(self):
        class Model(tracking.AutoTrackable):
            def __init__(self):
                self.v = variables_lib.Variable(2.0)
    ...
    def test_initialize_with_root_object(self):
        ...
        model.deferred_variable = variables_lib.Variable(5.0)
    ...
    def test_initialize_with_root_object_and_kwargs(self):
        ...
        new_model = self._create_trackable()
        separate_variable = variables_lib.Variable(1.0)
        load_checkpoint = trackable_utils.Checkpoint(
        ...
        new_model.separate_variable = variables_lib.Variable(200.0)
        v = variables_lib.Variable(100.0)
        ...
        separate_variable = variables_lib.Variable(200.0)
        v = variables_lib.Variable(100.0)
        load_checkpoint = trackable_utils.Checkpoint(
    ...
    def test_checkpoint_saved_model_compatibility(self):
        ...
        with self.assertRaisesRegex(
            errors_impl.NotFoundError, "Failed to restore from checkpoint or SavedModel"
        ):
...
class TemplateTests(parameterized.TestCase, test.TestCase):
    ...
    def test_trackable_save_restore_nested(self):
        ...
        self.evaluate(inner_template_one.variables[0].assign([20.0]))
        self.evaluate(inner_template_two.variables[0].assign([25.0]))
        checkpoint_directory = self.get_temp_dir()
        checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
        ...
        status.assert_consumed().run_restore_ops()
        self.assertAllEqual([20.0], self.evaluate(v1))
        self.assertAllEqual([25.0], self.evaluate(v2))
        self.assertAllEqual([25.0], self.evaluate(v3))
