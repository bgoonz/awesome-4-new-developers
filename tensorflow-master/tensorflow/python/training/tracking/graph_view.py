...
def _object_prefix_from_path(path_to_root):
    return "/".join((_escape_local_name(trackable.name) for trackable in path_to_root))
...
def _slot_variable_naming_for_optimizer(optimizer_path):
    ...
    def _name_slot_variable(variable_path, slot_name):
        ...
        return variable_path + optimizer_identifier + _escape_local_name(slot_name)
...
def _serialize_slot_variables(trackable_objects, node_ids, object_names):
    ...
    for trackable in non_slot_objects:
        if (
            isinstance(trackable, optimizer_v1.Optimizer)
            ...
            for slot_name in slot_names:
                for original_variable_node_id, original_variable in enumerate(
                    ...
                    if (
                        slot_variable._checkpoint_dependencies
                    ...
                    slot_variable_proto = trackable_object_graph_pb2.TrackableObjectGraph.TrackableObject.SlotVariableReference(
                        slot_name=slot_name,
...
class ObjectGraphView(object):
        ...
        while to_visit:
            ...
            for name, dependency in self.list_dependencies(current_trackable):
                if dependency not in path_to_root:
                    path_to_root[dependency] = path_to_root[current_trackable] + (
            ...
            for (
                ...
                for saveable in saveables:
                    if optional_restore is None:
                        ...
                        optional_restore = (
                            optional_restore and saveable.optional_restore
                        )
    ...
    def _fill_object_graph_proto(
        self, trackable_objects, node_ids, slot_variables, object_graph_proto=None
    ):
    ...
    def _serialize_gathered_objects(
        ...
        named_saveable_objects, feed_additions = self._add_attributes_to_object_graph(
            trackable_objects=trackable_objects,
    ...
    """
        ...
        return self._serialize_gathered_objects(trackable_objects, path_to_root)
    ...
    def frozen_saveable_objects(
        ...
        with target_context():
            named_saveable_objects, graph_proto, _ = self._serialize_gathered_objects(
                trackable_objects, path_to_root, object_map, call_with_mapped_captures
            )
