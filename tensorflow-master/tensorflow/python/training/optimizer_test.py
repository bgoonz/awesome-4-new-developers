...
class OptimizerTest(test.TestCase):
    ...
    def testBasic(self):
        for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
            ...
            var0 = resource_variable_ops.ResourceVariable(
                [1.0, 2.0], dtype=dtype, name="a_%d" % i
            )
            var1 = resource_variable_ops.ResourceVariable(
                [3.0, 4.0], dtype=dtype, name="b_%d" % i
            )
        ...
        for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
            with self.cached_session():
                ...
                opt_op = sgd_op.minimize(
                    ...
                    global_step,
                    [var0, var1],
                    aggregation_method=gradients_util.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N,
                )
    ...
    def testNoGradients(self):
        for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
            ...
            # seem to be getting deleted at the end of the loop.
            var0 = resource_variable_ops.ResourceVariable(
                [1.0, 2.0], dtype=dtype, name="a%d" % i
            )
            var1 = resource_variable_ops.ResourceVariable(
                [3.0, 4.0], dtype=dtype, name="b%d" % i
            )
            # pylint: disable=cell-var-from-loop
            def loss():
                return 5 * var0
