...
class DynamicLossScaleTest(test.TestCase, parameterized.TestCase):
    def _get_tensor(self, is_finite):
        tensor = control_flow_ops.cond(is_finite, lambda: 1.0, lambda: float("NaN"))
        ...
        def get():
            ...
            return control_flow_ops.cond(
                math_ops.equal(rep_id, 0), lambda: tensor, lambda: 1.0
            )
    ...
    def test_trigger_loss_scale_update_each_step(self, strategy_fn):
        with strategy_fn().scope():
            ...
            self._test_helper(
                inputs, expected_outputs, init_loss_scale, increment_period
            )
