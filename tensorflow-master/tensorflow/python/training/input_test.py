...
class MatchFilenamesOnceTest(test_lib.TestCase):
    def test(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertItemsEqual(map(compat.as_bytes, filenames), self.evaluate(star))
            self.assertItemsEqual(
                map(compat.as_bytes, additional), self.evaluate(question)
            )
            self.assertItemsEqual([compat.as_bytes(additional[1])], self.evaluate(one))
...
class InputProducerTest(test_lib.TestCase):
    def testNoShuffle(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertAllEqual(input_tensor * num_epochs, self.evaluate(dequeue_many))
    ...
    def testSharedName(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertProtoEquals(
                "s: 'SHARED_NAME_XYZ'", queue.queue_ref.op.node_def.attr["shared_name"]
            )
...
class SliceInputProducerTest(test_lib.TestCase):
    ...
    def testSharedName(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            source_ints = [7, 3, 5, 2]
            slices = inp.slice_input_producer(
                [source_strings, source_ints], shared_name="SHARED_NAME_XYZ", name="sip"
            )
...
class BatchTest(test_lib.TestCase):
    def _testOneThreadHelper(self, use_dict):
        with ops.Graph().as_default(), self.cached_session():
            ...
            for i in range(num_batches):
                ...
                self.assertAllEqual(
                    results[0], np.arange(i * batch_size, (i + 1) * batch_size)
                )
                self.assertAllEqual(
                    results[1].indices,
                    np.vstack(
    ...
    def testOneThreadDynamicPad(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            examples = variables.Variable(zero64)
            counter = examples.count_up_to(num_batches * batch_size)
            string = array_ops.tile(
                ["string"], math_ops.cast(array_ops.stack([counter]), dtypes.int32)
            )
            self.evaluate(variables.global_variables_initializer())
            variables.local_variables_initializer().run()
            batched = inp.batch(
    ...
    def testOneThreadSmallerBatch(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertAllEqual(
                results[0],
                np.arange(
                    num_batches * batch_size, num_batches * batch_size + extra_elements
                ),
            )
            self.assertAllEqual(
                results[1].indices,
    ...
    def testManyThreadsSmallerBatch(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertAllEqual(results[2], [b"string"] * extra_elements)
            self.assertItemsEqual(
                all_counts, range(num_batches * batch_size + extra_elements)
            )
    ...
    def _testKeepInputHelper(self, num_threads, enqueue_many, keep_input_vector=False):
        with ops.Graph().as_default(), self.cached_session():
            batch_size = 5
            num_batches = 4
            ...
            sparse_counter = sparse_tensor.SparseTensor(
                indices=array_ops.zeros([1, 1], dtype=dtypes.int64),
                values=array_ops.stack([math_ops.cast(counter, dtypes.float32)]),
                dense_shape=[1],
            )
    ...
    @test_util.run_v1_only(
        "Input pipelines based on Queues are not supported "
    ...
    def testInvalidKeepInputVector(self):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegex(
                ValueError, "must be known at graph construction"
            ):
                inp.maybe_batch(
                    [array_ops.zeros(5)],
...
class BatchJoinTest(test_lib.TestCase):
    def _testTwoThreadsHelper(self, use_dict):
        with ops.Graph().as_default(), self.cached_session():
            ...
            ninety_nine = inp.limit_epochs(
                constant_op.constant(99, dtype=dtypes.int64), num_b
            )
            ...
            self.assertAllEqual((None,), batched_fetch[1].values.get_shape().as_list())
            self.assertAllEqual(
            ...
            for i in range(num_batches):
                ...
                seen_b += len(which_b)
                self.assertAllEqual(
                    [99] * len(which_b), [results[0][i] for i in which_b]
                )
            ...
            tf_logging.info(
                "testTwoThreads%s saw both count: %s",
                "Dict" if use_dict else "",
                saw_both,
            )
    ...
    def testMismatchedDictKeys(self):
        with ops.Graph().as_default(), self.assertRaisesRegex(
            ...
            inp.batch_join(
                [{"c": 12, "s": 123, "S": "a"}, {"cool": -12, "s": 99, "S": "b"}],
                batch_size=8,
            )
    ...
    def testTwoThreadsDynamicPad(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            batch_size = 5
            a = array_ops.tile(
                ["a"], math_ops.cast(array_ops.stack([counter + 1]), dtypes.int32)
            )
            b = array_ops.tile(
                ["b"], math_ops.cast(array_ops.stack([ninety_nine]), dtypes.int32)
            )
            batched = inp.batch_join(
    ...
    def testTwoThreadsSmallerBatch(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            self.assertAllEqual(
                results[1].indices,
                np.vstack(
                    (np.arange(2 * extra_elements), np.zeros(2 * extra_elements))
                ).T,
            )
    ...
    def testTwoThreadsDynamicPadSmallerBatch(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            # self.assertGreater(saw_both, 1)
            tf_logging.info(
                "testTwoThreadsDynamicPadSmallerBatch saw both count: %s", saw_both
            )
