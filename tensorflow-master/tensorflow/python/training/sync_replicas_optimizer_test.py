        ...
        with graph.as_default():
            ...
            with ops.device("/job:worker/task:" + str(worker_id)):
                ...
                grads_sparse = ops.IndexedSlices(
                    constant_op.constant([0.1 + worker_id * 0.2], shape=[1, 1]),
                ...
                train_op = [
                    sync_rep_opt.apply_gradients(
                        zip(
                            [grads_0, grads_1, grads_sparse], [var_0, var_1, var_sparse]
                        ),
...
class SyncReplicasOptimizerTest(test.TestCase):
    ...
    def test2Workers(self):
        ...
        sessions, graphs, train_ops = get_workers(
            num_workers, replicas_to_aggregate, workers
        )
        ...
        self.assertAllClose(
            [[3.0], [4.0 - (0.1 + 0.3) / 2 * 2.0]], sessions[1].run(var_sparse_g_1)
        )
        ...
        threads = []
        threads.append(
            self.checkedThread(target=self._run, args=(train_ops[0], sessions[0]))
        )
        threads.append(
            self.checkedThread(target=self._run, args=(train_ops[1], sessions[1]))
        )
...
class SyncReplicasOptimizerHookTest(test.TestCase):
    ...
    def testFetchVariableList(self):
        opt = training.SyncReplicasOptimizer(
            opt=adam.AdamOptimizer(0.01), replicas_to_aggregate=1, total_num_replicas=1
        )
        v = variables.VariableV1([0.0], name="fetch_variable_test")
        global_step = variables.VariableV1(0, name="global_step", trainable=False)
        opt.minimize(v, global_step=global_step)
