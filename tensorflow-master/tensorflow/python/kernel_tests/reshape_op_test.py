...
class ReshapeTest(test.TestCase):
    ...
    def testBoolBasic(self):
        x = np.arange(1.0, 7.0).reshape([1, 6]) > 3
    ...
    def testDoubleBasic(self):
        x = np.arange(1.0, 7.0).reshape([1, 6]).astype(np.float64)
        self._testBothReshape(x, [2, 3])

    def testInt32Basic(self):
        x = np.arange(1.0, 7.0).reshape([1, 6]).astype(np.int32)
        self._testBothReshape(x, [2, 3])

    def testComplex64Basic(self):
        x = np.arange(1.0, 7.0).reshape([1, 6]).astype(np.complex64)
        self._testBothReshape(x, [2, 3])

    def testComplex128Basic(self):
        x = np.arange(1.0, 7.0).reshape([1, 6]).astype(np.complex128)
        self._testBothReshape(x, [2, 3])

    def testFloatReshapeThreeDimensions(self):
        x = np.arange(1.0, 28.0).reshape([1, 27]).astype(np.float32)
        self._testBothReshape(x, [3, 3, 3])

    def testFloatUnspecifiedDimOnly(self):
        x = np.arange(1.0, 7.0).reshape([6]).astype(np.float32)
    ...
    def testFloatReshapeGradThreeDimensions(self):
        x = np.arange(1.0, 25.0).reshape([2, 3, 4]).astype(np.float32)
    ...
    def testErrors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Cannot reshape a tensor with 4096 elements"
        ):
    ...
    def testPartialShapes(self):
        ...
        with ops.Graph().as_default():
            ...
            y = array_ops.reshape(
                x,
                array_ops.concat(
                    [array_ops.placeholder(dtypes.int32, shape=(2,)), [37, 42]], 0
                ),
            )
            ...
            y = array_ops.reshape(
                x,
                array_ops.shape(
                    array_ops.placeholder(dtypes.float32, shape=[None, 37, None])
                ),
            )
    ...
    def testTensorShape(self):
        ...
        y = array_ops.reshape(
            x, [tensor_shape.Dimension(100), tensor_shape.Dimension(1)]
        )
