...
@parameterized.parameters(
    (gen_string_ops.regex_replace), (gen_string_ops.static_regex_replace)
)
class RegexReplaceOpVariantsTest(test.TestCase, parameterized.TestCase):
    ...
    def testForwarding(self, op):
        with self.cached_session():
            ...
            inp = string_ops.substr(
                constant_op.constant(["AbCdEfG", "HiJkLmN"], dtypes.string),
                pos=0,
                len=5,
            )
    ...
    def testRemovePrefix(self, op):
        ...
        with self.cached_session():
            ...
            self.assertAllEqual(
                [b"foo", b"bar", b"foo", b"baz", b"qux", b"ca:b"], stripped
            )
...
class RegexReplaceTest(test.TestCase, parameterized.TestCase):
    @parameterized.parameters(
        (as_string, as_tensor), (as_tensor, as_string), (as_tensor, as_tensor)
    )
    @test_util.run_deprecated_v1
