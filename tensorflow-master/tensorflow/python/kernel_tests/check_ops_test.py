...
class AssertV2Asserts(test.TestCase):
    def test_passes_when_it_should(self):
        ...
        with context.eager_mode():
            ...
            c4 = constant_op.constant(
                [3.0, 3.5], name="three_and_a_half", dtype=dtypes.float32
            )
            scalar = c1
            non_scalar = c3
            integer = c1
            ...
            cases = [
                ...
                (
                    check_ops.assert_type_v2,
                    (c1, dtypes.int32),
                    (c1, dtypes.float32),
                    TypeError,
                ),
                (check_ops.assert_integer_v2, (integer,), (non_integer,), TypeError),
                (check_ops.assert_scalar_v2, (scalar,), (non_scalar,), ValueError),
                (check_ops.assert_rank_v2, (c1, 0), (c3, 2), ValueError),
                (check_ops.assert_rank_in_v2, (c1, [0, 1]), (c1, [1, 2]), ValueError),
                (
                    check_ops.assert_rank_at_least_v2,
                    (non_scalar, 1),
                    (scalar, 1),
                    ValueError,
                ),
        ...
        with context.eager_mode():
            ...
            with self.assertRaisesRegex(
                ...
                check_ops.assert_equal(
                    big, small, message="big does not equal small", summarize=10
                )
            with self.assertRaisesRegex(
            ...
            with self.assertRaisesRegex(
                ...
                check_ops.assert_equal(
                    big, small, message="big does not equal small", summarize=2
                )
...
class AssertAllCloseTest(test.TestCase):
    ...
    def test_doesnt_raise_when_close_enough_32_bit_due_to_default_rtol(self):
        ...
        with ops.control_dependencies(
            [check_ops.assert_near(x, y, atol=0.0, message="failure message")]
        ):
            out = array_ops.identity(x)
            self.evaluate(out)
    ...
    def test_doesnt_raise_when_close_enough_32_bit_due_to_default_atol(self):
        ...
        with ops.control_dependencies(
            [check_ops.assert_near(x, y, rtol=0.0, message="failure message")]
        ):
            out = array_ops.identity(x)
            self.evaluate(out)
    ...
    def test_doesnt_raise_when_close_enough_due_to_custom_rtol(self):
        ...
        with ops.control_dependencies(
            [check_ops.assert_near(x, y, atol=0.0, rtol=0.5, message="failure message")]
        ):
            out = array_ops.identity(x)
            self.evaluate(out)
    ...
    def test_doesnt_raise_when_close_enough_due_to_custom_atol(self):
        ...
        with ops.control_dependencies(
            [check_ops.assert_near(x, y, atol=0.5, rtol=0.0, message="failure message")]
        ):
            out = array_ops.identity(x)
            self.evaluate(out)
    ...
    def test_raises_when_atol_violated(self):
        ...
        with self.assertRaisesOpError(  # pylint:disable=g-error-prone-assert-raises
            ...
            with ops.control_dependencies(
                [check_ops.assert_near(x, y, atol=0.1, message="failure message")]
            ):
                out = array_ops.identity(x)
                self.evaluate(out)
...
class AssertLessTest(test.TestCase):
    ...
    def test_raises_when_equal(self):
        ...
        with self.assertRaisesOpError(  # pylint:disable=g-error-prone-assert-raises
            ...
            with ops.control_dependencies(
                [check_ops.assert_less(small, small, message="failure message")]
            ):
                out = array_ops.identity(small)
            self.evaluate(out)
...
class AssertLessEqualTest(test.TestCase):
    ...
    def test_raises_when_greater(self):
        ...
        with self.assertRaisesOpError(  # pylint:disable=g-error-prone-assert-raises
            ...
            with ops.control_dependencies(
                [check_ops.assert_less_equal(big, small, message="fail")]
            ):
                out = array_ops.identity(small)
            self.evaluate(out)
...
class AssertGreaterEqualTest(test.TestCase):
    ...
    def test_raises_when_less(self):
        ...
        with self.assertRaisesOpError(  # pylint:disable=g-error-prone-assert-raises
            ...
            with ops.control_dependencies(
                [check_ops.assert_greater_equal(small, big, message="fail")]
            ):
                out = array_ops.identity(small)
            self.evaluate(out)
...
class AssertPositiveTest(test.TestCase):
    ...
    def test_raises_when_negative(self):
        ...
        with self.assertRaisesOpError("fail"):
            with ops.control_dependencies(
                [check_ops.assert_positive(freddie, message="fail")]
            ):
...
class EnsureShapeTest(test.TestCase):
    ...
    # Dynamic shape check
    @test_util.run_deprecated_v1
    @test_util.disable_xla("b/123337890")  # Dynamic shapes not supported now with XLA
...
class EnsureShapeBenchmark(test.Benchmark):
    ...
    def benchmark_n_ensure_ops(self):
        ...
        n_ensure_ops = self._apply_n_times(
            lambda x: check_ops.ensure_shape(array_ops.identity(x), shape), input_op
        )
...
class AssertRankTest(test.TestCase):
    ...
    def test_rank_zero_tensor_raises_if_rank_too_small_static_rank(self):
        ...
        with self.assertRaisesRegex(ValueError, "fail.*must have rank 1"):
            with ops.control_dependencies(
                [check_ops.assert_rank(tensor, desired_rank, message="fail")]
            ):
            ...
            for desired_ranks in ((0, 1, 2), (1, 0, 2), (1, 2, 0)):
                with ops.control_dependencies(
                    ...
                    array_ops.identity(tensor_rank1).eval(
                        feed_dict={tensor_rank1: (42.0, 43.0)}
                    )
            ...
            with self.assertRaisesOpError("Rank must be a scalar"):
                with ops.control_dependencies(
                    ...
                    array_ops.identity(tensor).eval(
                        feed_dict={desired_ranks[0]: 1, desired_ranks[1]: [2, 1]}
                    )
...
class AssertTypeTest(test.TestCase):
    ...
    def test_sparsetensor_doesnt_raise_when_correct_type(self):
        ...
        with ops.control_dependencies(
            ...
            out = sparse_tensor.SparseTensor(
                sparse_float.indices,
...
class AssertShapesTest(test.TestCase):
    ...
    def test_raises_static_incorrect_rank(self):
        ...
        raises_static_rank_error(
            rank_three_shapes, array_ops.ones([1, 1]), correct_rank=3, actual_rank=2
        )
        raises_static_rank_error(
...
class AssertShapesSparseTensorTest(test.TestCase):
    ...
    def test_assert_shapes_sparse_tensor_nonscalar_target_fail(self):
        ...
        with self.assertRaisesRegexp(ValueError, r"must have rank 2.*Received rank 0"):
    ...
    def test_assert_shapes_sparse_tensor_wrong_rank_fail(self):
        ...
        with self.assertRaisesRegexp(
            ValueError, r"must have rank 3\..* Received rank 2"
        ):
...
class FloatDTypeTest(test.TestCase):
    ...
    def test_assert_same_float_dtype(self):
        ...
        self.assertIs(
            dtypes.float32, check_ops.assert_same_float_dtype([], dtypes.float32)
        )
        self.assertIs(
            dtypes.float32, check_ops.assert_same_float_dtype(None, dtypes.float32)
        )
        self.assertIs(
            dtypes.float32, check_ops.assert_same_float_dtype([None, None], None)
        )
        self.assertIs(
            dtypes.float32,
        ...
        self.assertRaises(
            ValueError, check_ops.assert_same_float_dtype, [const_float], dtypes.int32
        )
        ...
        self.assertIs(
            dtypes.float32,
            check_ops.assert_same_float_dtype([sparse_float], dtypes.float32),
        )
        self.assertRaises(
            ValueError, check_ops.assert_same_float_dtype, [sparse_float], dtypes.int32
        )
        self.assertRaises(
            ValueError,
            check_ops.assert_same_float_dtype,
            [const_float, None, sparse_float],
            dtypes.float64,
        )

        self.assertIs(
            dtypes.float32,
            check_ops.assert_same_float_dtype([const_float, sparse_float]),
        )
        self.assertIs(
            dtypes.float32,
            check_ops.assert_same_float_dtype(
        ...
        self.assertRaises(
            ValueError, check_ops.assert_same_float_dtype, [sparse_float, const_int]
        )
        self.assertRaises(
            ValueError,
            check_ops.assert_same_float_dtype,
