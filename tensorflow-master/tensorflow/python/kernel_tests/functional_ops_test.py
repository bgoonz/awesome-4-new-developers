...
def simple_scoped_fn(a, x):
    ...
    with variable_scope.variable_scope("body"):
        ...
        two = variable_scope.get_variable(
            "two", [], dtype=dtypes.int32, initializer=init_ops.constant_initializer(2)
        )
...
class FunctionalOpsTest(test.TestCase):
    ...
    def testFoldl_Simple(self):
        ...
        r = functional_ops.foldl(
            lambda a, x: math_ops.multiply(math_ops.add(a, x), 2), elems, initializer=10
        )
    ...
    def testFoldl_MultiInputSingleOutput(self):
        ...
        r = functional_ops.foldl(
            lambda a, x: a + x[0] + x[1], (elems, -elems), initializer
        )
    ...
    def testFoldl_MultiInputDifferentDimsSingleOutput(self):
        ...
        r = functional_ops.foldl(
            lambda a, x: a + x[0] * x[1], (elems, other_elems), initializer
        )
    ...
    def testFoldl_Scoped(self):
        with self.cached_session() as sess:
            with variable_scope.variable_scope("root") as varscope:
                ...
                self.assertEqual(
                    variables.trainable_variables()[0].name, "root/body/two:0"
                )
    ...
    def testScan_Reverse(self):
        ...
        # pylint: disable=unnecessary-lambda
        r = functional_ops.scan(
            lambda a, x: math_ops.multiply(a, x), elems, reverse=True
        )
        self.assertAllEqual([720.0, 720.0, 360.0, 120.0, 30.0, 6.0], self.evaluate(r))
        r = functional_ops.scan(
            lambda a, x: math_ops.multiply(a, x), elems, initializer=v, reverse=True
        )
        self.assertAllEqual(
            [1440.0, 1440.0, 720.0, 240.0, 60.0, 12.0], self.evaluate(r)
        )
        # pylint: enable=unnecessary-lambda
    ...
    def testScan_Control(self):
        with self.cached_session() as sess:
            ...
            self.assertAllClose(
                np.array([1.0, 3.0, 9.0]), sess.run(c, {s: [1, 3, 3], b: True})
            )
    ...
    def testScanGradientWithPartStopGradient(self):
        ...
        l0, l1 = functional_ops.scan(
            lambda elem_, input_: (a, b), elems, initializer=(0.0, 0.0)
        )
    ...
    def testScanVaryingShape(self):
        with self.cached_session() as sess:
            ...
            sess.run(
                [result, result_t, result_grad, result_t_grad],
                feed_dict={x: [[1.0, 2.0]]},
            )
        ...
        with ops.device("/job:localhost/replica:0/task:0/device:GPU:0"):
            remote_op = (
                functional_ops.remote_call(
                    ...
                    Tout=[dtypes.float32],
                    f=_remote_fn,
                    target="/job:localhost/replica:0/task:0/cpu:0",
                )[0]
                + 3.0
            )
        ...
        with ops.device("/job:worker/replica:0/task:0/cpu:0"):
            remote_op = (
                functional_ops.remote_call(
                    ...
                    Tout=[dtypes.float32],
                    f=_remote_fn,
                    target="/job:worker/replica:0/task:1/cpu:0",
                )[0]
                + 3.0
            )
    ...
    def testRemoteFunctionCancellation(self):
        ...
        with ops.device("/cpu:0"):
            collective_ops.all_reduce_v2(
                [1.0], group_size=2, group_key=1, instance_key=1
            )
        ...
        def Run(n, fetch_by_name):
            for use_gpu in (True, False):
                with ops.Graph().as_default() as g:
                    ...
                    # outputs: [0, n*(n+1)/2]
                    outputs = functional_ops.While(
                        [n, 0.0], Cond, Body, name="my_while"
                    )
                    ...
                    outputs[0].op._set_attr(
                        "_lower_using_switch_merge", attr_value_pb2.AttrValue(b=True)
                    )
            ...
            with ops.Graph().as_default() as g:
                ...
                with self.session(graph=g, use_gpu=use_gpu):
                    with self.assertRaisesRegex(
                        ...
                        functional_ops.While([5.0, 0.0], CondReturnsTooManyArgs, Body)[
                            0
                        ].eval()
                    with self.assertRaisesRegex(
                        errors.InvalidArgumentError,
                        ...
                        functional_ops.While([5.0, 0.0], Cond, BodyReturnsTooManyArgs)[
                            0
                        ].eval()
        ...
        for use_gpu in (True, False):
            with ops.Graph().as_default() as g:
                ...
                with self.session(graph=g, use_gpu=use_gpu) as sess:
                    ...
                    self.assertAllEqual(
                        5050.0, sess.run([result, c], feed_dict={n: 100.0})[0]
                    )
        ...
        with ops.Graph().as_default() as g:
            with self.session(graph=g, use_gpu=use_gpu) as sess:
                ...
                xs = [
                    # 1 + 2  + ... + 20
                    functional_ops.For(
                        1, 21, 1, [0.0], Body, rewrite_with_while=rewrite_with_while
                    )[0],
                    # 100 + 99 + ... + 1
                    functional_ops.For(
                        100, 0, -1, [0.0], Body, rewrite_with_while=rewrite_with_while
                    )[0],
                ]
    ...
    def testForWithWhileNaming(self):
        ...
        with g.as_default():
            ...
            _ = functional_ops.For(1, 21, 1, [0.0], TestBody, rewrite_with_while=True)[
                0
            ]
        ...
        for rewrite_with_while in (True, False):
            ...
            with self.test_session(use_gpu=use_gpu) as sess:
                result_nullary = functional_ops.For(
                    1, 10, 1, [], TestNullary, rewrite_with_while=rewrite_with_while
                )
                result_unary = functional_ops.For(
                    1, 10, 1, [0.0], TestUnary, rewrite_with_while=rewrite_with_while
                )
    ...
    def testForError(self):
        ...
        with self.test_session():
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "must be a scalar"
            ):
                functional_ops.For([0], 10, 1, [0.0], Foo)[0].eval()
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "Invalid start/limit/delta"
            ):
                functional_ops.For(0, 10, -1, [0.0], Foo)[0].eval()
            with self.assertRaisesRegex(
                errors.InvalidArgumentError,
...
class PartitionedCallTest(test.TestCase):
        ...
        with session.Session(workers[0].target) as sess:
            with ops.device(worker0_device):
                a = variable_scope.get_variable(
                    "a", initializer=constant_op.constant(1.0), use_resource=True
                )
            with ops.device(worker1_device):
                b = variable_scope.get_variable(
                    "b", initializer=constant_op.constant(1.0), use_resource=True
                )
    ...
    def testBasicSingleDevice(self):
        ...
        output, = self.evaluate(
            functional_ops.partitioned_call(
                args=[constant_op.constant(1.0), constant_op.constant(2.0)], f=Body
            )
        )
    ...
    def testBasicMultiDevice(self):
        ...
        with self.test_session(config=config):
            ...
            self.assertEqual(self.evaluate(output), 12.0)
    ...
    def testFunctionWithResourcesOnDifferentDevices(self):
        ...
        with self.test_session(
            ...
            result = sess.run(
                functional_ops.partitioned_call(args=defined.captured_inputs, f=defined)
            )
    ...
    def testCase(self):
        ...
        def f(branch, x):
            ...
            return array_ops.identity(
                gen_functional_ops.case(
                    branch,
                    input=[x],
                    Tout=[dtypes.float32],
                    branches=[
                        f.get_concrete_function(tmpl) for f in (two, three, four)
                    ],
                )[0]
            )
        ...
        for use_gpu in (True, False):
            ...
            def Run(branch, x, fetch_by_name, use_gpu=use_gpu):
                with ops.Graph().as_default() as g:
                    ...
                    outputs = gen_functional_ops.case(
                        branch,
                        input=[x],
            ...
            self.assertAllEqual(4 * -4.0, Run(7, -4.0, False))  # >= N default
            self.assertAllEqual(4 * -4.0, Run(7, -4.0, True))  # >= N default
            self.assertAllEqual(4 * -5.0, Run(-1, -5.0, False))  # <0 default
            self.assertAllEqual(4 * -5.0, Run(-1, -5.0, True))  # <0 default
    ...
    def testCaseLowering(self):
        for use_gpu in (True, False):
            ...
            with ops.device(test.gpu_device_name() if use_gpu else "CPU:0"):
                ...
                self.assertAllEqual(
                    4 * -4.0, self.evaluate(Run(7, -4.0))
                )  # >=N default
                self.assertAllEqual(
                    4 * -5.0, self.evaluate(Run(-1, -5.0))
                )  # <0 default
