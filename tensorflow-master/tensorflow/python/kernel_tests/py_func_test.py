...
class PyFuncTest(PyFuncTestBase):
    ...
    def testArray(self):
        with self.cached_session():
            ...
            self.assertAllEqual(
                z[0], np_func([1.0, 2.0], [2.0, 3.0]).astype(np.float64)
            )
    ...
    def testTuple(self):
        ...
        with self.cached_session():
            x = constant_op.constant(0.0, dtypes.float64)
            y = self.evaluate(
                script_ops.py_func(tuple_func, (x,), (dtypes.float64, dtypes.float64))
            )
            self.assertAllClose(y, [0.0, 1.0])
    ...
    def testStrings(self):
        ...
        with self.cached_session():
            ...
            y = self.evaluate(
                script_ops.py_func(read_fixed_length_numpy_strings, [], dtypes.string)
            )
            z = self.evaluate(
    ...
    def testBadNumpyReturnType(self):
        with self.cached_session():
            ...
            with self.assertRaisesRegex(
                errors.InternalError, "Unsupported numpy data type"
            ):
                self.evaluate(y)
    ...
    def testBadReturnType(self):
        with self.cached_session():
            ...
            with self.assertRaisesRegex(
                errors.InternalError, "Unsupported object type"
            ):
                self.evaluate(z)
    ...
    def testCOrder(self):
        with self.cached_session():
            val = [[1, 2], [3, 4]]
            x, = script_ops.py_func(
                lambda: np.array(val, order="F"), [], [dtypes.int64]
            )
...
class EagerPyFuncTest(PyFuncTestBase):
    ...
    def testEagerReturningVariableRaisesError(self):
        ...
        with self.assertRaisesRegex(
            errors.UnknownError, "Attempting to return a variable"
        ):
            output = script_ops.eager_py_func(
    ...
    def testEagerGradientGraphTwoOutputs(self):
        ...
        fa, fb = script_ops.eager_py_func(
            f, inp=[x, y], Tout=[dtypes.float32, dtypes.float32]
        )
