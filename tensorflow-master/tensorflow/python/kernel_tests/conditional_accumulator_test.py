...
class ConditionalAccumulatorTest(test.TestCase):
    ...
    def testAccumulatorApplyAndTakeGradWithShape(self):
        with self.cached_session():
            ...
            elems_ave = [
                [(a + b) / len(elems) for a, b in zip(x, y)]
    ...
    def testAccumulatorDynamicShape(self):
        with self.cached_session() as sess:
            ...
            elems_ave = [
                [(a + b) / len(elems) for a, b in zip(c, d)]
    ...
    def testAccumulatorSetGlobalStepPreventsAccumulation(self):
        with self.cached_session():
            ...
            for ls in local_steps:
                ...
                self.assertEqual(
                    0.0
                    + sum(x for x in local_steps if x >= ls)
                    / sum(1 for x in local_steps if x >= ls),
                    val,
                )
    ...
    def testParallelApplyGrad(self):
        ...
        with self.cached_session() as sess:
            ...
            threads = [
                self.checkedThread(target=apply_grad, args=(o,)) for o in accum_ops
            ]
