...
) + combinations.combine(device="GPU", communication=["RING", "NCCL"], required_gpus=1)
...
two_worker_pool_runner = multi_process_runner.MultiProcessPoolRunner(
    ...
    initializer=lambda: enable_collective_ops(
    ...
    def testAbortInstanceParamsResolution(self, device, communication):
        ...
        with ops.device(dev0):
            if cluster_resolver.task_id == 0:
                collective_ops.broadcast_send(
                    in_tensor, (1,), dtypes.float32, group_size, group_key, instance_key
                )
            else:
