...
class LeakyReluTest(test.TestCase):
    ...
    def testNaNPropagation(self):
        for t in [np.float16, np.float32, np.float64]:
            self._testLeakyRelu(np.array([-1, np.nan, 1, np.nan]).astype(t), alpha=0.2)
    ...
    def testUnexpectedAlphaValue(self):
        self.assertAllClose(
            ...
            nn_ops.leaky_relu(
                np.array([[-0.9, 0.7, -0.5, 0.3, -0.01], [0.1, -0.3, 0.5, -2.7, 0.9]]),
                alpha=10,
            ),
        )
        self.assertAllClose(
...
class EluTest(test.TestCase):
    ...
    def testNpElu(self):
        self.assertAllClose(
            np.array(
                [
                    [-0.59343034025, 0.7, -0.39346934028, 0.3, -0.09516258196],
...
class SeluTest(test.TestCase):
    def _npSelu(self, np_features):
        ...
        return np.where(
            np_features < 0,
            scale_alpha * (np.exp(np_features) - 1),
            scale * np_features,
        )
    ...
    def testNpSelu(self):
        self.assertAllClose(
            np.array(
                [
                    [-1.0433095, 0.73549069, -0.6917582, 0.3152103, -0.16730527],
    ...
    def testNumbers(self):
        for t in [np.float16, np.float32, np.float64]:
            self._testSelu(np.array([[-9, 7, -5, 3, -1], [1, -3, 5, -7, 9]]).astype(t))
...
class CreluTest(test.TestCase):
    ...
    def _testCrelu(self, np_features):
        ...
        np_crelu = np.concatenate((np_relu, np_neg_relu), len(np_features.shape) - 1)
    ...
    def testNumbersGPU(self):
        if not test.is_gpu_available():
            self.skipTest("No GPU available")
        for t in [np.float16, np.float32, np.float64]:
            self._testCrelu(np.array([[-9, 7, -5, 3, -1], [1, -3, 5, -7, 9]]).astype(t))
    ...
    def testNumbersWithAxis0(self):
        ...
        np_crelu = np.array(
            [[0, 7, 0, 3, 0], [1, 0, 5, 0, 9], [9, 0, 5, 0, 1], [0, 3, 0, 7, 0]]
        )
    ...
    def testNumbersWithAxis1(self):
        ...
        np_crelu = np.array(
            [[0, 7, 0, 3, 0, 9, 0, 5, 0, 1], [1, 0, 5, 0, 9, 0, 3, 0, 7, 0]]
        )
        self.assertAllEqual(np_crelu, tf_crelu)
