...
class SpectralOpsTest(test.TestCase, parameterized.TestCase):
    ...
    def _np_inverse_stft(stft, fft_length, hop_length, window_length):
        ...
        frames = np.pad(
            frames,
            [[0, 0]] * (frames.ndim - 1)
            + [[0, max(0, window_length - frames.shape[-1])]],
            "constant",
        )
        window = SpectralOpsTest._np_hann_periodic_window(window_length)
    ...
    def test_shapes(self):
        ...
        stft = spectral_ops.stft(
            signal, frame_length=8, frame_step=8, fft_length=16, pad_end=True
        )
        self.assertAllEqual([64, 9], stft.shape.as_list())
        self.assertAllEqual([64, 9], self.evaluate(stft).shape)

        stft = spectral_ops.stft(
            signal, frame_length=16, frame_step=8, fft_length=8, pad_end=True
        )
        self.assertAllEqual([64, 5], stft.shape.as_list())
        self.assertAllEqual([64, 5], self.evaluate(stft).shape)
        ...
        inverse_stft = spectral_ops.inverse_stft(
            stft, frame_length=8, fft_length=16, frame_step=8
        )
    ...
    def test_stft_and_inverse_stft(
        self, signal_length, frame_length, frame_step, fft_length, np_rtype, tol
    ):
    ...
    def test_mdct_round_trip(
        self, signal_length, frame_length, np_rtype, norm, window_type, pad_end
    ):
        ...
        if window_type == "vorbis":
            ...
            window_fn = None
        mdct = spectral_ops.mdct(
            signal, frame_length, norm=norm, window_fn=window_fn, pad_end=pad_end
        )
