    ...
    def testBijectorDynamicEventNdims(self):
        ...
        with self.cached_session():
            with self.assertRaisesOpError("Expected scalar"):
                bij.forward_log_det_jacobian(1.0, event_ndims=event_ndims).eval(
                    {event_ndims: (1, 2)}
                )
            with self.assertRaisesOpError("Expected scalar"):
                bij.inverse_log_det_jacobian(1.0, event_ndims=event_ndims).eval(
                    {event_ndims: (1, 2)}
                )
...
class BijectorReduceEventDimsTest(test.TestCase):
    ...
    def testReduceEventNdimsForward(self):
        ...
        bij = ExpOnlyJacobian()
        self.assertAllClose(
            np.log(x), self.evaluate(bij.forward_log_det_jacobian(x, event_ndims=0))
        )
        self.assertAllClose(
    ...
    def testReduceEventNdimsInverse(self):
        ...
        bij = ExpOnlyJacobian()
        self.assertAllClose(
            -np.log(x), self.evaluate(bij.inverse_log_det_jacobian(x, event_ndims=0))
        )
        self.assertAllClose(
            np.sum(-np.log(x), axis=-1),
    ...
    def testReduceEventNdimsForwardConstJacobian(self):
        ...
        self.assertAllClose(
            -2.0, self.evaluate(bij.forward_log_det_jacobian(x, event_ndims=0))
        )
        self.assertAllClose(
            -4.0, self.evaluate(bij.forward_log_det_jacobian(x, event_ndims=1))
        )
        self.assertAllClose(
            -8.0, self.evaluate(bij.forward_log_det_jacobian(x, event_ndims=2))
        )
    ...
    def testReduceEventNdimsInverseConstJacobian(self):
        ...
        bij = ConstantJacobian()
        self.assertAllClose(
            2.0, self.evaluate(bij.inverse_log_det_jacobian(x, event_ndims=0))
        )
        self.assertAllClose(
            4.0, self.evaluate(bij.inverse_log_det_jacobian(x, event_ndims=1))
        )
        self.assertAllClose(
            8.0, self.evaluate(bij.inverse_log_det_jacobian(x, event_ndims=2))
        )
