...
def RandMatrix(rows, cols, tr, round_bfloat=False):
    ...
    return (
        (np.clip(rand_func(low=-256.0, high=256.0, size=rows * cols), -64, 64) / 128.0)
        .reshape([rows, cols])
        .astype(np.float32)
    )
    ...
    def _testGradients(self, tr_a, tr_b, sp_a, sp_b, a_dtype, b_dtype, delta, name):
        with self.cached_session():
            a = constant_op.constant(
                RandMatrix(3, 2, tr_a, round_bfloat=True), dtype=dtypes.float32
            )
            b = constant_op.constant(
                RandMatrix(2, 4, tr_b, round_bfloat=True), dtype=dtypes.float32
            )
                        ...
                        for a_dtype in (dtypes.float32, dtypes.bfloat16):
                            for b_dtype in (dtypes.float32, dtypes.bfloat16):
                                ...
                                if (
                                    ...
                                    and b_dtype == dtypes.bfloat16
                                ):
                                    delta = 1 / 16.0
                                else:
                                    delta = 1 / 64.0
                                name = "sparse_matmul_%s_%s_%s_%s" % (
                                    tr_a,
                                    tr_b,
                                    sp_a,
                                    sp_b,
