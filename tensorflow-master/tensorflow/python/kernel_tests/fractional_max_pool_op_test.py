...
class FractionalMaxPoolTest(test.TestCase):
    ...
    """
        ...
        for i in range(row_seq.shape[0] - 1):
            ...
            output_image = np.vstack(
                (output_image, np.amax(input_matrix[row_start:row_end, :], axis=0))
            )  # axis 0 is along row
    ...
    def _GetExpectedFractionalMaxPoolResult(
    ...
    def _ValidateFractionalMaxPoolResult(
    ...
    """
        ...
        for overlapping in True, False:
            ...
            with self.cached_session() as sess:
                p, r, c = nn_ops.fractional_max_pool_v2(
                    rand_mat, pooling_ratio, pseudo_random, overlapping, seed=self._SEED
                )
                tensor_output, row_seq, col_seq = self.evaluate([p, r, c])
                expected_result = self._GetExpectedFractionalMaxPoolResult(
                    rand_mat, row_seq, col_seq, overlapping
                )
                print("row sequence:")
                print(row_seq)
    ...
    """
        ...
        self._ValidateFractionalMaxPoolResult(
            rand_mat, [1, math.sqrt(3), math.sqrt(2), 1], pseudo_random, overlapping
        )
...
class FractionalMaxPoolGradTest(test.TestCase):
  ...
  """
                ...
                for col_window_size in [2, 4]:
                    ...
                    for num_channels in [1, 2]:
                        ...
                        with self.cached_session() as _:
                            ...
                            fmp_input_backprop = self.evaluate(
                                fmp_input_backprop_tensor
                            )
                            self.assertShapeEqual(
                                input_backprop, fmp_input_backprop_tensor
                            )
    ...
    """
        ...
        input_size = [1, 4, 4, 1]
        output_backprop = [12.0, 15.0, 17.0, -5.0, 6.0, 21.0]  # pyformat: disable
        row_seq = [0, 1, 3, 4]
        col_seq = [0, 2, 4]
        output_data_not_overlapping = [
            5.0,
            7.0,
            8.0,
            9.0,
        ...
        output_data_overlapping = [9.0, 9.0, 9.0, 9.0, 7.0, 0.0]  # pyformat: disable
        output_size = [1, 3, 2, 1]
        ...
        with self.cached_session() as _:
            ...
            r = gen_nn_ops.fractional_max_pool_grad(
                input_tensor, output_tensor, grad, row_seq, col_seq, overlapping=False
            )
            ...
            self.assertAllClose(
                expected_input_backprop_not_overlapping, input_backprop_not_overlapping
            )
            ...
            self.assertAllClose(
                expected_input_backprop_overlapping, input_backprop_overlapping
            )
