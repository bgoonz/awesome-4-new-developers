    ...
    def Test(self):
        def RandomInput():
            ...
            if dtype_ in [np.complex64, np.complex128]:
                a += 1j * np.random.uniform(low=-1.0, high=1.0, size=shape_).astype(
                    dtype_
                )
            return a
                    ...
                    for cols in 0, 1, 2, 5, 10, 32, 100:
                        for batch_dims in [(), (3,)] + [(3, 2)] * (
                            ...
                            for use_static_shape in set([True, False]):
                                name = (
                                    "%s_%s_static_shape_%s__compute_uv_%s_full_%s"
                                    % (
                                        dtype.__name__,
                                        "_".join(map(str, full_shape)),
                                        use_static_shape,
                                        compute_uv,
                                        full_matrices,
                                    )
                ...
                for mat_shape in mat_shapes:
                    for batch_dims in [(), (3,)]:
                        ...
                        _AddTest(
                            ...
                            name,
                            _GetSvdGradOpTest(
                                dtype, full_shape, compute_uv, full_matrices
                            ),
                        )
