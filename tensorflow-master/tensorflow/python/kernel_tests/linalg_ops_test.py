                ...
                with self.session():
                    ...
                    for k in range(1, 3):
                        with self.subTest(n=n, np_type=np_type, atol=atol, k=k):
                            ...
                            self.assertAllClose(
                                rhs, math_ops.matmul(array, x), atol=atol
                            )
            ...
            for np_dtype, atol in [
                ...
                with self.subTest(n=n, np_dtype=np_dtype, atol=atol):
                    ...
                    with self.session():
                        ...
                        self.assertAllClose(
                            logdet_np, self.evaluate(logdet_tf), atol=atol
                        )
...
class EyeTest(parameterized.TestCase, test.TestCase):
    ...
    def testShapeInferenceStaticBatch(self):
        batch_shape = (2, 3)
        self.assertEqual(
            (2, 3, 2, 2), linalg_ops.eye(num_rows=2, batch_shape=batch_shape).shape
        )
        self.assertEqual(
            (2, 3, 2, 3),
            linalg_ops.eye(num_rows=2, num_columns=3, batch_shape=batch_shape).shape,
        )
    ...
    def testShapeInferenceStaticBatchWith(self, num_rows_fn, num_columns_fn):
        ...
        identity_matrix = linalg_ops.eye(
            num_rows=num_rows, num_columns=num_columns, batch_shape=batch_shape
        )
    ...
    def test_eye_no_placeholder(self, num_rows, num_columns, batch_shape, dtype):
        ...
        eye_tf = self.evaluate(
            linalg_ops.eye(
                num_rows, num_columns=num_columns, batch_shape=batch_shape, dtype=dtype
            )
        )
...
class _MatrixRankTest(object):
    def test_batch_default_tolerance(self):
        x_ = np.array(
            [
                [[2, 3, -2], [-1, 1, -2], [3, 2, 0]],  # = row2+row3
                [[0, 2, 0], [0, 1, 0], [0, 3, 0]],  # = 2*row2  # = 3*row2
                [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
            ],
            self.dtype,
        )
    ...
    def test_custom_tolerance_broadcasts(self):
        ...
        self.assertAllEqual(
            [3, 2, 1, 0],
            self.evaluate(linalg.matrix_rank(a, tol=[[0.09], [0.19], [0.29], [0.31]])),
        )

    def test_nonsquare(self):
        x_ = np.array(
            [
                [[2, 3, -2, 2], [-1, 1, -2, 4], [3, 2, 0, -2]],  # = row2+row3
                [[0, 2, 0, 6], [0, 1, 0, 3], [0, 3, 0, 9]],  # = 2*row2
            ],  # = 3*row2
            self.dtype,
        )
        x = array_ops.placeholder_with_default(
...
class _PinvTest(object):
    ...
    def test_symmetric(self):
        ...
        a_ = np.stack([a_ + 1.0, a_], axis=0)  # Batch of matrices.
        ...
        if self.use_default_rcond:
            ...
            rcond = self.dtype([0.0, 0.01])  # Smallest 1 component is forced to zero.
    ...
    def test_nonsquare(self):
        a_ = self.dtype(
            [[1.0, 0.4, 0.5, 1.0], [0.4, 0.2, 0.25, 2.0], [0.5, 0.25, 0.35, 3.0]]
        )
...
class _LUReconstruct(object):
    ...
    def test_non_batch(self):
        ...
        self.assertAllClose(x_, y_, atol=0.0, rtol=1e-3)
...
class _LUMatrixInverse(object):
    ...
    def test_non_batch(self):
        ...
        if self.use_static_shape:
            self.assertAllEqual(x_.shape, y.shape)
        self.assertAllClose(np.linalg.inv(x_), y_, atol=0.0, rtol=1e-3)

    def test_batch(self):
        x_ = np.array(
            [[[1, 2], [3, 4]], [[7, 8], [3, 4]], [[0.25, 0.5], [0.75, -2.0]]],
            dtype=self.dtype,
        )
        x = array_ops.placeholder_with_default(
            x_, shape=x_.shape if self.use_static_shape else None
...
class _LUSolve(object):
    ...
    def test_non_batch(self):
        ...
        self.assertAllClose(expected_, y_, atol=0.0, rtol=1e-3)
...
class EighTridiagonalTest(test.TestCase, parameterized.TestCase):
    ...
    def check_orthogonality(self, eigvectors, tol):
        ...
        vtv = math_ops.matmul(eigvectors, eigvectors, adjoint_a=True) - linalg.eye(
