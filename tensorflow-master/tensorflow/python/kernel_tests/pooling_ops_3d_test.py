...
class PoolingTest(test.TestCase):
    ...
    """
        ...
        input_data = 1.0
        input_tensor = constant_op.constant(input_data, shape=input_sizes, name="input")
    ...
    def _ConstructAndTestGradient(self, pool_func, **kwargs):
        """Runs _ConstructAndTestGradientForConfig for all tests configurations."""

        for data_format, use_gpu in GetTestConfigs():
            self._ConstructAndTestGradientForConfig(
                pool_func, data_format=data_format, use_gpu=use_gpu, **kwargs
            )
