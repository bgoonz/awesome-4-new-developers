...
def get_device():
    ...
    for device_type in ("XLA_GPU", "GPU", "XLA_CPU", "CPU"):
        ...
        if devices:
            ...
            return _cached_device
    raise ValueError(
        "Cannot find any suitable device. Available devices: %s"
...
def float_cases(shape_dtypes=(None,)):
    ...
    def wrap(op, dtype, shape, shape_dtype, seed, **kwargs):
        ...
        if (
            ...
            and device_type in ("XLA_GPU", "XLA_CPU")
            or dtype == dtypes.bfloat16
            and device_type == "GPU"
        ):
            dtype = dtypes.float32
        shape_ = (
            constant_op.constant(shape, dtype=shape_dtype)
            ...
            for shape in (), (3,), (2, 5):
                for name, stateless_op, stateful_op, kwargs in cases:
                    yield (
                        (
                            "%s_%s_%s_%s"
                            % (name, _name(dtype), shape, _name(shape_dtype))
                        ).replace(" ", ""),
                        functools.partial(
                            wrap, stateless_op, dtype, shape, shape_dtype, **kwargs
                        ),
                        functools.partial(
                            wrap, stateful_op, dtype, shape, shape_dtype, **kwargs
                        ),
                    )
...
def int_cases(shape_dtypes=(None,), minval_maxval=None):
    def wrap(op, minval, maxval, shape, shape_dtype, dtype, seed, **kwargs):
        ...
        return op(
            seed=seed, shape=shape_, minval=minval, maxval=maxval, dtype=dtype, **kwargs
        )
        ...
        for output_dtype in dtypes.int32, dtypes.int64:
            for logits in (
                ...
                yield (
                    ...
                    functools.partial(
                        wrap,
                        stateless.stateless_multinomial,
                        logits,
                        logits_dtype,
                        output_dtype,
                    ),
                    functools.partial(
                        wrap, random_ops.multinomial, logits, logits_dtype, output_dtype
                    ),
                )
        ...
        for alpha in ([[0.5, 1.0, 2.0]], [[0.5, 0.5], [0.8, 0.2], [0.25, 0.75]]):
            yield (
                ...
                functools.partial(
                    ...
                    alpha,
                    dtype,
                    (10,) + tuple(np.shape(alpha)),
                ),
                functools.partial(wrap, random_ops.random_gamma, alpha, dtype, (10,)),
            )
...
class StatelessOpsTest(test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        (
            "_%s_%s_%s" % (case[0], case_id, seed_id),
            case,
            seed,
    ...
    def testMatchFloat(self, case, seed):
        if get_device().device_type in ("XLA_GPU", "XLA_CPU"):
            ...
            # CPU kernels.
            self.skipTest(
                "Skip on XLA because XLA kernels do not support int64 "
    ...
    @parameterized.named_parameters(
        (
            "_%s_%s_%s" % (case[0], seed_type.name, case_id),
            case,
            seed_type,
        )  # pylint: disable=g-complex-comprehension
    ...
    def testDeterminismFloat(self, case, seed_type):
        if seed_type == dtypes.int64 and get_device().device_type in (
            "XLA_GPU",
            "XLA_CPU",
        ):
            # This test was passing before because soft placement silently picked the
            # CPU kernels.
            self.skipTest("Skip on XLA because XLA kernels do not support int64 seeds.")
