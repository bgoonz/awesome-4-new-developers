...
def composed_sampler(logits, num_samples):
    ...
    unif = random_ops.random_uniform(
...
class MultinomialBenchmark(test.Benchmark):
    def benchmarkNativeOpVsComposedOps(self):
        num_iters = 50
        print(
            "Composition of existing ops vs. Native Multinomial op [%d iters]"
            % num_iters
        )
        print(
        ...
        for batch_size in [32, 128]:
            for num_classes in [10000, 100000]:
                for num_samples in [1, 4, 32]:
                    n_dt, c_dt = native_op_vs_composed_ops(
                        batch_size, num_classes, num_samples, num_iters
                    )
                    print(
                        "%d\t%d\t%d\t%.3f\t%.3f\t%.2f"
                        % (
