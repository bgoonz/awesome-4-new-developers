...
def test_moment_matching(samples, number_moments, dist, stride=0):
  ...
  """
    ...
    for i in range(1, number_moments + 1):
        ...
        total_variance = (
            variance_sample_moments[i - 1] + i * np.finfo(samples.dtype).eps
        )
        ...
        z_test_scores.append(
            abs(
                (sample_moments[i - 1] - expected_moments[i - 1])
...
def test_truncated_normal(
    ...
    expected_median = (
        mu + probit((_normal_cdf(alpha) + _normal_cdf(beta)) / 2.0) * sigma
    )
