...
class TridiagonalMulOpTest(test.TestCase):
    def _testAllFormats(
        self, superdiag, maindiag, subdiag, rhs, expected, dtype=dtypes.float64
    ):
        ...
        diags_matrix = (
            np.diag(superdiag, 1) + np.diag(maindiag, 0) + np.diag(subdiag, -1)
        )

        diags_sequence = (
            constant_op.constant(superdiag_extended, dtype),
    ...
    def _randomComplexArray(self, shape):
        np.random.seed(43)
        return (
            np.random.uniform(-10, 10, shape) + np.random.uniform(-10, 10, shape) * 1j
        )

    def _gradientTest(self, diags, rhs, dtype=dtypes.float64):
        def reference_matmul(diags, rhs):
            matrix = self._makeTridiagonalMatrix(
                diags[..., 0, :-1], diags[..., 1, :], diags[..., 2, 1:]
            )
    ...
    class TridiagonalMatMulBenchmark(test.Benchmark):
        ...
        def _generateData(self, batch_size, m, n, seed=42):
            ...
            return (
                variables.Variable(data[:, :, 0], dtype=dtypes.float64),
            ...
            for device_option, size_option in itertools.product(devices, self.sizes):
                ...
                with ops.Graph().as_default(), session.Session(
                    ...
                    self.run_op_benchmark(
                        ...
                        name=(
                            "tridiagonal_matmul_baseline_%s"
                            "_batch_size_%d_m_%d_n_%d" % (device_name, batch_size, m, n)
                        ),
                    )
                    ...
                    self.run_op_benchmark(
                        ...
                        store_memory_usage=False,
                        name=(
                            "tridiagonal_matmul_%s_batch_size_%d_m_%d_n_%d"
