...
class StringFormatOpTest(test.TestCase):
    ...
    def testFormatSummarizeOne(self):
        with self.cached_session():
            tensor = array_ops.reshape(math_ops.range(100), [10, 10])
            format_output = string_ops.string_format(
                "tensor summary: {}", tensor, summarize=1
            )
            out = self.evaluate(format_output)
    ...
    def testFormatSummarizeTwo(self):
        with self.cached_session():
            tensor = array_ops.reshape(math_ops.range(100), [10, 10])
            format_output = string_ops.string_format(
                "tensor summary: {}", tensor, summarize=2
            )
            out = self.evaluate(format_output)
            expected = (
                "tensor summary: [[0 1 ... 8 9]\n"
                " [10 11 ... 18 19]\n"
                " ...\n"
                " [80 81 ... 88 89]\n"
                " [90 91 ... 98 99]]"
    ...
    def testFormatPlaceholder(self):
        with self.cached_session():
            tensor = array_ops.reshape(math_ops.range(100), [10, 10])
            format_output = string_ops.string_format(
                "tensor summary: %t%", tensor, placeholder="%t%"
            )
            out = self.evaluate(format_output)
            expected = (
                "tensor summary: [[0 1 2 ... 7 8 9]\n"
    ...
    def testTensorCountMustMatchPlaceholderCount(self):
        with self.cached_session():
            with self.assertRaisesRegex(
                ValueError,
                r"The template expects 2 tensors, but the inputs only has" r" 1\.\s.*",
            ):
                tensor = math_ops.range(10)
                format_output = string_ops.string_format("{} {}", tensor)
        ...
        with self.cached_session():
            with self.assertRaisesRegex(
                ValueError,
                r"The template expects 1 tensors, but the inputs only has" r" 2\.\s.*",
            ):
                tensor = math_ops.range(10)
