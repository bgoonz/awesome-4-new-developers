            ...
            for width in [9, 10]:  # Test both odd and even width.
                ...
                for kernel_height in range(1, 4):
                    for kernel_width in range(1, 4):
                        ...
                        f = np.arange(np.prod(f_shape), dtype=np.float32).reshape(
                            f_shape
                        )
                ...
                for width in range(15, 17):
                    ...
                    for kernel in [1, 3, 5]:  # The kernel size needs to be odd.
                        ...
                        for rate in range(2, 4):
                            ...
                            pad_bottom = (
                                0 if height % rate == 0 else rate - height % rate
                            )
                            pad_right = 0 if width % rate == 0 else rate - width % rate
                            pad = [[0, pad_bottom], [0, pad_right]]
                            y2 = array_ops.space_to_batch(
                                x, paddings=pad, block_size=rate
                            )
                            y2 = nn_ops.conv2d(
                                y2, f, strides=[1, 1, 1, 1], padding=padding
                            )
                            y2 = nn_ops.conv2d(
                                y2, f, strides=[1, 1, 1, 1], padding=padding
                            )
                            ...
                            y2 = array_ops.batch_to_space(
                                y2, crops=pad, block_size=rate
                            )
                ...
                for kernel_height in range(1, 4):
                    for kernel_width in range(1, 4):
                        ...
                        for rate in range(1, 4):
                            ...
                            kernel_width_up = kernel_width + (kernel_width - 1) * (
                                rate - 1
                            )
                            ...
                            for padding in ["SAME", "VALID"]:
                                ...
                                y1 = nn_ops.atrous_conv2d_transpose(
                                    x, f, y_shape, rate, padding
                                )
