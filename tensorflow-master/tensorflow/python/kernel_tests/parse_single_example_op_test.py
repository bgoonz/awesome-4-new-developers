...
def empty_sparse(dtype, shape=None):
    if shape is None:
        shape = [0]
    return (
        np.empty(shape=(0, len(shape)), dtype=np.int64),
...
def flatten_values_tensors_or_sparse(tensors_list):
    ...
    return list(
        flatten(
            [
                [v.indices, v.values, v.dense_shape]
...
class ParseExampleTest(test.TestCase):
            ...
            if expected_err:
                ...
                for result_dict in [out, out_with_example_name]:
                    ...
                    _compare_output_to_expected(
                        self, result_dict, expected_values, tf_result
                    )
    ...
    def testEmptySerializedWithoutDefaultsShouldFail(self):
        ...
        self._test(
            {"serialized": original.SerializeToString(), "features": input_features},
            expected_err=(
                errors_impl.OpError,
        ...
        self._test(
            {"serialized": "", "features": input_features},
            expected_err=(
                errors_impl.OpError,
                "Feature: c \\(data type: float\\) is required",
            ),
    ...
    def testSerializedContainingSparse(self):
        ...
        expected_outputs = [
            ...
            {"st_c": empty_sparse(np.float32), "st_d": empty_sparse(bytes)},
            {"st_c": empty_sparse(np.float32), "st_d": empty_sparse(bytes)},
            {
                "st_c": (
            ...
            self._test(
                {
                    "serialized": ops.convert_to_tensor(proto.SerializeToString()),
                    "features": {
                        "sp1": parsing_ops.SparseFeature(
                            "idx", "val1", dtypes.float32, 13
                        ),
    ...
    def testSerializedContaining3DSparseFeature(self):
        ...
        expected_outputs = [
            ...
            {
                "sp": (
                    np.array([[0, 1], [3, 2], [9, 0]], dtype=np.int64),
                    np.array([1.0, -1.0, 2.0], dtype=np.float32),
                    np.array([13, 3], dtype=np.int64),
                )
            },
    ...
    def testSerializedContainingVarLenDense(self):
        ...
        expected_outputs = [
            ...
            {
                aname: np.array([[[1], [1]]], dtype=np.float32),
                bname: np.array(["b0_str", "b1_str"], dtype=bytes).reshape(2, 1, 1, 1),
        ...
        self._test(
            {
                "serialized": ops.convert_to_tensor(original[2].SerializeToString()),
                "features": {
                    aname: parsing_ops.FixedLenSequenceFeature(
                        (2, 1), dtype=dtypes.float32, allow_missing=True
                    ),
...
class ParseSingleExampleTest(test.TestCase):
    ...
    def testExampleLongerThanSpec(self):
        serialized = example(
            features=features({"a": bytes_feature([b"a", b"b"])})
        ).SerializeToString()
        self._test(
            {
                "serialized": ops.convert_to_tensor(serialized),
                "features": {"a": parsing_ops.FixedLenFeature(1, dtypes.string)},
            },
