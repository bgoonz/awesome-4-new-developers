    ...
    def testAssertDivideByZero(self):
        with self.cached_session() as sess:
            ...
            with sess.graph.control_dependencies(
                [control_flow_ops.Assert(math_ops.less(epsilon, y), ["Divide-by-zero"])]
            ):
...
class PrintV2Test(test.TestCase):
    ...
    def testPrintComplexTensorStruct(self):
        ...
        with self.captureWritesToStream(sys.stderr) as printed:
            print_op = logging_ops.print_v2(
                ...
                tensor,
                "middle:",
                {"small": small_tensor, "Big": big_tensor},
                10,
                [tensor * 2, tensor],
            )
            self.evaluate(print_op)
    ...
    def testPrintTensorsToFile(self):
        ...
        print_op_0 = logging_ops.print_v2(
            tensor_0, output_stream="file://" + tmpfile_name
        )
        self.evaluate(print_op_0)
        tensor_1 = math_ops.range(11, 20)
        print_op_1 = logging_ops.print_v2(
            tensor_1, output_stream="file://" + tmpfile_name
        )
