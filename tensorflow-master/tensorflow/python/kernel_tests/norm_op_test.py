    ...
    def testBadOrder(self):
        ...
        for ord_ in "fro", -7, -1.1, 0:
            with self.assertRaisesRegex(
                ValueError, "'ord' must be a supported vector norm"
            ):
        ...
        for ord_ in "foo", -7, -1.1, 1.1:
            with self.assertRaisesRegex(
                ValueError, "'ord' must be a supported matrix norm"
            ):
...
def _GetNormOpTest(dtype_, shape_, ord_, axis_, keep_dims_, use_static_shape_):
    ...
    @test_util.run_v1_only("b/120545219")
    def Test(self):
        is_matrix_norm = (isinstance(axis_, tuple) or isinstance(axis_, list)) and len(
            axis_
        ) == 2
