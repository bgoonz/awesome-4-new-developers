...
class AssertBroadcastableTest(test.TestCase):
    ...
    def _test_valid(self, weights, values):
        ...
        with self.cached_session():
            static_op.run()
            dynamic_op.run(
                feed_dict={weights_placeholder: weights, values_placeholder: values}
            )
    ...
    def test1x1x1(self):
        self._test_valid(
            weights=np.asarray((5,)).reshape((1, 1, 1)), values=_test_values((3, 2, 4))
        )
    ...
    def testInvalid1x1(self):
        self._test_invalid(
            weights=np.asarray((5,)).reshape((1, 1)), values=_test_values((3, 2, 4))
        )
...
class BroadcastWeightsTest(test.TestCase):
    ...
    def testScalar(self):
        self._test_valid(
            weights=5, values=_test_values((3, 2, 4)), expected=5 * np.ones((3, 2, 4))
        )
