...
class CriticalSectionTest(test.TestCase, parameterized.TestCase):
    ...
    def testCriticalSectionWithControlFlow(self, outer_cond, inner_cond):
        ...
        r = [
            control_flow_ops.cond(array_ops.identity(outer_cond), execute, v.read_value)
        ...
        if inner_cond and outer_cond:
            self.assertAllClose(
                [2.0 * i for i in range(num_concurrent)], sorted(r_value)
            )
        else:
    ...
    def testCollection(self):
        ...
        execute_op = [
            x
            for x in execute.graph.get_operations()
        ...
        self.assertIn(
            ...
            [
                signature.op
                for signature in ops.get_collection(
    ...
    def testRecursiveCriticalSectionAccessWithinLoopIsProtected(self):
        ...
        (i_n, j_n) = control_flow_ops.while_loop(
            lambda i, _: i < 1000, body_implicit_capture, [0, 0], parallel_iterations=25
        )
        ...
        (i_n, j_n) = control_flow_ops.while_loop(
            lambda i, _: i < 1000, body_args_capture, [0, 0], parallel_iterations=25
        )
        # For consistency between eager and graph mode.
        i_n = array_ops.identity(i_n)
        logging.warn(
