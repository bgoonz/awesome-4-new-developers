...
class ResourceVariableOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
        ...
        with self.cached_session():
            ...
            with self.assertRaises(ValueError):
                resource_variable_ops.assign_variable_op(
                    handle, constant_op.constant(0.0, dtype=dtypes.float32)
                ).run()
            with self.assertRaises(ValueError):
                resource_variable_ops.assign_variable_op(
                    handle, constant_op.constant([0], dtype=dtypes.int32)
                ).run()
            resource_variable_ops.assign_variable_op(
                handle, constant_op.constant(0, dtype=dtypes.int32)
            ).run()
    ...
    def testEagerPickle(self):
        with context.eager_mode():
            ...
            with open(fname, "wb") as f:
                v = resource_variable_ops.ResourceVariable(
                    10.0, dtype=dtypes.float16, name="v"
                )
                pickle.dump(v, f)
    ...
    def testMetagraph(self):
        with ops.Graph().as_default():
            ...
            momentum.MomentumOptimizer(learning_rate=0.001, momentum=0.1).minimize(
                a,
    ...
    def testScatterUpdateString(self):
        handle = resource_variable_ops.var_handle_op(dtype=dtypes.string, shape=[1, 1])
        self.evaluate(
            resource_variable_ops.assign_variable_op(
    ...
    def testGPU(self):
        with test_util.use_gpu():
            ...
            self.evaluate(variables.global_variables_initializer())
            self.assertEqual(
                self.evaluate(resource_variable_ops.var_is_initialized_op(abc.handle)),
                True,
            )
    ...
    def testToFromProtoCachedValue(self):
        with ops.Graph().as_default():
            ...
            other_v_def = resource_variable_ops.ResourceVariable(
                caching_device="cpu:0", initial_value=constant_op.constant(3.0)
            ).to_proto()
    ...
    def testAssignDifferentShapesEagerNotAllowed(self):
        with context.eager_mode():
            with variable_scope.variable_scope("foo"):
                var = variable_scope.get_variable(
                    "x", shape=[1, 1], dtype=dtypes.float32
                )
    ...
    @test_util.disable_xla(
        "XLA doesn't allow changing shape at assignment, as "
        "dictated by tf2xla/xla_resource.cc:SetTypeAndShape"
    )
    @test_util.run_in_graph_and_eager_modes
    def testAssignDifferentShapesAllowed(self):
        var = resource_variable_ops.ResourceVariable(
            initial_value=np.zeros(shape=[1, 1]), shape=tensor_shape.TensorShape(None)
        )
        self.evaluate(variables.global_variables_initializer())
    ...
    def testAssignReturnsVariable(self):
        ...
        assert_eq(
            var.scatter_nd_update([[1]], [4.0])
            .scatter_nd_add([[0]], [2.0])
            .scatter_nd_sub([[1]], [3]),
            [5.0, 1.0],
        )
        ...
        assert_eq(
            batch_var.batch_scatter_update(batch_slices1).batch_scatter_update(
                batch_slices2
            ),
            [[1, 3], [2, 3]],
        )
    ...
    def testInitValueWrongShape(self):
        with self.assertRaisesWithPredicateMatch(ValueError, r"not compatible with"):
            var = resource_variable_ops.ResourceVariable(
                initial_value=np.zeros(shape=[3]), shape=[4]
            )
            self.evaluate(variables.global_variables_initializer())
    ...
    def testNumpyDotArray(self):
        with context.eager_mode():
            ...
            v1 = resource_variable_ops.ResourceVariable(
                initial_value=lambda: 1, name="v1"
            )
            self.assertEqual(1, np.array(v1))

            v2 = resource_variable_ops.ResourceVariable(
                initial_value=lambda: [1, 2], name="v2"
            )
        ...
        with context.eager_mode():
            ...
            with self.assertRaisesRegex(
                ...
                resource_variable_ops.destroy_resource_op(
                    var_handle, ignore_lookup_error=False
                )
    ...
    def testScatterUpdateVariant(self):
        with context.eager_mode():
            ...
            v.scatter_update(
                ops.IndexedSlices(
                    list_ops.tensor_list_from_tensor([1.0, 2.0], element_shape=[]), 0
                )
            )
            self.assertAllEqual(
    ...
    def testVariantInitializer(self):
        ...
        read_variant_shape_and_type = resource_variable_ops.get_eager_safe_handle_data(
            read
        )
        self.assertEqual(read_variant_shape_and_type, variant_shape_and_type_data)
        gather = v.sparse_read([0])
        gather_variant_shape_and_type = resource_variable_ops.get_eager_safe_handle_data(
            gather
        )
    ...
    @test_util.run_in_graph_and_eager_modes
    def testGatherWithBatchDimsMatchesTensor(
        self, params_shape, indices_shape, batch_dims, output_shape
    ):
