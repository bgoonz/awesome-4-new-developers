...
class TridiagonalSolveOpTest(test.TestCase):
    ...
    def testSequenceFormat(self):
        self._test(
            diags=(_tfconst([2, 1, 4]), _tfconst([1, 3, 2, 2]), _tfconst([1, -1, 1])),
    ...
    def testAdjointRhs(self):
        self._testWithLists(
            ...
            expected=np.array([_sample_result * (1 - 1j), _sample_result * (1 + 2j)]).T,
            transpose_rhs=True,
            conjugate_rhs=True,
        )
    ...
    def testTransposeRhsWithBatching(self):
        self._testWithLists(
            diags=np.array([_sample_diags, -_sample_diags]),
            rhs=np.array(
                [[_sample_rhs, 2 * _sample_rhs], [3 * _sample_rhs, 4 * _sample_rhs]]
            ),
            expected=np.array(
                [
    ...
    def testGradientWithMultipleRhs(self):
        self._gradientTestWithLists(
            ...
            y=[[1, 5], [2, 6], [3, 7], [4, 8]],
            expected_grad_diags=(
                [[-20, 28, -60, 0], [36, -35, 60, 80], [0, 63, -75, -80]]
            ),
    ...
    class TridiagonalSolveBenchmark(test.Benchmark):
        ...
        def _generateData(self, matrix_size, batch_size, num_rhs, seed=42):
            ...
            rhs = data[:, :, 3:]
            return (
                variables.Variable(diags, dtype=dtypes.float64),
        ...
        def _generateMatrixData(self, matrix_size, batch_size, num_rhs, seed=42):
            ...
            return (
                variables.Variable(np.tile(matrix, (batch_size, 1, 1))),
        ...
        def _benchmark(self, generate_data_fn, test_name_format_string):
            ...
            for device_option, pivoting_option, size_option in itertools.product(
                devices, self.pivoting_options, self.sizes
            ):
                ...
                with ops.Graph().as_default(), session.Session(
                    ...
                    self.run_op_benchmark(
                        ...
                        name=test_name_format_string.format(
                            device_name, matrix_size, batch_size, num_rhs, pivoting_name
                        ),
                    )
...
if __name__ == "__main__":
    for name, fun in dict(TridiagonalSolveOpTest.__dict__).items():
        ...
        if not hasattr(fun, FLAG_REQUIRES_PIVOTING):
            setattr(
                TridiagonalSolveOpTest, name + "_noPivoting", decor(fun, pivoting=False)
            )
