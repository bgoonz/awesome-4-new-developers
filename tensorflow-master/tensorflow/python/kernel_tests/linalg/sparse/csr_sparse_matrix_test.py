...
class SparseMatrixMatmulTest(test.TestCase):
        ...
        for dtype in dtypes_to_test:
            a_mats = sparsify(
                (
                    ...
                    + 1.0j * np.random.randn(*dense_shape_a)
                )
            ).astype(dtype)
            b_mats = sparsify(
                (
                    np.random.randn(*dense_shape_b)
                    + 1.0j * np.random.randn(*dense_shape_b)
                )
            ).astype(dtype)
            a_sm = sparse_csr_matrix_ops.CSRSparseMatrix(a_mats)
    ...
    def _testSparseDense(self, transpose_a, transpose_b, adjoint_a, adjoint_b):
        ...
        for dtype in dtypes_to_test:
            ...
            b_mats = (
                np.random.randn(*dense_shape_b) + 1.0j * np.random.randn(*dense_shape_b)
            ).astype(dtype)
            a_sm = sparse_csr_matrix_ops.CSRSparseMatrix(a_mats)
            c_dense = test_util.matmul_without_tf32(
                a_mats,
    ...
    def _testDenseSparse(self, transpose_a, transpose_b, adjoint_a, adjoint_b):
        ...
        for dtype in dtypes_to_test:
            a_mats = (
                np.random.randn(*dense_shape_a) + 1.0j * np.random.randn(*dense_shape_a)
            ).astype(dtype)
            b_mats = sparsify(
                (
                    np.random.randn(*dense_shape_b)
