    ...
    def testLargeBatchConversionGrad(self):
        ...
        for dense_shape in ([53, 65, 127], [127, 65]):
            ...
            with self.test_session() as sess:
                ...
                grad_out = gradients_impl.gradients([dense_mats], [mats], [grad_vals])[
                    0
                ]
                ...
                tf_logging.info(
                    "testLargeBatchConversionGrad: Testing shape %s" % dense_shape
                )
                nonzero_indices = abs(mats_val) > 0.0
                self.assertAllEqual(
                    grad_out_value[nonzero_indices], grad_vals[nonzero_indices]
                )
                self.assertTrue(
    ...
    def testLargeBatchSparseConversionGrad(self):
        ...
        for dense_shape in ([53, 65, 127], [127, 65]):
            ...
            with self.session(use_gpu=True) as sess:
                ...
                new_coo_tensor = sparse_csr_matrix_ops.csr_sparse_matrix_to_sparse_tensor(
                    csr_matrix, type=dtypes.float32
                )
                grad_out = gradients_impl.gradients(
                    [new_coo_tensor.values], [values], [grad_vals]
                )[0]
                self.assertEqual(grad_out.dtype, dtypes.float32)
    ...
    def testLargeBatchSparseMatrixAddGrad(self):
        ...
        for dense_shape in ([53, 65, 127], [127, 65]):
            ...
            with self.test_session() as sess:
                ...
                a_grad, b_grad = gradients_impl.gradients(
                    [c_dense], [a_mats, b_mats], [grad_vals]
                )
