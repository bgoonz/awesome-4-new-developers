...
class IoOpsTest(test.TestCase):
    ...
    def testReadFile(self):
        ...
        for contents in cases:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="ReadFileTest", dir=self.get_temp_dir(), delete=False
            ) as temp:
                temp.write(contents)
    ...
    def testWriteFile(self):
        ...
        for contents in cases:
            contents = compat.as_bytes(contents)
            with tempfile.NamedTemporaryFile(
                prefix="WriteFileTest", dir=self.get_temp_dir(), delete=False
            ) as temp:
                pass
    ...
    def _subset(self, files, indices):
        return set(
            compat.as_bytes(files[i].name) for i in range(len(files)) if i in indices
        )
    ...
    def testMatchingFiles(self):
        ...
        files = [
            tempfile.NamedTemporaryFile(prefix=c, dir=self.get_temp_dir(), delete=True)
            for c in cases
        ]
        ...
        with self.cached_session():
            ...
            # Test multiple globs.
            self.assertItemsEqual(
                io_ops.matching_files([pattern % "?", directory_path + "X?Z*"]).eval(),
