...
class NumericColumn(
    ...
    def get_config(self):
        ...
        config[
            "normalizer_fn"
        ] = serialization._serialize_keras_object(  # pylint: disable=protected-access
            self.normalizer_fn
        )
        config["dtype"] = self.dtype.name
        return config
    ...
    def from_config(cls, config, custom_objects=None, columns_by_name=None):
        ...
        kwargs[
            "normalizer_fn"
        ] = serialization._deserialize_keras_object(  # pylint: disable=protected-access
            config["normalizer_fn"], custom_objects=custom_objects
        )
        kwargs["dtype"] = dtypes.as_dtype(config["dtype"])
...
class VocabularyFileCategoricalColumn(
    ...
    def _make_table_from_tfrecord_gzip_file(self, key_dtype, name):
        ...
        def key_dtype_fn(key):
            return (
                key
                if key_dtype is dtypes.string
                else string_ops.string_to_number(key, out_type=key_dtype)
            )
