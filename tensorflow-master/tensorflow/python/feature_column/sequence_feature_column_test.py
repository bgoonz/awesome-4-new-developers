...
class ConcatenateContextInputTest(test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        {"testcase_name": "rank_lt_3", "seq_input_arg": np.arange(100).reshape(10, 10)},
        {
    ...
    @parameterized.named_parameters(
        {"testcase_name": "rank_lt_2", "context_input_arg": np.arange(100)},
        {
...
def _get_sequence_dense_tensor_state(column, features):
    state_manager = fc._StateManagerImpl(fc_lib.DenseFeatures(column), trainable=True)
...
class SequenceCategoricalColumnWithVocabularyFileTest(
    ...
    def setUp(self):
        ...
        self._wire_vocabulary_file_name = self._write_vocab(
            vocab_strings, "wire_vocabulary.txt"
        )
...
@test_util.run_all_in_graph_and_eager_modes
class SequenceEmbeddingColumnTest(test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        {
            "testcase_name": "2D",
            "inputs_args": {
    ...
    def test_get_sequence_dense_tensor(self):
        ...
        with ops.Graph().as_default():
            ...
            embedding_lookup_a = _get_sequence_dense_tensor(
                shared_embedding_columns[0], {"aaa": sparse_input_a}
            )[0]
            embedding_lookup_b = _get_sequence_dense_tensor(
                shared_embedding_columns[1], {"bbb": sparse_input_b}
            )[0]
    ...
    @parameterized.named_parameters(
        ...
        },
        {
            ...
            "inputs_args": {
                ...
                "values": (20, 3, 5.0, 3.0, 8.0),
                "dense_shape": (2, 2, 2),
            },
            "expected": [[[20.0], [3.0], [5.0], [0.0]], [[3.0], [0.0], [8.0], [0.0]]],
        },
    )
    def test_get_sequence_dense_tensor(self, inputs_args, expected):
        inputs = sparse_tensor.SparseTensorValue(**inputs_args)
