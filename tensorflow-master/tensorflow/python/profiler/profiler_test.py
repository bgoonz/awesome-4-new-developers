...
class ProfilerTest(test.TestCase):
    ...
    def testProfileBasic(self):
        ...
        model_analyzer.profile(sess.graph, cmd="graph", run_meta=run_meta, options=opts)
        with gfile.Open(outfile, "r") as f:
            pma_str = f.read()
        ...
        model_analyzer.profile(sess.graph, cmd="scope", run_meta=run_meta, options=opts)
        with gfile.Open(outfile, "r") as f:
            pma_str = f.read()
        self.assertEqual(pma_str, profiler_str)
        ...
        model_analyzer.profile(sess.graph, cmd="code", run_meta=run_meta, options=opts)
        with gfile.Open(outfile, "r") as f:
            pma_str = f.read()
        self.assertEqual(pma_str, profiler_str)
        ...
        model_analyzer.profile(sess.graph, cmd="op", run_meta=run_meta, options=opts)
        with gfile.Open(outfile, "r") as f:
            pma_str = f.read()
        self.assertEqual(pma_str, profiler_str)
    ...
    def testMultipleProfilePerStep(self):
        ...
        opts = (
            ...
            .with_empty_output()
            .with_accounted_types([".*"])
            .select(["micros", "bytes", "peak_bytes", "residual_bytes", "output_bytes"])
            .build()
        )
