    ...
    def benchmark_batch_sparse(self):
        ...
        for non_zeros_per_row in non_zeros_per_row_values:
            ...
            for batch_size in batch_size_values:
                dataset = (
                    dataset_ops.Dataset.from_tensors(tensor).repeat().batch(batch_size)
                )
    ...
    def _benchmark_batch_dense(self, parallel_copy, benchmark_id):
        for element_exp in [10, 12, 14, 16, 18, 20, 22]:
            for batch_exp in [3, 6, 9]:
                ...
                dataset = (
                    dataset_ops.Dataset.from_tensors(np.random.rand(element_size))
                    .repeat()
                    .batch(batch_size)
                )
    ...
    def benchmark_parallel_batch(self):
        ...
        for num_parallel_calls in nums_parallel_calls:
            ...
            op_str = (
                ...
                else ("parallel_batch_num_parallel_calls_%s" % num_parallel_calls_str)
            )

            dataset = (
                dataset_ops.Dataset.range(num_range)
                .map(f)
                .batch(batch_size, num_parallel_calls=num_parallel_calls)
            )
            self.run_and_report_benchmark(
                dataset,
