...
class _PerDeviceGenerator(dataset_ops.DatasetV2):
    ...
    def __init__(
        ...
        multi_device_iterator_string_handle = gen_dataset_ops.multi_device_iterator_to_string_handle(
            multi_device_iterator_resource
        )
        ...
        # TODO(b/124254153): Enable autograph once the overhead is low enough.
        @function.defun(
            input_signature=[tensor_spec.TensorSpec([], dtypes.string)], autograph=False
        )  # Pure graph code.
        def _next_func(string_handle):
            # pylint: disable=protected-access
            multi_device_iterator = gen_dataset_ops.multi_device_iterator_from_string_handle(
...
class _ReincarnatedPerDeviceGenerator(dataset_ops.DatasetV2):
  ...
  """
    ...
    def __init__(self, per_device_dataset, incarnation_id):
        ...
        self._next_captured_args[
            per_device_dataset._incarnation_id_index
        ] = incarnation_id
...
def _create_device_dataset(
    prototype_ds, incarnation_id, prefetch_buffer_size, experimental_slack
):
...
class MultiDeviceIterator(object):
    ...
    """
        ...
        with ops.device(self._source_device):
            ...
            self._multi_device_iterator_resource = gen_dataset_ops.multi_device_iterator(
        ...
        for i, device in enumerate(self._devices):
            with ops.device(device):
                ds = _create_device_dataset(
                    self._prototype_device_datasets[i],
                    self._incarnation_id,
...
class MultiDeviceIteratorSpec(type_spec.TypeSpec):
    ...
    def from_value(value):
        ...
        return MultiDeviceIteratorSpec(
            value._devices, value._source_device, value.element_spec
        )
...
class OwnedMultiDeviceIterator(composite_tensor.CompositeTensor):
  ...
  """
    ...
    """
        ...
        if dataset is None:
            ...
            for it in self._device_iterators:
                iterator_handles.append(
                    it._iterator_resource
                )  # pylint: disable=protected-access
        else:
            ...
            # Create the MultiDeviceIterator.
            with ops.device(self._source_device):
                self._multi_device_iterator_resource, self._deleter = gen_dataset_ops.anonymous_multi_device_iterator(
                    devices=self._devices, **dataset._flat_structure
                )  # pylint: disable=protected-access
    ...
    def _type_spec(self):
        return MultiDeviceIteratorSpec(
            self._devices, self._source_device, self._element_spec
        )
