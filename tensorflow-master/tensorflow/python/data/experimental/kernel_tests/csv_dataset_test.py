...
class CsvDatasetTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def _test_dataset(
        ...
        **kwargs
        ...
        if expected_err_re is not None:
            ...
            dataset = readers.CsvDataset(filenames, **kwargs)
            expected_output = [
                tuple(v.encode("utf-8") if isinstance(v, str) else v for v in op)
    ...
    def testErrWithUnescapedQuotes(self):
        ...
        self._test_dataset(
            inputs,
            expected_err_re="Quote inside a string has to be escaped by another quote",
            record_defaults=record_defaults,
        )
    ...
    def testWithHeader(self):
        ...
        expected = [[1, 2]]
        self._test_dataset(
            inputs, expected, record_defaults=record_defaults, header=True
        )
    ...
    def testWithChainedOps(self):
        ...
        self.assertDatasetsEqual(
            ds_actual.repeat(5).prefetch(1), ds_expected.repeat(5).prefetch(1)
        )
    ...
    def testWithTypeDefaults(self):
        ...
        self._test_dataset(
            inputs, [[1.0, 2.0], [3.0, 4.0]], record_defaults=record_defaults
        )
    ...
    def testErrorWithTerminateMidRecord(self):
        ...
        self._test_dataset(
            inputs,
            expected_err_re="Reached end of file without closing quoted field in record",
            record_defaults=record_defaults,
        )
    ...
    def _test_dataset_on_buffer_sizes(
        self,
        inputs,
        expected,
        linebreak,
...
class CsvDatasetCheckpointTest(
    ...
    def setUp(self):
        ...
        inputs = [
            ",".join(str(self._num_cols * j + i) for i in range(self._num_cols))
            for j in range(self._num_rows)
        ]
        contents = "\n".join(inputs).encode("utf-8")

        self._filename = os.path.join(self.get_temp_dir(), "file.csv")
        self._compressed = os.path.join(
            self.get_temp_dir(), "comp.csv"
        )  # GZip compressed
