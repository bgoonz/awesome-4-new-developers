...
class ShuffleAndRepeatTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testCorrectOutput(self):
        ...
        self.assertSequenceEqual(
            sorted(output), sorted(np.array([range(20) for _ in range(5)]).flatten())
        )
        for i in range(5):
    ...
    def testInfiniteEmpty(self):
        with self.assertRaises(errors.OutOfRangeError):
            self._gen_outputs(
                lambda: self._build_ds(10, count=None, num_elements=0), 100
            )
        with self.assertRaises(errors.OutOfRangeError):
    ...
    def testRerandomizeOnReplicate(self):
        ...
        ds = dataset_ops.Dataset.range(num_elements).apply(
            shuffle_ops.shuffle_and_repeat(buffer_size=num_elements, count=num_epochs)
        )
...
class ShuffleAndRepeatCheckpointTest(
