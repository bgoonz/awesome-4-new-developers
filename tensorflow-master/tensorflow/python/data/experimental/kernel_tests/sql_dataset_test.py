...
class SqlDatasetTestBase(test_base.DatasetTestBase):
    ...
    def _createSqlDataset(
        self, query, output_types, driver_name="sqlite", num_repeats=1
    ):
        dataset = readers.SqlDataset(
            driver_name, self.data_source_name, query, output_types
        ).repeat(num_repeats)
        return dataset
    ...
    def setUp(self):
        ...
        c.executemany(
            ...
            [("Benjamin", "Franklin", "Pennsylvania"), ("John", "Doe", "California")],
        )
        c.execute(
...
class SqlDatasetTest(SqlDatasetTestBase, parameterized.TestCase):
    ...
    def testReadResultSetNullTerminator(self):
        ...
        self.assertEqual((b"Jane", b"Moe", b"nonsense\0"), self.evaluate(get_next()))
        with self.assertRaises(errors.OutOfRangeError):
            self.evaluate(get_next())
    ...
    def testReadResultSetReuseSqlDataset(self):
        ...
        self.assertEqual(
            (b"Benjamin", b"Franklin", b"Pennsylvania"), self.evaluate(get_next())
        )
        with self.assertRaises(errors.OutOfRangeError):
            self.evaluate(get_next())
    ...
    def testReadResultSetFloat64LargestConsecutiveWholeNumbersNotEqual(self):
        ...
        self.assertNotEqual(
            (b"George", b"Washington", 9007199254740992.0), self.evaluate(get_next())
        )
        self.assertNotEqual(
            (b"John", b"Adams", 9007199254740991.0), self.evaluate(get_next())
        )
        with self.assertRaises(errors.OutOfRangeError):
            self.evaluate(get_next())
...
class SqlDatasetCheckpointTest(
    SqlDatasetTestBase, checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    def _build_dataset(self, num_repeats):
        ...
        query = (
            "SELECT first_name, last_name, motto FROM students ORDER BY "
            "first_name DESC"
        )
        output_types = (dtypes.string, dtypes.string, dtypes.string)
        return readers.SqlDataset(
