...
class GroupByReducerTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testAverage(self):
        def reduce_fn(x, y):
            return (
                (x[0] * x[1] + math_ops.cast(y, dtypes.float32)) / (x[1] + 1),
                x[1] + 1,
            )
    ...
    def testConcat(self):
        ...
        for i in range(1, 11):
            ...
            ).apply(grouping.group_by_reducer(lambda x, y: y % 2, reducer))
            self.assertDatasetProduces(
                dataset,
                expected_shapes=tensor_shape.TensorShape([]),
    ...
    def testSparseSum(self):
        ...
        for i in range(1, 11):
            dataset = (
                dataset_ops.Dataset.range(2 * i)
                .map(_sparse)
                .apply(grouping.group_by_reducer(lambda x: x.values[0] % 2, reducer))
            )
            self.assertDatasetProduces(
                dataset,
                expected_shapes=tensor_shape.TensorShape([]),
    ...
    def testChangingStateShape(self):
        ...
        for i in range(1, 11):
            dataset = (
                dataset_ops.Dataset.from_tensors(np.int64(0))
                .repeat(i)
                .apply(grouping.group_by_reducer(lambda x: x, reducer))
            )
    ...
    def testTuple(self):
        ...
        dataset = dataset_ops.Dataset.zip(
            (dataset_ops.Dataset.range(10), dataset_ops.Dataset.range(10))
        ).apply(grouping.group_by_reducer(lambda x, y: np.int64(0), reducer))
        get_next = self.getNext(dataset)
        x, y = self.evaluate(get_next())
