...
@deprecation.deprecated(
    None,
    "`make_saveable_from_iterator` is intended for use in TF1 with "
  ...
  """
    ...
    """
        ...
        if estimator._config.num_worker_replicas > 1:
            # Distributed setting.
            suffix = "_{}_{}".format(
                estimator._config.task_type, estimator._config.task_id
            )
    ...
    def begin(self):
        ...
        # pylint: disable=protected-access
        if (
            self._checkpoint_saver_hook._saver is None
            and self._checkpoint_saver_hook._scaffold is None
        ):
  ...
  """
    ...
    def save(
        ...
        return super(_CustomSaver, self).save(
            ...
            save_path,
            global_step,
            latest_filename or self._latest_filename,
            meta_graph_suffix,
            write_meta_graph,
            write_state,
            strip_default_attrs,
