...
class MapAndBatchBenchmark(benchmark_base.DatasetBenchmarkBase):
    ...
    def _benchmark_series(self, label, series, benchmark_id):
        ...
        def make_dataset(
            map_num_calls, element_size, batch_size, batch_num_calls, apply_fusion
        ):
            k = 1024 * 1024
        ...
        def make_name(
            ...
            map_num_calls_str = (
                "autotuned"
                if map_num_calls == dataset_ops.AUTOTUNE
                else str(map_num_calls)
            )
            batch_num_calls_str = (
                "autotuned"
                if batch_num_calls == dataset_ops.AUTOTUNE
                else str(1 if batch_num_calls is None else batch_num_calls)
            )
            name_str = (
        ...
        for (
            ...
            self.run_and_report_benchmark(
                ...
                warmup=True,
                extras={
                    "model_name": "map_and_batch.benchmark.%d" % benchmark_id,
                    "parameters": "%d.%d.%d.%d.%d.%s"
                    % (
                        map_num_calls,
                        inter_op,
                        element_size,
    ...
    """
        ...
        seq_elem_size_series = itertools.product(
            [1], [1], [1, 2, 4, 8], [16], [None], [False, True]
        )
        seq_batch_size_series = itertools.product(
            [1], [1], [1], [8, 16, 32, 64], [None], [False, True]
        )

        # Parallel pipeline configuration.
        par_elem_size_series = itertools.product(
            [32], [32], [1, 2, 4, 8], [256], [None], [False, True]
        )
        par_batch_size_series = itertools.product(
            [32], [32], [1], [128, 256, 512, 1024], [None], [False, True]
        )
        par_map_num_calls_series = itertools.product(
            [8, 16, 32, 64], [32], [1], [512], [None], [False, True]
        )
        par_inter_op_series = itertools.product(
            [32], [8, 16, 32, 64], [1], [512], [None], [False, True]
        )
        ...
        self._benchmark_series(
            "Sequential element size evaluation", seq_elem_size_series, benchmark_id=2
        )
        self._benchmark_series(
            "Sequential batch size evaluation", seq_batch_size_series, benchmark_id=3
        )
        self._benchmark_series(
            "Parallel element size evaluation", par_elem_size_series, benchmark_id=4
        )
        self._benchmark_series(
