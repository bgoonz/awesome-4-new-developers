...
class FromTensorsTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testFromTensorsTensorArray(self):
        ...
        components = tensor_array_ops.TensorArray(
            dtypes.float32, element_shape=(), size=2
        ).unstack([1.0, 2.0])
    ...
    def testFromTensorsSparse(self):
        ...
        components = (
            sparse_tensor.SparseTensorValue(
                indices=np.array([[0]]), values=np.array([0]), dense_shape=np.array([1])
            ),
            sparse_tensor.SparseTensorValue(
    ...
    def testNestedDict(self):
        ...
        dataset = dataset_ops.Dataset.from_tensors(components)
        self.assertEqual(
            dtypes.int32, dataset_ops.get_legacy_output_types(dataset)["a"]["aa"]
        )
        self.assertEqual(
            dtypes.float32, dataset_ops.get_legacy_output_types(dataset)["a"]["ab"]
        )
        self.assertEqual(
            dtypes.int32, dataset_ops.get_legacy_output_types(dataset)["b"]
        )
...
class FromTensorsCheckpointTest(
