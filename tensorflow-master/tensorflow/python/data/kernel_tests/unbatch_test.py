...
class UnbatchTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testUnbatchMultiElementTupleDataset(self):
        data = tuple(
            [
                (math_ops.range(10 * i, 10 * i + 10), array_ops.fill([10], "hi"))
                for i in range(3)
            ]
        )
        data = dataset_ops.Dataset.from_tensor_slices(data)
        ...
        self.assertDatasetProduces(
            data, [((i, b"hi"), (10 + i, b"hi"), (20 + i, b"hi")) for i in range(10)]
        )
    ...
    def testUnbatchEmpty(self):
        data = dataset_ops.Dataset.from_tensors(
            (
                ...
                constant_op.constant([], shape=[0, 4]),
    ...
    def testUnbatchDynamicShapeMismatch(self):
        ...
        with self.cached_session() as sess:
            ...
            sess.run(
                iterator.initializer,
                feed_dict={ph1: np.arange(7).astype(np.int32), ph2: 7},
            )
            with self.assertRaises(errors.InvalidArgumentError):
                self.evaluate(next_element)
    ...
    def testNoneComponent(self):
        dataset = (
            dataset_ops.Dataset.from_tensors((list(range(10)), None))
            .unbatch()
            .map(lambda x, y: x)
        )
