...
class ScanTest(test_base.DatasetTestBase, parameterized.TestCase):
    def _counting_dataset(self, start, scan_fn):
        return (
            ...
            .repeat()
            .scan(initial_state=start, scan_func=scan_fn)
        )
    ...
    def testCount(self):
        ...
        for start_val, step_val, take_val in [
            (0, 1, 10),
            (0, 1, 0),
            (10, 1, 10),
            (10, 2, 10),
    ...
    def testFibonacci(self):
        data = (
            ...
            .repeat(None)
            .scan(
                initial_state=[0, 1], scan_func=lambda a, _: ([a[1], a[0] + a[1]], a[1])
            )
        )
    ...
    def testSparseCount(self):
        ...
        def dataset_fn(start, step, take):
            return self._counting_dataset(_sparse(start), make_scan_fn(step)).take(take)
    ...
    def testTensorArraySimple(self):
        ...
        ds = dataset_ops.Dataset.range(5).scan(initial_state=start, scan_func=scan_fn)
    ...
    def testTensorArrayWithCondReset(self):
        ...
        ds = dataset_ops.Dataset.range(6).scan(initial_state=start, scan_func=scan_fn)

        self.assertDatasetProduces(
            ds,
            expected_output=[[-1, 0], [1], [1, 2], [1, 2, 3], [4], [4, 5]],
            requires_initialization=True,
            num_test_iterations=2,
        )
    ...
    def testTensorArrayWithCondResetByExternalCaptureBreaks(self):
        ...
        with self.assertRaisesRegex(
            NotImplementedError, r"construct a new TensorArray inside the function"
        ):
    ...
    def testChangingStateShape(self):
        ...
        dataset = (
            dataset_ops.Dataset.from_tensors(0)
            .repeat(5)
            .scan(initial_state=([0], 1), scan_func=_scan_fn)
        )
        self.assertEqual(
            [None], dataset_ops.get_legacy_output_shapes(dataset)[0][0].as_list()
        )
        self.assertIs(None, dataset_ops.get_legacy_output_shapes(dataset)[0][1].ndims)
        self.assertEqual([], dataset_ops.get_legacy_output_shapes(dataset)[1].as_list())
