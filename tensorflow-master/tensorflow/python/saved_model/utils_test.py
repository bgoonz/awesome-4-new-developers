...
class UtilsTest(test.TestCase):
    ...
    def testBuildTensorInfoSparse(self):
        ...
        x_tensor_info = utils.build_tensor_info(x)
        self.assertEqual(x.values.name, x_tensor_info.coo_sparse.values_tensor_name)
        self.assertEqual(x.indices.name, x_tensor_info.coo_sparse.indices_tensor_name)
        self.assertEqual(
            x.dense_shape.name, x_tensor_info.coo_sparse.dense_shape_tensor_name
        )
        self.assertEqual(types_pb2.DT_FLOAT, x_tensor_info.dtype)
    ...
    def testBuildTensorInfoRagged(self):
        ...
        # Check components
        self.assertEqual(
            x.values.name, x_tensor_info.composite_tensor.components[0].name
        )
        self.assertEqual(
            types_pb2.DT_INT32, x_tensor_info.composite_tensor.components[0].dtype
        )
        self.assertEqual(
            x.row_splits.name, x_tensor_info.composite_tensor.components[1].name
        )
        self.assertEqual(
            types_pb2.DT_INT64, x_tensor_info.composite_tensor.components[1].dtype
        )
        # Check type_spec.
    ...
    def testGetTensorFromInfoInOtherGraph(self):
        ...
        actual = utils.get_tensor_from_tensor_info(tensor_info, graph=expected_graph)
        self.assertIsInstance(actual, ops.Tensor)
    ...
    def testGetTensorFromInfoInScope(self):
        ...
        with ops.Graph().as_default():
            ...
            actual = utils.get_tensor_from_tensor_info(tensor_info, import_scope="foo")
            self.assertEqual(expected.name, actual.name)
