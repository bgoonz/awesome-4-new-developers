  ...
  """
    ...
    def _call_flat(self, args, captured_inputs, cancellation_manager=None):
        ...
        def get_unused_handle(x):
            return (
                _unused_handle() if distribute_utils.is_distributed_variable(x) else x
            )

        if (
            ds_context.get_replica_context() is not None
...
class Loader(object):
        ...
        while nodes_to_visit:
            ...
            for reference in self._proto.nodes[node_id].children:
                ...
                if child_object is None and node is not None:
                    child_object = node._lookup_dependency(
    ...
    def _create_saveable_object_factories(self):
        for node_id, proto in self._iter_all_nodes():
            ...
            node._self_saveable_object_factories = (
                {}
            )  # pylint: disable=protected-access
            for name, saveable_object_proto in proto.saveable_objects.items():
                node._self_saveable_object_factories[
                    name
                ] = saveable_object_util.restored_saved_object_factory(  # pylint: disable=protected-access
            ...
            for node_path in self._node_filters:
                ...
                for name in path[1:-1]:
                    if not hasattr(current_node, name):
                        setattr(
                            current_node, name, self._recreate_base_user_object()[0]
                        )
    ...
    def _setup_functions_structures(self):
        ...
        for name, proto in sorted(self._proto.concrete_functions.items()):
            ...
            # and types.
            concrete_function._func_graph.structured_outputs = (
                original_outputs
            )  # pylint: disable=protected-access
    ...
    def _setup_functions_captures(self):
        ...
        for name, proto in concrete_functions:
            ...
            # itself.
            concrete_function._captured_inputs = (
                bound_inputs
            )  # pylint: disable=protected-access
            concrete_function._func_graph.variables = (
                bound_variables
            )  # pylint: disable=protected-access
            if bound_inputs:
    ...
    def _iter_all_nodes(self):
        if self._filtered_nodes is None:
            ...
            return [
                (node_id, self._proto.nodes[node_id])
    ...
    def _restore_checkpoint(self):
        ...
        if self._save_options.allow_partial_checkpoint:
            load_status = saver.restore(
                variables_path, self._checkpoint_options
            ).expect_partial()
                ...
                if restore_ops:
                    if resource_variable_ops.is_resource_variable(obj):
                        ...
                        ops.add_to_collection(
                            ops.GraphKeys.TABLE_INITIALIZERS, restore_ops
                        )
                    else:
    ...
    def _recreate_default(self, proto, node_id):
        ...
        if kind not in factory:
            raise ValueError(
                f"Unknown SavedObject type: {kind}. Expected one of "
                f"{list(factory.keys())}."
            )
        return factory[kind]()
...
def load_internal(export_dir, tags=None, options=None, loader_cls=Loader, filters=None):
