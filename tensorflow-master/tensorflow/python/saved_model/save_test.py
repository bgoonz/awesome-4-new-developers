...
class SaveTest(test.TestCase, parameterized.TestCase):
    ...
    def test_method_save_list_func(self):
        ...
        root.f = def_function.function(
            lambda x: 2.0 * case_fn(x),
            input_signature=[tensor_spec.TensorSpec(None, dtypes.float32)],
        )
        root.f(constant_op.constant(1.0))
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
        save.save(root, save_dir, root.f)
        self.assertEqual({"output_0": 4.0}, _import_and_infer(save_dir, {"x": 1.0}))

    def test_method_save_concrete(self):
        root = tracking.AutoTrackable()
        root.f = def_function.function(lambda z: {"out": 2.0 * z})
        root.f(constant_op.constant(1.0))
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
        save.save(
            root,
    ...
    def test_untracked_variable_useful_message(self):
        root = module.Module()
        v = variables.Variable(1.0, name="some_unique_name")
    ...
    def test_nested_outputs(self):
        root = tracking.AutoTrackable()
        root.f = def_function.function(lambda x: (2.0 * x, (3.0 * x, 4.0 * x)))
        root.f(constant_op.constant(1.0))
    ...
    def test_single_function_default_signature(self):
        ...
        model.f = def_function.function(lambda: 3.0, input_signature=())
        model.f()
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
        save.save(model, save_dir)
        self.assertAllClose({"output_0": 3.0}, _import_and_infer(save_dir, {}))

    def test_single_function_no_signature(self):
        model = tracking.AutoTrackable()
        model.f = def_function.function(lambda: 3.0)
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
        save.save(model, save_dir)
    ...
    def test_signature_attribute_reserved(self):
        root = util.Checkpoint(signatures=variables.Variable(1.0))
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
    ...
    @parameterized.named_parameters(
        ...
        ("_DiscardDevices_ExportMetaGraph", save_options.VariablePolicy.NONE, True),
        ("_SaveDevices_Save", save_options.VariablePolicy.SAVE_VARIABLE_DEVICES, False),
    ...
    def test_save_variable_devices(self, save_devices, meta_graph_only):
        ...
        with ops.device("CPU:0"):
            root.v0 = variables.Variable(1.0, name="v0")
        with ops.device("CPU:1"):
            root.v1 = variables.Variable(1.0, name="v1")

        options = save_options.SaveOptions(experimental_variable_policy=save_devices)
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            save_options.VariablePolicy.NONE,
            True,
        ),
        (
    ...
    def test_expand_distributed_variables(self, expand_strategy, policy):
        ...
        with strategy.scope():
            root = tracking.AutoTrackable()
            root.v = variables.Variable([1.0, 1.0], name="v")

            @def_function.function(input_signature=[])
            def f():
                root.v.assign([2.0, 2.0])
        ...
        v = next(
            (
                ...
                if n.HasField("variable") and n.variable.name == "v"
            ),
            None,
        )
        saved_function = next(
            (
                f
                for f in graph_def.library.function
                if "inference_f_" in f.signature.name
            ),
            None,
        )
        self.assertIsNotNone(saved_function)
        if expand_strategy == save_options.VariablePolicy.EXPAND_DISTRIBUTED_VARIABLES:
    ...
    def test_save_uninitialized_variable(self):
        ...
        root.initialized_variable = variables.Variable(1.0, name="initialized_variable")
        ...
        with self.assertRaisesRegex(FileNotFoundError, "Key uninitialized_variable"):
    ...
    def test_save_composite_tensor_signature(self):
        ...
        root = tracking.AutoTrackable()
        path = os.path.join(self.get_temp_dir(), "saved_model")
        inp = ragged_factory_ops.constant([[[1.0, 2.0], [3.0]], [[5.0]]])
        flat_inp = {
            "x": constant_op.constant([1.0, 2.0, 3.0, 5]),
        ...
        imported = load.load(path)
        self.assertAllEqual(inp, imported.signatures["key"](**flat_inp)["output_key"])
        graph = ops.Graph()
        ...
        with graph.as_default(), session_lib.Session() as session:
            ...
            output_components = signature.outputs[
                "output_key"
            ].composite_tensor.components
            fetches = {}
    ...
    def test_save_uses_sanitized_signature_name(self):
        ...
        root = tracking.AutoTrackable()
        path = os.path.join(self.get_temp_dir(), "saved_model")
        save.save(root, path, signatures={unsanitized_name: f.get_concrete_function()})
        graph = ops.Graph()
        with graph.as_default(), session_lib.Session() as session:
            meta_graph_def = loader.load(session, [tag_constants.SERVING], path)
            ...
            self.assertCountEqual(
                tensor_names, ["foo_bar_x:0", "foo_bar_x_1:0", "foo_bar_x_2:0"]
            )
...
class VariablePolicyEnumTest(test.TestCase):
    def testFromObj(self):
        self.assertEqual(
            save_options.VariablePolicy.NONE, save_options.VariablePolicy.from_obj(None)
        )
        self.assertEqual(
            save_options.VariablePolicy.SAVE_VARIABLE_DEVICES,
...
class SavingOptionsTest(test.TestCase):
    ...
    def test_accepts_variable_policy(self):
        options = save_options.SaveOptions()
        self.assertEqual(
            save_options.VariablePolicy.NONE, options.experimental_variable_policy
        )
        ...
        options = save_options.SaveOptions(
            experimental_variable_policy=save_options.VariablePolicy.EXPAND_DISTRIBUTED_VARIABLES
        )
        self.assertEqual(
            save_options.VariablePolicy.EXPAND_DISTRIBUTED_VARIABLES,
            options.experimental_variable_policy,
...
class AssetTests(test.TestCase):
    ...
    def test_untracked_table_useful_message(self):
        ...
        table = lookup_ops.HashTable(initializer, default_value=-1)
        root.table_user = def_function.function(
            table.lookup, input_signature=[tensor_spec.TensorSpec(None, dtypes.string)]
        )
