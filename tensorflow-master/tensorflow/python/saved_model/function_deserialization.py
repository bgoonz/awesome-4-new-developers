...
def _call_concrete_function(function, inputs):
  ...
  """
    ...
    result = function._call_flat(
        tensor_inputs, function._captured_inputs
...
def _deserialize_function_spec_as_nonmethod(function_spec_proto, coder):
    ...
    return function_lib.FunctionSpec(
...
def setup_bare_concrete_function(saved_bare_concrete_function, concrete_functions):
    ...
    if saved_bare_concrete_function.HasField("function_spec"):
        ...
        function_spec = _deserialize_function_spec_as_nonmethod(
            saved_bare_concrete_function.function_spec, coder
        )
...
def load_function_def_library(
    library, load_shared_name_suffix=None, wrapper_function=None
):
  ...
  """
    ...
    for fdef in _sort_function_defs(library, function_deps):
        copy = _fix_fdef(
            fdef, functions, load_shared_name_suffix, new_gradient_op_types
        )
...
def _restore_gradient_functions(func_graph, renamed_functions, loaded_gradients):
    ...
    for op in func_graph.get_operations():
        ...
        if op.type in ["StatefulPartitionedCall", "PartitionedCall"]:
            function = renamed_functions[
                compat.as_bytes(op.node_def.attr["f"].func.name)
            ]
            op._gradient_function = (
                function._get_gradient_function()
            )  # pylint: disable=protected-access
        try:
            ...
            if gradient_op_type in loaded_gradients:
                ...
                grad_fn._num_positional_args = len(
                    op.inputs
                )  # pylint: disable=protected-access
                grad_fn._arg_keywords = [
...
def _sort_function_defs(library, function_deps):
    ...
    if len(output) != len(library.function):
        ...
        raise ValueError(
            "There is a cyclic dependency between functions. ",
...
def _get_gradient_op_type(node_def):
    ...
    if "_gradient_op_type" in node_def.attr and node_def.op not in [
...
def fix_node_def(node_def, functions, shared_name_suffix):
    ...
    if node_def.op == "HashTableV2":
        if (
            "use_node_name_sharing" not in node_def.attr
    ...
    if op_def:
        ...
        if attr:
            ...
            node_def.attr["shared_name"].s = shared_name + compat.as_bytes(
                shared_name_suffix
            )
...
_FUNCTION_WRAPPER_NAME_REGEX = r"^%s(.*)_\d+$" % (
    function_lib._INFERENCE_PREFIX
)  # pylint:disable=protected-access
