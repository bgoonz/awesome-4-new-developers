  ...
  """
    ...
    """
        ...
        _add_op_to_signature_def_map(
            signature_def_map, init_op, constants.INIT_OP_SIGNATURE_KEY
        )
        _add_op_to_signature_def_map(
            signature_def_map, train_op, constants.TRAIN_OP_SIGNATURE_KEY
        )
...
class SavedModelBuilder(_SavedModelBuilder):
    ...
    """
        ...
        # graph.
        asset_filename_map = _maybe_save_assets(
            _add_asset_to_collection, assets_collection_to_add
        )
    ...
    """
        ...
        if not isinstance(main_op, ops.Operation):
            raise TypeError(
                f"Expected {main_op} to be an Operation but got type "
    ...
    """
        if train_op is not None:
            if not isinstance(train_op, ops.Tensor) and not isinstance(
                train_op, ops.Operation
            ):
    ...
    add_meta_graph_and_variables.__doc__ = _SavedModelBuilder.add_meta_graph_and_variables.__doc__.replace(
        "assets_list", "assets_collection"
    )
...
def _asset_path_from_tensor(path_tensor):
  ...
  """
    ...
    if path_tensor.op.type != "Const":
        raise TypeError(
            f"Asset path tensor {path_tensor} must be of type constant."
            f"Has type {path_tensor.op.type} instead."
        )
    if path_tensor.dtype != dtypes.string:
        raise TypeError(
            f"Asset path tensor {path_tensor}` must be of dtype string."
...
def copy_assets_to_destination_dir(asset_filename_map, destination_dir):
    ...
    assets_destination_dir = saved_model_utils.get_or_create_assets_dir(destination_dir)

    # Copy each asset from source path to destination path.
    for asset_basename, asset_source_filepath in asset_filename_map.items():
        asset_destination_filepath = os.path.join(
            compat.as_bytes(assets_destination_dir), compat.as_bytes(asset_basename)
        )
