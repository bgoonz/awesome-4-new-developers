...
def classification_signature_def(examples, classes, scores):
  ...
  """
    ...
    if not isinstance(examples, ops.Tensor):
        raise ValueError(
            "Classification `examples` must be a string Tensor. "
            f"Found `examples` of type {type(examples)}."
        )
    if classes is None and scores is None:
        raise ValueError(
            "Classification `classes` and `scores` cannot both be " "None."
        )
    ...
    signature_def = build_signature_def(
        signature_inputs, signature_outputs, signature_constants.CLASSIFY_METHOD_NAME
    )
...
def predict_signature_def(inputs, outputs):
  ...
  """
    ...
    signature_outputs = {
        key: utils.build_tensor_info(tensor) for key, tensor in outputs.items()
    }
...
def is_valid_signature(signature_def):
    ...
    if signature_def is None:
        return False
    return (
        _is_valid_classification_signature(signature_def)
        or _is_valid_regression_signature(signature_def)
        or _is_valid_predict_signature(signature_def)
    )
...
def _is_valid_regression_signature(signature_def):
    ...
    if set(signature_def.inputs.keys()) != set([signature_constants.REGRESS_INPUTS]):
        return False
    if (
        signature_def.inputs[signature_constants.REGRESS_INPUTS].dtype
        != types_pb2.DT_STRING
    ):
        return False
    ...
    if set(signature_def.outputs.keys()) != set([signature_constants.REGRESS_OUTPUTS]):
        return False
    if (
        signature_def.outputs[signature_constants.REGRESS_OUTPUTS].dtype
        != types_pb2.DT_FLOAT
    ):
        return False
...
def _is_valid_classification_signature(signature_def):
    ...
    if set(signature_def.inputs.keys()) != set([signature_constants.CLASSIFY_INPUTS]):
        return False
    if (
        signature_def.inputs[signature_constants.CLASSIFY_INPUTS].dtype
        != types_pb2.DT_STRING
    ):
        return False
    ...
    if set(signature_def.outputs.keys()) - allowed_outputs:
        return False
    if (
        signature_constants.CLASSIFY_OUTPUT_CLASSES in signature_def.outputs
        and signature_def.outputs[signature_constants.CLASSIFY_OUTPUT_CLASSES].dtype
        != types_pb2.DT_STRING
    ):
        return False
    if (
        signature_constants.CLASSIFY_OUTPUT_SCORES in signature_def.outputs
        and signature_def.outputs[signature_constants.CLASSIFY_OUTPUT_SCORES].dtype
        != types_pb2.DT_FLOAT
    ):
        return False
