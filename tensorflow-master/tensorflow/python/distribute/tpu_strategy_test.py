...
class TPUTest(test.TestCase):
    ...
    def test_tpu_output_device(self):
        ...
        func2 = function.defun_with_attributes(
            foo, attributes={"_OutputsOnOpDevice": True, "_XlaMustCompile": False}
        )
        ...
        self.assertAllEqual(
            ret1.backing_device, "/job:localhost/replica:0/task:0/device:CPU:0"
        )
        self.assertAllEqual(
            ret2.backing_device, "/job:localhost/replica:0/task:0/device:TPU:0"
        )
...
class TPUStrategyTest(test.TestCase, parameterized.TestCase):
    ...
    def testStaticHashTableDatasetFnHostTrainingLoop(self, enable_packed_var):
        ...
        def dataset_fn(input_context):
            ...
            dataset = (
                dataset_ops.Dataset.from_tensors(tensor)
                .repeat()
    ...
    def test_device_switch_case(self, enable_packed_var):
        ...
        self.assertAllEqual(3.0, run_inference(1))  # Use TPU core 1.
    ...
    def test_dynamic_shape_with_outside_compilation_failure(self, enable_packed_var):
        ...
        strategy = get_tpu_strategy(enable_packed_var)
        dataset = (
            dataset_ops.Dataset.from_tensors(("string", 1.0))
            .repeat()
    ...
    def test_tpu_run_arg_parsing(self, enable_packed_var):
        ...
        with self.assertRaisesRegex(TypeError, r"got multiple values for argument"):
            ...
            def step():
                strategy.run(
                    named_args, args=(1,), kwargs={"first_arg": 2, "second_arg": 3}
                )

            step()

        with self.assertRaisesRegex(
            ValueError, r"cannot handle Variables passed to \*args"
        ):
        ...
        with self.assertRaisesRegex(ValueError, r"mix of positional args and \*args"):
    ...
    def test_using_external_variable_inside_tf_function(self, enable_packed_var):
        ...
        dataset = dataset_ops.Dataset.range(
            strategy.num_replicas_in_sync * 2, output_type=dtypes.float32
        ).batch(strategy.num_replicas_in_sync)
    ...
    def test_all_reduce_on_sync_on_read_variable(self, enable_packed_var):
        strategy = get_tpu_strategy(enable_packed_var)
        dataset = dataset_ops.Dataset.range(
            strategy.num_replicas_in_sync, output_type=dtypes.float32
        ).batch(strategy.num_replicas_in_sync, drop_remainder=True)
        input_iterator = iter(strategy.experimental_distribute_dataset(dataset))
    ...
    def test_run_output_on_device(self, enable_packed_var):
        ...
        self.assertAllEqual(
            "/job:localhost/replica:0/task:0/device:TPU:0", results[0].backing_device
        )
        self.assertAllEqual(
            "/job:localhost/replica:0/task:0/device:TPU:1", results[1].backing_device
        )
            ...
            def make_sparse(_):
                return sparse_tensor.SparseTensor(
                    indices=array_ops.constant(
                        [[0, 0], [1, 0], [1, 1]], dtype=dtypes.int64
                    ),
...
class TPUStrategyDataPrefetchTest(test.TestCase):
    ...
    def test_prefetch_to_device_tpu(self):
        ...
        dataset_item = next(
            iter(
                strategy.experimental_distribute_dataset(dataset, options=input_options)
            )
        )
        dataset_location = tf_device.DeviceSpec.from_string(
            dataset_item.values[0].device
    ...
    def test_prefetch_to_device_sparse_dataset(self):
        ...
        dataset = dataset_ops.Dataset.from_tensors(
            sparse_tensor.SparseTensor(
                indices=[[0, 0], [0, 1], [1, 0]], values=[1, 2, 3], dense_shape=[2, 2]
            )
        )
...
class TPUStrategyDistributionTest(
    ...
    def test_reduce(self):
        ...
        inputs = strategy.make_input_fn_iterator(
            lambda _: dataset_ops.Dataset.from_tensor_slices([2.0, 3.0])
        )
    ...
    def test_constraint(self):
        ...
        with strategy.scope():
            variable = variables.Variable(
                initial_value=2.0, constraint=lambda x: 0.0 * x + 1.0
            )
    ...
    def test_model_parallelism(self):
        ...
        with self.cached_session():
            ...
            self.assertEqual(30.0, self.evaluate(result))
    ...
    def test_model_parallelism_checkpointing(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertEqual(20.0, self.evaluate(result))
...
class DeviceAssignmentTest(test.TestCase):
    ...
    def test_device_assignment_strategy_properties(self):
        ...
        self.assertEqual(
            strategy.extended.num_replicas_per_host, 1
    ...
    def test_variables_mismatched_device_assignment(self):
        ...
        with self.cached_session():
            ...
            self.assertAllEqual(
                [1.0, 42.0], self.evaluate(strategy0.experimental_local_results(v))
            )
        ...
        with self.cached_session():
            self.assertAllEqual(
                [42.0, 1.0], self.evaluate(strategy0.experimental_local_results(v_read))
            )
