...
class _VariableInterfaceTestBase(test.TestCase, parameterized.TestCase):
    ...
    def testDenseUpdate(self):
        v = self.create_variable(1.0)
        self.assertEqual(
            v.assign(2.0, use_locking=True, name="assign", read_value=True), 2.0
        )
        self.assertIsNone(v.assign(3.0, read_value=False))
        self.assertEqual(v, 3.0)
        self.assertEqual(
            v.assign_add(1.0, use_locking=True, name="assign_add", read_value=True), 4.0
        )
        self.assertIsNone(v.assign_add(1.0, read_value=False))
        self.assertEqual(v, 5.0)
        self.assertEqual(
            v.assign_sub(1.0, use_locking=True, name="assign_sub", read_value=True), 4.0
        )
    ...
    def testSparseUpdate(self):
        ...
        self.assertAllEqual(
            v.scatter_add(
                _make_index_slices(values=[1.0, 2.0], indices=[0, 2]),
                use_locking=True,
                name="add",
            ),
            [1.0, 0.0, 2.0],
        ...
        self.assertAllEqual(
            v.scatter_div(
                _make_index_slices(values=[4.0, 2.0], indices=[0, 2]),
                use_locking=True,
                name="div",
            ),
            [0.25, 0.0, 1.0],
        ...
        self.assertAllEqual(
            v.scatter_max(
                _make_index_slices(values=[1.0, 0.5], indices=[1, 2]),
                use_locking=True,
                name="max",
            ),
            [0.25, 1.0, 1.0],
        ...
        self.assertAllEqual(
            v.scatter_min(
                _make_index_slices(values=[1.0, 0.5], indices=[0, 1]),
                use_locking=True,
                name="min",
            ),
            [0.25, 0.5, 1.0],
        ...
        self.assertAllEqual(
            v.scatter_mul(
                _make_index_slices(values=[2.0, 0.5], indices=[0, 1]),
                use_locking=True,
                name="mul",
            ),
            [0.5, 0.25, 1.0],
        ...
        self.assertAllEqual(
            v.batch_scatter_update(
                _make_index_slices(values=[1.0, 1.5], indices=[0, 1]),
                use_locking=True,
                name="update",
            ),
            [1.0, 1.5, 1.0],
    ...
    def testSparseNdUpdate(self):
        ...
        self.assertAllEqual(
            v.scatter_nd_sub([[3], [1]], [1.0, 2.0], name="sub"), [0.0, -2.0, 0.0, -1.0]
        )
        self.assertAllEqual(
            v.scatter_nd_add([[2], [0]], [1.0, 2.0], name="add"), [2.0, -2.0, 1.0, -1.0]
        )
        self.assertAllEqual(
            v.scatter_nd_update([[1], [3]], [3.0, 3.0], name="update"),
            [2.0, 3.0, 1.0, 3.0],
        )
    ...
    def testSparseRead(self):
        ...
        self.assertAllEqual(
            v.sparse_read([1, 0], name="read"), [[3.0, 4.0], [1.0, 2.0]]
        )
...
class DistributedVariableTest(test.TestCase, parameterized.TestCase):
    ...
    def testStrategyRun(self, strategy, enable_packed_handle, tf_function):
        if (
            test_util.is_tpu_strategy(strategy)
