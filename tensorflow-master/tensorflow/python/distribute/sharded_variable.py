...
class MinSizePartitioner(Partitioner):
  ...
  """
    ...
    """
        ...
        if max_shards < 1:
            raise ValueError(
                "Argument `max_shards` must be positive. " f"Received: {max_shards}"
            )
...
class ShardedVariableMixin(trackable.Trackable):
    ...
    def __init__(
        self, variables: Sequence[variables_lib.Variable], name="ShardedVariable"
    ):
    ...
    """
        ...
        save_slice_info = [
            v._get_save_slice_info() for v in variables
    ...
    """
        if (
            isinstance(slice_spec.start, ops.Tensor)
            or isinstance(slice_spec.stop, ops.Tensor)
            or isinstance(slice_spec.step, ops.Tensor)
        ):
            raise TypeError(
    ...
    def _map_resources(self, save_options):
        ...
        obj_map[self] = ShardedVariable(
            [obj_map[self._saving_variable]], name=self.name
        )
...
def embedding_lookup(
    ...
    return embedding_ops.embedding_lookup(
        params.variables, ids, partition_strategy, name, validate_indices, max_norm
    )
