...
class MultiProcessRunnerTest(test.TestCase, parameterized.TestCase):
    ...
    def test_start_in_process_as(self):
        ...
        mpr = multi_process_runner.MultiProcessRunner(
            fn,
            multi_worker_test_base.create_cluster_spec(has_chief=True, num_workers=1),
            return_output=True,
        )
        ...
        for job in ["worker", "evaluator"]:
            for iteration in range(5):
                self.assertTrue(
                    any(
                        "{}-0, i: {}".format(job, iteration) in line
                        for line in list_to_assert
                    )
                )
    ...
    def test_auto_restart_failure_immediate_after_restart(self):
        ...
        mpr = multi_process_runner.MultiProcessRunner(
            fn,
            multi_worker_test_base.create_cluster_spec(has_chief=False, num_workers=2),
            auto_restart=True,
        )
        mpr.start()
    ...
    def test_auto_restart_terminate(self):
        ...
        mpr = multi_process_runner.MultiProcessRunner(
            fn,
            multi_worker_test_base.create_cluster_spec(has_chief=False, num_workers=1),
            args=(counter,),
            auto_restart=True,
        )
        mpr.start()
