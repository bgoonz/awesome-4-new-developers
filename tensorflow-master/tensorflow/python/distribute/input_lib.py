...
def get_iterator_spec_from_dataset(strategy, dataset):
  ...
  """
    ...
    if isinstance(
        dataset._type_spec,  # pylint: disable=protected-access
        (DistributedDatasetSpec, DistributedDatasetsFromFunctionSpec),
    ):
...
class InputWorkers(object):
    ...
    """
        ...
        if canonicalize_devices:
            self._fed_devices = tuple(
                tuple(device_util.canonicalize(d) for d in f)
                for _, f in self._worker_device_pairs
            )
        else:
            self._fed_devices = tuple(
                tuple(device_util.canonicalize_without_job_and_task(d) for d in f)
                for _, f in self._worker_device_pairs
            )
    ...
    def __repr__(self):
        ...
        debug_repr = ",\n".join(
            "  %d %s: %s" % (i, devices[i], self._fed_devices[i])
  ...
  """
    ...
    with distribution_strategy_context.enter_or_assert_strategy(strategy):
        if distribution_strategy_context.get_replica_context() is not None:
            raise ValueError(
                "next(iterator) should be called from outside of "
                "replica_fn. e.g. strategy.run(replica_fn, "
                "args=(next(iterator),))"
            )

    for i, worker in enumerate(
        iterator._input_workers.worker_devices
    ):  # pylint: disable=protected-access
        with ops.device(worker):
            worker_has_value, next_element = iterator._iterators[
                i
            ].get_next_as_list()  # pylint: disable=protected-access
  ...
  """
    ...
    for spec in nest.flatten(element_spec):
        if isinstance(
            ...
            for component in nest.flatten(
                spec._component_specs
            ):  # pylint: disable=protected-access
...
class DistributedIteratorBase(DistributedIteratorInterface):
    ...
    def __init__(self, input_workers, iterators, strategy, enable_get_next_as_optional):
    ...
    def get_next_as_optional(self):
        ...
        return control_flow_ops.cond(
            global_has_value,
            lambda: optional_ops.Optional.from_value(replicas),
            return_none,
        )
    ...
    def get_next(self, name=None):
        ...
        if not self._enable_get_next_as_optional:
            with distribution_strategy_context.enter_or_assert_strategy(self._strategy):
                if distribution_strategy_context.get_replica_context() is not None:
                    raise ValueError(
                        "next(iterator) should be called from outside of "
                        "replica_fn. e.g. strategy.run(replica_fn, "
...
class DistributedDatasetAndIteratorSpec(type_spec.TypeSpec):
    ...
    def __init__(
        ...
        if isinstance(input_workers, tuple):
            raise NotImplementedError(
                "DistributedIteratorSpec does not have support " "for deserialization."
            )
        else:
            self._input_workers = input_workers
            ...
            if self._strategy:
                self._canonicalize_devices = getattr(
                    self._strategy, "_canonicalize_devices", True
                )
            else:
    ...
    """
        ...
        if self._input_workers.serialize() != other._input_workers.serialize():
            raise ValueError(
                "_input_workers is not compatible with both %s "
                "and %s" % (self, other)
            )
        if self._strategy is not other._strategy:
            raise ValueError(
                "tf.distribute strategy is not compatible with both %s "
                "and %s" % (self, other)
            )
...
class DistributedIteratorSpec(DistributedDatasetAndIteratorSpec):
    ...
    def __init__(
        ...
        super(DistributedIteratorSpec, self).__init__(
            input_workers, element_spec, strategy, options, enable_get_next_as_optional
        )
    ...
    """
        ...
        element_spec = nest.map_structure(
            lambda a, b: a.most_specific_compatible_type(b),
            self._element_spec,
            other._element_spec,
        )
...
class DistributedIterator(DistributedIteratorBase, composite_tensor.CompositeTensor):
    ...
    def __init__(
        ...
        if iterators is None:
            ...
            super(DistributedIterator, self).__init__(
                input_workers, iterators, strategy, enable_get_next_as_optional
            )
    ...
    def element_spec(self):
        ...
        if (
            self._enable_get_next_as_optional
            and self._strategy.extended._in_multi_worker_mode()
        ):  # pylint: disable=protected-access
...
class DistributedDatasetSpec(DistributedDatasetAndIteratorSpec):
    ...
    def __init__(
        ...
        super(DistributedDatasetSpec, self).__init__(
            input_workers, element_spec, strategy, options, enable_get_next_as_optional
        )
...
class DistributedDataset(_IterableInput, composite_tensor.CompositeTensor):
    ...
    def build(self, dataset_to_replace=None):
        ...
        self._create_cloned_datasets_from_dataset(
            dataset,
            self._input_context,
            self._input_workers,
            self._strategy,
    ...
    def _create_cloned_datasets_from_dataset(
        self, dataset, input_context, input_workers, strategy, num_replicas_in_sync
    ):
        ...
        if num_replicas_in_sync is not None:
            num_workers = (
                ...
                if input_context
                else len(input_workers.worker_devices)
            )
            rebatch_fn = self._make_rebatch_fn(
                dataset, num_workers, num_replicas_in_sync
            )
        else:
            rebatch_fn = None
            ...
            for i, worker in enumerate(input_workers.worker_devices):
                with ops.device(worker):
                    ...
                    cloned_dataset = input_ops.auto_shard_dataset(
                        cloned_dataset,
                        len(input_workers.worker_devices),
                        i,
                        num_replicas_in_sync,
                    )
        ...
        def rebatch_fn(dataset, worker_index):
            try:
                ...
                def apply_rebatch():
                    ...
                    return distribute._RebatchDataset(dataset, batch_sizes).prefetch(
    ...
    def __iter__(self):
        if not (
            context.executing_eagerly() or ops.get_default_graph().building_function
        ):
            raise RuntimeError(
        ...
        if not self._built:
            raise ValueError(
                "To use this dataset, you need to pass this dataset to "
        ...
        enable_legacy_iterators = getattr(
            self._strategy, "_enable_legacy_iterators", False
        )
...
class DistributedDatasetV1(DistributedDataset):
    ...
    def _get_iterator(self):
        worker_iterators = _create_iterators_per_worker(
            self._cloned_datasets, self._input_workers, True, self._options
        )
        iterator = DistributedIteratorV1(
            self._input_workers,
            worker_iterators,
...
class DistributedDatasetsFromFunctionSpec(DistributedDatasetAndIteratorSpec):
    ...
    def __init__(self, input_workers, element_spec, strategy, options):
        super(DistributedDatasetsFromFunctionSpec, self).__init__(
            input_workers, element_spec, strategy, options
        )
    ...
    """
        ...
        return DistributedDatasetsFromFunctionSpec(
            self._input_workers, element_spec, self._strategy, self._options
        )
...
class DistributedDatasetsFromFunction(_IterableInput, composite_tensor.CompositeTensor):
    ...
    def build(self):
        assert not self._built
        self._datasets, element_spec = _create_datasets_from_function_with_input_context(
    ...
    def __iter__(self):
        ...
        if not self._built:
            raise ValueError(
                "You need to use this dataset in "
                "ClusterCoordinator.create_per_worker_dataset."
            )
    ...
    def _type_spec(self):
        return DistributedDatasetsFromFunctionSpec(
            self._input_workers, self._element_spec, self._strategy, self._options
        )
...
class DistributedDatasetsFromFunctionV1(DistributedDatasetsFromFunction):
    ...
    def _get_iterator(self):
        iterators = _create_iterators_per_worker(
            self._datasets, self._input_workers, True, self._options
        )
        iterator = DistributedIteratorV1(
            self._input_workers,
            iterators,
...
class DatasetIterator(DistributedIteratorV1):
    ...
    """
        ...
        worker_iterators = _create_iterators_per_worker(
            dist_dataset._cloned_datasets, input_workers, True
        )  # pylint: disable=protected-access
...
def _dummy_tensor_fn(value_structure):
    ...
    def create_dummy_tensor(spec):
        ...
        dims = (
            [dim if dim is not None else 0 for dim in feature_shape.as_list()]
            if feature_shape
            else []
        )
        if dims and (
            isinstance(spec, ragged_tensor.RaggedTensorSpec)
            or feature_shape.is_fully_defined()
        ):
...
class _SingleWorkerDatasetIteratorBase(object):
    ...
    """
        if (
            self._options
            and self._options.experimental_replication_mode
            == InputReplicationMode.PER_REPLICA
            and not self._options.experimental_fetch_to_device
        ):
            return [data_list]
        else:
            return data_list
...
class _SingleWorkerDatasetIteratorSpec(type_spec.TypeSpec):
    ...
    def __init__(
        self, worker, devices, element_spec, options, canonicalize_devices=True
    ):
        self._worker = worker
        if canonicalize_devices:
        ...
        self._options = (
            options if options is not None else distribute_lib.InputOptions()
        )
        self._canonicalize_devices = canonicalize_devices
    ...
    def from_value(value):
        # pylint: disable=protected-access
        return _SingleWorkerDatasetIteratorSpec(
            value._worker,
            value._devices,
            value._element_spec,
            value._options,
...
class _SingleWorkerOwnedDatasetIterator(
    _SingleWorkerDatasetIteratorBase, composite_tensor.CompositeTensor
):
    ...
    """
        ...
        error_message = (
            "Either `dataset` or both `components` and `element_spec` "
            "need to be provided."
        )
        ...
        if dataset is None:
            ...
            if components is not None or element_spec is not None:
                raise ValueError(error_message)
            super(_SingleWorkerOwnedDatasetIterator, self).__init__(
                dataset, worker, devices, self._options
            )
    ...
    def _make_iterator(self):
        ...
        if not self._worker:
            raise ValueError(
                "Worker device must be specified when creating an " "owned iterator."
            )
        if _should_use_multi_device_iterator(self._options):
...
def _create_iterators_per_worker(
    worker_datasets,
    input_workers,
    enable_legacy_iterators,
    options=None,
        ...
        with ops.device(worker):
            ...
            if tf2.enabled() and not enable_legacy_iterators:
                ...
                iterator = _SingleWorkerDatasetIterator(
                    worker_datasets[i], worker, worker_devices, options
                )
            iterators.append(iterator)
    return iterators


def _create_datasets_from_function_with_input_context(
...
def _get_dataset_attributes(dataset):
    ...
    elif isinstance(dataset, dataset_ops.DatasetV1Adapter) and isinstance(
        dataset._dataset, dataset_ops.PrefetchDataset
    ):
...
def _enable_get_next_as_optional(strategy, dataset):
    ...
    if not getattr(
        ...
        getattr(strategy.extended, "experimental_enable_get_next_as_optional", False),
    ):
        return False
    ...
    if context.executing_eagerly():
        ...
        # cardinality.
        with ops.device(
            dataset._variant_tensor.device
        ):  # pylint: disable=protected-access
            if dataset.cardinality().numpy() == cardinality.INFINITE:
                return False

    return (
        not _is_statically_shaped(dataset.element_spec)
        or strategy.extended._in_multi_worker_mode()
    )  # pylint: disable=protected-access
...
def _always_wrap(strategy):
    ...
    return (
        ...
        or len(strategy.extended.worker_devices) > 1  # pylint: disable=protected-access
    )
