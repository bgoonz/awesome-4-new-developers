...
from tensorflow.python.tpu import (
    device_assignment as device_assignment_lib,
...
class TPUStrategyV2(distribute_lib.Strategy):
  ...
  """
    ...
    def __init__(self, tpu_cluster_resolver=None, experimental_device_assignment=None):
    ...
    """
        ...
        distribute_lib.distribution_strategy_replica_gauge.get_cell("num_workers").set(
            self.extended.num_hosts
        )
        distribute_lib.distribution_strategy_replica_gauge.get_cell(
            "num_replicas_per_worker"
        ).set(self.extended.num_replicas_per_host)
    ...
    """
        num_logical_devices_per_replica = self.extended._tpu_devices.shape[
            1
        ]  # pylint: disable=protected-access
        if (
            logical_device_id < 0
            or logical_device_id >= num_logical_devices_per_replica
        ):
            raise ValueError(
                "`logical_core_id` to assign must be lower then total "
    ...
    """
        ...
        if tensor_rank != len(partition_dimensions):
            raise ValueError(
                "Length of `partition_dimensions` must equal to the "
        ...
        for dim_index, dim_size in enumerate(input_shape):
            ...
            if dim_size % split_size != 0:
                raise ValueError(
                    "Tensor shape at `partition_dimensions[{}]` must be "
  ...
  """
    ...
    """
        ...
        super(TPUStrategy, self).__init__(
            TPUExtended(self, tpu_cluster_resolver, device_assignment=device_assignment)
        )
        distribute_lib.distribution_strategy_gauge.get_cell("V2").set("TPUStrategy")
        distribute_lib.distribution_strategy_replica_gauge.get_cell("num_workers").set(
            self.extended.num_hosts
...
class TPUStrategyV1(distribute_lib.StrategyV1):
    ...
    def __init__(
        self, tpu_cluster_resolver=None, steps_per_run=None, device_assignment=None
    ):
    ...
    """
        super(TPUStrategyV1, self).__init__(
            TPUExtended(self, tpu_cluster_resolver, steps_per_run, device_assignment)
        )
...
class TPUExtended(distribute_lib.StrategyExtendedV1):
    ...
    def _make_input_fn_iterator(
        ...
        return input_lib.InputFunctionIterator(
            input_fn, input_workers, input_contexts, self._container_strategy()
        )
    ...
    def _gather_to_implementation(self, value, destinations, axis, options):
        ...
        # pylint: disable=protected-access
        if (
            isinstance(value, values.DistributedVariable)
    ...
    def _reduce_to(self, reduce_op, value, destinations, options):
        if (
            isinstance(value, values.DistributedValues) or tensor_util.is_tf_type(value)
    ...
    def _update(self, var, fn, args, kwargs, group):
        ...
        if (
            var.synchronization != variables_lib.VariableSynchronization.ON_READ
            and var.aggregation != variables_lib.VariableAggregation.NONE
        ):
        ...
        for i, value_and_device in enumerate(values_and_devices):
            ...
            name = "update_%d" % i
            with ops.device(device), distribute_lib.UpdateContext(i), ops.name_scope(
                name
            ):
    ...
    def num_hosts(self):
        ...
        return len(
            set(
                [
                    self._device_assignment.host_device(r)
    ...
    def num_replicas_per_host(self):
        ...
        max_models_per_host = (
            self._tpu_metadata.num_of_cores_per_host
    ...
    def _tpu_function_creator(self, fn, options):
        ...
        def tpu_function(args, kwargs):
            ...
            if options.experimental_enable_dynamic_batch_size and replicate_inputs:
                ...
                maximum_shapes = nest.pack_sequence_as(
                    replicate_inputs[0], maximum_shapes
                )
            else:
                maximum_shapes = None
...
class _TPUReplicaContext(distribute_lib.ReplicaContext):
    ...
    def all_gather(self, value, axis, experimental_hints=None):
        ...
        def _all_to_all(value, axis):
            ...
            squeezed = [
                array_ops.squeeze(replica, axis=0) for replica in splited_with_extra_dim
            ]
...
def _set_last_step_outputs(ctx, last_step_tensor_outputs):
    ...
    for (
        name,
        reduce_op,
    ) in ctx._last_step_outputs_reduce_ops.items():  # pylint: disable=protected-access
