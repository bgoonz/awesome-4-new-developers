...
class SaveModelForMultipleWorkers(test.TestCase, parameterized.TestCase):
    def test_read_sync_on_read_variable(self, strategy):
        ...
        export_dir = os.path.join(self._get_tempdir_path_test(), "test-file-failure")
        with strategy.scope():
            m = Model()
            m.v.assign(1.0)
...
class SaveAndLoadForServingTest(test.TestCase, parameterized.TestCase):
    ...
    def test_read_sync_on_read_variable(self, strategy):
        ...
        self.assertEqual(self.evaluate(loaded.v.read_value()), 1.0)
        self.assertEqual(self.evaluate(loaded()), 1.0)
    ...
    def test_read_mirrored_variable(self, strategy):
        ...
        class Model(tf.Module):
            def __init__(self):
                self.v = tf.Variable(
                    0.0, synchronization=tf.VariableSynchronization.ON_WRITE
                )
    ...
    def test_update_sync_on_read_variable(self, strategy):
        ...
        loaded = tf.saved_model.load(export_dir)
        loaded.update()
        self.assertEqual(self.evaluate(loaded.v), 1.0)
    ...
    def test_update_mirrored_variable(self, strategy):
        ...
        loaded = tf.saved_model.load(export_dir)
        self.assertEqual(self.evaluate(loaded.v), 0.0)
        loaded.update()
        self.assertEqual(self.evaluate(loaded.v), 1.0)
    ...
    def test_training_only_device(self, strategy):
        ...
        self.assertEqual(loaded().numpy(), 1.0)
    ...
    def test_model_with_loaded_layer(self, strategy):
        ...
        loaded = tf.saved_model.load(export_dir)
        self.assertAllEqual(
            self.evaluate(strategy.experimental_local_results(strategy.run(loaded))),
            [1.0, 1.0],
        )
...
class SaveAndLoadForTrainingTest(test.TestCase, parameterized.TestCase):
    ...
    def test_read_sync_on_read_variable(self, strategy):
        ...
        value = strategy.experimental_distribute_values_from_function(
            lambda ctx: tf.identity([3.0, 7.0][ctx.replica_id_in_sync_group])
        )
        with strategy.scope():
            m = Model()
            strategy.run(m.v.assign, args=(value,))
            self.assertAllEqual(
                self.evaluate(strategy.experimental_local_results(m.v)), [3.0, 7.0]
            )
            self.assertEqual(self.evaluate(m.v.read_value()), 10.0)
            tf.saved_model.save(m, export_dir)
            del m
        ...
        self.assertEqual(self.evaluate(loaded.v.read_value()), 10.0)
    ...
    def test_update_sync_on_read_variable(self, strategy):
        ...
        with strategy.scope():
            m = Model()
            tf.saved_model.save(m, export_dir)
            self.evaluate(m.v.assign(10.0))
            self.assertAllEqual(
                self.evaluate(strategy.experimental_local_results(m.v)), [5.0, 5.0]
            )
            del m
        ...
        with strategy.scope():
            loaded = tf.saved_model.load(export_dir)
            self.evaluate(loaded.v.assign(10.0))
            self.assertAllEqual(
                self.evaluate(strategy.experimental_local_results(loaded.v)), [5.0, 5.0]
            )
    ...
    def test_update_mirrored_variable(self, strategy):
        ...
        export_dir = self.get_temp_dir()
        value = strategy.experimental_distribute_values_from_function(
            lambda ctx: tf.identity([1.0, 2.0][ctx.replica_id_in_sync_group])
        )
        with strategy.scope():
            m = Model()
            tf.saved_model.save(m, export_dir)
        ...
        with strategy.scope():
            loaded = tf.saved_model.load(export_dir)
        self.assertAllEqual(
            self.evaluate(strategy.experimental_local_results(loaded.v)), [0.0, 0.0]
        )
        self.evaluate(loaded.v.assign(1.0))
        self.assertAllEqual(
            self.evaluate(strategy.experimental_local_results(loaded.v)), [1.0, 1.0]
        )
        strategy.run(loaded.update, args=(value,))
        self.assertAllEqual(
            self.evaluate(strategy.experimental_local_results(loaded.v)), [2.0, 3.0]
        )
    ...
    def test_model_with_loaded_v1_layer_broken(self, strategy):
        ...
        with tf1.Graph().as_default(), tf1.Session() as sess:
            v = tf1.Variable(1.0, use_resource=True)
...
class PSStrategySaveAndLoadTest(test.TestCase):
    ...
    class Model(tf.Module):
        ...
        @tf.function(
            input_signature=[tf.TensorSpec(shape=(), dtype=tf.dtypes.int32, name="x")]
        )
        def __call__(self, x):
