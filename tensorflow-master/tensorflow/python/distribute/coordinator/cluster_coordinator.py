...
class InputError(Exception):
    def __init__(self, original_exception):
        ...
        message = (
            "Input has an error, the original exception is %r, "
            "error message is %s." % (original_exception, str(original_exception))
        )
...
def _maybe_rebuild_remote_values(worker, structure):
    ...
    def _get_error(val):
        if isinstance(val, RemoteValue):
            if (
                val._status is RemoteValueStatus.ABORTED
...
def _maybe_as_type_spec(val):
    if isinstance(val, (RemoteValue, PerWorkerValues)):
        if val._type_spec is None:  # pylint: disable=protected-access
            raise ValueError(
                "Output of a scheduled function that is not "
...
def _select_worker_slice(worker_id, structured):
    ...
    def _get(x):
        return (
            x._values[worker_id] if isinstance(x, PerWorkerValues) else x
        )  # pylint: disable=protected-access
...
class Closure(object):
    ...
    def __init__(self, function, cancellation_mgr, args=None, kwargs=None):
        if not callable(function):
            raise ValueError(
                "Function passed to `ClusterCoordinator.schedule` must "
    ...
    """
        ...
        e = _maybe_rebuild_remote_values(
            worker, replica_args
        ) or _maybe_rebuild_remote_values(worker, replica_kwargs)
        if e:
        ...
        self.maybe_call_with_output_remote_value(
            lambda r: r._set_values(output_values)
        )  # pylint: disable=protected-access
...
class _CoordinatedClosureQueue(object):
  ...
  """
    ...
    """
        if self._error:
            logging.error(
                "Start cancelling closures due to error %r: %s",
                self._error,
                self._error,
            )
...
class WorkerPreemptionHandler(object):
    ...
    def __init__(self, server_def, cluster):
        ...
        self._preemption_handler_thread = threading.Thread(
            target=self._preemption_handler, name="WorkerPreemptionHandler", daemon=True
        )
    ...
    def _validate_preemption_failure(self, e):
        ...
        if _is_worker_failure(e) and (
            not self._cluster.closure_queue._cancellation_mgr.is_cancelled
        ):  # pylint: disable=protected-access
            return
        raise e
    ...
    """
        ...
        try:
            ...
            if self._cluster._record_and_ignore_transient_ps_failure(
                e
            ):  # pylint: disable=protected-access
                logging.error(
        ...
        while True:
            ...
            with self._cluster_update_lock:
                try:
                    ...
                    # too frequently.
                    logging.error(
                        "Cluster update failed with error: %s. Retrying...", e
                    )
...
class Worker(object):
  ...
  """
        ...
        try:
            ...
            if not isinstance(e, errors.CancelledError):
                logging.error(
                    ...
                    "processing closure: %r:%s",
                    self.worker_index,
                    e,
                    e,
                )
            closure.maybe_call_with_output_remote_value(
                lambda r: r._set_error(e)
            )  # pylint: disable=protected-access
    ...
    def _maybe_delay(self):
        ...
        if delay_secs > 0:
            logging.info(
                "Worker %d sleeping for %d seconds before running function",
                self.worker_index,
                delay_secs,
            )
    ...
    def _register_resource(self, resource_remote_value):
        if not isinstance(resource_remote_value, RemoteValue):
            raise ValueError(
                "Resource being registered is not of type "
    ...
    def _record_and_ignore_transient_ps_failure(self, e):
        ...
        with self._potential_ps_failures_lock:
            for t in ps_tasks:
                ...
                if (
                    self._potential_ps_failures_count[t]
  ...
  """
    ...
    """
        ...
        with self.strategy.scope():
            self.strategy.extended._being_scheduled = (
                True
            )  # pylint: disable=protected-access
    ...
    """
        ...
        for w in self._cluster.workers:
            results.append(
                w.create_resource(fn, args=args, kwargs=kwargs)
            )  # pylint: disable=protected-access
...
def _is_ps_failure(error):
    ...
    return isinstance(error, errors.UnavailableError) and _RPC_ERROR_FROM_PS in str(
        error
    )
...
def _is_worker_failure(error):
    ...
    if isinstance(error, errors.InvalidArgumentError):
        if "unknown device" in str(
            error
        ) or "Unable to find the relevant tensor remote_handle" in str(error):
    ...
    if isinstance(error, errors.NotFoundError):
        if (
            "is neither a type of a primitive operation nor a name of a function "
