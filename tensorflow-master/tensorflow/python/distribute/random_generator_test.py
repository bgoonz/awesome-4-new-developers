...
class GeneratorTest(test.TestCase, parameterized.TestCase):
    ...
    @ds_combinations.generate(
        combinations.combine(strat=all_strategies, mode=["eager"])
    )
    ...
    @ds_combinations.generate(
        combinations.combine(
            strat=[strategy_combinations.parameter_server_strategy_1worker_2ps_cpu],
            mode=["eager"],
        )
    )
    ...
    @ds_combinations.generate(
        combinations.combine(
            strat=all_strategies, mode=["eager"], jit_replica_fn=[False, True]
        )
    )
    ...
    @ds_combinations.generate(
        combinations.combine(
            strat=all_strategies, mode=["eager"], is_save_in_scope=[True, False]
        )
    )
