...
if _is_enabled():
    ...
    class AbslForkServerProcess(
        _AbslProcess, multiprocessing.context.ForkServerProcess
    ):
...
def _if_spawn_run_and_exit():
    ...
    # "semaphore_tracker".
    is_spawned = "-c" in sys.argv[1:] and sys.argv[sys.argv.index("-c") + 1].startswith(
