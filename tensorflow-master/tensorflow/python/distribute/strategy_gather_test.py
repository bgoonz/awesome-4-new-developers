...
class GatherTest(test.TestCase, parameterized.TestCase):
    def _gather_same_shape_and_verify(
        self, value_on_replica, axis, pure_eager, strategy
    ):
    ...
    def testGatherRaiseDiffShapeAtNonAxis(self, strategy, pure_eager):
        ...
        if (
            isinstance(strategy, CollectiveAllReduceStrategy)
        ...
        if isinstance(strategy, CollectiveAllReduceStrategy):
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, r"Dimensions of inputs should match"
            ):
                run()
        else:
            with self.assertRaisesRegex(
                ValueError, r"Dimension \d in both shapes must be equal"
            ):
                run()
    ...
    def testGatherRaiseSparse(self, strategy, pure_eager):
        ...
        t0 = _make_indexed_slices(
            values=[[1.0, 2.0]], indices=[2], dense_shape=dense_shape
        )
        ...
        with self.assertRaisesRegex(
            NotImplementedError, r"gather does not support IndexedSlices"
        ):
            if pure_eager:
                run(t0)
            else:
    ...
    def testGatherRaiseDifferentRank(self, strategy, pure_eager):
        ...
        if isinstance(strategy, CollectiveAllReduceStrategy):
            ...
            with self.assertRaisesRegex(
                ValueError, r"Shape must be rank \d but is rank \d"
            ):
                run()
    ...
    def _all_gather_same_shape_and_verify(
        self, value_on_replica, axis, pure_eager, strategy
    ):
    ...
    def testAllGatherDiffValueTPU(self, strategy, pure_eager):
        ...
        if not _is_tpu_strategy(strategy):
            self.skipTest(
                "Test for TPU only. For other strategies case already"
    ...
    def testAllGatherNest(self, strategy, pure_eager):
        ...
        self.assertAllEqual(
            expected_per_replica_1, strategy.experimental_local_results(result[0])
        )
        self.assertAllEqual(
            expected_per_replica_2, strategy.experimental_local_results(result[1])
        )
    ...
    def testAllGatherRaiseSparse(self, strategy, pure_eager):
        ...
        with self.assertRaisesRegex(
            NotImplementedError, r"all_gather does not support IndexedSlices"
        ):
            if not pure_eager:
