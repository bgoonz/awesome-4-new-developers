    ...
    """
        ...
        if default is not cls.NO_DEFAULT:
            default = _convert_value(
                default, value_type, (f"default value for {name}",)
            )
...
def validate_field_value_type(
  ...
  """
    ...
    if value_type in (int, float, str, bytes, bool, None, _NoneType, dtypes.DType):
        ...
        value_type in (ops.Tensor, tensor_shape.TensorShape)
        or isinstance(value_type, type_spec.TypeSpec)
        or (
  ...
  """
    ...
    for field in fields:
        ...
        converted[field.name] = _convert_value(
            field_value, field.value_type, (field.name,), for_spec
        )
...
def _convert_tensor_spec(value, expected_type, path, for_spec):
    ...
    if for_spec:
        if not (
            isinstance(value, tensor_spec.TensorSpec)
            and expected_type.is_compatible_with(value)
        ):
            raise TypeError(
                f'{"".join(path)}: expected a TensorSpec compatible '
    ...
    if not isinstance(value, ops.Tensor):
        try:
            ...
            raise TypeError(
                f'{"".join(path)}: expected a {expected_type.dtype!r} '
                f"Tensor, got {value!r}"
            )
    if not expected_type.is_compatible_with(value):
        raise TypeError(
            f'{"".join(path)}: expected a Tensor compatible with '
...
def _convert_type_spec(value, expected_type, path, for_spec):
    ...
    if for_spec:
        if not (
            ...
            raise TypeError(
                f'{"".join(path)}: expected a TypeSpec compatible '
                f"with {expected_type}, got {value!r}"
            )
        return value

    if isinstance(value, type_spec.TypeSpec) or not expected_type.is_compatible_with(
        value
    ):
        raise TypeError(
            f'{"".join(path)}: expected {expected_type!r}, ' f"got {value!r}"
        )
    return value
