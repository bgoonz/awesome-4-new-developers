...
class ExtensionTypeFieldTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        [
            ...
            (
                "t",
                tensor_spec.TensorSpec([2], dtypes.int32),
            ...
            (
                "z",
                typing.Mapping[ops.Tensor, int],
                {},
                "In field 'z': Key must be hashable.",
            ),
        ]
    )
    ...
    @parameterized.parameters(
        [
            (
                ...
                "default=ExtensionTypeField.NO_DEFAULT)",
                "i",
                int,
            ),
            (
            ...
            (
                "ExtensionTypeField(name='j', value_type=<class 'int'>, default=3)",
                "j",
                int,
                3,
            ),
    ...
    def testIsReservedName(self, name, expected):
        self.assertEqual(
            extension_type_field.ExtensionTypeField.is_reserved_name(name), expected
        )
...
class ValidateFieldPyTypeTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                tp=typing.Union[int, list], error="Unsupported type annotation `list`"
            ),
            dict(
...
class FieldValueConverterTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            ({"x": 1}, "Missing required fields: {'y'}"),
            ({"x": 1, "y": 2.0, "z": 3}, "Got unexpected fields: {'z'}"),
        ]
    )
    ...
    @parameterized.parameters(
        [
            ...
            ({"a": 12}, typing.Mapping[str, int]),
            ({"a": (12, 3.0)}, typing.Mapping[str, typing.Tuple[int, float]]),
        ]
    )
    ...
    def testConvertFieldsForSpec(self):
        ...
        field_values = {"x": 1, "y": [1, True, 3], "z": tensor_spec.TensorSpec([5, 3])}
