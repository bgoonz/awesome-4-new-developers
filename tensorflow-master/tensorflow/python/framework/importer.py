...
def _ProcessInputMapParam(input_map):
    ...
    if input_map is None:
        ...
        if not (
            ...
            and all(isinstance(k, compat.bytes_or_text_types) for k in input_map.keys())
        ):
            raise TypeError(
        ...
        if node.op in producer_op_dict:
            ...
            for key in list(node.attr):
                if _FindAttrInOpDef(key, op_def) is None:
                    ...
                    if (
                        attr_def
                        and attr_def.HasField("default_value")
...
def _PopulateTFImportGraphDefOptions(
    options, prefix, input_map, return_elements, validate_colocation_constraints
):
    ...
    for input_src, input_dst in input_map.items():
        ...
        if input_src.startswith("^"):
            ...
            c_api.TF_ImportGraphDefOptionsAddInputMapping(
                options, src_name, src_idx, dst_output
            )
    for name in return_elements or []:
        if ":" in name:
            ...
            c_api.TF_ImportGraphDefOptionsAddReturnOperation(
                options, compat.as_str(name)
            )
...
def _ProcessNewOps(graph):
    ...
    for new_op in graph._add_new_tf_operations(
        compute_devices=False
    ...
    for op, coloc_op_list in colocation_pairs.items():
        ...
        for coloc_op_name in coloc_op_list:
            try:
                coloc_op = graph._get_operation_by_name_unsafe(
                    coloc_op_name
                )  # pylint: disable=protected-access
            except KeyError:
                ...
                raise ValueError(
                    ...
                    "does not exist during import: %s in %s" % (coloc_op_name, op.name)
                )
            if coloc_op.device:
    ...
    for name in requested_return_elements:
        if ":" in name:
            combined_return_elements.append(
                graph._get_tensor_by_tf_output(return_outputs[outputs_idx])
            )  # pylint: disable=protected-access
            outputs_idx += 1
        else:
            combined_return_elements.append(
                graph._get_operation_by_tf_operation(return_opers[opers_idx])
            )  # pylint: disable=protected-access
            opers_idx += 1
...
def import_graph_def_for_function(graph_def, name=None):  # pylint: disable=invalid-name
...
def _import_graph_def_internal(  # pylint: disable=invalid-name
  ...
  """
        ...
        with c_api_util.tf_buffer(graph_def.SerializeToString()) as serialized:
            try:
                results = c_api.TF_GraphImportGraphDefWithResults(
                    graph._c_graph, serialized, options
                )  # pylint: disable=protected-access
    ...
    missing_unused_input_keys = c_api.TF_ImportGraphDefResultsMissingUnusedInputMappings_wrapper(
        results.results
    )
