...
class _Function(_Convertible):
  ...
  """
    ...
    def __init__(self, function, enclosing_graph):
        ...
        self._nodes = {
            n.name: _Node.new(node=n, function=self, enclosing_graph=enclosing_graph)
...
class _Merge(_Node):
    ...
    def convert_variable_to_constant(self, incoming_edge, tensor_data):
        ...
        super(_Merge, self).convert_variable_to_constant(
            _Edge(
                incoming_edge.source, _Edge(incoming_edge.destination.convertible, 0)
            ),
            tensor_data,
        )
...
class _VarHandle(_Node):
    ...
    def convert_variable_to_constant(self, incoming_edge, tensor_data):
        tensor_proto = tensor_util.make_tensor_proto(
            tensor_data.numpy, tensor_data.dtype, tensor_data.numpy.shape
        )
    ...
    def convert_variable_to_constant(self, incoming_edge, tensor_data):
        ...
        if index >= self._first_function_input:
            node.update_dtype(
                ...
                index - self._first_function_input,
                tensor_data.dtype,
            )
    ...
    def convert_variable_to_constant(self, incoming_edge, tensor_data):
        ...
        if node.node.attr["output_shapes"].list.shape:
            node.node.attr["output_shapes"].list.shape[
                incoming_edge.destination.index
            ].CopyFrom(
...
class _ConverterData(object):
  ...
  """
    ...
    def __init__(
        self, graph_def, variable_names_allowlist=None, variable_names_denylist=None
    ):
        self._graph_def = graph_def
        self._tensor_data = {}
    ...
    def _should_convert(self, name):
        ...
        return (
            self._variable_names_allowlist is None
            or name in self._variable_names_allowlist
        ) and (
            self._variable_names_denylist is None
            or name not in self._variable_names_denylist
        )
...
class _FunctionConverterData(_ConverterData):
    ...
    """
        ...
        graph_def = _run_inline_graph_optimization(
...
def _construct_concrete_function(func, output_graph_def, converted_input_indices):
  ...
  """
    ...
    new_func = wrap_function.function_from_graph_def(
        output_graph_def, new_input_names, new_output_names
    )
...
def _replace_variables_by_constants(converter_data):
  ...
  """
    ...
    converted_input_indices = {
        t.index for t in converter_data.tensor_data.values() if t.index is not None
    }
...
def convert_var_to_const_function_in_v1(
    func, lower_control_flow=True, aggressive_inlining=False
):
    """Replaces all the variables in a graph with constants of the same values.
...
def convert_variables_to_constants_v2_as_graph(
  ...
  """
    ...
    frozen_func = _construct_concrete_function(
        func, output_graph_def, converted_input_indices
    )
    ...
    for function in graph_def.library.function:
        if "_input_shapes" in function.attr:
            for input_arg, shape_attribute in zip(
                function.signature.input_arg, function.attr["_input_shapes"].list.shape
            ):
