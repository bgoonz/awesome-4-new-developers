...
class TensorUtilTest(test.TestCase, parameterized.TestCase):
    ...
    def testFloatNpArrayFloat64(self):
        ...
        self.assertAllClose(
            np.array([[10.0, 20.0, 30.0]], dtype=np.float64), tensor_util.MakeNdarray(t)
        )
    ...
    @parameterized.named_parameters(
        ("_int8", dtypes.int8, np.int8),
        ("_int16", dtypes.int16, np.int16),
        ("_int32", dtypes.int32, np.int32),
        ("_int64", dtypes.int64, np.int64),
        ("_uint8", dtypes.uint8, np.uint8),
        ("_uint16", dtypes.uint16, np.uint16),
    ...
    def testIntTypesWithImplicitRepeat(self, dtype, nptype):
        self.assertAllEqual(
            np.array(
                [[10, 11, 12, 12], [12, 12, 12, 12], [12, 12, 12, 12]], dtype=nptype
            ),
            tensor_util.MakeNdarray(
                tensor_util.make_tensor_proto([10, 11, 12], shape=[3, 4], dtype=dtype)
            ),
        )
    ...
    @parameterized.named_parameters(
        ("_int64", np.int64, "DT_INT64"), ("_uint64", np.uint64, "DT_UINT64")
    )
    ...
    def testNestedNumpyArrayWithDType(self):
        t = tensor_util.make_tensor_proto(
            [10.0, 20.0, np.array(30.0)], dtype=dtypes.float32
        )
        a = tensor_util.MakeNdarray(t)
        self.assertEqual(np.float32, a.dtype)
        self.assertAllClose(np.array([10.0, 20.0, 30.0], dtype=np.float32), a)
...
class ConstantValueTest(test.TestCase):
    ...
    def testRank(self):
        ...
        tf_val = array_ops.rank_internal(
            constant_op.constant(0.0, shape=[1, 2, 3]), optimize=False
        )
        c_val = tensor_util.constant_value(tf_val)
    ...
    def testEqual(self):
        ...
        tf_val = math_ops.equal(
            constant_op.constant([[0, 1]]), constant_op.constant([[0], [1]])
        )
        c_val = tensor_util.constant_value(tf_val)
...
class ConstantValueAsShapeTest(test.TestCase):
    ...
    def testConstant(self):
        ...
        self.assertEqual(
            tensor_shape.TensorShape([]), tensor_util.constant_value_as_shape(tf_val)
        )

    @test_util.run_in_graph_and_eager_modes
    def testCast(self):
        tf_val = math_ops.cast(
            array_ops.shape(constant_op.constant(0.0, shape=[1, 2, 3])), dtypes.int64
        )
    ...
    def testConcat(self):
        ...
        with ops.Graph().as_default():
            ...
            tf_val = array_ops.concat(
                [[16, 37], array_ops.placeholder(dtypes.int32, shape=(1,)), [48]], 0
            )
            c_val = tensor_util.constant_value_as_shape(tf_val)
    ...
    def testSlice(self):
        ...
        with ops.Graph().as_default():
            tf_val_orig = array_ops.concat(
                [[1, 2, 16, 37], array_ops.placeholder(dtypes.int32, shape=(1,)), [48]],
                0,
            )
