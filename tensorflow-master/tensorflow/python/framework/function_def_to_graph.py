  ...
  """
    ...
    with func_graph.as_default():
        ...
        for ret_arg_def, tensor_name in zip(
            ...
            output_names[
                ops.tensor_id(func_graph.get_tensor_by_name(tensor_name))
            ] = ret_arg_def.name
...
def function_def_to_graph_def(fdef, input_shapes=None):
  ...
  """
    ...
    if input_shapes and len(input_shapes) != len(fdef.signature.input_arg):
        raise ValueError(
            "Length of `input_shapes` must match the number "
    ...
    for node_def in fdef.node_def:
        ...
        if f is not None:
            ...
            op_def = default_graph._get_op_def(
                node_def.op
            )  # pylint: disable=protected-access
        ...
        for attr in op_def.attr:
            if attr.type == "func":
                ...
                if not is_function(fname):
                    raise ValueError(
                        f"Function {fname} was not found. Please make sure "
                ...
                for fn in node_def.attr[attr.name].list.func:
                    ...
                    if not is_function(fname):
                        raise ValueError(
                            f"Function {fname} was not found. Please make "
