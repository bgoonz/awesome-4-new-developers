...
class DimensionTest(test_util.TensorFlowTestCase):
    ...
    def testAsDimension(self):
        ...
        self.assertEqual(
            tensor_shape.Dimension(None).value, tensor_shape.as_dimension(None).value
        )
    ...
    def testInequality(self):
        ...
        self.assertIsNone(
            tensor_shape.Dimension(None) != None
...
class ShapeTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testMergeFullShapes(self):
        self.assertEqual(
            [3, 4, 7],
            tensor_shape.TensorShape([3, 4, 7])
            .merge_with(tensor_shape.TensorShape([3, 4, 7]))
            .as_list(),
        )
        with self.assertRaises(ValueError):
    ...
    def testMergeFullAndUnknownShape(self):
        self.assertEqual(
            [3, 4, 7],
            tensor_shape.TensorShape([3, 4, 7])
            .merge_with(tensor_shape.TensorShape(None))
            .as_list(),
        )
    ...
    def testSlice(self):
        ...
        self.assertEqual(
            tensor_shape.Dimension(None).value, tensor_shape.dimension_value(unknown[2])
        )
        tensor_shape.TensorShape([None, None, None]).assert_is_compatible_with(
            unknown[1:4]
        )
    ...
    @parameterized.named_parameters(
        ("Concatenate", lambda x, y: x.concatenate(y)), ("Add", lambda x, y: x + y)
    )
    def testConcatenateWithDimension(self, concatenate_fn):
        tensor_shape.TensorShape([1, 2, 3]).assert_is_compatible_with(
            concatenate_fn(tensor_shape.TensorShape([1, 2]), tensor_shape.Dimension(3))
        )
    ...
    def testMostSpecificCompatibleShape(self):
        ...
        self._testMostSpecificCompatibleShapeHelper(
            [None, 2, 3], [1, 1, 3], [None, None, 3]
        )
        self._testMostSpecificCompatibleShapeHelper(
            [1, 1, 3], [None, 2, 3], [None, None, 3]
        )
    ...
    def testStr(self):
        ...
        self.assertEqual(
            "(None,)", str(tensor_shape.unknown_shape(rank=1)).replace("?", "None")
        )
        self.assertEqual(
            "(None, None)", str(tensor_shape.unknown_shape(rank=2)).replace("?", "None")
        )
        self.assertEqual(
            "(None, None, None)",
    ...
    def testEquality(self):
        ...
        s3 = tensor_shape.TensorShape(
            [tensor_shape.Dimension(3), tensor_shape.Dimension(4), None]
        )
    ...
    def testReduce(self):
        ...
        self.assertEqual(
            args, ([tensor_shape.Dimension(2), tensor_shape.Dimension(3)],)
        )
        reconstructed = ctor(*args)
