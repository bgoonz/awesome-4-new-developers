...
class SubscribeTest(test_util.TensorFlowTestCase):
    ...
    def testSupportedTypes(self):
        ...
        # Dictionaries.
        subscribed = subscribe.subscribe(
            {"first": a, "second": b}, lambda t: script_ops.py_func(sub, [t], [t.dtype])
        )
        ...
        with self.assertRaisesRegex(TypeError, "has invalid type"):
            subscribe.subscribe(
                c.name, lambda t: script_ops.py_func(sub, [t], [t.dtype])
            )
    ...
    def testResourceType(self):
        ...
        tensor_array = tensor_array_ops.TensorArray(
            dtype=dtypes.float32, tensor_array_name="test", size=3, infer_shape=False
        )
    ...
    def testMultipleOutputs(self):
        ...
        subscribe.subscribe(
            sparse_add.op.outputs, lambda t: script_ops.py_func(sub, [t], [t.dtype])
        )
