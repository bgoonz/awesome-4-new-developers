...
class TwoTensorsSpec(type_spec.TypeSpec):
    ...
    def _component_specs(self):
        return (
            tensor_spec.TensorSpec(self.x_shape, self.x_dtype),
    ...
    def from_value(cls, value):
        return cls(
            value.x.shape, value.x.dtype, value.y.shape, value.y.dtype, value.color
        )
...
class TwoCompositesSpec(type_spec.TypeSpec):
    ...
    def from_value(cls, value):
        return cls(
            type_spec.type_spec_from_value(value.x),
...
class NestOfTensorsSpec(type_spec.TypeSpec):
    ...
    def __repr__(self):
        if (
            hasattr(self.spec, "_fields")
            and isinstance(self.spec._fields, collections_abc.Sequence)
            and all(isinstance(f, six.string_types) for f in self.spec._fields)
        ):
            return "%s(%r)" % (type(self).__name__, self._serialize())
        return super(
            type_spec.TypeSpec, self
        ).__repr__()  # pylint: disable=bad-super-call
...
class TypeSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        (
            ...
            TwoTensorsSpec(
                ...
                [3],
                dtypes.bool,
                (np.int32(1), np.float32(1.0), np.array([[1, 2], [3, 4]])),
            ),
            TwoTensorsSpec(
                [5, 3],
                dtypes.int32,
    ...
    @parameterized.named_parameters(
        ...
        (
            "DiffMetadataNumpy",
            TwoTensorsSpec(
                [5, 3], dtypes.int32, [3], dtypes.bool, np.array([[1, 2], [3, 4]])
            ),
            TwoTensorsSpec(
                [5, 3], dtypes.int32, [3], dtypes.bool, np.array([[1, 2], [3, 8]])
            ),
        ),
        (
        ...
        ("Non-TypeSpec", TwoTensorsSpec([5, 3], dtypes.int32, [8], dtypes.bool), 5),
    )
    ...
    def testMostSpecificCompatibleTypeNamedTupleIsNotTuple(self):
        named_tuple_spec_a = NestOfTensorsSpec.from_value(
            NestOfTensors(_TestNamedTuple(a=1, b="aaa"))
        )
        named_tuple_spec_b = NestOfTensorsSpec.from_value(
            NestOfTensors(_TestNamedTuple(a=2, b="bbb"))
        )
        named_tuple_spec_c = NestOfTensorsSpec.from_value(
            NestOfTensors(_TestNamedTuple(a=3, b="ccc"))
        )
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            NestOfTensorsSpec(
                _TestNamedTupleSingleField(a=tensor_spec.TensorSpec((), dtypes.int32))
            ),
        ),
        (
            "DifferentFieldName",
    ...
    def testRegistry(self):
        self.assertEqual("tf.TwoCompositesSpec", type_spec.get_name(TwoCompositesSpec))
        self.assertEqual("tf.TwoTensorsSpec", type_spec.get_name(TwoTensorsSpec))
        self.assertEqual(TwoCompositesSpec, type_spec.lookup("tf.TwoCompositesSpec"))
    ...
    def testRegistryDuplicateErrors(self):
        with self.assertRaisesRegex(
            ValueError,
            "Name tf.TwoCompositesSpec has already been registered "
            "for class __main__.TwoCompositesSpec.",
        ):

            @type_spec.register(
                "tf.TwoCompositesSpec"
        ...
        with self.assertRaisesRegex(
            ValueError,
            "Class __main__.TwoCompositesSpec has already been "
