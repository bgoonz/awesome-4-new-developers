...
class TestWholeModelSaving(keras_parameterized.TestCase):
    ...
    """
        ...
        if check_metrics:
            self.assertAllEqual(
                [m.name for m in model.metrics], [m.name for m in loaded_model.metrics]
            )
    ...
    def test_sequential_model_saving_2(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            new_model = keras.models.load_model(
                saved_model_dir,
                custom_objects={"CustomOp": CustomOp, "custom_loss": custom_loss},
            )
            self._assert_same_weights_and_metrics(model, new_model)
    ...
    def test_saving_model_with_long_layer_names(self):
        ...
        with self.cached_session():
            ...
            model.compile("adam", loss=keras.losses.MeanSquaredError(), metrics=["acc"])
    ...
    def test_model_saving_to_pre_created_h5py_file(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            with h5py.File("_", driver="core", mode="w", backing_store=False) as h5file:
                keras.models.save_model(model, h5file)
                loaded_model = keras.models.load_model(h5file)
                out2 = loaded_model.predict(x)
            self.assertAllClose(out, out2, atol=1e-05)
    ...
    def test_model_saving_to_new_dir_path(self):
        saved_model_dir = os.path.join(self._save_model_dir(), "newdir", "saved_model")
        save_format = testing_utils.get_save_format()
        ...
        with self.cached_session() as session:
            ...
            if save_format in ["tf", "tensorflow"]:
                ...
                self.assertIsInstance(
                    loaded.optimizer, keras.optimizer_v2.optimizer_v2.OptimizerV2
                )
    ...
    def test_functional_model_with_getitem_op_layer(self):
        ...
        self.assertAllEqual(loaded_model.predict(args, batch_size=batch_size), expected)
    ...
    def test_shared_objects(self):
        class OuterLayer(keras.layers.Layer):
            ...
            def from_config(cls, config):
                return cls(
                    generic_utils.deserialize_keras_object(config["inner_layer"])
                )
    ...
    def test_shared_objects_wrapper(self):
        ...
        model = keras.Model(inputs=input_, outputs=[unwrapped(input_), wrapped(input_)])
    ...
    """
        ...
        model.compile(
            optimizer="rmsprop", loss="mse", metrics={"head_0": [metric, "accuracy"]}
        )
...
def _make_sequential(input_size, output_size):
    ...
    return keras.Sequential(
        [keras.layers.Dense(8, activation="relu"), keras.layers.Dense(output_size)]
    )
