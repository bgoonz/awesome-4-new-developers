...
class TestSavedModelFormatAllModes(keras_parameterized.TestCase):
    ...
    def _test_evaluation(self, model, loaded):
        ...
        self.assertAllClose(self.evaluate(model.weights), self.evaluate(loaded.weights))

        input_arr = constant_op.constant(np.random.random((1, 3)).astype(np.float32))
        self.assertAllClose(
        ...
        if context.executing_eagerly():
            ...
            self.assertAllClose(
                self.evaluate(model.get_losses_for(None)),
    ...
    def test_standard_loader(self):
        ...
        self.evaluate(variables.variables_initializer(loaded.variables))
        all_close = ["variables", "trainable_variables", "non_trainable_variables"]
        for attr in all_close:
            self.assertAllClose(
    ...
    def test_metadata_input_spec(self):
        ...
        saved_model_dir = self._save_model_dir()
        model = testing_utils.get_model_from_layers([layer], model_type="subclass")
        model(
            {
                "a": constant_op.constant([[2, 4]]),
    ...
    def testSaveWithSignatures(self):
        ...
        feature_configs = {
            "inputs": parsing_ops.FixedLenFeature(shape=[2, 3], dtype=dtypes.float32)
        }
        ...
        model.save(
            saved_model_dir,
            save_format="tf",
            signatures={"predict": predict, "parse_and_predict": parse_and_predict},
        )
        model.save(
            "/tmp/saved",
            save_format="tf",
        ...
        self.assertAllClose(
            ...
            loaded.signatures["predict"](
                ops.convert_to_tensor_v2_with_dispatch(input_arr.astype("float32"))
            )["predictions"],
        )
        ...
        example = example_pb2.Example(features=feature_pb2.Features(feature=feature))
    ...
    def testTrainingDefaults(self):
        ...
        assert_training_default(load.layer_with_training_default_none.__call__, False)
        assert_training_default(load.layer_with_training_default_true.__call__, True)
    ...
    def testFeatureColumns(self):
        ...
        if context.executing_eagerly():
            ...
            cat_vocab = fc.categorical_column_with_vocabulary_list("b", ["1", "2", "3"])
    ...
    def testModelWithTfFunctionCall(self):
        ...
        self.assertAllEqual(
            [[1, 5]],
            self.evaluate(loaded(array_ops.constant([[1, 5.0]]), training=True)),
        )
        self.assertAllEqual(
            [[0, 0]],
            self.evaluate(loaded(array_ops.constant([[1, 5.0]]), training=False)),
        )
    ...
    def testSaveTimeDistributedLayer(self):
        ...
        loaded = keras_load.load(saved_model_dir)
        self.assertAllClose(
            loaded.predict_on_batch(array_ops.ones((3, 2, 1))), predictions
        )

    @parameterized.named_parameters([("with_unrolling", True), ("no_unrolling", False)])
    ...
    def testSaveConvLSTM2D(self, stateful):
        ...
        input_arr = np.ones((batch, timesteps, channels, rows, cols)).astype("float32")
        layer = keras.layers.ConvLSTM2D(
            filters=16, kernel_size=(1, 1), data_format=data_format, stateful=stateful
        )
    ...
    def testSaveWithRaggedInputs(self):
        ...
        feature_1 = ragged_factory_ops.constant([[0.0], [1, 3]])
        feature_2 = ragged_factory_ops.constant([[1.0, 2], [4]])
    ...
    def testSaveLayerMultipleInputs(self):
        ...
        model = CustomModel()
        inp = [
            constant_op.constant(i, shape=[1, 1], dtype=dtypes.float32)
            for i in range(1, 5)
        ]
        expected = model(inp)
        ...
        self.assertAllEqual(self.evaluate(expected_loss), self.evaluate(actual_loss))
    ...
    def test_wrapped_layer_training(self):
        ...
        model = Custom()
        x = constant_op.constant(1.0, shape=[1, 1])
...
class TestSavedModelFormat(test.TestCase):
    ...
    def test_load_with_partially_failed_serialization(self):
        ...
        class Model(keras.models.Model):
            ...
            @def_function.function(input_signature=[tensor_spec.TensorSpec([None, 1])])
            def call(self, inputs):
                return self.layer(inputs)
    ...
    def test_save_without_tracing(self):
        ...
        with self.assertRaisesRegex(ValueError, "Cannot call custom layer"):
            loaded.attached_layer(constant_op.constant([1.0]))
...
class MetricTest(test.TestCase, parameterized.TestCase):
    ...
    def _test_metric_save_and_load(
        self, metric, save_dir, num_tensor_args, shape=(1, 5), test_sample_weight=True
    ):
    ...
    def test_custom_metric(self, base_cls, num_tensor_args, requires_build):
        ...
        with self.cached_session():
            ...
            with self.assertRaisesRegex(ValueError, "Unable to restore custom object"):
