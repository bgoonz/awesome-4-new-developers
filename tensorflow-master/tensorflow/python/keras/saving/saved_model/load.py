...
from tensorflow.python.keras.saving.saved_model.serialized_attributes import (
    CommonEndpoints,
)
...
layers_module = LazyLoader("layers_module", globals(), "tensorflow.python.keras.layers")
input_layer = LazyLoader(
    "input_layer", globals(), "tensorflow.python.keras.engine.input_layer"
)
functional_lib = LazyLoader(
    "functional_lib", globals(), "tensorflow.python.keras.engine.functional"
)
training_lib = LazyLoader(
    "training_lib", globals(), "tensorflow.python.keras.engine.training"
)
training_lib_v1 = LazyLoader(
    "training_lib_v1", globals(), "tensorflow.python.keras.engine.training_v1"
)
metrics = LazyLoader("metrics", globals(), "tensorflow.python.keras.metrics")
recurrent = LazyLoader(
    "recurrent", globals(), "tensorflow.python.keras.layers.recurrent"
)
  ...
  """
    ...
    if gfile.Exists(path_to_metadata_pb):
        try:
            ...
            raise IOError(
                "Cannot parse keras metadata {}: {}.".format(
                    path_to_metadata_pb, str(e)
                )
            )
    else:
        logging.warning(
...
def _read_legacy_metadata(object_graph_def, metadata):
    ...
    for node_id, proto in enumerate(object_graph_def.nodes):
        if (
            ...
            and proto.user_object.identifier in constants.KERAS_OBJECT_IDENTIFIERS
        ):
  ...
  """
    ...
    def load_layers(self, compile=True):  # pylint: disable=redefined-builtin
        ...
        for node_metadata in self._metadata.values():
            ...
            self.loaded_nodes[node_metadata.node_id] = self._load_layer(
                node_metadata.node_id, node_metadata.identifier, node_metadata.metadata
            )
    ...
    def _reconstruct_all_models(self):
        ...
        if all_initialized_models != set(self.model_layer_dependencies.keys()):
            ...
            raise ValueError(
                "Error when loading from SavedModel -- the following "
                "models could not be initialized: {}".format(uninitialized_model_names)
            )
    ...
    def _reconstruct_model(self, model_id, model, layers):
        ...
        if model.inputs:
            ...
            (inputs, outputs, created_layers) = functional_lib.reconstruct_from_config(
...
def _finalize_config_layers(layers):
    ...
    for layer in layers:
        ...
        if (
            isinstance(layer, recurrent.RNN)
            and layer.stateful
...
def revive_custom_object(identifier, metadata):
    ...
    revived_classes = {
        constants.INPUT_LAYER_IDENTIFIER: (RevivedInputLayer, input_layer.InputLayer),
    ...
    if parent_classes is not None:
        ...
        raise ValueError(
            "Unable to restore custom object of type {} currently. "
            "Please make sure that the layer implements `get_config`"
            "and `from_config` when saving. In addition, please use "
            "the `custom_objects` arg when calling `load_model()`.".format(identifier)
        )
...
def _get_keras_attr(layer):
    return getattr(layer, "_serialized_attributes", {}).get(constants.KERAS_ATTR, None)
