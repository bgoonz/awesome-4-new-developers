...
class TableHandler(object):
    ...
    def __init__(self, table, oov_tokens=None, mask_token=None, mask_value=0):
        self.table = table
        ...
        if oov_tokens is None:
            ...
            self.oov_tokens = math_ops.cast(
                oov_tokens, table._value_dtype
    ...
    def insert(self, keys, values):
        ...
        keys = ops.convert_to_tensor_v2_with_dispatch(
            keys, dtype=self.table._key_dtype
        )  # pylint: disable=protected-access
        values = ops.convert_to_tensor_v2_with_dispatch(
            values, dtype=self.table._value_dtype
        )  # pylint: disable=protected-access
        if values.shape.ndims != 1:
            raise ValueError(
                "`values` must be 1-dimensional, got an input with "
    ...
    def _replace_oov_buckets(self, inputs, lookups):
        ...
        oov_locations = math_ops.equal(
            lookups, self.table._default_value
        )  # pylint: disable=protected-access
    ...
    def _lookup_and_mask(self, inputs):
        ...
        return array_ops.where_v2(
            mask_locations,
            math_ops.cast(
                self.mask_value, self.table._value_dtype
            ),  # pylint: disable=protected-access
            lookups,
        )  # pylint: disable=protected-access
...
def find_repeated_tokens(vocabulary):
    ...
    if len(vocabulary) != len(vocabulary_set):
        return [
            item for item, count in collections.Counter(vocabulary).items() if count > 1
        ]
    else:
        return []
