...
class PreprocessingStageTest(
    ...
    def test_adapt_preprocessing_stage_with_list_input(self):
        ...
        stage = preprocessing_stage.FunctionalPreprocessingStage([x0, x1, x2], [y, z])
        stage.compile()
    ...
    def test_adapt_preprocessing_stage_with_dict_input(self):
        ...
        x1_sum = core.Lambda(lambda x: math_ops.reduce_sum(x, axis=-1, keepdims=True))(
            x1
        )
    ...
    def test_adapt_preprocessing_stage_with_dict_output(self):
        ...
        self.assertAllClose(outputs["y0"], np.ones((4, 3), dtype="float32") + 1.0)
        self.assertAllClose(outputs["y1"], np.ones((4, 3), dtype="float32") - 1.0)
        self.assertAllClose(outputs["z0"], np.ones((4, 3), dtype="float32") + 2.0)
