...
class IndexLookup(base_preprocessing_layer.CombinerPreprocessingLayer):
  ...
  """
        ...
        if has_vocab_path or restore_from_static_table:
            ...
            if vocabulary is None:
                ...
                if not gfile.Exists(vocabulary):
                    raise ValueError(
                        "Vocabulary file %s does not exist." % (vocabulary,)
                    )
    ...
    """
        ...
        if (
            self.output_mode in [MULTI_HOT, COUNT, TF_IDF]
            and self._called
        ...
        if not context.executing_eagerly() and (
            tensor_util.is_tensor(vocabulary) or tensor_util.is_tensor(idf_weights)
        ):
            raise RuntimeError(
    ...
    def _encode_output(self, lookup_result):
        ...
        if lookup_result.shape.rank > 2:
            raise ValueError(
                ...
                "`output_mode={}`.".format(
                    original_shape, lookup_result.shape.rank, self.output_mode
                )
            )
...
class _IndexLookupAccumulator(
    collections.namedtuple("Accumulator", ["data", "count_dict", "per_doc_count_dict"])
):
    pass
...
class _IndexLookupCombiner(base_preprocessing_layer.Combiner):
  ...
  """
    ...
    def __init__(
        self, vocab_size=None, mask_value=None, oov_value=None, compute_idf=False
    ):
