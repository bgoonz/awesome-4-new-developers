  ...
  """
    ...
    def _defun_gru_call(self, inputs, initial_state, training, mask, sequence_lengths):
        ...
        if _use_new_code():
            ...
            (last_output, outputs, new_h, runtime) = self._defun_wrapper.defun_layer(
                **gru_kwargs
            )
        else:
            gpu_gru_kwargs = {
            ...
            if context.executing_eagerly():
                ...
                can_use_gpu = (
                    ...
                    (
                        ...
                        or (device_type is None and config.list_logical_devices("GPU"))
                    )
                    and (
...
def gru_with_backend_selection(
  ...
  """
    ...
    if _use_new_code():
        ...
        (last_output, outputs, new_h, runtime) = control_flow_ops.execute_fn_for_device(
            {
        ...
        if is_ragged_input or not self._could_use_gpu_kernel:
            ...
            if _use_new_code():
                lstm_kwargs = {
                    ...
                    "kernel": _read_variable_value(self.cell.kernel),
                    "recurrent_kernel": _read_variable_value(
                ...
                if context.executing_eagerly():
                    ...
                    can_use_gpu = (
                        ...
                        (
                            ...
                            or (
                                device_type is None
                                and config.list_logical_devices("GPU")
                            )
                        )
                        and (
                            mask is None
                            or is_cudnn_supported_inputs(mask, self.time_major)
                        )
                    )
                    # Under eager context, check the device placement and prefer the
...
def has_fully_masked_sequence(mask):
    ...
    return math_ops.reduce_any(math_ops.reduce_all(math_ops.logical_not(mask), axis=1))
