...
class TestGetFileAndValidateIt(test.TestCase):
    ...
    """
        ...
        path = keras.utils.data_utils.get_file(
            "test.txt", origin, md5_hash=hashval_md5, untar=True, cache_subdir=dest_dir
        )
        path = keras.utils.data_utils.get_file(
            filepath,
            origin,
        ...
        self.assertTrue(keras.utils.data_utils.validate_file(filepath, hashval_sha256))
        ...
        hashval_md5 = keras.utils.data_utils._hash_file(zip_file_path, algorithm="md5")
        path = keras.utils.data_utils.get_file(
            "test", origin, md5_hash=hashval_md5, extract=True, cache_subdir=dest_dir
        )
        path = keras.utils.data_utils.get_file(
            "test",
            origin,
