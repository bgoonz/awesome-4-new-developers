...
def get_test_mode_kwargs():
    ...
    return {"run_eagerly": run_eagerly}
...
class CompositeTensorInternalTest(keras_parameterized.TestCase):
    ...
    def test_training_internal_ragged_tensors(self):
        ...
        expected_data = np.concatenate((input_data * 3, input_data * 0.5), axis=-1)
...
class CompositeTensorOutputTest(keras_parameterized.TestCase):
    ...
    def test_sparse_tensor_rebatched_outputs(self):
        ...
        expected_indices = np.array([[0, 0], [1, 0], [1, 1], [2, 0], [3, 0], [3, 1]])
    ...
    def test_sparse_tensors(self, use_dict, use_dataset, action):
        data = [
            ...
            (
                ...
                np.array([[[5, -1, -1, -1]], [[6, 7, -1, -1]], [[-1, 8, -1, -1]]]),
            ),
        ]
        ...
        model.compile(
            optimizer="sgd", loss="mse", metrics=["accuracy"], **get_test_mode_kwargs()
        )
...
class RaggedTensorInputTest(keras_parameterized.TestCase, test_util.TensorFlowTestCase):
    def test_ragged_input(self, use_dict, use_dataset, action):
        ...
        self.assertIsInstance(model_input._type_spec, ragged_tensor.RaggedTensorSpec)
...
class RaggedTensorInputValidationTest(
    ...
    def test_ragged_tensor_input_with_one_none_dimension(self, use_dict, use_dataset):
        # Define some input data.
        data = [
            (
                ragged_factory_ops.constant([[[1, 0]], [[2, 3]]], ragged_rank=1),
                np.array([[[1, 0]], [[2, 3]]]),
            )
        ]
    ...
    def test_ragged_tensor_input_with_no_none_dimension(self, use_dict, use_dataset):
        # Define some input data.
        data = [
            (
                ragged_factory_ops.constant([[[1, 0]], [[2, 3]]], ragged_rank=0),
                np.array([[[1, 0]], [[2, 3]]]),
            )
        ]
...
class CompositeTensorModelPredictTest(keras_parameterized.TestCase):
    ...
    def test_sparse_tensor_model_predict(self):
        ...
        model_input = input_layer.Input(shape=(3,), sparse=True, dtype=dtypes.float32)
    ...
    def test_ragged_tensor_model_predict(self):
        ...
        ragged_input = ragged_factory_ops.constant([[1, 2, 3, 4, 5], [2, 4]])
