...
from tensorflow.python.keras.distribute import (
    distribute_strategy_test as keras_test_lib,
)
...
class TestDistributionStrategyWithCallbacks(test.TestCase, parameterized.TestCase):
    ...
    def test_callbacks_in_predict(self, distribution):
        ...
        model.predict(
            keras_test_lib.get_predict_dataset(dataset), steps=5, callbacks=[counter]
        )
...
class TestDistributionStrategyErrorCases(test.TestCase, parameterized.TestCase):
    ...
    def test_distribution_strategy_on_deferred_sequential_model(self, distribution):
        with distribution.scope():
    ...
    @ds_combinations.generate(keras_test_lib.all_strategy_combinations_minus_default())
...
class TestDistributionStrategyValidation(test.TestCase, parameterized.TestCase):
    @ds_combinations.generate(
        combinations.times(keras_test_lib.all_strategy_combinations_minus_default())
    )
