...
class DatasetCreatorModelFitTestBase(test.TestCase, parameterized.TestCase):
    ...
    def _model_compile(
        ...
        class ResultAssertingCallback(callbacks_lib.Callback):
            ...
            def on_epoch_end(self, epoch, logs=None):
                ...
                is_loss_float = logs.get("loss", None) is not None and isinstance(
                    logs["loss"], (float, np.floating)
                )
    ...
    def _model_evaluate(
        ...
        model, default_callbacks = self._model_compile(
            strategy, steps_per_execution, run_eagerly, with_normalization_layer
        )
        callbacks += default_callbacks
