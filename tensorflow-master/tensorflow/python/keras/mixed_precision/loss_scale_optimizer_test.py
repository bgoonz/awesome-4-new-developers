...
class LossScaleOptimizerTest(test.TestCase, parameterized.TestCase):
    ...
    def _run_fn_with_grad_check(self, strategy, var, opt, expected_grad):
        grad_check_fn = mp_test_util.create_identity_with_grad_check_fn(expected_grad)
    ...
    def testDynamicAttrsWithFixedLossScale(self):
        ...
        opt = loss_scale_optimizer.LossScaleOptimizer(
            opt, dynamic=False, initial_scale=2.0
        )
    ...
    def testGetScaledLoss(self):
        ...
        loss = ops.convert_to_tensor_v2_with_dispatch(5.0)
        self.assertEqual(10.0, self.evaluate(opt.get_scaled_loss(loss)))
        self.assertEqual(10.0, self.evaluate(opt.get_scaled_loss(lambda: loss)()))
        loss = ops.convert_to_tensor_v2_with_dispatch(5.0, dtype="float16")
        self.assertEqual(10.0, self.evaluate(opt.get_scaled_loss(loss)))
        self.assertEqual(10.0, self.evaluate(opt.get_scaled_loss(lambda: loss)()))
    ...
    def testGetUnscaledGradients(self):
        ...
        scaled_grads = [
            ops.convert_to_tensor_v2_with_dispatch(3.0),
            None,
            ops.convert_to_tensor_v2_with_dispatch(-4.0, dtype="float16"),
        ]
    ...
    def testGetUnscaledSparseGradients(self):
        ...
        sparse_scaled_grad = ops.IndexedSlices(
            ops.convert_to_tensor_v2_with_dispatch([[4.0, 2.0], [8.0, 5.0]]),
            ops.convert_to_tensor_v2_with_dispatch([1, 3], dtype="int32"),
            dense_shape=ops.convert_to_tensor_v2_with_dispatch([5, 2], dtype="int32"),
        )
        sparse_grad = opt.get_unscaled_gradients([sparse_scaled_grad])[0]
        self.assertIsInstance(sparse_grad, ops.IndexedSlices)
        self.assertAllEqual([[2.0, 1.0], [4.0, 2.5]], self.evaluate(sparse_grad.values))

    @parameterized.named_parameters(*TESTCASES)
    def testDynamicLossScale(self, strategy_fn):
        strategy = strategy_fn()
        learning_rate = 2.0
        expected_gradient = variables.Variable(
    ...
    def testClipping(self, strategy_fn):
        ...
        for clip_type in ("clipnorm", "global_clipnorm", "clipvalue"):
            with strategy.scope(), self.subTest(clip_type=clip_type):
                ...
                run_op = strategy.experimental_run(run_fn)
                self._run_if_in_graph_mode(run_op)
                self.assertAllClose([-5.0], self.evaluate(var))  # Var does not change
                self.assertEqual(self.evaluate(opt.loss_scale), 4)
    ...
    def testNanOnOneReplicaOnly(self):
        ...
        with create_mirrored_strategy().scope() as strategy:
            ...
            def loss():
                rep_id = (
                    distribution_strategy_context.get_replica_context().replica_id_in_sync_group
                )
                # The last element of last replica's gradient is NaN.
                return control_flow_ops.cond(
                    constant_op.constant(rep_id == 0),
                    lambda: var * 2.0,
                    lambda: var * constant_op.constant([1.0, float("NaN")]),
                )
    ...
    def testDynamicLossScaleWithSlots(self, strategy_fn):
        ...
        if (
            isinstance(strategy_obj, mirrored_strategy.MirroredStrategy)
    ...
    def testArbitraryAttributesNotExposed(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "'LossScaleOptimizer' object has no attribute 'nesterov'"
        ):
    ...
    def testPassingV1LossScale(self, strategy_fn):
        ...
        with strategy.scope():
            ...
            var = variables.Variable([5.0])
            opt = gradient_descent.SGD(learning_rate)
            loss_scale = tf_loss_scale_module.FixedLossScale(2.0)
    ...
    def testLossScaleDelegationWithWrapper(self):
        ...
        with self.cached_session():
            ...
            self.assertEqual(self.evaluate(opt.learning_rate), 2.0)
            self.assertEqual(
                self.evaluate(opt.inner_optimizer.inner_optimizer.learning_rate), 2.0
            )
    ...
    def testCheckpoint(self, strategy_fn, save_with_ls, restore_with_ls):
        class MySGD(gradient_descent.SGD):
            ...
            def __init__(self, *args, **kwargs):
                ...
                self.my_var = variables.Variable(0.0)
                self._track_trackable(self.my_var, "my_var")

        strategy = strategy_fn()
        replicas = strategy.num_replicas_in_sync
        if (
            isinstance(strategy, mirrored_strategy.MirroredStrategy)
            and not context.executing_eagerly()
        ):
        ...
        with self.test_session(), strategy.scope():
            ...
            run_fn = lambda: opt.minimize(lambda: var / replicas + 1.0, var_list=[var])
    ...
    @combinations.generate(
        combinations.combine(get_config=["v1", "v2", "tf2_3"], from_config=["v1", "v2"])
    )
    def testGetConfigFixed(self, get_config, from_config):
        ...
        self.assertEqual(self.evaluate(opt.lr), 2.0)
        self.assertEqual(self.evaluate(opt.inner_optimizer.lr), 2.0)
    ...
    @parameterized.named_parameters(
        {"testcase_name": "V2", "use_v1": False},
        {"testcase_name": "V1", "use_v1": True},
    )
    def testSerializationWithBuiltInOptimizer(self, use_v1):
        opt = gradient_descent.SGD(2.0, momentum=0.5)
        if use_v1:
            loss_scale = tf_loss_scale_module.DynamicLossScale(
                initial_loss_scale=2.0, increment_period=3.0
            )
            opt = loss_scale_optimizer.LossScaleOptimizerV1(opt, loss_scale)
        else:
            opt = loss_scale_optimizer.LossScaleOptimizer(
        ...
        self.assertEqual(self.evaluate(opt.loss_scale), 2.0)
        self.assertEqual(opt.dynamic_growth_steps, 3.0)
        self.assertTrue(opt.dynamic, 4.0)
    ...
    def testInvalidArgsWithFixedLossScale(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            '"dynamic_growth_steps" must be None if "dynamic" is ' "False, but got: 2",
        ):
    ...
    def testErrorWrappingSameOptimizerMultipleTimes(self):
        ...
        with self.assertRaisesRegex(
            ValueError, '"inner_optimizer" is already wrapped by a LossScaleOptimizer.'
        ):
            loss_scale_optimizer.LossScaleOptimizer(inner_opt)
