...
class KerasModelTest(keras_parameterized.TestCase):
    ...
    def test_model(
        ...
        with strategy_fn().scope():
            ...
            with policy.policy_scope(cls(policy_name)):
                ...
                model.compile(
                    opt, loss=loss_fn, run_eagerly=testing_utils.should_run_eagerly()
                )
    ...
    def _test_saving(self, model, dataset, save_format, use_regularizer):
        ...
        self.assertEqual(get_layer_policy.get_layer_policy(layer).name, "mixed_float16")
        ...
        with strategy.scope():
            with policy.policy_scope(policy.Policy("mixed_float16")):
                ...
                layer3 = mp_test_util.MultiplyLayer(
                    assert_type=dtypes.float16, use_operator=False
                )
    ...
    def test_dynamic_loss_scaling(
        ...
        expected_gradient = backend.variable(
            [initial_loss_scale / batch_size], dtype=dtypes.float16
        )
        ...
        with strategy.scope():
            ...
            with policy.policy_scope(p):
                ...
                identity_with_grad_check_fn = mp_test_util.create_identity_with_grad_check_fn(
                    expected_dtype=dtypes.float16, expected_gradient=expected_gradient
                )
                y = core.Lambda(identity_with_grad_check_fn)(y)
                model = models.Model(inputs=x, outputs=y)
                if get_config:
        ...
        backend.set_value(expected_gradient, backend.get_value(expected_gradient * 2))
        model.fit(dataset)
    ...
    def test_loss_scale_optimizer_overrides_policy_v1_loss_scale(self):
        with policy.policy_scope(policy.PolicyV1("float32", loss_scale=10.0)):
            opt = gradient_descent.SGD(1.0)
            opt = loss_scale_optimizer.LossScaleOptimizer(
                opt, dynamic=False, initial_scale=5.0
            )
            x = layers.Input(shape=(1,))
            y = mp_test_util.MultiplyLayer()(x)
            model = models.Model(x, y)
            model.compile(opt, loss="mse")
            self.assertEqual(self.evaluate(model.optimizer.loss_scale), 5.0)

    @combinations.generate(combinations.combine(mode=["graph", "eager"]))
    def test_policy_v1_without_loss_scale(self):
        with policy.policy_scope(policy.PolicyV1("mixed_float16", loss_scale=None)):
            opt = gradient_descent.SGD(1.0)
            x = layers.Input(shape=(1,))
            y = mp_test_util.MultiplyLayer()(x)
            model = models.Model(x, y)
    ...
    @parameterized.named_parameters(
        {"testcase_name": "base", "strategy_fn": default_strategy_fn},
        {"testcase_name": "distribute", "strategy_fn": create_mirrored_strategy},
        {"testcase_name": "base_h5", "strategy_fn": default_strategy_fn, "h5": True},
        {
    ...
    def test_save_slot_variables_with_autocast_vars(self, strategy_fn, var_name="v"):
        p = policy.Policy("mixed_float16")
        with strategy_fn().scope(), policy.policy_scope(p):
            ...
            layer = mp_test_util.MultiplyLayer(
                assert_type=dtypes.float16, var_name=var_name
            )
            y = layer(x)
            model = models.Model(inputs=x, outputs=y)
    ...
    def test_restore_old_loss_scale_checkpoint(self):
        ...
        self.assertAllClose(
            self.evaluate(opt.get_slot(model.weights[0], "momentum")), expected_slot
        )
    ...
    def test_restore_old_saved_model(self):
        ...
        self.assertEqual(type(model.optimizer), loss_scale_optimizer.LossScaleOptimizer)
    ...
    def test_save_model_with_dynamic_loss_scaling(
        ...
        with strategy.scope():
            ...
            if use_v1_loss_scale_optimizer:
                loss_scale = loss_scale_module.DynamicLossScale(
                    initial_loss_scale=1.0, increment_period=2.0
                )
                opt = loss_scale_optimizer.LossScaleOptimizerV1(opt, loss_scale)
            else:
                opt = loss_scale_optimizer.LossScaleOptimizer(
