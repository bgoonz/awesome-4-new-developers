...
class OptimizerV2(trackable.Trackable):
  ...
  """
    ...
    def __init__(
        self, name, gradient_aggregator=None, gradient_transformers=None, **kwargs
    ):
        ...
        with ops.name_scope_v2(self._name):
            ...
            if optimizer_utils.strategy_supports_no_merge_call():
                return self._distributed_apply(
                    strategy, grads_and_vars, name, apply_state
                )
            else:
    ...
    def _create_hypers(self):
        ...
        with self._distribution_strategy_scope():
            ...
            for name, value in sorted(self._hyper.items()):
                if isinstance(value, (ops.Tensor, tf_variables.Variable)) or callable(
                    value
                ):
    ...
    """
        ...
        if len(params) != len(weights):
            raise ValueError(
                ...
                + str(len(weights))
                + ", but the optimizer was expecting "
                + str(len(params))
    ...
    """
        ...
        if (
            ...
            # variables.
            and (
                not ops.get_default_graph()._variable_creator_stack
