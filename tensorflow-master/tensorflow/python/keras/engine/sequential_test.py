...
class TestSequential(keras_parameterized.TestCase):
  ...
  """
    ...
    def test_multi_output_layer_not_accepted(self):
        ...
        with self.assertRaisesRegex(ValueError, "should have a single output tensor"):
    ...
    def test_sequential_layer_tracking(self):
        ...
        self.assertEqual(
            list(model._flatten_layers(include_self=False, recursive=False))[-1], layer
        )
        ...
        model.add(layer2)
        self.assertEqual(
            list(model._flatten_layers(include_self=False, recursive=False))[-1], layer2
        )
    ...
    def test_config_preserves_input_layer(self):
        ...
        layers = list(new_model._flatten_layers(include_self=False, recursive=False))
