  ...
  """
    ...
    def __init__(self, use_steps, num_samples=None, steps=None):
        super(MetricsAggregator, self).__init__(
            use_steps=use_steps, num_samples=num_samples, steps=steps, batch_size=None
        )
        ...
        if self.batch_size and self.batch_size < batch_element.shape[0]:
            raise ValueError(
                ...
                "Output shape = {}, expected output shape = shape {}".format(
                    batch_element.shape, (self.batch_size,) + batch_element.shape[1:]
                )
            )
  ...
  """
    ...
    def __init__(self, num_samples, batch_size):
        ...
        super(SliceAggregator, self).__init__(
            use_steps=False, num_samples=num_samples, steps=None, batch_size=batch_size
        )
...
def standardize_input_data(
  ...
  """
    ...
    if isinstance(data, dict):
        try:
            data = [
                data[x].values if data[x].__class__.__name__ == "DataFrame" else data[x]
                for x in names
            ]
...
def standardize_weights(
  ...
  """
    ...
    if sample_weight is not None:
        ...
        if (
            not tensor_util.is_tf_type(sample_weight)
...
def has_tensors(ls):
    ...
    if isinstance(ls, (list, tuple)):
        return any(
            tensor_util.is_tf_type(v) and not isinstance(v, ragged_tensor.RaggedTensor)
            for v in ls
        )
...
def prepare_loss_weights(training_endpoints, loss_weights=None):
  ...
  """
    if loss_weights is None:
        ...
        generic_utils.check_for_unexpected_keys(
            "loss_weights", loss_weights, [e.output_name for e in training_endpoints]
        )
        for e in training_endpoints:
...
def is_eager_dataset_or_iterator(data):
    return context.executing_eagerly() and isinstance(
        data, (dataset_ops.DatasetV1, dataset_ops.DatasetV2, iterator_ops.IteratorBase)
    )
