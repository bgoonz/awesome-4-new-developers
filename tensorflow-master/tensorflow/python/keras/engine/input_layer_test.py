...
class InputLayerTest(keras_parameterized.TestCase):
    ...
    def testBasicOutputShapeNoBatchSize(self):
        ...
        self.assertAllEqual(
            model(array_ops.ones((3, 32))), array_ops.ones((3, 32)) * 2.0
        )
    ...
    def testBasicOutputShapeWithBatchSize(self):
        ...
        model = functional.Functional(x, x * 2.0)
        self.assertAllEqual(
            model(array_ops.ones(x.shape)), array_ops.ones(x.shape) * 2.0
        )
    ...
    def testBasicOutputShapeNoBatchSizeInTFFunction(self):
        ...
        self.assertAllEqual(
            run_model(array_ops.ones((10, 8))), array_ops.ones((10, 8)) * 2.0
        )
    ...
    def testInputTensorArgInTFFunction(self):
        ...
        self.assertAllEqual(
            run_model(array_ops.ones((10, 16))), array_ops.ones((10, 16)) * 3.0
        )
    ...
    def testNoMixingArgsWithTypeSpecArg(self):
        ...
        with self.assertRaisesRegexp(
            ...
            input_layer_lib.Input(
                batch_size=4, type_spec=tensor_spec.TensorSpec((7, 32), dtypes.float32)
            )
        with self.assertRaisesRegexp(
            ValueError, "all other args except `name` must be None"
        ):
        ...
        with self.assertRaisesRegexp(
            ...
            input_layer_lib.Input(
                sparse=True, type_spec=tensor_spec.TensorSpec((7, 32), dtypes.float32)
            )
        with self.assertRaisesRegexp(
            ValueError, "all other args except `name` must be None"
        ):
            input_layer_lib.Input(
                ragged=True, type_spec=tensor_spec.TensorSpec((7, 32), dtypes.float32)
            )
    ...
    def testCompositeTypeSpecArgWithoutDtype(self):
        for assign_variant_dtype in [False, True]:
            ...
            def lambda_fn(tensors):
                return math_ops.cast(tensors.x, dtypes.float64) + math_ops.cast(
                    tensors.y, dtypes.float64
                )
