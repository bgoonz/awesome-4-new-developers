...
def _validate_arguments(
  ...
  """
    ...
    val_gen = data_utils.is_generator_or_sequence(validation_data) or isinstance(
        validation_data, iterator_ops.IteratorBase
    )
    if (
        val_gen
    ...
    if any(k != "steps" for k in kwargs):
        raise ValueError(
            "Invalid arguments passed: {}".format([k for k in kwargs if k != "steps"])
        )
...
def _make_enqueued_generator(
    generator, workers=1, use_multiprocessing=False, max_queue_size=10, shuffle=False
):
...
class EagerDatasetOrIteratorTrainingLoop(training_utils_v1.TrainingLoop):
    ...
    def fit(
        ...
        **kwargs
        ...
        if isinstance(x, (dataset_ops.DatasetV1, dataset_ops.DatasetV2)) and shuffle:
