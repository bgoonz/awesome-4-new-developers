...
class HasList(training.Model):
    def __init__(self):
        ...
        self.layer_list += data_structures.wrap_or_unwrap(
            [core.Dense(9)]
        ) + data_structures.wrap_or_unwrap([core.Dense(10)])
        self.layer_list.extend(
            data_structures.wrap_or_unwrap(list([core.Dense(11)]) + [core.Dense(12)])
        )
...
class ListTests(keras_parameterized.TestCase):
    ...
    def testTracking(self):
        with self.test_session():
            ...
            self.assertIs(
                model.layers_with_updates, model._checkpoint_dependencies[1].ref
            )
            self.assertEqual(
    ...
    def testTensorConversion(self):
        ...
        self.assertAllEqual(
            [1.0, 2.0, 3.0], self.evaluate(gen_array_ops.Pack(values=ListToTensor().l))
        )
...
class HasMapping(training.Model):
    def __init__(self):
        ...
        self.layer_dict["norm"].append(batch_normalization_v1.BatchNormalization())
        self.layer_dict["norm"].append(batch_normalization_v1.BatchNormalization())
...
class TupleTests(keras_parameterized.TestCase):
    ...
    def testTensorConversion(self):
        ...
        self.assertAllEqual(
            (1.0, 2.0, 3.0), self.evaluate(gen_array_ops.Pack(values=TupleToTensor().l))
        )
