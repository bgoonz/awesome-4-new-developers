...
class TestImage(keras_parameterized.TestCase):
    ...
    def test_image_data_generator_invalid_data(self):
        ...
        with self.assertRaises(ValueError):
            generator = preprocessing_image.ImageDataGenerator(data_format="unknown")

        generator = preprocessing_image.ImageDataGenerator(zoom_range=(2.0, 2.0))
    ...
    def test_directory_iterator(self):
        ...
        for test_images in _generate_test_images():
            for im in test_images:
                ...
                filename = os.path.join(
                    classpaths[count % len(classpaths)], "image-{}.jpg".format(count)
                )
        ...
        _ = preprocessing_image.load_img(
    ...
    def directory_iterator_with_validation_split_test_helper(self, validation_split):
        if PIL is None:
            return  # Skip test if PIL is not available.
    ...
    def test_img_utils(self):
        ...
        img = preprocessing_image.array_to_img(x, data_format="channels_first")
        self.assertEqual(img.size, (width, height))
        x = preprocessing_image.img_to_array(img, data_format="channels_first")
    ...
    def test_img_transforms(self):
        ...
        _ = preprocessing_image.random_shear(x, 2.0)
        ...
        _ = preprocessing_image.random_channel_shift(x, 2.0)
