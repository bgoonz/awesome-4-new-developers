    ...
    if condition_arg_error is not None:
        if infeed_queue is None:
            raise TypeError(
                ...
                "condition needs %s"
                % (input_arity, str([i.name for i in inputs]), condition_arg_error)
            )
        else:
            raise TypeError(
                "Supplied loop condition function cannot be called with the "
    ...
    def body_wrapper(*inputs):
        ...
        if tensor_tracer.TensorTracer.is_enabled():
            ...
            output_tensors = tt.trace_tpu(
                ops.get_default_graph(), output_tensors, None, num_replicas
            )
...
def repeat(
    n: int,
    body: Callable[
        ..., Union[core_types.TensorLike, Iterable]
    ],  # pylint:disable=g-bare-generic
    inputs: Optional[List[core_types.TensorLike]] = None,
    infeed_queue: Optional[tpu_feed.InfeedQueue] = None,
    name: Any = None,
) -> List[core_types.TensorLike]:
