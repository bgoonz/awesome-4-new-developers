...
class TPUEmbeddingCheckpointTest(parameterized.TestCase, test.TestCase):
    def setUp(self):
        ...
        with self.strategy.scope():
            ...
            self.second_mid_level = self.build_mid_level(
                ...
                self.second_mid_level_optimizer,
                initialize_tpu_embedding=False,
            )

        self.cpu_mid_level_optimizer = tpu_embedding_v2_utils.SGD(learning_rate=0.1)
    ...
    def test_checkpoint_save_retrieves(self):
        ...
        self.assertAllClose(
            ...
            self.make_checkpoint_and_get_embedding("before_load", self.first_mid_level),
        ...
        self.assertAllClose(
            self.second_mid_level_contents,
            self.make_checkpoint_and_get_embedding("after_load", self.first_mid_level),
    ...
    def test_checkpoint_restore_before_variable_creation(self):
        ...
        with self.strategy.scope():
            module1 = TestModule(
                init_ops_v2.Ones(), self.strategy.num_replicas_in_sync * 2
            )
        ...
        with self.strategy.scope():
            module2 = TestModule(
                init_ops_v2.Zeros(), self.strategy.num_replicas_in_sync * 2
            )
        ...
        self.assertAllClose(
            np.ones((self.strategy.num_replicas_in_sync * 2, 4)),
    ...
    def build_mid_level(
        ...
        mid_level = tpu_embedding_v2.TPUEmbedding(feature_config, optimizer)
    ...
    def make_checkpoint_and_get_embedding(self, name, model):
        ...
        if len(variables) != 1:
            raise RuntimeError(
                "Found {} copies of the parameter variable in the "
                "checkpoint. Exactly one copy exported.".format(len(variables))
            )
    ...
    def test_model_export_cpu(self):
        ...
        def serve_tensors(features):
            features = tpu_embedding_v2.cpu_embedding_lookup(
                features,
                None,
                self.cpu_mid_level.embedding_tables,
        ...
        self.assertAllClose(
            prediction.numpy(), self.first_mid_level_contents[input_feature_value]
        )
    ...
    def test_check_checkpoint_variable_names_are_same_on_cpu_and_tpu(self, optimizer):
        ...
        cpu_mid_level = self.build_mid_level(self.first_mid_level_contents, optimizer)
...
class TPUEmbeddingTest(parameterized.TestCase, test.TestCase):
    ...
    def test_variable_learning_rate(self):
        ...
        learning_rates = [
            starting_lr - (starting_lr - ending_lr) / num_steps * j
    ...
    def test_optimizer_with_slot_creation_fn(self, use_tpu):
        ...
        optimizer = tpu_embedding_v2_utils.Adagrad(
            learning_rate=0.1, slot_variable_creation_fn=slot_creation_fn
        )
        if use_tpu:
            strategy = self._get_strategy()
        else:
        ...
        self.assertAllClose(
            video_accumulator.numpy(),
            np.zeros((self.table_video.vocabulary_size, self.table_video.dim)),
        )
        self.assertAllClose(
            user_accumulator.numpy(),
            np.zeros((self.table_user.vocabulary_size, self.table_user.dim)),
        )
    ...
    def test_optimizer_with_slot_creation_fn_non_partial(self):
        def slot_creation_fn(table, slot_names, _):
            ...
            for slot in slot_names:
                ...
                slots[slot] = tf_variables.Variable(
                    name="{}_{}".format(table.name, slot),
                    initial_value=init_ops_v2.Zeros()(
                        shape=table.shape, dtype=dtypes.float32
                    ),
                    trainable=False,
                )
            return slots
        ...
        with strategy.scope():
            ...
            with self.assertRaisesRegex(
                ValueError, "Unable to extract initializer function"
            ):
                # We aren't going to actually run anything, so the batch_size here does
                # not matter.
                mid_level_api.build(self.batch_size)
