...
class CPUEmbeddingTest(test.TestCase):
    ...
    def test_cpu_sparse_lookup_with_weights(self):
        ...
        weighted_sum = []
        for feature, weight, config in zip(
            nest.flatten(features), nest.flatten(weights), self.feature_config
        ):
            table = mid_level.embedding_tables[config.table].numpy()
    ...
    def test_cpu_no_optimizer(self):
        ...
        mid_level.build()
        self.assertEqual(
            list(mid_level._variables[self.table_video.name].keys()), ["parameters"]
        )
