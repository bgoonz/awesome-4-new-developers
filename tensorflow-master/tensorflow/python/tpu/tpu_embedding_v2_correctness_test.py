...
flags.DEFINE_string(
    "model_dir", os.environ.get("TEST_TMPDIR"), "A temporary directory."
)
...
class TPUEmbeddingCorrectness(parameterized.TestCase, test.TestCase):
    def setUp(self):
        ...
        self.feature_config = (
            ...
            tpu_embedding_v2_utils.FeatureConfig(table=self.table_user, name="friends"),
        )
    ...
    def _create_ragged_dataset(self, strategy, include_weights=False, weight=0.5):
        # Create dataset for enqueue operation
        sparse_features = self._create_sparse_data(include_weights, weight)
        ragged_features = nest.map_structure(
            ragged_tensor.RaggedTensor.from_sparse, sparse_features
        )
        ...
        def input_fn(ctx):
            ...
            features = (
                constant_op.constant(
                    self.feature_watched_values[-2:], dtype=dtypes.int32
                ),
                constant_op.constant(
                    self.feature_favorited_values[-2:], dtype=dtypes.int32
                ),
                constant_op.constant(
                    self.feature_friends_values[-2:], dtype=dtypes.int32
                ),
            )
            if include_weights:
                weights = [
    ...
    def _check_results(
        ...
        loss_gold0 = _compute_loss(
            activation_watched_gold0,
        ...
        activation_watched_gold = np.concatenate(
            (
                np.concatenate(
                    (np.expand_dims(activation_watched_gold0, axis=0),)
                    * (num_replicas // 2)
                ),
                np.concatenate(
                    (np.expand_dims(activation_favorited_gold0, axis=0),)
                    * (num_replicas // 2)
                ),
            ),
            axis=1,
        ).reshape([self.batch_size * num_replicas, 4])
        ...
        if training:
            ...
            gradient_wrt_favorited_gold = (
                2 * activation_favorited_gold / global_batch_size
            )
            gradient_wrt_friends_gold = 2 * activation_friends_gold / global_batch_size

            # Calculate gradients wrt embedding tables.
            gradients_wrt_user = _compute_gradients_wrt_embedding_table(
                global_batch_size,
                gradient_wrt_friends_gold,
                embedding_table_user_before,
                input_data[2].indices.numpy(),
                input_data[2].values.numpy(),
                self.table_user.combiner,
            )
            gradients_wrt_video = _compute_gradients_wrt_embedding_table(
                global_batch_size,
                gradient_wrt_favorited_gold,
                embedding_table_video_before,
                input_data[1].indices.numpy(),
                input_data[1].values.numpy(),
                self.table_video.combiner,
            ) + _compute_gradients_wrt_embedding_table(
                global_batch_size,
                gradient_wrt_watched_gold,
                embedding_table_video_before,
                input_data[0].indices.numpy(),
                input_data[0].values.numpy(),
                self.table_video.combiner,
            )

            self._check_embedding_and_slot_variables(
                embedding_table_user_before,
                gradients_wrt_user,
                embedding_table_video_before,
                gradients_wrt_video,
    ...
    def _check_embedding_and_slot_variables(
        self,
        embedding_table_user_before,
        gradients_wrt_user,
        embedding_table_video_before,
    ...
    def _check_embedding_and_slot_variables_for_sgd(
        self, embedding_table_before, gradients, optimizer, variables
    ):
        embedding_table = np.copy(embedding_table_before)
        embedding_table -= optimizer.learning_rate * np.sum(gradients, axis=0)
        self.assertAllClose(
            _get_variable(variables["parameters"]).numpy(), embedding_table
        )

    def _check_embedding_and_slot_variables_for_adagrad(
        ...
        embedding_table -= (
            optimizer.learning_rate * np.sum(gradients, axis=0) / np.sqrt(accumulator)
        )
        self.assertAllClose(
    ...
    def _check_embedding_and_slot_variables_for_adam(
        self, embedding_table_before, gradients, optimizer, variable
    ):
        embedding_table = np.copy(embedding_table_before)
        ...
        self.assertAllClose(
            _get_variable(variable["parameters"]).numpy(), embedding_table, rtol=1e-4
        )
        self.assertAllClose(_get_variable(variable["momenta"]).numpy(), m, rtol=1e-4)
        self.assertAllClose(_get_variable(variable["velocities"]).numpy(), v, rtol=1e-4)

    def _check_embedding_and_slot_variables_for_ftrl(
        self, embedding_table_before, gradients, optimizer, variable
    ):
        embedding_table = np.copy(embedding_table_before)
        ...
        sigma = (
            accumulator ** neg_lr_p - optimizer.initial_accumulator_value ** neg_lr_p
    ...
    def test_dense_lookup(self):
        ...
        golden = (
            numpy_videos[self.feature_watched_values[-2:]],
...
def _compute_gradients_wrt_embedding_table(
  ...
  """
    ...
    for i in range(batch_size):
        ...
        count = 0
        for (batch_i, seq_index), vocabulary_id in zip(feature_indices, feature_values):
            if batch_i == i:
                count += 1
                if max_sequence_length > 0:
                    if seq_index < max_sequence_length:
                        grad[vocabulary_id, :] += gradient_wrt_activation[
                            i, seq_index, :
                        ]
                else:
    ...
    for activation in activations:
        losses.append(
            math_ops.reduce_mean(
                math_ops.reduce_sum(gen_math_ops.squared_difference(activation, 0), 1)
            )
        )
