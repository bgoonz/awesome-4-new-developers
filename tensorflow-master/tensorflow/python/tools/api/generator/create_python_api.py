...
class _ModuleInitCodeBuilder(object):
    ...
    def _check_already_imported(self, symbol_id, api_name):
        if (
            api_name in self._dest_import_to_id
            and symbol_id != self._dest_import_to_id[api_name]
            and symbol_id != -1
        ):
    ...
    def add_import(
        self, symbol, source_module_name, source_name, dest_module_name, dest_name
    ):
...
"""
        ...
        if self._api_version == 1 or self._lazy_loading:
            for dest_module, _ in self._module_imports.items():
                ...
                if (
                    not dest_module
                    and "lite" in self._module_imports
    ...
    def copy_imports(self, from_dest_module, to_dest_module):
        self._module_imports[to_dest_module] = self._module_imports[
            from_dest_module
        ].copy()
  ...
  """
    ...
    for module in list(sys.modules.values()):
        ...
        if (
            ...
            continue
        if packages_to_ignore and any(
            [p for p in packages_to_ignore if p in module.__name__]
        ):
            continue

        # Do not generate __init__.py files for contrib modules for now.
        if (
            ".contrib." in module.__name__ or module.__name__.endswith(".contrib")
        ) and ".lite" not in module.__name__:
            continue

        for module_contents_name in dir(module):
            if (
                module.__name__ + "." + module_contents_name
...
def get_module_docstring(module_name, package, api_name):
  ...
  """
    ...
    if (
        docstring_module_name in sys.modules
...
def create_api_files(
  ...
  """
    ...
    (module_text_map, deprecation_footer_map, root_module_footer) = get_api_init_text(
        packages,
        packages_to_ignore,
        output_package,
        api_name,
    ...
    for module, text in module_text_map.items():
        ...
        if module == root_module and root_init_template:
            ...
            contents = (
                ...
                % get_module_docstring(module, packages[0], api_name)
                + text
                + _GENERATED_FILE_FOOTER
            )
        if module in deprecation_footer_map:
            if "# WRAPPER_PLACEHOLDER" in contents:
                contents = contents.replace(
                    "# WRAPPER_PLACEHOLDER", deprecation_footer_map[module]
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--output_package", default="tensorflow", type=str, help="Root output package."
    )
    parser.add_argument(
        "--loading",
        default="default",
    ...
    if args.loading == "default":
        ...
        raise ValueError(
            "Invalid value for --loading flag: %s. Must be one of "
    ...
    create_api_files(
        ...
        packages,
        packages_to_ignore,
        args.root_init_template,
        args.apidir,
        args.output_package,
        args.apiname,
        args.apiversion,
        args.compat_apiversions,
        args.compat_init_templates,
        lazy_loading,
