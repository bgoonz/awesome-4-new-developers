...
class ParserTest(test.TestCase):
    def assertAstMatches(self, actual_node, expected_node_src, expr=True):
        ...
        msg = "AST did not match expected:\n{}\nActual:\n{}".format(
            pretty_printer.fmt(expected_node), pretty_printer.fmt(actual_node)
        )
    ...
    def test_parse_lambda_resolution_ambiguous(self):
        ...
        with self.assertRaisesRegex(
            errors.UnsupportedLanguageElementError, expected_exception_text
        ):
    ...
    def test_parse_lambda_in_expression(self):
        ...
        l = (lambda x: lambda y: x + y + 1, lambda x: lambda y: x + y + 2)
        ...
        node = gast.If(
            ...
            body=[
                gast.Assign(
                    targets=[
                        gast.Name(
                            "a", ctx=gast.Store(), annotation=None, type_comment=None
                        )
                    ],
                    value=gast.Name(
