...
class QN(object):
    ...
    def __init__(self, base, attr=None, subscript=None):
        if attr is not None and subscript is not None:
            raise ValueError(
                "A QN can only be either an attr or a subscript, not "
    ...
    def __eq__(self, other):
        return (
            ...
            and self.has_subscript() == other.has_subscript()
            and self.has_attr() == other.has_attr()
        )
    ...
    def ast(self):
        ...
        if isinstance(base, str):
            ...
            assert False, (
                "the constructor should prevent types other than " "str and Literal"
            )
