...
class ControlFlowTestBase(converter_testing.TestCase):
    def assertValuesEqual(self, actual, expected):
        values = nest.map_structure(
            lambda x: self.evaluate(x) if tensor_util.is_tf_type(x) else x, actual
        )
...
class NestedControlFlowTest(ControlFlowTestBase):
    def test_basic(self):
        ...
        self.assertTransformedResult(f, constant_op.constant(5), (25, 5, 0, 5))
...
class WhileStatementTest(ControlFlowTestBase):
    ...
    def test_composite_state_slice_initialized_in_loop(self):
        ...
        self.assertTransformedResult(f, (0, constant_op.constant(10)), {"subkey": 14})
        tr = self.transform(f, control_flow)
        with self.assertRaisesRegex(
    ...
    def test_composite_state_slice_aliased_to_local(self):
        ...
        self.assertTransformedResult(f, (0, constant_op.constant(10)), {"subkey": 11})
        tr = self.transform(f, control_flow)
        ...
        with self.assertRaises(errors.InaccessibleTensorError):
            ...
            self.evaluate(graph_fn(constant_op.constant(0), constant_op.constant(5)))
    ...
    def test_non_tensor_state_illegal_type(self):
        ...
        with self.assertRaisesRegex(ValueError, "tc.*must be defined before the loop"):
...
class IfStatementTest(ControlFlowTestBase):
    ...
    def test_sparse_tensor(self):
        ...
        st = sparse_tensor.SparseTensor(indices=((0,),), values=(0,), dense_shape=(1,))
    ...
    def test_unbalanced_multiple_composites(self):
        ...
        self.assertTransformedResult(
            f, (Foo(), constant_op.constant(True)), (7, 11, 13)
        )
        self.assertTransformedResult(f, (Foo(), constant_op.constant(False)), (2, 3, 5))
    ...
    def test_unbalanced_composite(self):
        ...
        self.assertTransformedResult(f, (Foo(), constant_op.constant(True)), (7, 13))
        self.assertTransformedResult(f, (Foo(), constant_op.constant(False)), (2, 5))
...
class ForStatementTest(ControlFlowTestBase):
    ...
    def test_with_comprehension_in_body(self):
        ...
        self.assertTransformedResult(
            f, (constant_op.constant([1, 2, 3]), 5), np.array(range(5)) * 4
        )
