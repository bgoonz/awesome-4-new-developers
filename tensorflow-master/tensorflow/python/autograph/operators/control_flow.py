  ...
  """
    ...
    for name, val, fi_val in zip(symbol_names, init_vars, first_iter_vars):
        if isinstance(val, variables.UndefinedReturnValue):
            if fi_val:
                raise ValueError(
                    "the return value from a TensorFlow loop may only be a {}; got {}".format(
...
def _verify_single_loop_var(name, check_shape, init, entry, exit_, shape_invariant):
    ...
    if entry.dtype != exit_.dtype:
        raise TypeError(
            ...
            " iteration".format(name, entry.dtype.name, exit_.dtype.name)
        )
    if check_shape:
...
def _verify_tf_cond_branch_vars(vars_, symbol_names, branch_name):
    ...
    for name, var_ in zip(symbol_names, vars_):
        ...
        if isinstance(var_, variables.UndefinedReturnValue):
            raise ValueError(
                "the {} branch must also have a return statement.".format(branch_name)
            )
...
def _tf_range_for_stmt(
    ...
    def aug_get_state():
        ...
        state_vars = tuple(
            _value_or(name, var, iterate) for name, var in zip(symbol_names, state_vars)
        )
    ...
    _add_max_iterations_hint(
        opts, math_ops.cast(misc.get_range_len(start, limit, delta), dtypes.int32)
    )
...
def _tf_iterator_for_stmt(
    ...
    def aug_body():
        ...
        # tensors.
        aug_set_state(control_flow_ops.cond(has_next, main_path, noop_path))
...
def _general_purpose_scan(ds, init_state, body):
    ...
    return dataset_ops._ScanDataset(ds, init_state, body, use_default_device=False)
...
def _py_while_stmt(test, body, get_state, set_state, opts):
    ...
    def guarded_test():
        ...
        try:
            ...
            ag_logging.log(
                1, "Caught error while evaluating while loop condition", exc_info=True
            )
        ...
        with ops.control_dependencies(
            [
                control_flow_ops.Assert(
                    final_loop_vars[0],
                    [
                        _runtime_zero_iterations_errmsg(
                            symbol_names, nulls, orig_init_vars
                        )
                    ],
                )
            ]
...
def _tf_if_stmt(cond, body, orelse, get_state, set_state, symbol_names, nouts):
    ...
    final_cond_vars = control_flow_ops.cond(cond, aug_body, aug_orelse, strict=True)
