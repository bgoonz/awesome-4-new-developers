...
def IsCondSwitch(op):
    ...
    for o in op.outputs:
        for c in o.consumers():
            ...
            is_cond_switch = is_cond_switch and (
                ctxt is not None and ctxt.IsCondContext()
            )
...
def OpInContext(op, ctxt):
    return IsContainingContext(
        op._get_control_flow_context(), ctxt
  ...
  """
    ...
    if not input_ctxt:
        ...
        if while_ctxt is None:
            ...
            # while loop at all).
            valid = True
        elif while_ctxt.grad_state and IsContainingContext(
            while_ctxt.grad_state.forward_context, input_while_ctxt
        ):
            ...
            while_ctxt.grad_state
            and while_ctxt.grad_state.forward_context is input_while_ctxt._outer_context
        ):  # pylint: disable=protected-access
            ...
            input_while_ctxt.grad_state
            and input_while_ctxt.grad_state.forward_context is while_ctxt
        ):
            ...
            input_while_ctxt.grad_state
            and input_ctxt.grad_state.forward_context.grad_state
            and input_ctxt.grad_state.forward_context.grad_state.forward_context
            is while_ctxt
        ):
    ...
    if not valid:
        ...
        log_msg += "\n\nTraceback for %s:\n%s\nTraceback for %s:\n%s\n" % (
            op.name,
            "".join(traceback.format_list(op.traceback)),
            input_op.name,
            "".join(traceback.format_list(input_op.traceback)),
        )
