  ...
  """
    ...
    with ops.name_scope(name):
        ...
        axis = array_ops.get_positive_axis(
            axis, params.shape.rank, ndims_name="params.shape.rank"
        )
...
def _expand_st_row_partitions(st, axis):
    ...
    if axis == 0:
        ...
        return st.row_partitions + (
            RowPartition.from_uniform_row_length(1, nvals, nrows=nvals, validate=False),
        )
    else:
        nvals = st.row_partitions[axis - 1].nrows() if (axis - 1 >= 0) else st.nrows()
        return (
            st.row_partitions[: axis - 1]
            + (
  ...
  """
    ...
    if isinstance(value, StructuredTensor):
        ...
        for k in value.field_names():
            new_fields[k] = _extend_op(
                [v.field_value(k) for v in values], leaf_op, empty_st_op
            )
...
def _structured_tensor_from_dense_tensor(t):
    ...
    if t.shape.is_fully_defined():
        ...
        return StructuredTensor.from_fields(
            {}, shape=t.shape, nrows=array_ops.shape(t)[0]
        )
    else:
        rt = ragged_tensor.RaggedTensor.from_tensor(t)
        return _structured_tensor_from_row_partitions(
            t.shape, rt._nested_row_partitions
        )
...
def _structured_tensor_like(t):
    ...
    return StructuredTensor.from_fields(
        {}, shape=t.shape, row_partitions=t.row_partitions, nrows=t.nrows()
    )
