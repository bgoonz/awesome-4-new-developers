  ...
  """
    ...
    """
        if internal is not _structured_tensor_factory_key:
            raise ValueError(
                "StructuredTensor constructor is private; please use "
    ...
    @classmethod
    def from_fields(
        cls, fields, shape=(), nrows=None, row_partitions=None, validate=False
    ):
    ...
    """
        ...
        if not isinstance(fields, dict):
            raise TypeError(
                "fields must be a dictionary, got %s" % type(fields).__name__
            )
        ...
        with ops.name_scope(None, "StructuredTensor", fields.values()):
            ...
            if rank == 1:
                ...
                for value in fields.values():
                    nrows, static_nrows = _merge_nrows(
                        nrows, static_nrows, value, shape_dtype, validate
                    )
                if nrows is None:
                    if static_nrows.value is None:
                        raise ValueError(
                            "nrows must be specified if rank==1 "
                            "and `fields` is empty."
                        )
                    else:
            ...
            if rank > 1:
                ...
                fields = dict(
                    [
                        (k, _replace_row_partitions(v, row_partitions))
    ...
    """
        updates_items = [
            (_normalize_field_name_to_tuple(name), value)
    ...
    def _with_updates_impl(
        ...
        def apply_value(
            name: str, value: Union["FieldValue", "FieldFn"]
        ) -> "FieldValue":
            if callable(value):
        ...
        for name, value in updates:
            ...
            if len(name) == 1:
                ...
                if not isinstance(current_value, StructuredTensor):
                    raise ValueError(
                        ...
                        "can contain sub-structures -- it is a `{}`.".format(
                            error_prefix + tuple(name),
                            name_fullpath(prefix),
                            type(current_value),
                        )
                    )
                ...
                value = current_value._with_updates_impl(
                    error_prefix + (prefix,), one_update, validate
                )
        ...
        if isinstance(field_name, (list, tuple)):
            ...
            for f in field_name:
                if not isinstance(value, StructuredTensor):
                    raise KeyError(
                        "Field path {} not found in {}".format(field_name, self)
                    )
    ...
    * If `struct_tensor` is non-scalar (i.e., a vector or higher-dimensional
    ...
    def _scalar_getitem(self, key):
        if (
            ...
            and key[0].step is None
        ):
            fields = dict(
                (field_name, field_value.__getitem__(key[1:]))
            ...
            raise ValueError(
                "Key for indexing a StructuredTensor must be a "
    ...
    def _tensor_getitem(self, key):
        ...
        if len(key) <= rank:
            new_fields = dict(
                (field_name, field_value.__getitem__(key))
                for (field_name, field_value) in self._fields.items()
            )
    ...
    * `StructTensors` with `rank>0` are converted to nested python `list`s,
    ...
    """
        ...
        if len(self._shape) > 0:  # pylint: disable=g-explicit-length-test
            if not result:  # special-case for StructuredTensors w/ no fields.
                return _empty_dict_pylist_from_row_partitions(
                    self._row_partitions, self._nrows
                )
    ...
    """
        if isinstance(pyval, dict):
            ...
            if rank is not None:
                return cls._from_pylist_of_dict(
                    pyval, keys, rank, typespec, path_so_far
                )
            else:
    ...
    def _from_pydict(cls, pyval, typespec, path_so_far):
        ...
        if typespec is None:
            fields = dict(
                (k, cls._from_pyval(v, None, path_so_far + (k,)))
            ...
            if not (
                isinstance(typespec, StructuredTensorSpec)
                and spec_shape.rank == 0
                and set(pyval) == set(field_specs)
            ):
                raise ValueError(
            ...
            fields = dict(
                (k, cls._from_pyval(v, field_specs[k], path_so_far + (k,)))
                for (k, v) in pyval.items()
            )
    ...
    def _from_pylist_of_dict(cls, pyval, keys, rank, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if shape.rank < rank:
                raise ValueError(
                    "Value at %r does not match typespec (rank mismatch): "
    ...
    def _from_pylist_of_value(cls, pyval, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if empty_rank is None:
                ...
                return cls._from_pylist_of_dict(
                    pyval, set(), empty_rank, typespec, path_so_far
                )
        else:
            raise ValueError(
                "Value at %r does not match typespec: %r vs %r"
    ...
    def _from_pyscalar(cls, pyval, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if not (
                isinstance(typespec, tensor_spec.TensorSpec)
                and typespec.shape.rank == 0
            ):
                raise ValueError(
                    "Value at %r does not match typespec: %r vs %r"
                    % (path_so_far, typespec, pyval)
    ...
    """
        ...
        if not outer_axis <= inner_axis:
            raise ValueError(
                "Expected outer_axis (%d) to be less than or equal to "
...
class StructuredTensorSpec(type_spec.BatchableTypeSpec):
    ...
    """
        ...
        for key, value in field_specs.items():
            ...
            if not isinstance(
                ...
                raise TypeError(
                    "field_specs must be a dictionary with " "TypeSpec values."
                )
    ...
    def from_value(cls, value):
        field_specs = dict(
            (k, type_spec.type_spec_from_value(v)) for (k, v) in value._fields.items()
        )
    ...
    def _from_compatible_tensor_list(self, tensor_list):
        ...
        for field_name, field_spec in sorted(
            ...
            fields[field_name] = field_spec._from_compatible_tensor_list(field_tensors)
...
def _convert_to_structured_field_value(value):
    ...
    if isinstance(value, (ops.Tensor, ragged_tensor.RaggedTensor, StructuredTensor)):
        return value
...
def _row_partitions_for_ragged_tensor(value, rank, dtype):
    ...
    assert rank > 1
    value_row_partitions = value._nested_row_partitions[
        : rank - 1
    ]  # pylint: disable=protected-access
...
def _replace_row_partitions(value, new_partitions):
  ...
  """
    if isinstance(value, ops.Tensor) or not new_partitions:
        ...
        new_fields = dict(
            (k, _replace_row_partitions(v, new_partitions))
        ...
        return StructuredTensor(
            ...
            shape=value.shape,
            nrows=value.nrows(),
            row_partitions=new_partitions + value.row_partitions[len(new_partitions) :],
            internal=_structured_tensor_factory_key,
        )
  ...
  """
    ...
    if isinstance(value, ops.Tensor) and not is_ragged:
        new_shape = array_ops.concat(
            [
                [row_partition.nrows(), row_partition.uniform_row_length()],
        ...
        fields = dict(
            (k, _partition_outer_dimension(v, row_partition))
            for (k, v) in value._fields.items()
        )
        return StructuredTensor(
            fields,
...
def _merge_dims(value, outer_axis, inner_axis):
    ...
    if isinstance(value, (ops.Tensor, ragged_tensor.RaggedTensor)):
        ...
        fields = dict(
            (k, _merge_dims(v, outer_axis, inner_axis))
            for (k, v) in value._fields.items()
        )
        ...
        if outer_axis == 0:
            ...
            for dim in range(outer_axis, inner_axis):
                merged_splits = array_ops.gather(
                    value.row_partitions[dim].row_splits(), merged_splits
                )
        ...
        return StructuredTensor(
            fields, shape, nrows, partitions, internal=_structured_tensor_factory_key
        )
