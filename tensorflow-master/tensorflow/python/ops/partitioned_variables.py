...
def variable_axis_size_partitioner(
  ...
  """
    ...
    """
        ...
        bytes_per_slice = (
            1.0 * (shape.num_elements() / shape.dims[axis].value) * element_size
        )
...
def min_max_variable_partitioner(
    max_partitions=1, axis=0, min_slice_size=256 << 10, bytes_per_string_element=16
):
  ...
  """
    ...
    """
        ...
        partitions_list[axis] = max(
            1, min(shape.dims[axis].value, max_partitions, int(math.ceil(partitions)))
        )
...
@deprecation.deprecated(
    date=None, instructions="Use `tf.get_variable` with a partitioner set."
)
def create_partitioned_variables(
  ...
  """
    ...
    if len(shape) < 1:
        raise ValueError(
            "A partitioned Variable must have rank at least 1: " f"shape: {shape}"
        )
