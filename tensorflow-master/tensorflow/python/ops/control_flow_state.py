    ...
    def __init__(self, forward_ctxt, outer_grad_state):
        ...
        if outer_grad_state:
            ...
            if not hasattr(forward_ctxt, "outer_context"):
                raise ValueError(
                    "Failed to call gradients on a while loop without"
    ...
    """
        ...
        if shape.is_fully_defined():
            ...
            if dead_branch:
                ...
                val = control_flow_ops._SwitchRefOrTensor(op.inputs[0], pred)[
                    1 - branch
                ]
            ...
            for _, b_merge in grad_state.switch_map.items():
                if b_merge.op.inputs[0] == b_merge.op.inputs[1]:
                    ...
                    if shape.is_fully_defined():
                        ...
                        grad_shape = array_ops.shape_internal(
                            enter_grad, optimize=False
                        )
...
def MaybeCreateControlFlowState(
...
def _ZerosLikeV2(op, index):
    ...
    if (
        isinstance(val.op.graph, control_flow_v2_func_graphs.WhileBodyFuncGraph)
