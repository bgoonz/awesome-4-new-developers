...
def _bin_op(tf_fun, a, b, promote=True):
    if promote:
        a, b = np_array_ops._promote_dtype_binary(
            a, b
...
def clip(a, a_min, a_max):  # pylint: disable=missing-docstring
    ...
    if a_min is None:
        ...
        a, a_min, a_max = np_array_ops._promote_dtype(
            a, a_min, a_max
        )  # pylint: disable=protected-access
    ...
    def f(a, b):  # pylint: disable=missing-docstring
        ...
        def move_last_to_axis(a, axis):
            ...
            return array_ops.transpose(
                a,
                array_ops.concat(
                    [math_ops.range(axis), [r - 1], math_ops.range(axis, r - 1)], axis=0
                ),
            )
...
def _tf_gcd(x1, x2):  # pylint: disable=missing-function-docstring
    ...
    def _gcd_body_fn(x1, x2):
        ...
        x1, x2 = (
            ...
            array_ops.where_v2(
                x2 != 0, math_ops.mod(x1, x2_safe), constant_op.constant(0, x2.dtype)
            ),
        )
        return (
    ...
    if not np.issubdtype(x1.dtype.as_numpy_dtype, np.integer) or not np.issubdtype(
        x2.dtype.as_numpy_dtype, np.integer
    ):
...
def diff(a, n=1, axis=-1):  # pylint: disable=missing-function-docstring
    def f(a):
        ...
        if (axis + nd if axis < 0 else axis) >= nd:
            raise ValueError(
                "axis %s is out of bounds for array of dimension %s" % (axis, nd)
            )
        if n < 0:
...
def average(
    ...
    if axis is not None and not isinstance(axis, six.integer_types):
        ...
        raise ValueError(
            "`axis` must be an integer. Tuple of ints is not "
            "supported yet. Got type: %s" % type(axis)
        )
    a = np_array_ops.array(a)
    if weights is None:  # Treat all weights as 1
        if not np.issubdtype(a.dtype.as_numpy_dtype, np.inexact):
            a = a.astype(np_utils.result_type(a.dtype, np_dtypes.default_float_type()))
        ...
        if np.issubdtype(a.dtype.as_numpy_dtype, np.inexact):
            ...
            out_dtype = np_utils.result_type(
                a.dtype, weights, np_dtypes.default_float_type()
            )
...
def einsum(subscripts, *operands, **kwargs):  # pylint: disable=missing-docstring
    ...
    if casting == "safe":
        operands = np_array_ops._promote_dtype(
            *operands
        )  # pylint: disable=protected-access
    elif casting == "no":
...
def enable_numpy_methods_on_tensor():
    ...
    setattr(
        ops.Tensor, "reshape", np_array_ops._reshape_method_wrapper
    )  # pylint: disable=protected-access
