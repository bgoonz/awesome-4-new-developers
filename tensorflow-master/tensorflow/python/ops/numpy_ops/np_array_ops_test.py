...
class ArrayCreationTest(test.TestCase):
    def setUp(self):
        ...
        self.shape_transforms = [
            ...
            lambda x: np_array_ops.array(x, dtype=int),
    ...
    def testArray(self):
        ...
        def test_copy_equal_false():
            ...
            self.assertIsNot(
                np_array_ops.array(zeros_list, copy=False, ndmin=2), zeros_list
            )
            self.assertIsNot(
                np_array_ops.array(zeros_list, copy=False, ndmin=2), zeros_list
            )
            self.assertIs(
                np_array_ops.array(zeros_list, copy=False, ndmin=1), zeros_list
            )
            self.assertIs(
                np_array_ops.array(zeros_list, copy=False, ndmin=1), zeros_list
            )
            ...
            self.assertIsNot(
                np_array_ops.array(zeros_list, copy=False, dtype=int), zeros_list
            )
            self.assertIsNot(
                np_array_ops.array(zeros_list, copy=False, dtype=int), zeros_list
            )
            self.assertIs(
                np_array_ops.array(zeros_list, copy=False, dtype=float), zeros_list
            )
            self.assertIs(
                np_array_ops.array(zeros_list, copy=False, dtype=float), zeros_list
            )
    ...
    def testAsAnyArray(self):
        for a, dtype in itertools.product(self.all_arrays, self.all_types):
            self.match(
                np_array_ops.asanyarray(a, dtype=dtype), np.asanyarray(a, dtype=dtype)
            )
        zeros_list = np_array_ops.zeros(5)
        # Same instance is returned if no dtype is specified and input is ndarray.
        ...
        for dtype in self.all_types:
            for start in all_values:
                ...
                for stop in all_values:
                    ...
                    self.match(
                        np_array_ops.arange(start, stop),
    ...
    def testIndexedSlices(self):
        ...
        expected = array_ops.scatter_nd(
            [[1], [9]], array_ops.ones([2, 3], dtype=dtype), [10, 3]
        )
...
class ArrayMethodsTest(test.TestCase):
    ...
    def testAllAny(self):
        def run_test(arr, *args, **kwargs):
            for fn in self.array_transforms:
                arr = fn(arr)
                self.match(
                    np_array_ops.all(arr, *args, **kwargs), np.all(arr, *args, **kwargs)
                )
                self.match(
                    np_array_ops.any(arr, *args, **kwargs), np.any(arr, *args, **kwargs)
                )
    ...
    def testSize(self):
        ...
        @def_function.function(
            input_signature=[tensor_spec.TensorSpec(dtype=dtypes.float64, shape=None)]
        )
        def f(arr):
    ...
    def testTake(self):
        ...
        axis = 1
        self.assertAllEqual(
            np.take(a, indices, axis=axis), np_array_ops.take(a, indices, axis=axis)
        )

    def testWhere(self):
        self.assertAllEqual(
            [[1.0, 1.0], [1.0, 1.0]],
            np_array_ops.where([True], [1.0, 1.0], [[0, 0], [0, 0]]),
        )
    ...
    def testSwapaxes(self):
        ...
        x = [[[0, 1], [2, 3]], [[4, 5], [6, 7]]]
        self.assertAllEqual(
            [[[0, 4], [2, 6]], [[1, 5], [3, 7]]], np_array_ops.swapaxes(x, 0, 2)
        )
        self.assertAllEqual(
            [[[0, 4], [2, 6]], [[1, 5], [3, 7]]], np_array_ops.swapaxes(x, -3, -1)
        )
        ...
        for dtype in test_types:
            for shape in test_shapes:
                if np.issubdtype(dtype, np.complexfloating):
                    arr = np.asarray(
                        state.randn(*shape) * 100, dtype=dtype
...
class ArrayManipulationTest(test.TestCase):
    ...
    def testBroadcastTo(self):
        def run_test(arr, shape):
            for fn in self.array_transforms:
                arg1 = fn(arr)
                self.match(
                    np_array_ops.broadcast_to(arg1, shape), np.broadcast_to(arg1, shape)
                )
    ...
    def testIx_(self):
        possible_arys = [
            ...
            [False, False],
            list(range(5)),
            np_array_ops.empty(0, dtype=np.int64),
        ]
