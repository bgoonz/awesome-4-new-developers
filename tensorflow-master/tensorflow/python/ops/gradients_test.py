...
class GradientsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testStopGradients(self):
        def _MakeGraph(rng, stop_gradients=()):
            ...
            if "a" in stop_gradients:
                a = array_ops.stop_gradient(a)
            b = _FunctionOf([a])
            if "b" in stop_gradients:
                b = array_ops.stop_gradient(b)
            c = _FunctionOf([a, b])
            if "c" in stop_gradients:
                c = array_ops.stop_gradient(c)
            d = _FunctionOf([b, c])
            if "d" in stop_gradients:
                d = array_ops.stop_gradient(d)
    ...
    def testUnconnectedGradientsZeroConnectedGradients(self):
        with ops.Graph().as_default():
            x = constant(1.0)
            y = x * 3.0
            grad = gradients.gradients([y], [x], unconnected_gradients="zero")
    ...
    def testInt64Indices(self):
        with self.cached_session():
            ...
            c_sparse = ops.IndexedSlices(
                c_sparse.values,
                math_ops.cast(c_sparse.indices, dtypes.int64),
...
class ResourceCondTest(test_util.TensorFlowTestCase):
    ...
    def testBasic(self):
        ...
        output = control_flow_ops.cond(training, TestFn, lambda: inputs)

        loss = output

        grads = gradients.gradients(loss, [gamma])
...
class CustomGradientTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    def testCustomGradientTrivial(self):
        ...
        with ops.Graph().as_default():
            ...
            with session.Session():
                self.assertEqual(9.0, self.evaluate(dy))
    ...
    def testCustomGradient(self):
        ...
        with ops.Graph().as_default():
            ...
            self.assertAllEqual([3.0, 5.0], self.evaluate(dy))
    ...
    def testCustomGradientWithCapture(self):
        with ops.Graph().as_default():
            ...
            y = constant(5.0)
            dy = F(y)
            self.assertAllEqual(5.0, self.evaluate(dy))
    ...
    def testCustomGradientVariablesKwonlyArgs(self, anonymous_varargs):
        with context.eager_mode():
            x_captured = variables.Variable(3.0)  # Used by FuncMult
            ...
            def FuncMult(x):
                def ActualGrad(dy, variables):  # pylint: disable=redefined-outer-name
                    ...
                    x_captured_grad = 5.0 * x * dy
                    return (4.0 * x_captured * dy, [x_captured_grad])
            ...
            x = variables.Variable(6.0)
            with backprop.GradientTape(persistent=True) as g:
                y = FuncMult(x)
            self.assertAllEqual(g.gradient(y, x), 4.0 * 3.0)
    ...
    @parameterized.named_parameters(
        [
            (
                ("_%s_%s" % (x_struct, y_struct))
                .replace(" ", "")
                .replace("None", ""),  # pylint: disable=g-complex-comprehension
                x_struct,
                y_struct,
            )
    ...
    def testCustomGradientStructuralInputOutput(self, x_struct, y_struct):
        ...
        def F(*x):
            ...
            def Vjp(*dy):
                self.assertEqual(len(nest.flatten(y_struct)), len(nest.flatten(dy)))
...
class TensorListGradientsTest(test_util.TensorFlowTestCase):
    def testDefaultGradYs(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(self.evaluate(grad), 5.0)


class VariablesGradientTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testRecomputeGradDifferentDtypesInputs(self):
        ...
        x2 = array_ops.constant(1.0, dtype=dtypes.float32)
    ...
    def testCustomGradientRecomputeGradHigherOrder(self):
        ...
        self._test_gradients(F, [constant_op.constant([1.0])], order=3)
    ...
    def testFnRecomputeWithScopeGradient(self, use_tape):
        ...
        with variable_scope.variable_scope(
            ...
            with test_util.AbstractGradientTape(
                use_tape=use_tape, persistent=True
            ) as tape:
    ...
    def testFnRecomputeSameTensor(self):
        ...
        with variable_scope.variable_scope("test", use_resource=True):
            ...
            grads_re, grads = self._TestFnVariablesGradient(x, TestFnMul, x)
            grads_re = self.evaluate(grads_re)
            grads = self.evaluate(grads)
            for g, g_re in zip(grads, grads_re):
                self.assertAllClose(g, g_re)
