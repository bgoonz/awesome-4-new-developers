  ...
  """
    ...
    def stopping_criterion(i, state):
        return math_ops.logical_and(
            i < max_iter, math_ops.reduce_any(linalg.norm(state.r, axis=-1) > tol)
        )

    def dot(x, y):
        return array_ops.squeeze(
            math_ops.matvec(x[..., array_ops.newaxis], y, adjoint_a=True), axis=-1
        )
    ...
    with ops.name_scope(name):
        ...
        _, state = control_flow_ops.while_loop(stopping_criterion, cg_step, [i, state])
