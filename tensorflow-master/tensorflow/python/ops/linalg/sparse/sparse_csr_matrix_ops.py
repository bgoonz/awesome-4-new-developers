...
def _make_handle_data(tensor):
    ...
    return _create_handle_data_proto(
        tensor.shape.as_proto(), tensor.dtype.as_datatype_enum
    )
...
def dense_shape_and_type(matrix):
  ...
  """
    ...
    if len(handle_data.shape_and_type) != 1:
        raise ValueError(
            "len(matrix.handle_data.shape_and_type) != 1: '%s'"
...
def matmul_shape_inference(a, b, c, transpose_a, transpose_b, adjoint_a, adjoint_b):
    ...
    if (
        c_handle is None
        and a_shape_and_type is not None
        and b_shape_and_type is not None
    ):
  ...
  """
    ...
    with ops.name_scope(name, "SparseMatrixMatMul", [a_matrix, b_matrix]):
        if isinstance(a, SparseMatrix) and isinstance(b, SparseMatrix):
            ...
            c_handle = matmul_shape_inference(
                a_matrix, b_matrix, c, transpose_a, transpose_b, adjoint_a, adjoint_b
            )
