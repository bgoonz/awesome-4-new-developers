    ...
    **Subclasses should copy-paste this `__init__` documentation.**
    ...
    def _assert_non_singular(self):
        ...
        if self._can_use_cholesky():
            ...
            cond = math_ops.reduce_max(singular_values, axis=-1) / math_ops.reduce_min(
                singular_values, axis=-1
            )
    ...
    def _assert_self_adjoint(self):
        ...
        return check_ops.assert_equal(
            dense, linalg.adjoint(dense), message="Matrix was not equal to its adjoint."
        )
    ...
    """
        if isinstance(x, LinearOperator):
            ...
            if (
                right_operator.range_dimension is not None
                and left_operator.domain_dimension is not None
                and right_operator.range_dimension != left_operator.domain_dimension
            ):
                raise ValueError(
    ...
    """
        with self._name_scope(name):  # pylint: disable=not-callable
            ...
            tensor_shape.dimension_at_index(
                self.shape, self_dim
            ).assert_is_compatible_with(x.shape[-1])
    ...
    """
        with self._name_scope(name):  # pylint: disable=not-callable
            ...
            tensor_shape.dimension_at_index(
                self.shape, self_dim
            ).assert_is_compatible_with(rhs.shape[-1])
    ...
    """
        ...
        if not self._can_use_cholesky():
            raise ValueError(
                "Cannot take the Cholesky decomposition: "
    ...
    """
        ...
        for i, t in enumerate(graph_parents):
            if t is None or not (
                linear_operator_util.is_ref(t) or tensor_util.is_tf_type(t)
            ):
...
class _LinearOperatorSpec(type_spec.TypeSpec):
    ...
    """
        ...
        kwargs = _extract_attrs(
            operator, keys=set(operator._composite_tensor_fields + validation_fields)
        ...
        for k, v in list(kwargs.items()):
            ...
            is_tensor = [
                isinstance(x, type_spec.TypeSpec) for x in nest.flatten(type_spec_or_v)
            ]
            if all(is_tensor):
                ...
                raise NotImplementedError(
                    f"Field {k} contains a mix of `Tensor` and "
                    f" non-`Tensor` values."
                )
        ...
        return cls(
            ...
            prefer_static_fields=operator._composite_tensor_prefer_static_fields,
        )  # pylint: disable=protected-access
...
def make_composite_tensor(cls, module_name="tf.linalg"):
    ...
    cls._type_spec = property(
        spec_type.from_operator
    )  # pylint: disable=protected-access
    return cls
...
def _extract_attrs(op, keys):
  ...
  """
    ...
    for k in keys:
        srcs = [
            getattr(op, k, not_found),
            getattr(op, "_" + k, not_found),
        ...
        if (
            k in op._composite_tensor_prefer_static_fields and kwargs[k] is not None
        ):  # pylint: disable=protected-access
...
def _extract_type_spec_recursively(value):
  ...
  """
    ...
    if isinstance(value, dict):
        return type(value)(
            (k, _extract_type_spec_recursively(v)) for k, v in value.items()
        )
    return value
...
def _matmul(  # pylint:disable=missing-docstring
    ...
    if not isinstance(a, LinearOperator):
        ...
        adjoint_matmul = b.matmul(
            a, adjoint=(not adjoint_b), adjoint_arg=(not adjoint_a), name=name
        )
...
def _solve(matrix, rhs, adjoint=False, name=None):
    if not isinstance(matrix, LinearOperator):
        raise ValueError(
            "Passing in `matrix` as a Tensor and `rhs` as a "
