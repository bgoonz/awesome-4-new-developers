...
def is_ref(x):
  ...
  """
    return (
        ...
        isinstance(x, variables_module.Variable)
        or (
            isinstance(x, module.Module) and hasattr(x, "dtype") and hasattr(x, "shape")
        )
    )
...
def _reshape_for_efficiency(
    ...
    b_extra_sz_ = (
        ...
        if b.shape[:b_extra_ndims].is_fully_defined()
        else None
    )
    ...
    perm = np.concatenate(
        (np.arange(b_extra_ndims, b.shape.ndims), np.arange(0, b_extra_ndims)), 0
    )
...
def arg_is_blockwise(block_dimensions, arg, arg_split_dim):
    ...
    if isinstance(arg, (tuple, list)) and len(arg) == len(block_dimensions):
        ...
        if not any(nest.is_nested(x) for x in arg):
            ...
            if all(
                self_d == arg_d or self_d is None
