...
class LinearOperatorDerivedClassTest(test.TestCase):
  ...
  """
    ...
    def operator_and_matrix(
        self, shapes_info, dtype, use_placeholder, ensure_self_adjoint_and_pd=False
    ):
    ...
    with self.session(graph=ops.Graph()) as sess:
        ...
        if adjoint_arg:
            op_matmul = operator.matmul(
                linalg.adjoint(x), adjoint=adjoint, adjoint_arg=adjoint_arg
            )
        else:
        ...
        if blockwise_arg and len(operator.operators) > 1:
            ...
            block_dimensions = (
                operator._block_range_dimensions()
                if adjoint
                else operator._block_domain_dimensions()
            )
            block_dimensions_fn = (
                operator._block_range_dimension_tensors
                if adjoint
                else operator._block_domain_dimension_tensors
            )
    ...
    with self.session(graph=ops.Graph()) as sess:
        ...
        if adjoint_arg:
            op_solve = operator.solve(
                linalg.adjoint(rhs), adjoint=adjoint, adjoint_arg=adjoint_arg
            )
        else:
        ...
        if blockwise_arg and len(operator.operators) > 1:
            ...
            split_rhs = linear_operator_util.split_arg_into_blocks(
                block_dimensions, block_dimensions_fn, rhs, axis=-2
            )
            if adjoint_arg:
            ...
            class Model(module.Module):
                def __init__(self, init_x):
                    self.x = nest.map_structure(
                        lambda x_: variables.Variable(x_, shape=None), init_x
                    )

                @def_function.function(
                    input_signature=(operator._type_spec,)
                )  # pylint: disable=protected-access
        ...
        for dtype, use_placeholder, shape_info in itertools.product(
            ...
            base_test_name = "_".join(
                [
                    "test",
                    name,
                    "_shape={},dtype={},use_placeholder={}".format(
            ...
            if name in tests_with_adjoint_args:
                for adjoint in test_cls.adjoint_options():
                    for adjoint_arg in test_cls.adjoint_arg_options():
                        test_name = (
                            base_test_name
                            + ",adjoint={},adjoint_arg={}".format(adjoint, adjoint_arg)
                        )
                        if hasattr(test_cls, test_name):
                            raise RuntimeError(
                                "Test %s defined more than once" % test_name
                            )
                        setattr(
                            test_cls,
                            test_name,
                ...
                if hasattr(test_cls, base_test_name):
                    raise RuntimeError(
                        "Test %s defined more than once" % base_test_name
                    )
                setattr(
                    test_cls,
                    base_test_name,
                    test_util.run_deprecated_v1(
                        test_template_fn(use_placeholder, shape_info, dtype)
                    ),
                )
...
def random_positive_definite_matrix(
    shape, dtype, oversampling_ratio=4, force_well_conditioned=False
):
    ...
    with ops.name_scope("random_tril_matrix"):
        ...
        if force_well_conditioned:
            ...
            diag = random_sign_uniform(
                shape[:-1], dtype=dtype, minval=1.0, maxval=maxval
            )
...
def random_sign_uniform(
    shape, minval=None, maxval=None, dtype=dtypes.float32, seed=None
):
  ...
  """
    ...
    with ops.name_scope("random_sign_uniform"):
        ...
        if seed is not None:
            seed += 12
        signs = math_ops.sign(
            random_ops.random_uniform(shape, minval=-1.0, maxval=1.0, seed=seed)
        )
