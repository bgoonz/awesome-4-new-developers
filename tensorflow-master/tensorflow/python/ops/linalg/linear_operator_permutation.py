  ...
  """
    ...
    def _check_perm(self, perm):
        ...
        if perm.shape.ndims is not None and perm.shape.ndims < 1:
            raise ValueError(
                "Argument perm must have at least 1 dimension.  " "Found: %s" % perm
            )
        if not perm.dtype.is_integer:
            raise TypeError("Argument perm must be integer dtype. Found:" " %s" % perm)
    ...
    def _diag_part(self):
        ...
        return math_ops.cast(
            math_ops.equal(
                math_ops.range(0, self._domain_dimension_tensor(perm)), perm
            ),
            self.dtype,
        )
