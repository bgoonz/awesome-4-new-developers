...
def _unvec_by(y, num_col):
    ...
    return array_ops.matrix_transpose(
        array_ops.reshape(
            y, array_ops.concat([array_ops.shape(y)[:-1], [num_col, -1]], axis=0)
        )
    )
    ...
    def _determinant(self):
        ...
        for operator in self.operators:
            determinant = determinant * operator.determinant() ** math_ops.cast(
                total / operator.domain_dimension_tensor(), dtype=operator.dtype
            )
        return determinant
    ...
    def _diag_part(self):
        ...
        for operator in self.operators[1:]:
            ...
            diag_part = array_ops.reshape(
                diag_part,
                shape=array_ops.concat([array_ops.shape(diag_part)[:-2], [-1]], axis=0),
            )
    ...
    def _assert_non_singular(self):
        if all(operator.is_square for operator in self.operators):
            ...
            raise errors.InvalidArgumentError(
                node_def=None,
                op=None,
                message="All Kronecker factors must be "
