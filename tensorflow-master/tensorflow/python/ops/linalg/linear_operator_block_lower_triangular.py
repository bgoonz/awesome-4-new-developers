  ...
  """
    ...
    def _validate_operator_dimensions(self):
        ...
        for i in range(1, len(self.operators)):
            for j in range(i):
                ...
                if (
                    op.domain_dimension is not None
                    and above_op.domain_dimension is not None
                ):
                ...
                if (
                    op.range_dimension is not None
                    and right_op.range_dimension is not None
                ):
                ...
                for index in reversed(range(len(self.operators))):
                    ...
                    for j in reversed(range(index + 1, len(self.operators))):
                        y = y - self.operators[j][index].matmul(
                            solution_list[len(self.operators) - 1 - j], adjoint=adjoint
                        )
    ...
    def _to_dense(self):
        ...
        flat_broadcast_operators = linear_operator_util.broadcast_matrix_batch_dims(
            [op.to_dense() for row in self.operators for op in row]
