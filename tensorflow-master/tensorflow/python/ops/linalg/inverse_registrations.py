...
def _inverse_scaled_identity(identity_operator):
    return linear_operator_identity.LinearOperatorScaledIdentity(
        ...
        multiplier=1.0 / identity_operator.multiplier,
...
def _inverse_block_diag(block_diag_operator):
    ...
    return linear_operator_block_diag.LinearOperatorBlockDiag(
        operators=[operator.inverse() for operator in block_diag_operator.operators],
...
def _inverse_block_lower_triangular(block_lower_triangular_operator):
  ...
  """
    if len(block_lower_triangular_operator.operators) == 1:
        return linear_operator_block_lower_triangular.LinearOperatorBlockLowerTriangular(
            ...
            is_positive_definite=(block_lower_triangular_operator.is_positive_definite),
            is_square=True,
        )
    ...
    upper_left_inverse = linear_operator_block_lower_triangular.LinearOperatorBlockLowerTriangular(
        block_lower_triangular_operator.operators[:-1]
    ).inverse()
    ...
    for i in range(blockwise_dim - 1):
        ...
        block = linear_operator_identity.LinearOperatorScaledIdentity(
            ...
            multiplier=math_ops.cast(-1, dtype=block.dtype),
        ).matmul(block)
...
def _inverse_kronecker(kronecker_operator):
    ...
    return linear_operator_kronecker.LinearOperatorKronecker(
        operators=[operator.inverse() for operator in kronecker_operator.operators],
...
def _inverse_circulant(circulant_operator):
    ...
    return linear_operator_circulant.LinearOperatorCirculant(
        spectrum=1.0 / circulant_operator.spectrum,
