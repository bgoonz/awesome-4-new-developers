...
def _EinsumGrad(op, grad):
    ...
    """
        ...
        grad_reduced = gen_linalg_ops.einsum(
            [output_grad, other_operand],
            "{},{}->{}".format(output_subs, other_subs, left_subs),
        )
...
def _QrGrad(op, dq, dr):
    ...
    if (
        ...
        raise NotImplementedError(
            "QrGrad not implemented when nrows > ncols " "and full_matrices is true."
        )
    ...
    dx = _QrGradSquareAndDeepMatrices(
        q, u, dq + math_ops.matmul(y, dv, adjoint_b=True), du
    )
...
def _MatrixSolveGrad(op, grad):
    ...
    if adjoint_a:
        grad_a = -math_ops.matmul(
            c, grad_b, adjoint_b=True
        )  # pylint: disable=invalid-unary-operand-type
    else:
        grad_a = -math_ops.matmul(
            grad_b, c, adjoint_b=True
        )  # pylint: disable=invalid-unary-operand-type
...
def _TridiagonalSolveGrad(op, grad):
    ...
    grad_diags = -_MatmulExtractingThreeDiagonals(
        grad_rhs, x
    )  # pylint: disable=invalid-unary-operand-type
