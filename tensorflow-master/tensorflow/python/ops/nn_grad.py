...
def _AvgPool3DGradGrad(op, grad):
    return (
        array_ops.stop_gradient(op.inputs[0]),
...
def _BiasAddGrad(op, received_grad):
  ...
  """
    ...
    return (
        received_grad,
        gen_nn_ops.bias_add_grad(out_backprop=received_grad, data_format=data_format),
    )
...
def _EluGradGrad(op, grad):
    ...
    return (
        ...
        array_ops.where(elu_x < 0, grad * op.inputs[0], array_ops.zeros_like(elu_x)),
    )
...
def _SeluGradGrad(op, grad):
    selu_x = op.inputs[1]
    return (
        gen_nn_ops.selu_grad(grad, selu_x),
        array_ops.where(
            selu_x < 0.0, grad * op.inputs[0], array_ops.zeros_like(selu_x)
        ),
    )
...
def _Dilation2DGrad(op, grad):
    return [
        nn_ops.dilation2d_backprop_input(
            op.inputs[0],
            op.inputs[1],
            grad,
            op.get_attr("strides"),
        ...
        nn_ops.dilation2d_backprop_filter(
            op.inputs[0],
            op.inputs[1],
            grad,
            op.get_attr("strides"),
...
def _LRNGrad(op, grad):
    ...
    return [
        gen_nn_ops.lrn_grad(
            grad, op.inputs[0], op.outputs[0], depth_radius, bias, alpha, beta
        )
    ]
...
def _FractionalAvgPoolGrad(op, grad_0, unused_grad_1, unused_grad_2):
  ...
  """
    return gen_nn_ops.fractional_avg_pool_grad(
...
def _TopKGrad(op, grad, _):
  ...
  """
    ...
    ind_lastdim = array_ops.gather(
        math_ops.cast(ind_shape, dtypes.int64), array_ops.size(ind_shape) - 1
    )
    # Flatten indices to 2D.
    ind_2d = array_ops.reshape(op.outputs[1], array_ops.stack([-1, ind_lastdim]))

    in_lastdim = array_ops.gather(
        math_ops.cast(in_shape, dtypes.int64), array_ops.size(in_shape) - 1
    )
    ...
    ind = array_ops.reshape(
        ...
        + math_ops.cast(
            array_ops.expand_dims(
                math_ops.range(
                    0, math_ops.cast(outerdim, dtypes.int64) * in_lastdim, in_lastdim
                ),
                -1,
            ),
            dtypes.int32,
