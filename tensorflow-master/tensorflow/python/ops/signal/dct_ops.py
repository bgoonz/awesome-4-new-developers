...
def dct(
  ...
  """
    ...
    with _ops.name_scope(name, "dct", [input]):
        ...
        seq_len = (
            tensor_shape.dimension_value(input.shape[-1]) or _array_ops.shape(input)[-1]
        )
        if n is not None:
            if n <= seq_len:
                input = input[..., 0:n]
        ...
        if type == 2:
            scale = 2.0 * _math_ops.exp(
                _math_ops.complex(
                    zero,
                    -_math_ops.range(axis_dim_float) * _math.pi * 0.5 / axis_dim_float,
                )
            )
            ...
            dct2 = _math_ops.real(
                fft_ops.rfft(input, fft_length=[2 * axis_dim])[..., :axis_dim] * scale
            )
            ...
            dct3 = _math_ops.real(
                fft_ops.irfft(
                    scale * _math_ops.complex(input, zero), fft_length=[2 * axis_dim]
                )
            )[..., :axis_dim]
