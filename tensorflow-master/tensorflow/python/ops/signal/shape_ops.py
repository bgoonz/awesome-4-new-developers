...
def frame(
  ...
  """
    with ops.name_scope(name, "frame", [signal, frame_length, frame_step, pad_value]):
        ...
        result_shape = _infer_frame_shape(
        ...
        slice_shape = array_ops.concat(
            [outer_dimensions, [num_subframes * subframe_length], inner_dimensions], 0
        )
        ...
        subframes = array_ops.reshape(
            array_ops.strided_slice(
                signal, array_ops.zeros_like(signal_shape), slice_shape
            ),
            subframe_shape,
        )
        ...
        frames = array_ops.reshape(
            ...
            array_ops.concat(
                [outer_dimensions, [num_frames, frame_length], inner_dimensions], 0
            ),
        )
