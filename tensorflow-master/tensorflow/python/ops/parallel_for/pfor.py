    ...
    if t.dtype == dtypes.variant:
        ...
        if shapes_and_types[0].type.type_id == full_type_pb2.TFT_ARRAY:
            if len(shapes_and_types) != 1:
                raise ValueError(
                    "Expected handle data of length 1, got {!r} of length {}".format(
                        shapes_and_types, len(shapes_and_types)
                    )
                )
            ...
            raise ValueError(
                (
                    ...
                    "type {!r} ({!r})"
                ).format(shapes_and_types[0].type, t)
            )
        ...
        with ops.control_dependencies(args):
            for output in concrete_function.outputs:
                if not isinstance(output, ops.Tensor):
                    raise ValueError(
                        "Got a non-Tensor output %s while running reduce" % output
                    )
                ...
                # None of the inputs and control inputs were converted.
                    ...
                    if converter is None:
                        ...
                        if (
                            ...
                            message = (
                                "No pfor vectorization defined for %s\n"
                                "%s\n"
                                "inputs: %s. " % (y_op.type, y_op, converted_inputs)
                            )
...
def _convert_parse_example_v2(pfor_input):
    ...
    if serialized.shape.ndims not in (None, 1):
        raise ValueError(
            "ParseExampleV2 can only be converted if `serialized` " "is scalar."
        )
...
class WhileV2(object):
    ...
    def __init__(self, pfor_input):
        ...
        if self._cond_func is None or self._body_func is None:
            raise ValueError(
                "Error extracting cond and body functions for op %s."
                % (self._pfor_input.op)
            )
