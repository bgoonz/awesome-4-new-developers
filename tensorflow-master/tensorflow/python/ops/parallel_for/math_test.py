...
class MathTest(PForTestCase, parameterized.TestCase):
                ...
                with g:
                    ...
                    for out in (y, y_i):
                        if out.dtype == dtypes.float32:
                            for output_gradients in (
                                None,
                                out * math_ops.cast(i, out.dtype),
                            ):
                                grad = g.gradient(
                    ...
                    for stack_b in (True, False):
                        ...
                        def loop_fn(i):
                            ...
                            return math_ops.matmul(
                                a, b, transpose_a=tr_a, transpose_b=tr_b
                            )
        ...
        for segment_ids_dtype in (dtypes.int32, dtypes.int64):
            for num_segments_dtype in (dtypes.int32, dtypes.int64):
                segment_ids = constant_op.constant(
                    [[0, 0, 2], [0, 1, 2], [2, 2, 2]], dtype=segment_ids_dtype
                )
                ...
                def loop_fn(i):
                    ...
                    return (
                        reduction_op(data, seg_ids, num_segments),
    ...
    @parameterized.parameters(
        (math_ops.sparse_segment_sum_v2, True),
    ...
    def test_cast(self):
        ...
        def loop_fn(i):
            return (
                math_ops.cast(array_ops.gather(x, i), dtypes.float32),
...
class LinalgTest(PForTestCase):
    def test_cholesky(self):
        ...
        x = math_ops.matmul(
            z, array_ops.matrix_transpose(z)
        ) + linalg_ops.eye(  # Ensure pos. def.
            3
    ...
    def test_matrix_inverse(self):
        x = random_ops.random_uniform([3, 4, 2, 2]) + 10 * linalg_ops.eye(
            2
        )  # Ensure well-conditioned.
        ...
        for adjoint in (True, False):
            for stack_a in (True, False):
                for stack_b in (True, False):
                    ...
                    x = random_ops.random_uniform(shape_a) + 10 * linalg_ops.eye(
                        3
                    )  # Ensure well-conditioned.
                    y = random_ops.random_uniform(shape_b)
    ...
    def test_self_adjoint_eig(self):
        ...
        def loop_fn(i):
            return (
                linalg_ops.self_adjoint_eig(array_ops.gather(x, i)),
