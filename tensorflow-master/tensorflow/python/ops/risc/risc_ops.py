...
def risc_add(input_lhs, input_rhs, name="RISC_ADD"):
...
def risc_condition(
    pred, input_true, input_false, func_true, func_false, name="RISC_CONDITION"
):
...
def risc_conv(x, kernel, strides, data_format="NHWC", dilations=None, name="RISC_CONV"):
    return gen_risc_ops.risc_conv(
        x, kernel, strides, data_format=data_format, dilations=dilations, name=name
    )
...
def risc_dot(
    input_lhs, input_rhs, transpose_a=False, transpose_b=False, name="RISC_DOT"
):
...
def risc_gather(
    params, indices, validate_indices=None, axis=None, batch_dims=0, name="RISC_GATHER"
):
...
def risc_triangular_solve(
    matrix, rhs, lower=True, adjoint=False, name="RISC_TRIANGULAR_SOLVE"
):
