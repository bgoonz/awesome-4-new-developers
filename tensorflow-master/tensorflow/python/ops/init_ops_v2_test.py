...
class InitializersTest(test.TestCase):
    def _identical_test(
        self, init1, init2, assertion, shape=None, dtype=dtypes.float32
    ):
        if shape is None:
            shape = [100]
...
class ConstantInitializersTest(InitializersTest):
    ...
    def testZeros(self):
        self._range_test(
            init_ops_v2.Zeros(), shape=(4, 5), target_mean=0.0, target_max=0.0
        )
    ...
    def testZerosInvalidKwargs(self):
        ...
        with self.assertRaisesWithLiteralMatch(
            TypeError, r"Unknown keyword arguments: dtpye"
        ):
    ...
    def testOnes(self):
        self._range_test(
            init_ops_v2.Ones(), shape=(4, 5), target_mean=1.0, target_max=1.0
        )
...
class OrthogonalInitializerTest(InitializersTest):
    ...
    def testRangeInitializer(self):
        self._range_test(
            init_ops_v2.Orthogonal(seed=123), shape=(20, 20), target_mean=0.0
        )
...
class IdentityInitializerTest(InitializersTest):
    ...
    def testGain(self):
        ...
        for dtype in [dtypes.float32, dtypes.float64]:
            ...
            with test_util.use_gpu():
                self.assertAllClose(
                    self.evaluate(init_custom(shape, dtype=dtype)), np.eye(*shape) * 0.9
                )
...
class GlorotInitializersTest(InitializersTest):
    ...
    def testGlorotUniform(self):
        ...
        self._range_test(
            init_ops_v2.GlorotUniform(seed=123), shape, target_mean=0.0, target_std=std
        )
    ...
    def test_GlorotNormal(self):
        ...
        std = np.sqrt(2.0 / (fan_in + fan_out))
        self._range_test(
            init_ops_v2.GlorotNormal(seed=123), shape, target_mean=0.0, target_std=std
        )
...
class MethodInitializers(InitializersTest):
    ...
    def testLecunUniform(self):
        ...
        self._range_test(
            init_ops_v2.lecun_uniform(seed=123), shape, target_mean=0.0, target_std=std
        )
    ...
    def testHeUniform(self):
        ...
        self._range_test(
            init_ops_v2.he_uniform(seed=123), shape, target_mean=0.0, target_std=std
        )
    ...
    def testLecunNormal(self):
        ...
        std = np.sqrt(1.0 / fan_in)
        self._range_test(
            init_ops_v2.lecun_normal(seed=123), shape, target_mean=0.0, target_std=std
        )
    ...
    def testHeNormal(self):
        ...
        std = np.sqrt(2.0 / fan_in)
        self._range_test(
            init_ops_v2.he_normal(seed=123), shape, target_mean=0.0, target_std=std
        )
