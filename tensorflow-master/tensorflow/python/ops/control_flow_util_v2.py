...
def in_defun():
    ...
    while (
        isinstance(graph, CondBranchFuncGraph)
        or isinstance(graph, WhileBodyFuncGraph)
        or isinstance(graph, WhileCondFuncGraph)
    ):
...
def in_while_loop_defun(graph):
    ...
    if context.executing_eagerly():
        return False
    return isinstance(graph, WhileCondFuncGraph) or isinstance(
...
def maybe_set_lowering_attr(op, lower_using_switch_merge=None):
  ...
  """
    if lower_using_switch_merge is not None:
        ...
        not _DISABLE_LOWER_USING_SWITCH_MERGE
        and not control_flow_util.GraphOrParentsInXlaContext(op.graph)
            ...
            if input_index != _extract_input_index("else_branch"):
                raise AssertionError(
                    (
                        ...
                        "input->output mapping of resources."
                    ).format(node_def.op)
                )
...
def run_as_function_for_tape_gradients(make_op, inputs):
  ...
  """
    ...
    if (
        gradients_util.PossibleTapeGradientTypes(inputs)
        ...
        results = function.defun_with_attributes(
            make_op,
            autograph=False,
            attributes=dict(func_name="cflow_gradient_wrapper"),
        )(inputs)
        return results
    else:
