...
class DequantizeOpTest(test.TestCase):
    ...
    def _testDequantizeOp(
        ...
        with self.cached_session():
            ...
            dequantized = array_ops.dequantize(
                input_op, min_range, max_range, mode=mode, narrow_range=narrow_range
            )
        ...
        if mode == "MIN_COMBINED":
            if v_min < 0:
                ...
                half_range = 0.0
            np_ans = (
                (inputs.astype(np.float32) + half_range)
                * (max_range - min_range)
                / type_range
            ) + min_range
        elif mode == "SCALED":
    ...
    def testScaledMode(self):
        self._testDequantizeOp(
            np.array([-128, 0, 127]), -1.0, 2.0, dtypes.qint8, mode="SCALED"
        )
        self._testDequantizeOp(
            np.array([-2, 4, -17]), -5.0, -3.0, dtypes.qint8, mode="SCALED"
        )
        self._testDequantizeOp(
            np.array([0, -4, 42, -108]), 5.0, 40.0, dtypes.qint8, mode="SCALED"
        )
    ...
    def testAxis(self):
        ...
        dequant_values = np.array(
            [-2, -1.0, 0, 0.59375, 1.59375, 1.109375, 1.0], dtype=np.float32
        )
