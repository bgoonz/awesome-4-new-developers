...
class RaggedTensorToSparseOpTest(test_util.TensorFlowTestCase):
    ...
    def testBadSparseTensorRank(self):
        ...
        self.assertRaisesRegex(
            ValueError, r"rank\(st_input\) must be 2", RaggedTensor.from_sparse, st1
        )
        ...
        self.assertRaisesRegex(
            ValueError, r"rank\(st_input\) must be 2", RaggedTensor.from_sparse, st2
        )
        ...
        if not context.executing_eagerly():
            ...
            self.assertRaisesRegex(
                ValueError, r"rank\(st_input\) must be 2", RaggedTensor.from_sparse, st3
            )
    ...
    def testNonRaggedSparseTensor(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, r".*SparseTensor is not right-ragged"
        ):
