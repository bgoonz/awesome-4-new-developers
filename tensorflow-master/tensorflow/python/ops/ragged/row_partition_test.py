...
class RowPartitionTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testRaggedTensorConstruction(self):
        ...
        rp = RowPartition(
            row_splits=row_splits, internal=row_partition._row_partition_factory_key
        )
    ...
    @parameterized.named_parameters(
        [
            (
                "FromRowSplits",
                lambda: RowPartition.from_row_splits([0, 2, 8]),
                ["row_splits"],
            ),
            (
                "FromRowLengths",
                lambda: RowPartition.from_row_lengths([3, 0, 8]),
            ...
            (
                "FromRowLimits",
                lambda: RowPartition.from_row_limits([3, 7, 10]),
                ["row_splits"],
            ),
        ]
    )
    def testPrecomputedSplits(self, rp_factory, expected_encodings):
        rp = rp_factory()
        self.assertEqual(
            rp.has_precomputed_row_splits(), "row_splits" in expected_encodings
        )
        self.assertEqual(
            rp.has_precomputed_row_lengths(), "row_lengths" in expected_encodings
        )
        self.assertEqual(
            rp.has_precomputed_value_rowids(), "value_rowids" in expected_encodings
        )
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                ...
                x=lambda: RowPartition.from_row_splits([0, 3, 8]),
                y=lambda: RowPartition.from_row_splits(
                    [0, 3, 8]
    ...
    def testMergePrecomputedEncodings(self, x, y, expected_encodings):
        ...
        for validate in (True, False):
            ...
            self.assertEqual(
                result.has_precomputed_row_splits(), "row_splits" in expected_encodings
            )
            self.assertEqual(
            ...
            self.assertEqual(
                result.has_precomputed_nrows(), "nrows" in expected_encodings
            )
            self.assertEqual(
            ...
            for r in (x, y):
                if (
                    r.has_precomputed_row_splits()
                    and result.has_precomputed_row_splits()
                ):
                    self.assertAllEqual(r.row_splits(), result.row_splits())
                if (
                    r.has_precomputed_row_lengths()
                    and result.has_precomputed_row_lengths()
                ):
                    self.assertAllEqual(r.row_lengths(), result.row_lengths())
                if (
                    r.has_precomputed_value_rowids()
                    and result.has_precomputed_value_rowids()
                ):
                ...
                if (
                    r.uniform_row_length() is not None
                    and result.uniform_row_length() is not None
                ):
                    self.assertAllEqual(
                        r.uniform_row_length(), result.uniform_row_length()
                    )
...
class RowPartitionSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        [
            ...
            dict(nrows=8, nvals=10, uniform_row_length=5, error=".* not compatible .*"),
        ]
    )
    def testConstructionError(
        self,
        nrows=None,
        nvals=None,
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                ...
                expected=tensor_spec.TensorSpec([6], dtypes.int64),
            ),  # inferred nrow
        ]
    )
    ...
    @parameterized.parameters(
        [
            ...
            (
                RowPartitionSpec(nrows=7, nvals=12),
    ...
    def testFromValue(self):
        ...
        self.assertEqual(
            RowPartitionSpec.from_value(RowPartition.from_row_lengths([5, 3, 0, 2])),
            RowPartitionSpec(nrows=4),
        )
        self.assertEqual(
            RowPartitionSpec.from_value(RowPartition.from_value_rowids([0, 2, 2, 8])),
            RowPartitionSpec(nrows=9, nvals=4),
        )
        self.assertEqual(
            RowPartitionSpec.from_value(
                RowPartition.from_uniform_row_length(nvals=12, uniform_row_length=3)
            ),
...
def _assert_row_partition_equal(test_class, actual, expected):
    ...
    test_class.assertEqual(
        actual.has_precomputed_row_splits(), expected.has_precomputed_row_splits()
    )
    test_class.assertEqual(
        actual.has_precomputed_row_lengths(), expected.has_precomputed_row_lengths()
    )
    test_class.assertEqual(
        actual.has_precomputed_value_rowids(), expected.has_precomputed_value_rowids()
    )
    test_class.assertEqual(
        actual.has_precomputed_nrows(), expected.has_precomputed_nrows()
    )
    test_class.assertEqual(
        actual.uniform_row_length() is None, expected.uniform_row_length() is None
    )
    ...
    if expected.uniform_row_length() is not None:
        test_class.assertAllEqual(
            actual.uniform_row_length(), expected.uniform_row_length()
        )
