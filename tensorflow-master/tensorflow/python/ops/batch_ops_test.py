...
class BatchOpsTest(test.TestCase):
    ...
    def testMultipleBatch(self):
        ...
        with self.cached_session() as sess:
            ...
            def worker():
                thread_results.extend(
                    sess.run([batched], feed_dict={inp0: [1], inp1: [2]})
                )
    ...
    def testIllegalBatchDifferentDim0Sizes(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertGreater(
                raised.exception.message.find("must have equal 0th-dimension size"), 0
            )
    ...
    def testBasicUnbatch(self):
        ...
        with self.cached_session() as sess:
            ...
            result = batch_ops.unbatch(
                computation, index, id_t, timeout_micros=1000000, shared_name="unbatch"
            )
            thread_results = []
    ...
    def testBasicUnbatchDecorated(self):
        ...
        with self.cached_session() as sess:
            ...
            # be investigated.
            default_inp = array_ops.placeholder_with_default(
                2, shape=[]
    ...
    def testParallelRunsWithCpuAndGpu(self):
        ...
        num_calls = 10
        placeholders = [
            array_ops.placeholder(dtypes.float32, shape=(1,)) for _ in range(num_calls)
        ]
        results = []
    ...
    def testBatchFunctionOpWithLargeBatchSplitted(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertTrue(
                np.all(
                    np.equal(
                        thread1_results[0], np.array([8, 9, 10, 11, 12], dtype=np.int32)
                    )
                )
            )
            self.assertTrue(
                np.all(np.equal(main_results[0], np.array([5, 6, 7], dtype=np.int32)))
            )
    ...
    def testUnbatchTimeout(self):
        ...
        with self.cached_session() as sess:
            ...
            timeout_micros = 10
            result = batch_ops.unbatch(
                computation, index, id_t, timeout_micros, shared_name="shared_unbatch"
            )
            ...
            computation_delayed = script_ops.py_func(
                delayed_plus1, [batched[0]], dtypes.int32
            )
            result_delayed = batch_ops.unbatch(
                computation_delayed,
                index,
                id_t,
                timeout_micros,
