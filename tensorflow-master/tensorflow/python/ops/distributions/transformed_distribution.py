...
class TransformedDistribution(distribution_lib.Distribution):
  ...
  """
    ...
    """
        ...
        name = name or (("" if bijector is None else bijector.name) + distribution.name)
        with ops.name_scope(name, values=[event_shape, batch_shape]) as name:
            ...
            self._is_maybe_batch_override = bool(
                ...
                or tensor_util.constant_value(self._override_batch_shape).size != 0
            )
            ...
            self._is_maybe_event_override = bool(
                ...
                or tensor_util.constant_value(self._override_event_shape).size != 0
            )
        ...
        super(TransformedDistribution, self).__init__(
            ...
            graph_parents=(
                distribution._graph_parents
                + bijector.graph_parents  # pylint: disable=protected-access
            ),
            name=name,
        )
    ...
    def _log_cdf(self, y):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "log_cdf is not implemented when overriding " "event_shape"
            )
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "log_cdf is not implemented when " "bijector is not injective."
            )
        x = self.bijector.inverse(y)
    ...
    def _cdf(self, y):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "cdf is not implemented when overriding " "event_shape"
            )
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "cdf is not implemented when " "bijector is not injective."
            )
        x = self.bijector.inverse(y)
    ...
    def _log_survival_function(self, y):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "log_survival_function is not implemented when "
    ...
    def _survival_function(self, y):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "survival_function is not implemented when " "overriding event_shape"
            )
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "survival_function is not implemented when "
                "bijector is not injective."
            )
        x = self.bijector.inverse(y)
    ...
    def _quantile(self, value):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "quantile is not implemented when overriding " "event_shape"
            )
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "quantile is not implemented when " "bijector is not injective."
            )
    ...
    def _entropy(self):
        ...
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "entropy is not implemented when " "bijector is not injective."
            )
        ...
        if self._is_maybe_event_override:
            ...
            entropy *= math_ops.cast(
                math_ops.reduce_prod(self._override_event_shape),
        ...
        dummy = array_ops.zeros(
            shape=array_ops.concat(
                [self.batch_shape_tensor(), self.event_shape_tensor()], 0
            ),
            dtype=self.dtype,
        )
        event_ndims = (
        ...
        ildj = self.bijector.inverse_log_det_jacobian(dummy, event_ndims=event_ndims)
    ...
    def _maybe_validate_shape_override(
        self, override_shape, base_is_scalar, validate_args, name
    ):
        """Helper to __init__ which ensures override batch/event_shape are valid."""
        if override_shape is None:
            override_shape = []

        override_shape = ops.convert_to_tensor(
            override_shape, dtype=dtypes.int32, name=name
        )
        ...
        if tensor_util.constant_value(override_shape) is not None:
            ...
            dynamic_assertions += [
                check_ops.assert_positive(
                    override_shape, message="shape override must have positive elements"
                )
            ]

        is_both_nonscalar = _logical_and(
            _logical_not(base_is_scalar), _logical_not(override_is_scalar)
        )
        ...
        return control_flow_ops.with_dependencies(dynamic_assertions, override_shape)
