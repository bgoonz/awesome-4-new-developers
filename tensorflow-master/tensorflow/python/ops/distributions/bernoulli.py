...
class Bernoulli(distribution.Distribution):
  ...
  """
    ...
    def _log_prob(self, event):
        ...
        if not (
            event.get_shape().is_fully_defined()
            and logits.get_shape().is_fully_defined()
            and event.get_shape() == logits.get_shape()
        ):
    ...
    def _entropy(self):
        return -self.logits * (
            math_ops.sigmoid(self.logits) - 1
        ) + nn.softplus(  # pylint: disable=invalid-unary-operand-type
            -self.logits
    ...
    def _variance(self):
        return self._mean() * (1.0 - self.probs)
...
def _kl_bernoulli_bernoulli(a, b, name=None):
  ...
  """
    with ops.name_scope(name, "kl_bernoulli_bernoulli", values=[a.logits, b.logits]):
        ...
        return (
            math_ops.sigmoid(a.logits) * delta_probs0
