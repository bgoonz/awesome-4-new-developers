...
def assert_scalar_congruency(
    bijector, lower_x, upper_x, n=int(10e3), rtol=0.01, sess=None
):
  ...
  """
    ...
    # Jacobian.
    np.testing.assert_allclose(dy_dx_v, np.divide(1.0, dx_dy_v), atol=1e-5, rtol=1e-3)
...
def assert_bijective_and_finite(
  ...
  """
    ...
    [x_from_x, y_from_y, ildj_f_x, fldj_x, ildj_y, fldj_g_y, f_x_v, g_y_v] = sess.run(
        [
