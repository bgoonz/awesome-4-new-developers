...
class CondWithManyIntermediatesBenchmark(test.Benchmark):
    ...
    def _create_cond(self, x):
        def branch_fn():
            ...
            return x + sum(
                random_ops.random_normal([]) for _ in range(self.NUM_INTERMEDIATES)
            )

        # Use a dynamic predicate to make sure the cond isn't constant folded.
        return control_flow_ops.cond(math_ops.not_equal(x, -1), branch_fn, lambda: 0.0)
            ...
            with ops.Graph().as_default():
                ...
                with session.Session() as sess:
                    ...
                    self.report_benchmark(
                        wall_time=time.time() - start_time, iters=self.NUM_ITERS
                    )
