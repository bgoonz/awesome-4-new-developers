  ...
  """
    ...
    def _ExtractInputShapes(inputs):
        ...
        for x in inputs:
            ...
            if (
                not isinstance(input_shape, ops.Tensor)
        ...
        if context.executing_eagerly() or isinstance(concat_dim, ops.EagerTensor):
            ...
            if constant_op.is_constant(concat_dim):
                ...
                if dim_context != grad_context:
                    ...
                    concat_dim = constant_op.constant(
                        value=value, dtype=concat_dim.dtype
                    )
            ...
            if len(sizes) > 16:
                ...
                sizes = array_ops.squeeze(
                    array_ops.slice(
                        array_ops.stack(sizes, axis=1), [non_neg_concat_dim, 0], [1, -1]
                    )
                )
                out_grads = array_ops.split(grad, sizes, non_neg_concat_dim)
            else:
...
def _TensorStridedSliceUpdateGrad(
    op, grad
...
def _MatrixDiagV3Grad(op, grad):
    return (
        array_ops.matrix_diag_part(grad, k=op.inputs[1], align=op.get_attr("align")),
        None,
        None,
        None,
        None,
    ...
    if matrix_shape.is_fully_defined():
        return (
            array_ops.matrix_diag(
                grad, k=op.inputs[1], num_rows=matrix_shape[0], num_cols=matrix_shape[1]
            ),
            None,
            None,
        )
...
def _MatrixDiagPartV3Grad(op, grad):
    ...
    if matrix_shape.is_fully_defined():
        ...
        return (
            array_ops.matrix_set_diag(
                array_ops.zeros_like(op.inputs[0]), grad, k=op.inputs[1], align=align
            ),
            None,
            None,
        )
...
def _MatrixSetDiagGradV2(op, grad):
    ...
    if not diag_shape.is_fully_defined():
        ...
        postfix = control_flow_ops.cond(
            ...
            lambda: ops.convert_to_tensor([d_upper - d_lower + 1, max_diag_len]),
        )
        # pyformat: enable
...
def _MatrixSetDiagGradV3(op, grad):
    ...
    grad_input = array_ops.matrix_set_diag(
        grad, array_ops.zeros(diag_shape, dtype=grad.dtype), k=op.inputs[2], align=align
    )
...
def _ConjugateTransposeGrad(op, grad):
    ...
    p = op.inputs[1]
    return [
        array_ops.transpose(grad, array_ops.invert_permutation(p), conjugate=True),
        None,
    ]
...
def _PadGrad(op, grad):
    ...
    pad_before = array_ops.slice(a, [0, 0], array_ops.stack([array_ops.rank(x), 1]))
...
def _ExtractVolumePatchesGrad(op, grad):
    ...
    input_idx_patched = gen_array_ops.extract_volume_patches(
        input_idx, op.get_attr("ksizes"), op.get_attr("strides"), op.get_attr("padding")
    )
...
def _TensorScatterMinOrMaxGrad(op, grad):
    ...
    ys_indicators = array_ops.scatter_nd(indices, y_indicators, array_ops.shape(x))
