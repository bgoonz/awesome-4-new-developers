...
from tensorflow.lite.toco.logging import (
    toco_conversion_log_pb2 as _toco_conversion_log_pb2,
)
...
class HTMLGenerator(object):
    ...
    """
        ...
        html_dict["<!--CONVERSION_STATUS-->"] = (
            ...
            if toco_err_log
            else r'<span class="label label-success">Success</span>'
        )
        ...
        for i in range(len(toco_conversion_log_before.op_list)):
            ...
            pre_op_profile += (
                "<tr><td>" + toco_conversion_log_before.op_list[i] + "</td>"
            )
            # Append input type column.
            if i < len(toco_conversion_log_before.op_signatures):
                pre_op_profile += (
                    "<td>"
                    + get_input_type_from_signature(
                        toco_conversion_log_before.op_signatures[i]
                    )
                    + "</td></tr>"
                )
            else:
        ...
        for op in toco_conversion_log_after.op_list:
            ...
            post_op_profile += (
                "<tr><td>" + op + "</td><td>" + supported_type + "</td></tr>"
            )
        ...
        if toco_err_log:
            ...
            success_info = (
                "TFLite graph conversion successful. You can preview the "
                "converted model at: "
            ) + tflite_graph_path
...
def gen_conversion_log_html(
  ...
  """
    ...
    if not os.path.exists(template_filename):
        raise IOError(
            "Failed to generate HTML: file '{0}' doesn't exist.".format(
                template_filename
            )
        )
    ...
    if not os.path.exists(dot_before_path):
        raise IOError(
            "Failed to generate HTML: file '{0}' doesn't exist.".format(dot_before_path)
        )
    if not os.path.exists(dot_after_path):
        raise IOError(
            "Failed to generate HTML: file '{0}' doesn't exist.".format(dot_after_path)
        )
