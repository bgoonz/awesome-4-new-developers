...
def make_zip_of_tests(
  ...
  """
    ...
    if options.make_edgetpu_tests:
        ...
        parameter_count = 0
        for parameters in test_parameters:
            if True in parameters.get("fully_quantize", []) and False in parameters.get(
                "quant_16x8", [False]
            ):
                parameter_count += functools.reduce(
                    operator.mul,
    ...
    for parameters in test_parameters:
        ...
        """
                ...
                if tflite_model_binary:
                    ...
                    archive.writestr(
                        zipinfo, example_fp.getvalue(), zipfile.ZIP_DEFLATED
                    )
                    ...
                    archive.writestr(
                        zipinfo, example_fp2.getvalue(), zipfile.ZIP_DEFLATED
                    )
