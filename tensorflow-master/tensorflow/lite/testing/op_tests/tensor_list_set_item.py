...
def make_tensor_list_set_item_tests(options):
    ...
    def build_graph(parameters):
        ...
        tensor_list = list_ops.tensor_list_set_item(
            tensor_list, parameters["index"], item
        )
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        ...
        item = create_tensor_data(
            parameters["element_dtype"], parameters["element_shape"]
        )
        return (
            [data, item],
            sess.run(outputs, feed_dict=dict(zip(inputs, [data, item]))),
        )
