...
parser = argparse.ArgumentParser(
    description="Script to move TFLite models from pre-release schema to " "new schema."
)
parser.add_argument(
    "input",
    type=str,
  ...
  """
    ...
    def __init__(self):
        ...
        self._upgrade_dispatch = {
            version: dispatch for version, unused1, unused2, dispatch in self._schemas
        }
        ...
        with TemporaryDirectoryResource() as tempdir:
            ...
            if extension in [".bin", ".tflite"]:
                ...
                returncode = subprocess.call(
                    [self._flatc_path, "-t", "--strict-json", "--defaults-json"]
                    + raw_binary
                    + ["-o", tempdir, schema, "--", input_file]
                )
                if returncode != 0:
    ...
    """
        ...
        with TemporaryDirectoryResource() as tempdir:
            if extension == ".json":
                ...
                if returncode != 0:
                    raise RuntimeError(
                        "flatc failed to convert upgraded json to binary."
                    )
    ...
    """
      ...
      """
            ...
            return (
                old_name_to_new_name[opcode_name]
                if opcode_name in old_name_to_new_name
                else opcode_name
            )
      ...
      """
            ...
            return (
                old_to_new[operator_type]
                if operator_type in old_to_new
                else operator_type
            )
        ...
        for operator_code in data["operator_codes"]:
            ...
            if not isinstance(operator_code["builtin_code"], type(u"")):
                raise ValueError(
                    "builtin_code %r is non-string. this usually means "
    ...
    """
        ...
        for version, schema, raw_binary, _ in self._schemas:
            ...
            if data_candidate["version"] == version:
                ...
                return
        raise RuntimeError(
            "No schema that the converter understands worked with "
