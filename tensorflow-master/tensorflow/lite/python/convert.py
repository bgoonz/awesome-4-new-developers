...
def _requires_input_stats(toco_flags: _toco_flags_pb2.TocoFlags()) -> bool:
  ...
  """
    ...
    return (
        toco_flags.inference_type in quantized_inference_types
...
def toco_convert_protos(
  ...
  """
    ...
    return _run_toco_binary(
        model_flags_str, toco_flags_str, input_data_str, debug_info_str
    )
...
def _run_toco_binary(
    model_flags_str, toco_flags_str, input_data_str, debug_info_str=None
):
...
Alternative, use virtualenv."""
    ...
    try:
        ...
        with _tempfile.NamedTemporaryFile(
            delete=False
        ) as fp_toco, _tempfile.NamedTemporaryFile(
            delete=False
...
def build_toco_flags(
    ...
    **_
    ...
    toco.default_to_single_batch_in_tensor_list_ops = (
...
def deduplicate_readonly_buffers(tflite_model):
  ...
  """
    ...
    sorted(
        read_only_buffer_indices, key=lambda idx: model.buffers[idx].data.data.tobytes()
    )
