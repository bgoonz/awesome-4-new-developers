...
class FromConcreteFunctionTest(lite_v2_test_util.ModelTest):
    ...
    def testInvalidFloat(self, inference_input_output_type):
        ...
        with self.assertRaises(ValueError) as error:
            ...
            converter.convert()
        self.assertEqual(
            "The inference_input_type and inference_output_type " "must be tf.float32.",
            str(error.exception),
        )
    ...
    def testInvalidIntegerQuantization(
        self, is_int16_quantize, inference_input_output_type
    ):
        root, func, calibration_gen = self._getIntegerQuantizeModel()
    ...
    def testNewQuantizerBlocklistingArgs(
        ...
        quantized_tflite_model = mlir_quantize(
            calibrated, denylisted_ops=denylisted_ops, denylisted_nodes=denylisted_nodes
        )
        interpreter = Interpreter(model_content=quantized_tflite_model)
...
class FromSavedModelTest(lite_v2_test_util.ModelTest):
    ...
    def testKerasSequentialModel(self):
        ...
        model = tf.keras.models.Sequential(
            [tf.keras.layers.Dropout(0.2), tf.keras.layers.Dense(1)]
        )
    ...
    def testKerasConvLSTM2DWithMoreThanOneDilationRate(self):
        input_tensor = tf.keras.layers.Input(
            batch_size=8, shape=[9, 10, 11, 12], name="input_tensor", dtype=tf.float32
        )
...
class ControlFlowTest(lite_v2_test_util.ModelTest):
    ...
    def testKerasRNN(self, rnn_layer, default_to_single_batch):
        ...
        model = tf.keras.models.Sequential(
            [tf.keras.layers.Input(shape=(10, 10), name="input"), rnn_obj]
        )
...
class UnknownShapes(lite_v2_test_util.ModelTest):
    ...
    def _getIntegerQuantizeModelWithUnknownShapes(self):
        ...
        def calibration_gen():
            for batch in range(5, 20, 5):
                for _ in range(5):
                    yield [
                        np.random.uniform(-1, 1, size=(batch, 33)).astype(np.float32)
                    ]
    ...
    def testSizeInvalid(self):
        @tf.function(
            input_signature=[tf.TensorSpec(shape=[1, None, 16, 3], dtype=tf.float32)]
        )
        def model(in_tensor):
...
class ResourceAndVariantTypes(lite_v2_test_util.ModelTest):
    ...
    @parameterized.named_parameters(
        ("EnableLoweringTensorListOps", True), ("DisableLoweringTensorListOps", False)
    )
    @test_util.run_v2_only
