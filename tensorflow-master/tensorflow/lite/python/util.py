...
def _convert_tflite_enum_type_to_tf_type(tflite_enum_type):
  ...
  """
    ...
    if tf_type is None:
        raise ValueError(
            "Unsupported enum {}. The valid map of enum to tf types is : {}".format(
  ...
  """
    ...
    for name in tensor_names:
        if not isinstance(name, six.string_types):
            raise ValueError(
                "Invalid type for a tensor name in the provided graph. "
                "Expected type for a tensor name is 'str', instead got "
                "type '{}' for tensor name '{}'".format(type(name), name)
            )
            ...
            if quant_type not in _MAP_QUANT_TO_IO_TYPES:
                raise ValueError(
                    ...
                    "tensor with name '{}' should be in {}, instead type is {}".format(
                        quant_tensor.name,
                        tuple(
                            get_tf_type_name(t) for t in _MAP_QUANT_TO_IO_TYPES.keys()
                        ),
...
def modify_model_io_type(
    model, inference_input_type=dtypes.float32, inference_output_type=dtypes.float32
):
  ...
  """
    if (
        inference_input_type == dtypes.float32
        and inference_output_type == dtypes.float32
    ):
        return model
