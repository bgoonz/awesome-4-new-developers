...
class TestModels(test_util.TensorFlowTestCase):
    ...
    def _getKerasModelFile(self):
        ...
        model = keras.models.Sequential(
            [keras.layers.Dropout(0.2, input_shape=(1,)), keras.layers.Dense(1)]
        )
...
class TfLiteConvertV1Test(TestModels):
    def _run(self, flags_str, should_succeed, expected_ops_in_converted_model=None):
        ...
        super(TfLiteConvertV1Test, self)._run(
            flags_str, should_succeed, expected_ops_in_converted_model
        )
    ...
    def testFrozenGraphDef(self):
        ...
        flags_str = (
            ...
            "--output_arrays={2}".format(graph_def_file, "Placeholder", "add")
        )
    ...
    def testQATFrozenGraphDefInt8(self):
        with ops.Graph().as_default():
            ...
            _ = array_ops.fake_quant_with_min_max_args(
                in_tensor_1 + in_tensor_2, min=0.0, max=1.0, name="output", num_bits=16
        ...
        flags_str = (
            "--inference_type=INT8 --std_dev_values=128,128 "
            "--mean_values=128,128 "
            "--graph_def_file={0} --input_arrays={1},{2} "
            "--output_arrays={3}".format(graph_def_file, "inputA", "inputB", "output")
        )
        self._run(flags_str, should_succeed=True)
        os.remove(graph_def_file)
    ...
    def testQATFrozenGraphDefUInt8(self):
        ...
        flags_str = (
            "--std_dev_values=128,128 --mean_values=128,128 "
            "--graph_def_file={0} --input_arrays={1} "
            "--output_arrays={2}".format(graph_def_file, "inputA,inputB", "output")
        )
    ...
    def testEnsureCustomOpdefsFlag(self):
        ...
        flags_str = (
            "--saved_model_dir={0} --allow_custom_ops "
    ...
    def testSavedModelWithCustomOpdefsFlag(self):
        ...
        flags_str = (
            ...
            "--experimental_new_converter".format(saved_model_dir, custom_opdefs_str)
        )
        self._run(
            flags_str,
            should_succeed=True,
    ...
    def testSavedModelWithInvalidCustomOpdefsFlag(self):
        ...
        flags_str = (
            '--saved_model_dir={0} --custom_opdefs="{1}" --allow_custom_ops '
            "--experimental_new_converter".format(
                saved_model_dir, invalid_custom_opdefs_str
            )
        )
        self._run(flags_str, should_succeed=False)
    ...
    def testKerasFileMLIR(self):
        ...
        flags_str = "--keras_model_file={} --experimental_new_converter".format(
            keras_file
        )
        self._run(flags_str, should_succeed=True)
        os.remove(keras_file)
    ...
    def testConversionSummary(self):
        ...
        flags_str = (
            "--keras_model_file={} --experimental_new_converter  "
    ...
    def testConversionSummaryWithOldConverter(self):
        ...
        flags_str = (
            "--keras_model_file={} --experimental_new_converter=false "
            "--conversion_summary_dir={}".format(keras_file, log_dir)
        )
        self._run(flags_str, should_succeed=True)
        os.remove(keras_file)
...
class TfLiteConvertV2Test(TestModels):
    ...
    def testFunctionalKerasModel(self):
        ...
        flags_str = "--keras_model_file={}".format(keras_file)
        self._run(
            flags_str, should_succeed=True, expected_output_shapes=[[1, 1], [1, 2]]
        )
        os.remove(keras_file)
    ...
    def testMutuallyExclusive(self):
        self._run(
            "--keras_model_file=model.h5 --saved_model_dir=/tmp/", should_succeed=False
        )
