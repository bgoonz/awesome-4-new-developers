  ...
  var c = 1,
    ...
    f = {
      ...
      set: function (b, c) {
        ...
        g !== -1
          ? (e = d.replace(new RegExp("(" + b + ":{[a-zA-Z0-9,-]*})"), f))
          : (e = d.length > 0 ? d + "," + f : f),
          (a.location.hash = e);
      },
      remove: function (b) {
        a.location.hash = a.location.hash.replace(
          new RegExp("([,]?" + b + ":{[a-zA-Z0-9,-]*}[,]?)"),
          ""
        );
      },
    },
    g = {
      numberRange: /^([0-9]+)-([0-9]+)$/,
      pageNumber: /-([0-9]+)$/,
      multilineBegin: /<span class="([\w-_][^"]+)">(?:.[^<]*(?!<\/span>)|)$/gi,
      multilineEnd: /(<span class="([\w-_][^"]+)">)?(?:.[^<]*)?(<\/span>)/gi,
    };
        ...
        j.filter("pre,code").each(function () {
          function r(b, c, e) {
            var h = !1,
              i = a.find(".vg-line");
            c &&
              (a.find(".vg-highlight").removeClass("vg-highlight"),
              f.remove(d),
              (k = [])),
              (b = $.type(b) === "array" ? b : [b]),
              $.each(b, function (b, c) {
                if (k.indexOf(c) <= -1) {
                  !isNaN(parseFloat(c, 10)) &&
                    isFinite(c) &&
                    (c = parseInt(c, 10));
                  if ($.type(c) === "string") {
                    ...
                    if (e) {
                      ...
                      a.find(".vg-line:contains(" + c + ")").each(function () {
                        ...
                        a.html(
                          a
                            ...
                            .replace(
                              c,
                              '<span class="vg-highlight">' + c + "</span>"
                            )
                        );
                      }),
                        k.push(c);
          ...
          if (!i) {
            ...
            $.each(o, function (a, b) {
              var c = a + h.firstLine,
                ...
                f = b;
              h.numbers &&
                (p += '<a class="vg-number" rel="#' + e + '">' + c + "</a>");
              if (s[t]) {
                var i = g.multilineEnd.exec(b);
                i && !i[1]
                  ? ((f = '<span class="' + s[t] + '">' + f), delete s[t], t--)
                  : (f = '<span class="' + s[t] + '">' + f + "</span>");
              }
              var j = g.multilineBegin.exec(b);
              j && (t++, (s[t] = j[1])),
                (q += '<div class="vg-line" id="' + e + '">' + f + "</div>");
            }),
              (q = '<code class="vg-code">' + q + "</code>"),
              h.numbers && (q = '<div class="vg-gutter">' + p + "</div>" + q),
              a.html(q),
              (e = a.find("code")),
              a.find(".vg-number").click(function (b) {
                var c = $(this),
                  e = c.attr("rel"),
                  h = a.find(e);
                if (h.hasClass("vg-highlight")) {
                  h.removeClass("vg-highlight"),
                    k.splice(k.indexOf(c.text()), 1),
                    f.set(d, k),
                    (j = !1);
                  return !1;
                }
                var i = j;
                (j = parseInt(g.pageNumber.exec(e)[1], 10)),
                  b.shiftKey && j
                    ? r(i < j ? i + "-" + j : j + "-" + i, !0)
                    : r(j, b.ctrlKey ? !1 : !0);
                return !1;
  ...
  $.fn.repo = function (options) {
    ...
    var _this = this,
      ...
      decode64 = function (a) {
        ...
        do {
          f = keyStr64.indexOf(a.charAt(j++));
          g = keyStr64.indexOf(a.charAt(j++));
          h = keyStr64.indexOf(a.charAt(j++));
          i = keyStr64.indexOf(a.charAt(j++));
          c = (f << 2) | (g >> 4);
          d = ((g & 15) << 4) | (h >> 2);
          e = ((h & 3) << 6) | i;
      ...
      calculateHeight = function (el) {
        ...
        return (
          el[0].clientHeight + _this.container.find("h1").outerHeight(true) + 15
        );
      },
    ...
    _this.namespace = _this.settings.name
      .toLowerCase()
      .replace(/[^a-z0-9_-]/g, "");
    ...
    // Insert CSS
    if (
      typeof _this.settings.css != "undefined" &&
      _this.settings.css !== "" &&
      $("#repojs_css").length <= 0
    )
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response.data.tree, function (i) {
            ...
            if (!exists) {
              ...
              ctx.folders.push({
                ...
                element: $(
                  '<div class="page" id="' +
                    id +
                    '"><ul><li class="titles"><span>name</span></li><li class="back"><a href="#">..</a></li></ul></page>'
          ...
          output
            ...
            .append(
              $(
                '<li class="file"><a href="#" data-path="' +
                  path +
                  '" data-id="' +
                  id +
                  '">' +
          ...
          if (parent.hasClass("file")) {
            ...
            if (el.legnth > 0) {
              ...
              $.ajax({
                ...
                success: function (response) {
                  var fileContainer = $(
                      '<div class="file page" id="' +
                        link.data("id") +
                        '"></div>'
                    ),
                    extension = response.data.name
                      .split(".")
                      .pop()
                      .toLowerCase(),
                  ...
                  if ("image" === mimeType.split("/").shift()) {
                    el = fileContainer
                      .append(
                        $(
                          '<div class="image"><span class="border-wrap"><img src="" /></span></div>'
                        )
                      )
                      .appendTo(repo);
                    el.find("img")
                      .attr(
                        "src",
                        "data:" + mimeType + ";base64," + response.data.content
                      )
                    ...
                    el.find("code").html(
                      String(decode64(response.data.content))
                        .replace(/&/g, "&amp;")
                        .replace(/</g, "&lt;")
                        .replace(/>/g, "&gt;")
                        .replace(/"/g, "&quot;")
                ...
                error: function (response) {
                  if (console && console.log) console.log("Request Error:", e);
                },
              });
            }
            ...
            el =
              el == _this.namespace
                ? _this.container.find(".page").first()
                : $("#" + el);
            transition(el, "right");
            ...
            el = el.length
              ? el
              : _this.container.find(".page").eq(link.index());

            if (link[0] !== _this.container.find("h1 a")[0])
              link.addClass("active");
            _this.container
              .find("h1 a")
              .slice(link.index() + 1, _this.container.find("h1 a").length)
              .remove();
            transition(el, "right");
          }
        });
      ...
      error: function (response) {
        if (console && console.log) console.log("Request Error:", response);
      },
    });
    ...
    return this.each(function () {
      _this.container = $(
        '<div class="repo"><h1><a href="#" data-id="' +
          _this.namespace +
          '_split_default">' +
          _this.settings.name +
          '</a></h1><div class="loader"></div><div class="page" id="' +
          _this.namespace +
          '_split_default"><ul><li class="titles"><span>name</span></li></ul></div></div>'
      ).appendTo($(this));
    });
  };
})(jQuery);
