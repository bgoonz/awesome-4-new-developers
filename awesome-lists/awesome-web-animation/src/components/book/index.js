...
function Container({ googleBookId }) {
  ...
  const alreadyAtLocalStorage =
    localStorage.getItem(googleBookId) && localStorage.getItem(googleBookId) !== 'undefined';
  const bookDataLocalStorage =
    alreadyAtLocalStorage && JSON.parse(localStorage.getItem(googleBookId));
  ...
  return (
    (!isLoading || alreadyAtLocalStorage) && (
      <Book bookData={bookDataLocalStorage || bookData} googleBookId={googleBookId} />
    )
  );
}
