    ...
    <div className="columns">
      <div className="column is-6">
        ...
        <h3 className="is-size-3 has-color-danger has-margin-top-5">
          <Link to={`/${stack.path}`}>{stack.name}</Link>
        </h3>
        <p className="has-margin-top-10">{truncate(stack.description, 150)}</p>
      </div>
          ...
          {stack.tools.slice(0, 3).map((tool) => (
            <div className="level-item" key={tool.name}>
              <Link to={`/${stack.path}`}>
                {tool.stackShareData ? (
                  <img
                    alt={tool.stackShareData.name}
                    src={tool.stackShareData.imageUrl}
                    style={{ width: "75px", borderRadius: "3px" }}
                  ></img>
                ) : tool.gitHubData ? (
                  <GitHubIcon github={tool.gitHubData} />
                ) : (
                  <div
                    className="is-size-7"
                    style={{
                      border: "1px dotted #8E9FA9",
                      width: "60px",
                      height: "60px",
                      padding: "2px",
                      display: "flex",
