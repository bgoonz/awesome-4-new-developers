  ...
  render() {
    ...
    return (
      <section ref="subcategory" className="list">
        <h3
          id={getAnchor(this.props.prefix, subcategory.name)}
          className="list_title"
        >
          <a href={`#${getAnchor(this.props.prefix, subcategory.name)}`}>
            {subcategory.name}
          </a>
        </h3>
        <ul className="list_list">
          {renderLinks(
            subcategory.links.filter((link) => link.state === LinkState.AWESOME)
          )}
          {renderLinks(
            subcategory.links.filter((link) => link.state === LinkState.DEFAULT)
          )}
          {renderLinks(
            subcategory.links.filter(
              (link) => link.state === LinkState.ARCHIVED
            )
          )}
          {renderLinks(
            subcategory.links.filter(
              (link) => link.state === LinkState.UNSUPPORTED
            )
          )}
        </ul>
      </section>
