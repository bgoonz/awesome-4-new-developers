  ...
  return (
    <div className={s.book} itemScope itemType="http://schema.org/Book">
      ...
      <div className={s.content}>
        <h2 itemProp="name" className={s.title}>
          {' '}
      ...
      <div className={s.info}>
        ...
        <time itemProp="datePublished" dateTime={bookData.volumeInfo.publishedDate}>
          {' '}
      ...
      {bookData.volumeInfo.categories &&
        bookData.volumeInfo.categories.map((category) => {
          return <meta key={category} itemProp="keywords" content={category} />;
        })}
    </div>
  );
}
