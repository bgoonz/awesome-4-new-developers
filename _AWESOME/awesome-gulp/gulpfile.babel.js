...
gulp.task("font", () => {
  return gulp.src(config.src.font).pipe(gulp.dest(`${config.dist}/font`));
});

gulp.task("img", () => {
  return gulp.src(config.src.img).pipe(gulp.dest(`${config.dist}/img`));
});

gulp.task("styles", () => {
  gulp
    .src(`${config.src.styles}/custom.scss`)
    .pipe(
      $.sass({
 ...
 */
gulp.task("gh-pages", () => {
  return gulp.src(`${config.dist}/**/*`).pipe($.ghPages());
});
