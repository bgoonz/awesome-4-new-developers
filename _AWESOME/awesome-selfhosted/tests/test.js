...
function parseArgs(args) {
  ...
  if (pr === true) {
    console.log(
      chalk.blue(
        `Running on PR. README.md: ${args[args.indexOf("-r", 2) + 1]} diff: ${
          args[args.indexOf("-d", 2) + 1]
        }`
      )
    );
  }
...
function findPattern(text) {
  const patt =
    /^\s{0,2}-\s\[.*?\]\(.*?\) (`⚠` )?- .{0,249}?\.( \(\[(Demo|Source Code|Clients)\]\([^)\]]*\)(, \[(Source Code|Clients)\]\([^)\]]*\))?(, \[(Source Code|Clients)\]\([^)\]]*\))*\))? \`.*?\` \`.*?\`$/;
...
function testSrcDemCli(text) {
  ...
  let testD =
    /(?<=\w. )(\(\[(Demo|Source Code|Clients)\]\([^)\]]*\)(, \[(Source Code|Clients)\]\([^)\]]*\))?(, \[(Source Code|Clients)\]\([^)\]]*\))*\))(?= \`?)/;
...
function testLicense(md) {
  ...
  try {
    ...
    console.log(
      chalk.yellow("Error in License syntax, license not checked against list.")
    );
    return [false, "", ""];
  }
  ...
  if (pr === true) {
    ...
    for (let e of entries) {
      ...
      if (!e.licenseTest[0]) {
        ...
        console.log(
          chalk.yellow(e.line + ": ") +
            `${e.name}'s license ${chalk.red(
              `'${e.licenseTest[1]}'`
            )} is not on the License list.\n`
        );
      }
      if (e.pass) {
        totalPass++;
