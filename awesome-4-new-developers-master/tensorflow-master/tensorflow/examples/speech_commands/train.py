...
def main(_):
    ...
    with tf.compat.v1.name_scope("train"), tf.control_dependencies(
        ...
        if FLAGS.optimizer == "gradient_descent":
            ...
            train_step = tf.compat.v1.train.MomentumOptimizer(
                learning_rate_input, 0.9, use_nesterov=True
            ).minimize(cross_entropy_mean)
        else:
    ...
    # Save list of words.
    with gfile.GFile(
        os.path.join(FLAGS.train_dir, FLAGS.model_architecture + "_labels.txt"), "w"
    ) as f:
    ...
    for training_step in xrange(start_step, training_steps_max + 1):
        ...
        if (
            training_step % FLAGS.save_step_interval == 0
            ...
            tf.compat.v1.logging.info(
                'Saving to "%s-%d"', checkpoint_path, training_step
            )
