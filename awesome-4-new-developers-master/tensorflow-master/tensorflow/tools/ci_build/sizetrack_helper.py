...
size.add_argument(
    "--artifact", type=argparse.FileType("r"), help="Local file you are measuring."
)
size.add_argument(
    "--manual_bytes",
    type=int,
...
def gcloud(tool, args, stdin=None):
  ...
  """
    ...
    if platform.system() == "Windows":
        tool = (
            r"C:\Program Files (x86)\Google\Cloud " r"SDK\google-cloud-sdk\bin\{}.cmd"
        ).format(tool)
...
def get_all_tested_commits():
    ...
    # COMMIT_HASH
    earliest_commit = bq(
        ["query", "--format", "csv", "--nouse_legacy_sql"],
    ...
    if earliest_commit:
        ...
        early_author_date, early_cl, early_commit_date = git_pretty(
            earliest_commit, PRETTY_EARLY, n=1
        )[0].split("\t")
...
def build_row():
    ...
    artifact_filename = (
        "NO_FILE" if not FLAGS.artifact else os.path.basename(FLAGS.artifact.name)
    )
...
def main():
    ...
    if FLAGS.print_schema:
        ...
        not FLAGS.team
        or not FLAGS.artifact_id
        or not (FLAGS.artifact or FLAGS.manual_bytes)
    ):
        print(
