...
from tensorflow.tools.tensorflow_builder.config_detector.data import (
...
cmds_linux = {
    "cpu_type": ("cat /proc/cpuinfo 2>&1 | grep 'vendor' | uniq"),
    "cpu_arch": ("uname -m"),
    "distrib": ("cat /etc/*-release | grep DISTRIB_ID* | sed 's/^.*=//'"),
    "distrib_ver": ("cat /etc/*-release | grep DISTRIB_RELEASE* | sed 's/^.*=//'"),
    "gpu_type": ("sudo lshw -C display | grep product:* | sed 's/^.*: //'"),
    "gpu_type_no_sudo": r"lspci | grep 'VGA compatible\|3D controller' | cut -d' ' -f 1 | "
    r"xargs -i lspci -v -s {} | head -n 2 | tail -1 | "
    r"awk '{print $(NF-2), $(NF-1), $NF}'",
    "gpu_count": ("sudo lshw -C display | grep *-display:* | wc -l"),
    "gpu_count_no_sudo": (r"lspci | grep 'VGA compatible\|3D controller' | wc -l"),
    "cuda_ver_all": ("ls -d /usr/local/cuda* 2> /dev/null"),
    "cuda_ver_dflt": (
        [
    ...
    "gcc_ver": ("gcc --version | awk '{print $NF}' | head -n 1"),
    "glibc_ver": ("ldd --version | tail -n+1 | head -n 1 | awk '{print $NF}'"),
    "libstdcpp_ver": "strings $(/sbin/ldconfig -p | grep libstdc++ | head -n 1 | "
    "awk '{print $NF}') | grep LIBCXX | tail -2 | head -n 1",
    "cpu_isa": ("cat /proc/cpuinfo | grep flags | head -n 1"),
}
...
def get_cuda_compute_capability(source_from_url=False):
  ...
  """
    if not GPU_TYPE:
        ...
        if FLAGS.debug:
            print(
                "Warning: Unknown GPU is detected. "
  ...
  """
    all_functions = collections.OrderedDict(
        [
            ...
            ("CUDA compute capability", get_cuda_compute_capability(get_gpu_type()[1])),
            ...
            if "ISA" in config:
                if not ret_val[1]:
                    ...
                    configs_found.append(
                        [
                            config,
                            "\033[91m\033[1mMissing "
                            + six.ensure_str(str(ret_val[1])[1:-1])
                            + "\033[0m",
                        ]
                    )
                    missing.append(
