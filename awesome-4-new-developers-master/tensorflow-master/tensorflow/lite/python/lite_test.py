...
class FromConstructor(TestModels):
    ...
    def testRedundantArgumentsWarning(self):
        ...
        frozen_graph_def = convert_to_constants.convert_variables_to_constants_from_session_graph(
    ...
    def testPartialShapeOverriding(self):
        ...
        # Convert model and ensure model is not None.
        converter = lite.TFLiteConverter(
            frozen_graph_def, None, None, [("in_tensor_a", [2, 16, 16, 3])], ["add"]
        )
    ...
    def testInvalidShapeOverriding(self):
        ...
        # Convert model and ensure model is not None.
        converter = lite.TFLiteConverter(
            frozen_graph_def, None, None, [("wrong_tensor", [2, 16, 16, 3])], ["add"]
        )
        with self.assertRaises(ConverterError):
            converter.convert()
...
class FromSessionTest(TestModels, parameterized.TestCase):
    ...
    def testFloatModelQuantizedInput(self):
        ...
        converter.quantized_input_stats = {"Placeholder": (0.0, 1.0)}  # mean, std_dev
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
        ...
        self.assertEqual(np.float32, output_details[0]["dtype"])
        self.assertAllEqual([1, 16, 16, 3], output_details[0]["shape"])
        self.assertEqual((0.0, 0.0), output_details[0]["quantization"])  # float
    ...
    def testDisablePerChannelQuantization(
        ...
        interpreter.allocate_tensors()
        detail = next(
            (d for d in interpreter.get_tensor_details() if d["name"] == k_conv_name)
        )
    ...
    @parameterized.named_parameters(
        ("EnableMlirConverter", True), ("DisableMlirConverter", False)  # enable mlir
    )  # disable mlir
    ...
    def testIntermediateInputArray(self):
        ...
        converter = lite.TFLiteConverter.from_session(
            sess, [in_tensor_final], [out_tensor]
        )
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
    ...
    def testSizeNoneInvalid(self):
        ...
        with self.assertRaises(ValueError) as error:
            converter.convert()
        self.assertEqual(
            "Provide an input shape for input array 'Placeholder'.",
            str(error.exception),
        )
    ...
    def testDumpGraphviz(self, enable_mlir_converter):
        ...
        self.assertIsNotNone(
            os.path.exists(os.path.join(graphviz_dir, "toco_AFTER_TRANSFORMATIONS.dot"))
        )
    ...
    @parameterized.named_parameters(
        ...
        ("UseRepresentativeData", True, False, True, False, False, False, False, False),
        # Quantize to Float16 if no rep data provided.
        ("NoRepresentativeData", False, False, True, False, False, False, False, False),
        # Quantize to Float16 and set Float16Accumulation
        (
            "SpecifyFloat16Accumulation",
            False,
            False,
            True,
            True,
        ...
        (
            "SampleDataIncludeInt8MlirQuant",
            True,
            True,
            False,
            False,
    ...
    def testQuantizeFloat16(
        self,
        use_rep_data,
        include_int8,
        is_float16_quantized,
        is_float16_accumulation,
        is_error,
        ...
        self.assertEqual(
            interpreter.get_tensor_details()[bias_idx]["dtype"], dtypes.float32
        )
        ...
        if is_float16_accumulation:
            quantized_converter.target_spec.experimental_supported_accumulation_type = (
                dtypes.float16
        ...
        if is_error:
            ...
            interpreter.allocate_tensors()
            self.assertEqual(
                interpreter.get_tensor_details()[bias_idx]["name"], bias_name
            )

            if is_float16_quantized:
                # Verify that bias constant is float16 type.
                self.assertEqual(
                    interpreter.get_tensor_details()[bias_idx]["dtype"], dtypes.float16
                )
            elif is_post_training_quantized:
                # Verify that bias constants is int32 type.
                self.assertEqual(
                    interpreter.get_tensor_details()[bias_idx]["dtype"], dtypes.int32
                )
            else:
    ...
    @parameterized.named_parameters(
        ("InferenceType_INT8", dtypes.int8), ("InferenceType_UINT8", dtypes.uint8)
    )
    ...
    def testInvalidQuantizeQATModelMissingInputStats(self):
        ...
        converter.inference_type = dtypes.uint8
        converter.quantized_input_stats = {"inputA": (0.0, 1.0)}  # mean, std_dev
        with self.assertRaises(ValueError) as error:
            converter.convert()
        self.assertEqual(
            "Quantization input stats are not available for input tensors " "'inputB'.",
            str(error.exception),
        )
    ...
    def testMultipleOutputNodeNames(self):
        ...
        # Convert model and ensure model is not None.
        converter = lite.TFLiteConverter.from_session(
            sess, [input_tensor], [out0, out1, out2, out3]
        )
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
    ...
    def testInferenceInputOutputTypeQuantizedUint8Default(self):
        with ops.Graph().as_default():
            ...
            out_tensor = array_ops.fake_quant_with_min_max_args(
                in_tensor + in_tensor, min=0.0, max=1.0, name="output"
            )
            sess = session.Session()
    ...
    def testResizingIntermediateDynamicTensor(self):
        ...
        input_details = interpreter.get_input_details()
        interpreter.set_tensor(
            input_details[1]["index"], np.array([[1, 1], [1, 1]], dtype=np.int32)
        )
        interpreter.invoke()
        ...
        interpreter.set_tensor(
            input_details[1]["index"], np.array([[2, 2], [2, 2]], dtype=np.int32)
        )
        interpreter.invoke()
...
class FromFrozenGraphFile(LiteTest):
    ...
    def testFreezeGraph(self):
        ...
        self.assertEqual(
            "Please freeze the graph using freeze_graph.py.", str(error.exception)
        )
...
class FromFrozenGraphObjectDetection(LiteTest):
    ...
    def testTFLiteGraphDefWithControlOutput(self):
        ...
        self.assertEqual(
            model.operatorCodes[1].builtinCode, schema_fb.BuiltinOperator.CUSTOM
        )
        self.assertEqual(model.operatorCodes[1].customCode, b"FlexStringFormat")
        self.assertEqual(
            model.operatorCodes[2].builtinCode, schema_fb.BuiltinOperator.CUSTOM
        )
    ...
    def testModifyIOToUint8(self):
        ...
        def representative_dataset_gen():
            for _ in range(2):
                yield [
                    np.random.uniform(low=0, high=1, size=(1, 300, 300, 3)).astype(
                        np.float32
                    )
                ]
        ...
        converter.experimental_new_quantizer = True
        converter.quantized_input_stats = {
            "normalized_input_image_tensor": (0.0, 1.0)
        }  # mean, std_dev
        converter.allow_custom_ops = True
        tflite_model = converter.convert()
...
class FromSavedModelTest(TestModels):
    ...
    def testNewConverterOptOut(self):
        ...
        handler = logging.StreamHandler(log)
        logging.root.addHandler(handler)
        optout_message = "Using experimental converter: " "If you encountered a problem"
        # Convert model and ensure model is not None.
        converter = lite.TFLiteConverter.from_saved_model(saved_model_dir)
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
    ...
    def testShapeOverriding(self):
        ...
        converter = lite.TFLiteConverter.from_saved_model(
            saved_model_dir,
            input_shapes={"inputA": [2, 16, 16, 3], "inputB": [2, 16, 16, 3]},
        )
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
...
class FromKerasFile(TestModels, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ("_graph", context.graph_mode), ("_eager", context.eager_mode)
    )
    ...
    def testSequentialModelOutputArray(self):
        ...
        self.assertEqual(
            "Invalid tensors 'invalid-output' were found.", str(error.exception)
        )
    ...
    def testShapeOverriding(self):
        ...
        # Convert to TFLite model.
        converter = lite.TFLiteConverter.from_keras_model_file(
            self._keras_file, input_shapes={"input_a": {2, 3}, "input_b": {2, 3}}
        )
        tflite_model = converter.convert()
        self.assertIsNotNone(tflite_model)
...
class GrapplerTest(TestModels, parameterized.TestCase):
    ...
    def testInputNodeIsNotFolded(self, enable_mlir_converter):
        ...
        # Convert model.
        converter = lite.TFLiteConverter.from_session(
            sess, [in_tensor, y_const], [out_tensor]
        )
        converter.experimental_new_converter = enable_mlir_converter
        tflite_model = converter.convert()
    ...
    def testGrapplerConstFolding(self):
        ...
        with ops.Graph().as_default():
            ...
            out_tensor = plus_placeholder(
                array_ops.zeros([2, 2, 2]), array_ops.reshape(in_tensor, shape=[2, 2])
            )
            sess = session.Session()
