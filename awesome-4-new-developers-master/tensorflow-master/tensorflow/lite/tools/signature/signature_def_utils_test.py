...
class SignatureDefUtilsTest(tf.test.TestCase):
    def testAddSignatureDefToFlatbufferMetadata(self):
        ...
        if not tf.io.gfile.exists(filename):
            raise IOError(
                'File "{0}" does not exist in {1}.'.format(
                    filename,
        ...
        predict_signature_def = tf.compat.v1.saved_model.build_signature_def(
            sig_input_tensor_signature,
            sig_output_tensor_signature,
        ...
        saved_signature_def_map = signature_def_utils.get_signature_defs(tflite_model)
        signature_def = saved_signature_def_map.get(serving_key)
        self.assertIsNotNone(signature_def, "SignatureDef not found")
        self.assertEqual(
            signature_def.SerializeToString(), predict_signature_def.SerializeToString()
        )
        remove_tflite_model = signature_def_utils.clear_signature_defs(tflite_model)
        signature_def_map = signature_def_utils.get_signature_defs(remove_tflite_model)
        self.assertIsNone(
            signature_def_map.get(serving_key),
