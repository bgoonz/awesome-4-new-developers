...
BUFFER_TEST_V3 = {
    ...
    "subgraphs": [
        {
            "tensors": [{"buffer": 1}, {"buffer": 2}, {"buffer": 0}],
            "inputs": [],
            "outputs": [],
            "operators": [],
        }
...
class TestSchemaUpgrade(test_util.TensorFlowTestCase):
    ...
    """
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".json", mode="w+"
        ) as in_json, tempfile.NamedTemporaryFile(
            suffix=".json", mode="w+"
        ) as out_json, tempfile.NamedTemporaryFile(
            suffix=".bin", mode="w+b"
        ) as out_bin, tempfile.NamedTemporaryFile(
            suffix=".tflite", mode="w+b"
        ) as out_tflite:
            ...
            self.assertEqual(
                open(out_bin.name, "rb").read(), open(out_tflite.name, "rb").read()
            )
