...
def toco_options(
    data_types, input_arrays, output_arrays, shapes, extra_toco_options=None
):
  ...
  """
    ...
    s = (
        " --input_data_types=%s" % ",".join(data_types)
...
def toco_convert(options, graph_def, input_tensors, output_tensors, **kwargs):
  ...
  """
    ...
    if dynamic_range_quantize or fully_quantize:
                    ...
                    for _, shape, _ in input_tensors:
                        if shape:
                            ...
                            calibration_inputs.append(
                                np.random.uniform(
                                    min_value, max_value, tuple(dims)
                                ).astype(np.float32)
                            )
        ...
        with tempfile.NamedTemporaryFile() as graphdef_file, tempfile.NamedTemporaryFile() as output_file, tempfile.NamedTemporaryFile(
            "w+"
        ) as stdout_file:
            graphdef_file.write(graph_def_str)
            graphdef_file.flush()
