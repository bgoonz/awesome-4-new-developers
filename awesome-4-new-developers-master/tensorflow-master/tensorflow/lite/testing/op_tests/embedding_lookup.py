...
def make_embedding_lookup_tests(options):
    ...
    def build_graph(parameters):
        ...
        ids = tf.compat.v1.placeholder(
            dtype=parameters["ids_dtype"], name="ids", shape=parameters["ids_shape"]
        )
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        ...
        ids = create_tensor_data(
            parameters["ids_dtype"],
            parameters["ids_shape"],
            0,
            parameters["params_shape"][0] - 1,
        )
        return (
            [params, ids],
            sess.run(outputs, feed_dict=dict(zip(inputs, [params, ids]))),
        )
