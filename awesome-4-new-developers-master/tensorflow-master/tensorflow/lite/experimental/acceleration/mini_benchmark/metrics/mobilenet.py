...
from tensorflow.lite.experimental.acceleration.mini_benchmark.metrics import (
    kl_divergence,
)
...
def main(output_path):
    ...
    with tf.Graph().as_default():
        ...
        mse = tf.reshape(
            tf.math.reduce_mean((expected_scores - actual_scores) ** 2), [1], name="mse"
        )
        kld_metric = kl_divergence.symmetric_kl_divergence(
        ...
        ok = tf.reshape(tf.logical_and(kld_metric < 5.5, mse < 0.003), [1], name="ok")
        sess = tf.compat.v1.Session()
        converter = tf.lite.TFLiteConverter.from_session(
            sess, [expected_scores, actual_scores], [kld_metric, mse, ok]
        )
