        ...
        if test_params.get("fully_quantize", False):
            ...
            def representative_dataset(input_tensors):
                ...
                for _, shape, _ in input_tensors:
                    if shape:
                        dims = [
                            1 if dim.value is None else dim.value for dim in shape.dims
                        ]
                        calibration_inputs.append(
                            np.random.uniform(min_value, max_value, tuple(dims)).astype(
                                np.float32
                            )
                        )
            ...
            if extra_toco_options.inference_input_type:
                converter.inference_input_type = extra_toco_options.inference_input_type
...
def mlir_convert_file(
  ...
  """
    ...
    with tempfile.NamedTemporaryFile() as output_file, tempfile.NamedTemporaryFile(
        "w+"
    ) as stdout_file:
        input_shapes = []
        ...
        cmd = (
            "%s -tf-input-arrays=%s -tf-input-data-types=%s -tf-input-shapes=%s "
            "-tf-output-arrays=%s " + quant_flags + additional_flags + "%s -o %s"
        )
        cmd = cmd % (
            bin_path,
...
def mlir_convert_saved_model(
  ...
  """
    ...
    with tempfile.NamedTemporaryFile() as output_file, tempfile.NamedTemporaryFile(
        ...
        cmd = (
            "%s %s --tf-savedmodel-tags=%s --tf-savedmodel-exported-names=%s "
            + additional_flags
            + " %s --o=%s"
        )
        cmd = cmd % (
...
def mlir_convert_jax(input_file_name, is_hlotxt_format, additional_flags=""):
  ...
  """
    ...
    with tempfile.NamedTemporaryFile() as output_file, tempfile.NamedTemporaryFile(
        ...
        cmd = (
            "%s --import-hlo --enable-hlo-to-tf-conversion "
            "--hlo-import-type=%s" + additional_flags + " %s --o=%s"
        )
        cmd = cmd % (bin_path, hlo_import_type_str, input_file_name, output_file.name)
        exit_code = os.system(cmd)
        log = (
            cmd
            + "exited with code %d" % exit_code
