...
class LocalCLIDebugWrapperSession(framework.BaseDebugWrapperSession):
  ...
  """
    ...
    """
        ...
        if not dump_root:
            ...
            if os.path.isfile(dump_root):
                ...
                raise ValueError(
                    "dump_root path points to a non-empty directory: %s" % dump_root
                )
    ...
    """
        ...
        if self._run_call_count > 1 and not self._skip_debug:
            if self._run_through_times > 0:
                ...
                # debugging.
                return self._run_start_response or framework.OnRunStartResponse(
                    framework.OnRunStartAction.DEBUG_RUN, self._get_run_debug_urls()
                )
    ...
    def _prep_debug_cli_for_run_end(
        self, debug_dump, tf_error, passed_filter, passed_filter_exclude_node_names
    ):
    ...
    """
        ...
        self._run_cli.register_tab_comp_context(
            ["print_tensor", "pt"], dumped_tensor_names
        )
    ...
    def _prep_profile_cli_for_run_end(self, py_graph, run_metadata):
        ...
        self._run_cli = profile_analyzer_cli.create_profiler_ui(
            py_graph, run_metadata, ui_type=self._ui_type, config=self._run_cli.config
        )
    ...
    def _register_this_run_info(self, curses_cli):
        ...
        if self._feed_dict and hasattr(self._feed_dict, "keys"):
            ...
            feed_keys = [
                common.get_graph_element_name(key) for key in self._feed_dict.keys()
            ]
    ...
    def _update_run_calls_state(
        self, run_call_count, fetches, feed_dict, is_callable_runner=False
    ):
    ...
    """
        ...
        self._run_description = cli_shared.get_run_short_description(
            run_call_count, fetches, feed_dict, is_callable_runner=is_callable_runner
        )
        self._run_through_times -= 1
