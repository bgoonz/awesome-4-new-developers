...
class RichTextLinesTest(test_util.TensorFlowTestCase):
    ...
    def testFormatTensorHighlightsTensorNameWithDebugOp(self):
        ...
        self.assertEqual(
            [
                (8, 8 + len(tensor_name), "bold"),
                (
                    8 + len(tensor_name) + 1,
                    8 + len(tensor_name) + 1 + len(debug_op),
                    "yellow",
                ),
            ],
    ...
    def testFormatTensorWithMetadata(self):
        ...
        cli_test_utils.assert_lines_equal_ignoring_whitespace(
            self,
            ['Tensor "a":', "  dtype: float64", "  shape: (4, 4)", ""],
            out.lines[:4],
        )
        self.assertEqual(repr(a).split("\n"), out.lines[4:])
    ...
    def testFormatTensor2DNoEllipsisWithRowBreak(self):
        ...
        self.assertEqual(
            {"dtype": a.dtype, "shape": a.shape}, out.annotations["tensor_metadata"]
        )
    ...
    def testFormatTensorWithEllipses(self):
        ...
        self.assertEqual(
            {tensor_format.BEGIN_INDICES_KEY: [0, 1, 0]},
        ...
        self.assertEqual(
            {tensor_format.BEGIN_INDICES_KEY: [10, 10, 0]},
        ...
        while not out.lines[omitted_line].strip().startswith("..."):
            omitted_line -= 1
        self.assertEqual(
            {tensor_format.OMITTED_INDICES_KEY: [10, 2, 0]},
            out.annotations[omitted_line],
        )
    ...
    def testBatchModeWithErrors(self):
        ...
        with self.assertRaisesRegex(ValueError, r"Indices contain negative value\(s\)"):
    ...
    def testLocateTensorElement3DWithEllipsesBatchMode(self):
        ...
        (are_omitted, rows, start_cols, end_cols) = tensor_format.locate_tensor_element(
            out, [[0, 0, 0]]
        )
...
class NumericSummaryTest(test_util.TensorFlowTestCase):
    ...
    def testNumericSummaryOnEmptyFloat(self):
        ...
        out = tensor_format.numeric_summary(x)
        self.assertEqual(
            ["No numeric summary available due to empty tensor."], out.lines
        )
    ...
    def testNumericSummaryOnBool(self):
        ...
        out = tensor_format.numeric_summary(x)
        cli_test_utils.assert_lines_equal_ignoring_whitespace(
            self, ["| False  True | total |", "|     2     2 |     4 |"], out.lines
        )
    ...
    def testNumericSummaryOnStrTensor(self):
        ...
        out = tensor_format.numeric_summary(x)
        self.assertEqual(
            ["No numeric summary available due to tensor dtype: object."], out.lines
        )
