...
class DebugAnalyzer(object):
    ...
    _GRAPH_STRUCT_OP_TYPE_DENYLIST = (
        "_Send",
        "_Recv",
        "_HostSend",
        "_HostRecv",
    ...
    """
        ...
        ap = argparse.ArgumentParser(
            description="List dumped intermediate tensors.", usage=argparse.SUPPRESS
        )
        ap.add_argument(
            "-f",
            "--tensor_filter",
        ...
        ap.add_argument(
            ...
            help=(
                "the field to sort the data by: (%s | %s | %s | %s)"
                % (
                    SORT_TENSORS_BY_TIMESTAMP,
                    SORT_TENSORS_BY_DUMP_SIZE,
        ...
        ap.add_argument(
            "-t", "--op_type", action="store_true", help="Show op types of input nodes."
        )
        ...
        ap.add_argument(
            ...
            dest="recursive",
            action="store_true",
            help="Show recipients of the node recursively, i.e., the output " "tree.",
        )
        ap.add_argument(
            "-t",
            "--op_type",
        ...
        self._arg_parsers["print_tensor"] = command_parser.get_print_tensor_argparser(
    ...
    """
        ...
        if parsed.tensor_filter:
            ...
            data_to_show = self._debug_dump.find(
                filter_callable, exclude_node_names=parsed.filter_exclude_node_names
            )
        else:
        ...
        max_timestamp_width, max_dump_size_width, max_op_type_width = self._measure_tensor_list_column_widths(
            data_to_show
        )
        ...
        output.extend(
            self._tensor_list_column_heads(
                parsed, max_timestamp_width, max_dump_size_width, max_op_type_width
            )
        )
        ...
        for dump in data_to_show:
            ...
            output.append(
                ...
                font_attr_segs=[
                    (
                        len(line) - len(dumped_tensor_name),
                        len(line),
    ...
    """
        ...
        max_timestamp_width = max(
            max_timestamp_width, len(self._TIMESTAMP_COLUMN_HEAD) + 1
        )
        ...
        max_dump_size_width = max(
            max_dump_size_width, len(self._DUMP_SIZE_COLUMN_HEAD) + 1
        )
    ...
    """
        ...
        if parsed.sort_by == SORT_TENSORS_BY_TENSOR_NAME and not parsed.reverse:
            command += " -r"
        attr_segs[0].append(
            (prev_len, len(row), [debugger_cli_common.MenuItem("", command), "bold"])
        )
        row += " " * (
            max_op_type_width + max_dump_size_width + max_timestamp_width - len(row)
        )
    ...
    """
        ...
        try:
            ...
            for depth, (file_path, line, function_name, text) in enumerate(node_stack):
                ...
                attribute = (
                    debugger_cli_common.MenuItem("", "ps %s -b %d" % (file_path, line))
                    if text
                    else None
                )
                line_number_line = RL("  ")
    ...
    """
        ...
        tensor_name, tensor_slicing = command_parser.parse_tensor_name_with_slicing(
            parsed.tensor_name
        )

        node_name, output_slot = debug_graphs.parse_node_or_tensor_name(tensor_name)
        if self._debug_dump.loaded_partition_graphs() and not self._debug_dump.node_exists(
            node_name
        ):
            output = cli_shared.error(
            ...
            if parsed.number < 0:
                ...
                for i, datum in enumerate(matching_data):
                    ...
                    font_attr_segs[len(lines) - 1] = [
                        (
                            len(lines[-1]) - len(datum.watch_key),
                            len(lines[-1]),
    ...
    def _reconstruct_print_source_command(
        self, parsed, line_begin, max_elements_per_line_increase=0
    ):
        return "ps %s %s -b %d -m %d" % (
            parsed.source_file_path,
            "-t" if parsed.tensors else "",
            line_begin,
    ...
    def print_source(self, args, screen_info=None):
        ...
        source_annotation = source_utils.annotate_source(
            self._debug_dump, parsed.source_file_path, do_dumped_tensors=parsed.tensors
        )
        ...
        output = debugger_cli_common.rich_text_lines_from_rich_line_list(
            labeled_source_lines,
            annotations={
                debugger_cli_common.INIT_SCROLL_POS_KEY: actual_initial_scroll_target
            },
        )
    ...
    """
        ...
        path_column_width = (
            max(max(len(item[0]) for item in source_list), len(path_head)) + 1
        )
        num_nodes_column_width = (
            max(max(len(str(item[2])) for item in source_list), len(num_nodes_head)) + 1
        )
        num_tensors_column_width = (
            max(max(len(str(item[3])) for item in source_list), len(num_tensors_head))
            + 1
        )

        head = RL(path_head + " " * (path_column_width - len(path_head)), color)
        head += RL(
            num_nodes_head + " " * (num_nodes_column_width - len(num_nodes_head)), color
        )
        head += RL(
            num_tensors_head + " " * (num_tensors_column_width - len(num_tensors_head)),
            color,
        )
        ...
        for (
            ...
            if source_utils.is_extension_uncompiled_python_source(file_path):
                path_attributes.append(
                    debugger_cli_common.MenuItem(
                        None, "ps %s -b %d" % (file_path, first_line_num)
                    )
                )
            ...
            line += RL(
                str(num_tensors)
                + " " * (num_tensors_column_width - len(str(num_tensors))),
                color,
            )
    ...
    def list_source(self, args, screen_info=None):
        ...
        output.extend(
            self._make_source_table(
                [item for item in source_list if not item[1]], False
            )
        )
        output.extend(
            self._make_source_table([item for item in source_list if item[1]], True)
        )
        _add_main_menu(output, node_name=None)
        return output

    def _list_inputs_or_outputs(
        self, recursive, node_name, depth, control, op_type, do_outputs=False
    ):
...
def create_analyzer_ui(
    debug_dump, tensor_filters=None, ui_type="curses", on_ui_exit=None, config=None
):
