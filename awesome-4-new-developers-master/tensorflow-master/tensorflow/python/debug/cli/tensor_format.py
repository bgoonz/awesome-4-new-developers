  ...
  """
    ...
    for r in xrange(len(lines)):
        ...
        matching_indices_list = [
            ind
            for ind in indices_list[batch_pos:]
...
def _validate_indices_list(indices_list, formatted):
    ...
    for ind in indices_list:
        ...
        if len(ind) != len(dims):
            raise ValueError(
                "Dimensions mismatch: requested: %d; actual: %d" % (len(ind), len(dims))
            )
...
def numeric_summary(tensor):
  ...
  """
    ...
    def _counts_summary(counts, skip_zeros=True, total_count=None):
        ...
        if skip_zeros:
            counts = [
                (count_key, count_val) for count_key, count_val in counts if count_val
            ]
        max_common_len = 0
    ...
    if not isinstance(tensor, np.ndarray) or not np.size(tensor):
        ...
        counts = [
            ...
            (
                "-",
                np.sum(
                    np.logical_and(tensor < 0.0, np.logical_not(np.isneginf(tensor)))
                ),
            ),
            ("0", np.sum(tensor == 0.0)),
            (
                "+",
                np.sum(
                    np.logical_and(tensor > 0.0, np.logical_not(np.isposinf(tensor)))
                ),
            ),
