...
class ParseCommandTest(test_util.TensorFlowTestCase):
    ...
    def testParseCommandsWithBrackets(self):
        ...
        self.assertEqual(
            ["pt", "foo[1, 2, :]", "-a"], command_parser.parse_command(command)
        )

        command = "inject_value foo [1, 2,:] 0"
        self.assertEqual(
            ["inject_value", "foo", "[1, 2,:]", "0"],
    ...
    def testParseCommandWithTwoArgsContainingBrackets(self):
        ...
        self.assertEqual(
            ["pt", "foo[1, :]", "bar[:, 2]"], command_parser.parse_command(command)
        )

        command = "pt foo[] bar[:, 2]"
        self.assertEqual(
            ["pt", "foo[]", "bar[:, 2]"], command_parser.parse_command(command)
        )

    def testParseCommandWithUnmatchedBracket(self):
        command = "pt  foo[1, 2, :"
        self.assertNotEqual(
            ["pt", "foo[1, 2, :]"], command_parser.parse_command(command)
        )

    def testParseCommandsWithQuotes(self):
        command = 'inject_value foo "np.zeros([100, 500])"'
        self.assertEqual(
            ["inject_value", "foo", "np.zeros([100, 500])"],
            command_parser.parse_command(command),
        )
        ...
        self.assertEqual(
            ["command prefix with spaces", "arg1"],
            command_parser.parse_command(command),
        )
...
class ParseRangesTest(test_util.TensorFlowTestCase):
    ...
    def testParseSingleRange(self):
        ...
        self.assertAllClose(
            [[-0.1, self.INF_VALUE]], command_parser.parse_ranges("[-0.1, inf]")
        )
        self.assertAllClose(
            [[-self.INF_VALUE, self.INF_VALUE]],
    ...
    def testParseInvalidRangeString(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Incorrect number of elements in range"
        ):
...
class ParseReadableSizeStrTest(test_util.TensorFlowTestCase):
    ...
    def testParseKiloBytesWorks(self):
        ...
        self.assertEqual(
            1024 ** 2 * 2, command_parser.parse_readable_size_str("2048kB")
        )
    ...
    def testParseMegaBytesWorks(self):
        ...
        self.assertEqual(
            1024 ** 3 * 2, command_parser.parse_readable_size_str("2048MB")
        )
    ...
    def testParseGigaBytesWorks(self):
        ...
        self.assertEqual(
            1024 ** 4 * 2, command_parser.parse_readable_size_str("2048GB")
        )
