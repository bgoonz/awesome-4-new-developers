...
def get_run_start_intro(
    run_call_count, fetches, feed_dict, tensor_filters, is_callable_runner=False
):
  ...
  """
    ...
    if is_callable_runner:
        ...
        out.append("Fetch(es):")
        out.extend(
            debugger_cli_common.RichTextLines(["  " + line for line in fetch_lines])
        )
        out.append("")
    ...
    out.extend(
        _recommend_command(
            "run", "Execute the run() call with debug tensor-watching", create_link=True
        )
    )
    out.extend(
        _recommend_command(
        ...
        if len(feed_dict) == 1:
            for key in feed_dict:
                description += "1 feed (%s)" % (
                    key
                    if isinstance(key, six.string_types) or not hasattr(key, "name")
                    else key.name
                )
        else:
