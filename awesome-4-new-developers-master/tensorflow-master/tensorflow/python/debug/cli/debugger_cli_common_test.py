...
class RichTextLinesTest(test_util.TensorFlowTestCase):
    def testRichTextLinesConstructorComplete(self):
        ...
        screen_output = debugger_cli_common.RichTextLines(
            ...
            font_attr_segs={0: [(0, 5, "red")], 1: [(0, 7, "blue")]},
            annotations={0: "longer wavelength", 1: "shorter wavelength"},
        )
    ...
    def testRichLinesAppendRichLine(self):
        rtl = debugger_cli_common.RichTextLines(
            "Roses are red", font_attr_segs={0: [(0, 5, "red")]}
        )
        rtl.append_rich_line(
            debugger_cli_common.RichLine("Violets are ")
    ...
    def testMergeRichTextLines(self):
        ...
        screen_output_2 = debugger_cli_common.RichTextLines(
            ...
            font_attr_segs={0: [(0, 6, "white")], 1: [(0, 7, "yellow")]},
            annotations={"metadata": "foo", 0: "full spectrum", 1: "medium wavelength"},
        )
    ...
    def testMergeRichTextLinesEmptyOther(self):
        ...
        self.assertEqual(
            {0: [(0, 5, "red")], 1: [(0, 7, "blue")]}, screen_output_1.font_attr_segs
        )
        self.assertEqual(
            {0: [(0, 5, "red")], 1: [(0, 7, "blue")]}, screen_output_1.font_attr_segs
        )
        self.assertEqual(
            {0: "longer wavelength", 1: "shorter wavelength"},
            screen_output_1.annotations,
        )
    ...
    def testPrependALineWithAttributeSegmentsWorks(self):
        ...
        self.assertEqual(
            {0: [(0, 7, "blue")], 1: [(0, 5, "red")]}, screen_output_1.font_attr_segs
        )
...
class CommandHandlerRegistryTest(test_util.TensorFlowTestCase):
    ...
    def testHandlerWithWrongReturnType(self):
        registry = debugger_cli_common.CommandHandlerRegistry()
        registry.register_command_handler(
            "wrong_return", self._handler_returning_wrong_type, ""
        )
    ...
    def testDispatchHandlerRaisingException(self):
        registry = debugger_cli_common.CommandHandlerRegistry()
        registry.register_command_handler(
            "raise_exception", self._handler_raising_exception, ""
        )
        ...
        self.assertTrue(
            cmd_output.lines[0].startswith("Error occurred during handling of command")
        )
    ...
    def testGetHelpFull(self):
        ...
        self.assertTrue(
            help_lines[3].endswith("Show screen width in number of columns.")
        )
...
class RegexFindTest(test_util.TensorFlowTestCase):
    ...
    def testRegexFindWithoutExistingFontAttrSegs(self):
        new_screen_output = debugger_cli_common.regex_find(
            self._orig_screen_output, "are", "yellow"
        )
        ...
        self.assertEqual(
            [0, 1],
            new_screen_output.annotations[debugger_cli_common.REGEX_MATCH_LINES_KEY],
        )
    ...
    def testRegexFindWithNoMatches(self):
        new_screen_output = debugger_cli_common.regex_find(
            self._orig_screen_output, "infrared", "yellow"
        )
...
class WrapScreenOutputTest(test_util.TensorFlowTestCase):
    def setUp(self):
        self._orig_screen_output = debugger_cli_common.RichTextLines(
            ...
            annotations={1: "longer wavelength", 2: "shorter wavelength"},
        )
    ...
    def testWrappingWithAttrCutoff(self):
        ...
        self.assertEqual(
            [(0, 5, "red"), (6, 9, "gray"), (10, 11, "red")], out.font_attr_segs[1]
        )
    ...
    def testWrappingWithMultipleAttrCutoff(self):
        self._orig_screen_output = debugger_cli_common.RichTextLines(
            ["Folk song:", "Roses are red", "Violets are blue"],
            font_attr_segs={1: [(0, 12, "red")], 2: [(1, 16, "blue")]},
            annotations={1: "longer wavelength", 2: "shorter wavelength"},
        )
...
class SliceRichTextLinesTest(test_util.TensorFlowTestCase):
    ...
    def testSliceBeginning(self):
        ...
        self.assertEqual(
            {0: "longer wavelength", "foo_metadata": "bar"}, sliced.annotations
        )
    ...
    def testSliceEnd(self):
        ...
        self.assertEqual(
            {0: "shorter wavelength", "foo_metadata": "bar"}, sliced.annotations
        )
...
class TabCompletionRegistryTest(test_util.TensorFlowTestCase):
    def setUp(self):
        ...
        self._tc_reg.register_tab_comp_context(
            ["print_tensor", "pt"], ["node_b:1", "node_b:2", "node_a:1", "node_a:2"]
        )
        self._tc_reg.register_tab_comp_context(
            ["node_info"], ["node_c", "node_b", "node_a"]
        )
    ...
    def testTabCompletion(self):
        ...
        self.assertEqual(
            (["node_a:1", "node_a:2"], "node_a:"),
            self._tc_reg.get_completions("print_tensor", "node_a"),
        )

        self.assertEqual(
            (["node_a:1"], "node_a:1"), self._tc_reg.get_completions("pt", "node_a:1")
        )
    ...
    def testDeregisterContext(self):
        ...
        self.assertEqual(
            (None, None), self._tc_reg.get_completions("print_tensor", "node_")
        )
    ...
    def testDeregisterNonexistentContext(self):
        ...
        with self.assertRaisesRegex(
            KeyError, 'Cannot deregister unregistered context word "print_tensor"'
        ):
            self._tc_reg.deregister_context(["print_tensor"])
...
class CommandHistoryTest(test_util.TensorFlowTestCase):
    ...
    def testLookUpMostRecent(self):
        ...
        self.assertEqual(
            ["list_tensors", "node_info node_a"], self._cmd_hist.most_recent_n(2)
        )
        self.assertEqual(
            ["list_tensors", "node_info node_a"], self._cmd_hist.most_recent_n(3)
        )
        ...
        self.assertEqual(
            ["node_info node_a", "node_info node_b"], self._cmd_hist.most_recent_n(2)
        )
        self.assertEqual(
            ["list_tensors", "node_info node_a", "node_info node_b"],
        ...
        self.assertEqual(["node_info node_a"], self._cmd_hist.most_recent_n(1))
        self.assertEqual(
            ["node_info node_b", "node_info node_a"], self._cmd_hist.most_recent_n(2)
        )
        self.assertEqual(
    ...
    def testLookUpPrefix(self):
        ...
        self.assertEqual(
            ["node_info node_b", "node_info node_a"],
            self._cmd_hist.lookup_prefix("node_info", 10),
        )

        self.assertEqual(
            ["node_info node_a"], self._cmd_hist.lookup_prefix("node_info", 1)
        )
    ...
    def testCommandHistoryHandlesWritingIOErrorGraciously(self):
        ...
        os.chmod(self._history_file_path, stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH)
...
class MenuTest(test_util.TensorFlowTestCase):
    ...
    def testFormatAsSingleLineWithStrItemAttrsWorks(self):
        ...
        self.assertEqual(
            (6, 18, [self.node1, "underline"]), output.font_attr_segs[0][0]
        )
        self.assertEqual(
            (20, 38, [self.node2, "underline"]), output.font_attr_segs[0][1]
        )
    ...
    def testFormatAsSingleLineWithListItemAttrsWorks(self):
        ...
        self.assertEqual(["Menu: water flower, measure wavelength, "], output.lines)
        self.assertEqual(
            (6, 18, [self.node1, "underline", "bold"]), output.font_attr_segs[0][0]
        )
        self.assertEqual(
            (20, 38, [self.node2, "underline", "bold"]), output.font_attr_segs[0][1]
        )
        self.assertEqual({}, output.annotations)
    ...
    def testInsertNode(self):
        ...
        self.assertEqual(
            ["water flower", "write poem", "measure wavelength"], self.menu.captions()
        )

        output = self.menu.format_as_single_line(prefix="Menu: ", divider=", ")
        self.assertEqual(
            ["Menu: water flower, write poem, measure wavelength, "], output.lines
        )
    ...
    def testFormatAsSingleLineWithDisabledNode(self):
        ...
        self.assertEqual(
            ["Menu: water flower, measure wavelength, write poem, "], output.lines
        )
        self.assertEqual((6, 18, [self.node1]), output.font_attr_segs[0][0])
        self.assertEqual((20, 38, [self.node2]), output.font_attr_segs[0][1])
...
class GetTensorFlowVersionLinesTest(test_util.TensorFlowTestCase):
    def testGetVersionWithoutDependencies(self):
        ...
        self.assertEqual(
            "TensorFlow version: %s" % pywrap_tf_session.__version__, out.lines[0]
        )

    def testGetVersionWithDependencies(self):
        out = debugger_cli_common.get_tensorflow_version_lines(True)
        self.assertIn(
            "TensorFlow version: %s" % pywrap_tf_session.__version__, out.lines
        )
