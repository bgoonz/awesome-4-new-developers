...
_CoreMetadata = collections.namedtuple(
    "CoreMetadata",
    [
        "global_step",
        "session_run_index",
...
def device_path_to_device_name(device_dir):
  ...
  """
    path_items = os.path.basename(device_dir)[
        len(METADATA_FILE_PREFIX) + len(DEVICE_TAG) :
    ].split(",")
    return "/".join(
        [
...
class DebugTensorDatum(object):
  ...
  """
    ...
    """
        ...
        self._dump_size_bytes = (
            gfile.Stat(self._file_path).length
            if gfile.Exists(self._file_path)
            else None
        )

    def __str__(self):
        return "{DebugTensorDatum (%s) %s:%d @ %s @ %d}" % (
            self.device_name,
            self.node_name,
            self.output_slot,
            self.debug_op,
...
class DebugDumpDir(object):
  ...
  """
    ...
    def _load_all_device_dumps(self, partition_graphs, validate):
        ...
        device_dirs = _glob(
            os.path.join(self._dump_root, METADATA_FILE_PREFIX + DEVICE_TAG + "*")
        )
    ...
    """
        ...
        self._dump_tensor_data[device_name] = sorted(
            self._dump_tensor_data[device_name], key=lambda x: x.extended_timestamp
        )
    ...
    """
        return sum(
            len(self._dump_tensor_data[device_name])
    ...
    """
        ...
        for device_name in self._device_names:
            ...
            if device_name in self._dump_graph_file_paths:
                ...
                partition_graph = self._find_partition_graph(
                    client_partition_graphs, device_name
                )
        ...
        for partition_graph, maybe_device_name in partition_graphs_and_device_names:
            debug_graph = debug_graphs.DebugGraph(
                partition_graph, device_name=maybe_device_name
            )
    ...
    """
        ...
        for datum in self._dump_tensor_data[device_name]:
            if datum.node_name not in debug_graph.node_inputs:
                raise ValueError(
                    "Node name '%s' is not found in partition graphs of "
        ...
        for node in debug_graph.node_inputs:
            ...
            for inp in inputs:
                ...
                if (
                    inp_node in self._debug_watches[device_name]
                    and inp_output_slot in self._debug_watches[device_name][inp_node]
                    and debug_graph.node_op_types.get(inp)
                    not in ("Enter", "NextIteration")
                    and (inp_node, inp_output_slot) not in pending_inputs[node]
                ):
        ...
        for i, datum in enumerate(self._dump_tensor_data[device_name]):
            ...
            if not self._satisfied_at_timestamp(
                ...
                raise ValueError(
                    "Causality violated in timing relations of debug "
                    "dumps: %s (%d): "
    ...
    """
        ...
        for datum in self._dump_tensor_data[device_name][start_i:]:
            ...
            if (
                ...
                and (datum.node_name, datum.output_slot) in pending
            ):
    ...
    """
        if device_name is None:
            if node_name in self._node_devices:
                ...
                raise ValueError(
                    "None of the %d device(s) has a node named '%s'."
    ...
    """
        ...
        tracer = debug_graphs.DFSGraphTracer(
            input_lists, skip_node_names=self._get_merge_node_names(device_name)
        )
    ...
    def _get_merge_node_names(self, device_name):
        ...
        if device_name not in self._merge_node_names:
            ...
            self._merge_node_names[device_name] = [
                node
                for node in debug_graph.node_op_types
    ...
    """
        ...
        if src_device_name != dst_device_name:
            raise ValueError(
                ...
                "%s vs. %s"
                % (src_node_name, dst_node_name, src_device_name, dst_device_name)
            )
    ...
    """
        if device_name is None:
            matching_device_names = [
                name
                for name in self._watch_key_to_datum
    ...
    """
        ...
        return [
            datum.file_path
            for datum in self._watch_key_to_datum[device_name][watch_key]
        ]
