...
DebugWatch = collections.namedtuple(
...
class EventListenerBaseStreamHandler(object):
    ...
    """
        raise NotImplementedError(
            "on_core_metadata_event() is not implemented in the base servicer " "class"
        )
...
class EventListenerBaseServicer(debug_service_pb2_grpc.EventListenerServicer):
        ...
        for event in request_iterator:
            ...
            if event.summary and event.summary.value:
                ...
                if event.graph_def:
                    ...
                    maybe_graph_def, maybe_device_name, maybe_wall_time = self._process_encoded_graph_def_in_chunks(
    ...
    """
        ...
        while not self._debug_ops_state_change_queue.empty():
            ...
            if (
                ...
                logging.info(
                    "Removing watchpoint or breakpoint: %s:%d:%s",
                    state_change.node_name,
                    state_change.output_slot,
                    state_change.debug_op,
                )
    ...
    def _process_graph_def(self, graph_def):
        for node_def in graph_def.node:
            if (
                debug_graphs.is_debug_node(node_def.name)
                and node_def.attr["gated_grpc"].b
            ):
                node_name, output_slot, _, debug_op = debug_graphs.parse_debug_node_name(
                    node_def.name
                )
    ...
    def request_watch(
        self, node_name, output_slot, debug_op, breakpoint=False
