...
def _send_call_tracebacks(
  ...
  """
    ...
    destinations = [
        ...
        if dest.startswith(common.GRPC_URL_PREFIX)
        else dest
    ...
    graph_traceback = (
        tfprof_logger.merge_default_with_oplog(graph, add_trainable_var=False)
        if graph
        else None
    )
...
def send_eager_tracebacks(destinations, origin_stack, send_source=True):
  ...
  """
    _send_call_tracebacks(
        destinations, origin_stack, is_eager_execution=True, send_source=send_source
    )
