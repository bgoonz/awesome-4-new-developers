...
class SendTracebacksTest(test_util.TensorFlowTestCase):
    ...
    def setUpClass(cls):
        ...
        (
            ...
            cls._server_dump_dir,
            cls._server_thread,
            cls._server,
        ) = grpc_debug_test_server.start_server_on_separate_thread(poll_server=True)
        cls._server_address = "localhost:%d" % cls._server_port
        (
            cls._server_port_2,
            cls._debug_server_url_2,
            cls._server_dump_dir_2,
    ...
    def testSendGraphTracebacksToTwoDebugServers(self):
        ...
        with session.Session() as sess:
            ...
            with test.mock.patch.object(
                grpc, "insecure_channel", wraps=grpc.insecure_channel
            ) as mock_grpc_channel:
                source_remote.send_graph_tracebacks(
    ...
    def testSendEagerTracebacksToSingleDebugServer(self):
        ...
        self.assertEqual(
            [debug_service_pb2.CallTraceback.EAGER_EXECUTION],
            self._server.query_call_types(),
        )
        self.assertIn(
            (self._curr_file_path, send_lineno, this_func_name),
