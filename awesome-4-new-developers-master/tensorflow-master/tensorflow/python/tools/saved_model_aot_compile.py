  ...
  """
    ...
    if sysconfig:
        ...
        if not os.path.exists(expected_header):
            logging.error(
                "Could not find includes path.  Missing file: {}".format(
                    expected_header
                )
            )
    ...
    return _XLA_MAKEFILE_TEMPLATE.format(
        ...
        cxx_flags="-D_GLIBCXX_USE_CXX11_ABI={}".format(versions.CXX11_ABI_FLAG),
    )
    ...
    for (k, v) in signature_def.inputs.items():
        ...
        if tensor_name not in node_names:
            logging.warn(
                ...
                "while freezing the graph.  Removing it from the compiled signatures.".format(
                    k, tensor_name
                )
            )
...
def aot_compile_cpu_meta_graph_def(
  ...
  """
    ...
    if signature_def_key not in signature_def_map:
        raise ValueError(
            ...
            "Available keys: {}".format(
                signature_def_key, list(signature_def_map.keys())
            )
        )
    ...
    with session.Session(graph=ops_lib.Graph()) as sess:
        ...
        graph_def.CopyFrom(
            graph_util.convert_variables_to_constants(
                ...
                variable_names_blacklist=[n.name for n, _ in variable_nodes_to_feed],
            )
        )
    ...
    entry_point = re.sub(
        "[^0-9a-zA-Z]+", "_", "__xla_" + output_prefix + "__" + cpp_class
    )
...
def _optimize_graph(meta_graph_def, signature_def):
    ...
    for tensor_info in list(signature_def.inputs.values()) + list(
    ...
    for name, input_ in signature_def.inputs.items():
        ...
        if tensor_name not in name_to_node_map:
            raise RuntimeError(
                ...
                "Graph nodes are: {}".format(tensor_name, list(name_to_node_map.keys()))
            )
        ...
        if node.op not in ("Placeholder", "PlaceholderV2"):
            logging.info(
                ...
                "but it doesn't look like a placeholder: {}".format(tensor_name, node)
            )
            continue
        ...
        if not shape.is_fully_defined():
            raise ValueError(
                ...
                "tensor name: '{}'; but shape is: {}.".format(name, tensor_name, shape)
            )
...
def _signature_to_tf2xla_config(signature_def, variable_nodes_to_feed):
  ...
  """
    from tensorflow.compiler.tf2xla import (
        tf2xla_pb2,
    )  # pylint: disable=g-import-not-at-top
