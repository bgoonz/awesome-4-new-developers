...
_FEATURE_COLUMNS = [
    ...
    fc_lib.IndicatorColumn,
    fc_lib.NumericColumn,
    ...
    init_ops.TruncatedNormal,
    sfc_lib.SequenceNumericColumn,
]
...
def deserialize_feature_column(config, custom_objects=None, columns_by_name=None):
  ...
  """
    ...
    new_instance = cls.from_config(  # pylint: disable=protected-access
        cls_config, custom_objects=custom_objects, columns_by_name=columns_by_name
    )
...
def deserialize_feature_columns(configs, custom_objects=None):
  ...
  """
    columns_by_name = {}
    return [
        deserialize_feature_column(c, custom_objects, columns_by_name) for c in configs
    ]
...
def _deserialize_keras_object(
    ...
    if isinstance(identifier, dict):
        ...
        raise ValueError(
            "Could not interpret serialized %s: %s"
...
def _class_and_config_for_serialized_keras_object(
    ...
    for key, item in cls_config.items():
        if isinstance(item, dict) and "__passive_serialization__" in item:
            ...
            _get_registered_object(item, custom_objects)
        ):
