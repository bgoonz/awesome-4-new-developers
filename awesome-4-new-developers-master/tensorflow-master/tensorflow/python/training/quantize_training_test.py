...
class PywrapQuantizeTrainingTest(test.TestCase):
    ...
    def testQuantizedSaveRestore(self):
        ...
        with session.Session(graph=g) as sess:
            ...
            b = variables.VariableV1(constant_op.constant(7.0, shape=[1, 1]), name="b")
            c = math_ops.matmul(a, b, name="matmul")
        ...
        with ops.Graph().as_default() as g, session.Session(graph=g) as sess:
            ...
            self.assertEqual(
                7.0, sess.run(g.get_tensor_by_name("b/read/Min/Variable:0"))
            )
            self.assertEqual(
                7.0, sess.run(g.get_tensor_by_name("b/read/Max/Variable:0"))
            )
