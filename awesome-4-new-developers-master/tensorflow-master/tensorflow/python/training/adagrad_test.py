...
class AdagradOptimizerTest(test.TestCase):
    def doTestBasic(
        self, use_locking=False, use_resource=False, use_callable_params=False
    ):
        ...
        with ops.Graph().as_default():
            for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
                with self.cached_session():
                    ...
                    self.evaluate(variables.global_variables_initializer())
                    self.assertAllClose(
                        aggregated_update_var, self.evaluate(repeated_index_update_var)
                    )
                    for _ in range(3):
                        repeated_update.run()
        ...
        with ops.Graph().as_default():
            for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
                with self.cached_session():
                    ...
                    update_op_repeated = adagrad.AdagradOptimizer(2.0).minimize(
                        loss_repeated
                    )
                    update_op_aggregated = adagrad.AdagradOptimizer(2.0).minimize(
                        loss_aggregated
                    )
                    self.evaluate(variables.global_variables_initializer())
                    self.assertAllCloseAccordingToType(
    ...
    def testDynamicShapeVariableWithCallableInit(self):
        with ops.Graph().as_default():
            var0 = variable_scope.get_variable(
                "var0", initializer=constant_op.constant(1.0), validate_shape=False
            )
            ...
            # Validate updated params
            v0_val = self.evaluate([var0])
            self.assertAllCloseAccordingToType(np.array([-1.6026098728179932]), v0_val)
