...
class EvaluateOnceTest(test.TestCase):
    ...
    def testEvaluatePerfectModel(self):
        checkpoint_dir = os.path.join(
            self.get_temp_dir(), "evaluate_perfect_model_once"
        )
        ...
        self.assertGreater(final_ops_values["accuracy"], 0.99)

    def testEvaluateWithFiniteInputs(self):
        checkpoint_dir = os.path.join(
            self.get_temp_dir(), "evaluate_with_finite_inputs"
        )
        ...
        inputs, labels = training.batch(
            [single_input, single_label], batch_size=6, allow_smaller_final_batch=True
        )
        ...
        self.assertTrue(final_ops_values["accuracy"] > 0.99)
