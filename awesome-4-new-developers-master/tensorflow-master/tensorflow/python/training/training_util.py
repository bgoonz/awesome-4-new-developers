...
def assert_global_step(global_step_tensor):
  ...
  """
    if not (
        isinstance(global_step_tensor, variables.Variable)
        or isinstance(global_step_tensor, ops.Tensor)
        or resource_variable_ops.is_resource_variable(global_step_tensor)
    ):
        raise TypeError(
    ...
    if (
        global_step_tensor.get_shape().ndims != 0
        and global_step_tensor.get_shape().is_fully_defined()
    ):
        raise TypeError(
            'Existing "global_step" is not scalar: %s' % global_step_tensor.get_shape()
        )
...
def _get_global_step_read(graph=None):
  ...
  """
    ...
    if len(global_step_read_tensors) > 1:
        raise RuntimeError(
            "There are multiple items in collection {}. "
  ...
  """
    ...
    with graph.as_default() as g, g.name_scope(None):
        with g.name_scope(global_step_tensor.op.name + "/"):
            ...
            global_step_value = (
                global_step_tensor.initialized_value()
