...
class SessionManagerTest(test.TestCase):
    ...
    def testPrepareSessionFails(self):
        ...
        with ops.Graph().as_default():
            ...
            checkpoint_filename = os.path.join(
                checkpoint_dir, "prepare_session_checkpoint"
            )
        ...
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                ...
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("v:0")
                ).eval(session=sess),
            )

    def _test_recovered_variable(
        self, checkpoint_dir=None, checkpoint_filename_with_path=None
    ):
    ...
    def testRecoverSessionWithReadyForLocalInitOp(self):
        ...
        checkpoint_dir = os.path.join(
            self.get_temp_dir(), "recover_session_ready_for_local_init"
        )
        try:
            gfile.DeleteRecursively(checkpoint_dir)
        except errors.OpError:
        ...
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                True,
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("w:0")
                ).eval(session=sess),
            )
            self.assertEqual(1, sess.run(v))
    ...
    def testRecoverSessionFailsStillRunsLocalInitOp(self):
        # Create a checkpoint.
        checkpoint_dir = os.path.join(
            self.get_temp_dir(), "recover_session_ready_for_local_init_fails_stil_run"
        )
        try:
            gfile.DeleteRecursively(checkpoint_dir)
        except errors.OpError:
    ...
    def testWaitForSessionInsufficientReadyForLocalInitCheck(self):
        ...
        with self.assertRaisesRegex(
            errors_impl.DeadlineExceededError, "Session was not ready after waiting.*"
        ):
            sm.wait_for_session("", max_wait_secs=3)
    ...
    def testPrepareSessionWithReadyForLocalInitOp(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                True,
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("x:0")
                ).eval(session=sess),
            )
            self.assertEqual(1, sess.run(v))
            self.assertEqual(1, sess.run(w))
    ...
    def testPrepareSessionWithPartialInitOp(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                False,
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("v_res:0")
                ).eval(session=sess),
            )
            self.assertEqual(
                True,
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("w_res:0")
                ).eval(session=sess),
            )
            self.assertEqual(
                True,
                variables.is_variable_initialized(
                    sess.graph.get_tensor_by_name("x_res:0")
                ).eval(session=sess),
            )
    ...
    def testPrepareSessionWithReadyNotReadyForLocal(self):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegex(
                RuntimeError, "Init operations did not make model ready for local_init"
            ):
...
class ObsoleteSessionManagerTest(test.TestCase):
    ...
    def testWaitForSessionReturnsNoneAfterTimeout(self):
        with ops.Graph().as_default():
            variables.VariableV1(1, name="v")
            sm = session_manager.SessionManager(
                ready_op=variables.assert_variables_initialized(), recovery_wait_secs=1
            )
