...
class CheckpointsTest(test.TestCase):
    ...
    def testInitFromPartitionVar(self):
        ...
        with ops.Graph().as_default() as g:
            with self.session(graph=g) as session:
                ...
                checkpoint_utils.init_from_checkpoint(
                    checkpoint_dir,
                    {"scope/var1": "some_scope/my1", "scope/": "some_other_scope/"},
                )
        ...
        with ops.Graph().as_default() as g:
            with self.session(graph=g) as session:
                ...
                checkpoint_utils.init_from_checkpoint(
                    checkpoint_dir, {"scope/var1": my1_var_list}
                )
    ...
    def testNoAdditionalReadOpsForResourceVariables(self):
        ...
        ops_in_init_from_checkpoint_scope = [
            ...
            if (
                op.name.startswith("init_from_checkpoint/")
                and not op.name.startswith(
                    "init_from_checkpoint/checkpoint_initializer"
                )
