  ...
  """
    ...
    def should_execute_combination(self, kwargs):
        ...
        if required_physical_gpus and required_gpus:
            raise ValueError(
                "Only one of `required_physical_gpus`(number of physical"
...
def _set_total_phsyical_gpus():
    if in_main_process():
        env().total_phsyical_gpus = len(context.context().list_physical_devices("GPU"))
...
def _multi_worker_test(test_method):
  ...
  """
    ...
    def decorator(self, has_chief, num_workers, num_ps, share_gpu, runner, **kwargs):
        if (
            _num_total_workers(has_chief, num_workers) == 1
            or _running_in_worker
            or (
