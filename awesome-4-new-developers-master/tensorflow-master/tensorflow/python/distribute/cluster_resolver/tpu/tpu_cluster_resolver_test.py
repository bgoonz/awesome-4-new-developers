...
from tensorflow.python.distribute.cluster_resolver.tpu import (
    tpu_cluster_resolver as resolver,
)
...
class TPUClusterResolverTest(test.TestCase):
    ...
    def testNewNetworkEndpointFormat(self):
        tpu_map = {
            "projects/test-project/locations/us-central1-c/nodes/test-tpu-1": {
                "state": "READY",
                "health": "HEALTHY",
                "networkEndpoints": [{"ipAddress": "10.2.3.4", "port": 8470}],
            }
        }
    ...
    def testPodResolution(self):
        tpu_map = {
            "projects/test-project/locations/us-central1-c/nodes/test-tpu-1": {
                ...
                "health": "HEALTHY",
                "networkEndpoints": [
                    {"ipAddress": "10.2.3.4", "port": 8470},
                    {"ipAddress": "10.2.3.5", "port": 8470},
                    {"ipAddress": "10.2.3.6", "port": 8470},
                    {"ipAddress": "10.2.3.7", "port": 8470},
                ],
            }
        }
    ...
    def testNoCallComputeMetadata(self):
        ...
        self.assertEqual(
            server_lib.ClusterSpec({"worker": ["10.1.2.3:8470"]}).as_dict(),
    ...
    def testGetDeviceDictAndCoresWithCPUsAndGPUs(self):
        ...
        device_dict, num_cores = resolver.TPUClusterResolver._get_device_dict_and_cores(
            device_list
        )
    ...
    @mock.patch.object(session.BaseSession, "list_devices")
    @mock.patch.object(resolver, "is_running_in_gce", mock_is_not_running_in_gce)
    def testNumAcceleratorsRetryFailure(
