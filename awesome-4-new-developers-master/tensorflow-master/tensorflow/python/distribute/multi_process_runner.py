...
TestEnvironment = collections.namedtuple(
    "TestEnvironment",
    [
        "task_type",
        "task_id",
...
class MultiProcessRunner(object):
  ...
  """
    ...
    def _start_subprocess_and_reading_thread(
        self, task_type, task_id, cluster_spec=None, fn=None, args=None, kwargs=None
    ):
    ...
    """
        with self._process_lock:
            ...
            if self._joined:
                raise ValueError(
                    "cannot start new processes after"
    ...
    """
        ...
        if self._watchdog_thread.is_alive():
            ...
            if self._watchdog_thread.is_alive():
                logging.error(
                    "Timeout when waiting for child processes to "
    ...
    """
        ...
        for (task_type, task_id), p in self._processes.items():
            if p.exitcode is not None:
                logging.info(
                    "%s-%d has already exited. Not terminating.", task_type, task_id
                )
                continue
            try:
                os.kill(p.pid, sig)
                self._terminated.add((task_type, task_id))
                logging.info(
                    "%s-%d terminated with signal %r.", task_type, task_id, sig
                )
            except ProcessLookupError:
                logging.info(
                    "Attempting to kill %s-%d but it does not exist.",
                    task_type,
                    task_id,
                )
...
class _ProcFunc(object):
    ...
    def __call__(self, resources, test_env, fn, args, kwargs, use_dill_for_args):
        ...
        with self._runtime_mode(test_env.executing_eagerly):
            info = _run_contained(
                test_env.task_type, test_env.task_id, fn, args, kwargs
            )
...
@tf_export(
    "__internal__.distribute.multi_process_runner" ".SubprocessTimeoutError", v1=[]
)
...
@tf_export(
    "__internal__.distribute.multi_process_runner" ".UnexpectedSubprocessExitError",
    v1=[],
)
