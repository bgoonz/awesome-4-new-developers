  ...
  """
    if (
        ...
        if isinstance(dataset, dataset_ops.DatasetV1):
            return distribute._AutoShardDatasetV1(
                dataset, num_shards, index, num_replicas_in_sync
            )
        else:
            return distribute._AutoShardDataset(
                dataset, num_shards, index, num_replicas_in_sync
            )
    else:
        return dataset
...
def _clone_helper(op_to_clone, variant_tensor_ops):
  ...
  """
    ...
    new_op = g.create_op(
        op_to_clone.type,
        inputs_list,
        [o.dtype for o in op_to_clone.outputs],
