    ...
    def configure(
        ...
        if session_config:
            if (
                ...
                and self.extended.experimental_between_graph
            ):
...
class DistributeCoordinatorTestBase(test.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._cluster_spec = {
            WORKER: [_strip_protocol(_bytes_to_str(w.target)) for w in cls._workers],
    ...
    def _create_cluster_spec(
        self, has_chief=False, num_workers=1, num_ps=0, has_eval=False
    ):
    ...
    def _run_multiple_coordinator_in_threads(
        self, worker_fn, strategy, cluster_spec, **kwargs
    ):
        threads = {}
    ...
    def _dump_strategy_property(self, strategy):
        ...
        self.assertEqual(
            context.should_save_summary, strategy.extended.should_save_summary
        )
...
class DistributeCoordinatorTestStandaloneMode(DistributeCoordinatorTestBase):
    ...
    def testBetweenGraphContextWithChief(self):
        ...
        # and distributed_mode.
        self.assertEqual(
            self._worker_context[CHIEF][0], ("grpc://fake_chief", 4, True, True)
        )
        self.assertEqual(
            self._worker_context[WORKER][0],
    ...
    def testInGraphContextWithEval(self):
        ...
        self.assertEqual(
            self._worker_context["None"][0],
            (_strip_protocol(_bytes_to_str(self._workers[0].target)), 3, True, True),
        )
...
class DistributeCoordinatorTestIndependentWorkerMode(DistributeCoordinatorTestBase):
    ...
    def testBetweenGraphContext(self):
        ...
        with test.mock.patch.object(
            distribute_coordinator, "_run_std_server", self._run_mock_std_server
        ):
            threads = self._run_multiple_coordinator_in_threads(
                self._dump_worker_context,
                MockStrategy(between_graph=True),
    ...
    def testRunStdServerInGoogleEnvironment(self):
        ...
        ), test.mock.patch.object(time, "sleep", _fake_sleep):
...
class StrategyConfigureTest(test.TestCase):
    ...
    def test_session_config_in_std_server(self):
        ...
        with test.mock.patch.dict(
            ...
            distribute_coordinator, "_run_std_server", self._dump_device_filters
        ):
            distribute_coordinator.run_distribute_coordinator(
                lambda _: None,
                MockStrategy(between_graph=True),
    ...
    def test_session_config_in_session_creator(self):
        ...
        distribute_coordinator._thread_local = (
            threading.local()
