...
class CollectiveOpsTest(test.TestCase, parameterized.TestCase):
    ...
    def testReduceDense(
        ...
        if num_processes == 2 and implementation == CommunicationImplementation.NCCL:
            self.skipTest(
                "Skip NCCL + 2 processes combination. NCCL requires "
    ...
    def testBatchReduceSparse(
        ...
        if group_size == 2:
            ...
            expect = [
                IndexedSlices(
                    values=[[1.0], [2.0], [5.0], [6.0], [9.0], [10.0], [13.0], [14.0]],
                    indices=[0, 1, 1, 2, 3, 4, 8, 9],
                    dense_shape=[10, 1],
                ),
                IndexedSlices(
                    values=[[3.0], [4.0], [7.0], [8.0], [11.0], [12.0], [15.0], [16.0]],
            ...
            def collective_all_reduce():
                ...
                for replica_id, device in enumerate(devices):
                    with ops.device(device):
                        ...
                        results.append(
                            collective._all_reduce(
                                reduce_op, value, replica_id, options
                            )
                        )
                return results
    ...
    def testMultiThreadedCollectiveLaunchNoInterleave(
        self, num_processes, required_gpus, implementation, prefer_unique_instance_key
    ):
        def replica_fn():
            CollectiveReplicaLauncher._prefer_unique_instance_key = (
                prefer_unique_instance_key
    ...
    def testTimeoutReduceDense(
        self, num_processes, implementation, required_gpus, prefer_unique_instance_key
    ):
