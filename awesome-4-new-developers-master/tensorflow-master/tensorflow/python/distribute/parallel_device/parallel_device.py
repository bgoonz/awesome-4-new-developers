...
class ParallelDevice(object):
    ...
    """
        ...
        with _next_device_number_lock:
            ...
            # special-cased).
            self._name = "{}/device:CUSTOM:{}".format(
    ...
    def _pack_tensor(self, *tensors):
        ...
        with ops.device(None):
            ...
            tensors = [
                t.read_value() if isinstance(t, variables.Variable) else t
                for t in tensors
            ]
    ...
    """
        ...
        return [
            nest.pack_sequence_as(parallel_tensor, unpacked, expand_composites=True)
