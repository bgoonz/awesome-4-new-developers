...
class _VirtualDeviceTestCase(test.TestCase):
    def setUp(self):
        ...
        if ctx.list_physical_devices("TPU"):
            ...
            ctx.set_logical_device_configuration(
                gpus[0],
                [
...
class ParallelDeviceTests(_VirtualDeviceTestCase, parameterized.TestCase):
    ...
    def test_one_replica_eager_control_flow(self):
        device = parallel_device.ParallelDevice(
            components=["/job:localhost/device:{}:0".format(self.device_type)]
        )
    ...
    def test_string_representation(self):
        x = self.device.pack(
            [constant_op.constant([5.0, 6.0]), constant_op.constant([6.0, 7.0])]
        )
        parallel_str = str(x)
    ...
    def test_generator(self):
        ...
        for same_component, different_component in zip(
            same_unpacked[1:], different_unpacked[1:]
        ):
    ...
    @parameterized.named_parameters([("RunFunctionsEagerly", True), ("", False)])
        ...
        try:
            ...
            with self.device:
                result = control_flow_ops.cond(
                    pred,
                    def_function.function(lambda: capture * 2.0),
                    def_function.function(lambda: capture * 4.0),
                )
            self.assertAllClose([[2.0], [8.0]], self.device.unpack(result))
        finally:
    ...
    def test_cond_with_variable(self):
        ...
        with self.device:
            ...
            def true_branch():
                nonlocal v
                if v is None:
                    v = variables.Variable(constant_op.constant(2.0))
                return v * capture

            result = control_flow_ops.cond(
                pred, true_branch, def_function.function(lambda: capture * 4.0)
            )
        self.assertAllClose([[2.0], [8.0]], self.device.unpack(result))
        self.assertAllClose([2.0, 2.0], self.device.unpack(v))
        # There are two unique variable handles with separate storage.
        h1, _ = self.device.unpack(v.handle)
        gen_resource_variable_ops.assign_variable_op(h1, constant_op.constant(3.0))
        self.assertAllClose([3.0, 2.0], self.device.unpack(v))
    ...
    def test_pack_composite(self):
        ...
        datasets = [
            dataset_ops.Dataset.from_tensor_slices([i + 1, (i + 1) * 2, (i + 1) * 3])
    ...
    def test_pack_structure(self):
        x_parts = [
            {"a": constant_op.constant(float(i))}
            for i in range(len(self.device.components))
        ]
    ...
    def test_saved_model(self):
        ...
        assign_value = self.device.pack(
            [constant_op.constant(0.1), constant_op.constant(0.2)]
        )
        with self.device:
    ...
    def test_different_shapes(self):
        x = self.device.pack(
            [constant_op.constant([1.0, 2.0]), constant_op.constant([5.0])]
        )
        with self.device:
            y = x * 2.0
        ...
        with self.assertRaisesRegex(
            Exception, "components do not all have the same rank"
        ):
