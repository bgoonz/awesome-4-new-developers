...
class CoordinatedClosureQueueTest(test.TestCase):
    ...
    def _test_cancel_closure_when_error(self, call_wait):
        ...
        def fake_cancellation():
            self._set_error(
                closure_queue, closure2, ValueError("Fake cancellation error.")
            )
        ...
        self.assertEqual(
            closure2.output_remote_value._status, values_lib.RemoteValueStatus.READY
        )
...
def make_coordinator(num_workers, num_ps):
    ...
    cluster_def["chief"] = ["localhost:%d" % multi_worker_test_base.pick_unused_port()]
    cluster_resolver = SimpleClusterResolver(ClusterSpec(cluster_def), rpc_layer="grpc")
    strategy = parameter_server_strategy_v2.ParameterServerStrategyV2(cluster_resolver)
...
class ClusterCoordinatorTest(TestCaseWithErrorReportingThread, parameterized.TestCase):
    ...
    def testDatasetsShuffledDifferently(self, use_input_fn):
        ...
        elements_in_iterator_1 = [
            self.strategy.experimental_local_results(e) for e in iterator_1
        ]
        ...
        elements_in_iterator_2 = [
            self.strategy.experimental_local_results(e) for e in iterator_2
        ]
...
class StrategyIntegrationTest(test.TestCase, parameterized.TestCase):
    ...
    def testRunNotUsedWithClusterCoordinatorSchedule(self):
        ...
        self.assertAllEqual(
            tuple(expected_result), self.strategy.experimental_local_results(rv.fetch())
        )
    ...
    def testBasicVariableAssignment(self):
        ...
        logging.info(
            "Results of experimental_run_v2: %f", self.coordinator.fetch(results)
        )
    ...
    def testRunAndReduce(self):
        ...
        with self.strategy.scope():
            ...
            expected_result = (
                4.0 * self.strategy.num_replicas_in_sync,
                2.0 * self.strategy.num_replicas_in_sync,
            )
            ...
            result = self.coordinator.schedule(
                worker_fn, args=(constant_op.constant(3.0),)
            )
    ...
    def testVariableAggregation(self):
        ...
        with self.strategy.scope():
            ...
            v = variables.Variable(
                initial_value=1.0, aggregation=variable_scope.VariableAggregation.SUM
            )
    ...
    def testAsyncScheduleWithDistributedDataset(self):
        def input_fn():
            dataset = (
                dataset_ops.DatasetV2.from_tensor_slices([2.0])
                .repeat()
                .batch(self.strategy.num_replicas_in_sync)
            )
            return self.strategy.experimental_distribute_dataset(dataset)
    ...
    def testInputFunctionWithMapWithDistributedDataset(self):
        ...
        def input_fn():
            ...
            dataset = (
                dataset_ops.DatasetV2.range(0, 10)
                .batch(self.strategy.num_replicas_in_sync)
                .map(map_fn)
            )
            return self.strategy.experimental_distribute_dataset(dataset)
        ...
        expected_result = array_ops.split(
            math_ops.range(10.0, 10.0 + self.strategy.num_replicas_in_sync),
            num_or_size_splits=self.strategy.num_replicas_in_sync,
            axis=0,
        )
    ...
    def testPerWorkerDistributeDatasetsElementSpec(self):
        ...
        self.assertAllEqual(
            ...
            input_lib._create_distributed_tensor_spec(
                self.strategy, dataset.element_spec
            ),
