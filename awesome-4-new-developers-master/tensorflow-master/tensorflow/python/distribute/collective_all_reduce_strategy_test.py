...
def create_test_objects(cluster_spec=None, task_type=None, task_id=None, num_gpus=None):
...
class CollectiveAllReduceStrategyTestBase(multi_worker_test_base.MultiWorkerTestBase):
    ...
    def _test_minimize_loss_graph(self, task_type, task_id, num_gpus):
        d, master_target, config = self._get_test_object(task_type, task_id, num_gpus)
    ...
    def _test_variable_initialization(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
            ...
            self.assertTrue(
                ...
                msg=("x_value = %r, reduced_x_value = %r" % (x_value, reduced_x_value)),
            )
...
class DistributedCollectiveAllReduceStrategyTest(
    ...
    def test_num_replicas_in_sync(self):
        ...
        num_workers = len(
            self._cluster_spec.get("chief", []) + self._cluster_spec.get("worker", [])
        )
    ...
    def testMakeInputFnIterator(self, required_gpus, use_dataset):
        def _worker_fn(task_type, task_id, required_gpus):
            ...
            expected_values = [
                [i + j for j in range(devices_per_worker)]
...
class LocalCollectiveAllReduceStrategy(
    ...
    @combinations.generate(
        combinations.combine(mode=["graph"], required_gpus=2, use_dataset=[True, False])
    )
    def testMakeInputFnIterator(self, required_gpus, use_dataset):
        if use_dataset:
...
class CollectiveAllReduceStrategyV2Test(test.TestCase, parameterized.TestCase):
    def test_replica_id_in_sync_group(self, strategy):
        ...
        self.assertAllEqual(
            list(range(strategy.extended._num_replicas_in_sync)), results[0].numpy()
        )
        self.assertAllEqual(
...
class ExperimentalCompatibilityTest(test.TestCase):
    ...
    def testName(self):
        ...
        self.assertEqual(
            CollectiveAllReduceStrategy.__name__, "CollectiveAllReduceStrategy"
        )
        self.assertEqual(
