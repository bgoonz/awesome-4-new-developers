...
def build_ring_all_reduce(
    input_tensors, num_workers, num_subchunks, gpu_perm, red_op, un_op=None
):
  ...
  """
    ...
    chunks_by_dev, pad_len = _build_ring_gather(
        input_tensors, devices, num_subchunks, pred_by_s_d, rank_by_s_d, red_op
    )
    if un_op:
  ...
  """
    ...
    for h in reversed(range(0, num_hops)):
        ...
        for d in range(0, num_devices):
            ...
            with ops.device(left_dev):
                new_chunks[left_idx] = array_ops.concat(
                    [chunks[left_idx], chunks[right_idx]], 0
                )
            with ops.device(right_dev):
                new_chunks[right_idx] = array_ops.concat(
                    [chunks[left_idx], chunks[right_idx]], 0
                )
        chunks = new_chunks
    return chunks
...
def _build_shuffle_hybrid(input_tensors, gather_devices, red_op, upper_level_f):
  ...
  """
    ...
    if len(gather_devices) != num_workers:
        raise ValueError(
            "For shuffle hybrid, gather_devices must contain one " "device per worker. "
        )
    for w in range(0, num_workers):
...
def build_shuffle_then_ring(
    input_tensors, gather_devices, subdiv, red_n_op, red_op, un_op=None
):
...
def build_shuffle_then_shuffle(
