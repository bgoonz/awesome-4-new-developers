...
class CollectiveReplicaLauncher(object):
    ...
    def __init__(self, group_key, group_size, collective_keys, device):
    ...
    def all_reduce(
        self, input_tensor, control_input=None, communication_hint="AUTO", timeout=0
    ):
    ...
    def batch_all_reduce(
        self, input_tensor_packs, communication_hint="AUTO", timeout=0
    ):
            ...
            if context.executing_eagerly():
                ...
                with ops.device(self._device):
                    ...
                    reduced = self.all_reduce(
                        array_ops.concat(flat_tensors, axis=0),
                        control_input,
                        communication_hint,
                        timeout,
                    )
    ...
    def all_gather(self, input_tensor, axis, communication_hint="AUTO", timeout=0):
        """All-gather a dense tensor.
    ...
    """
        ...
        with ops.device(self._device), ops.control_dependencies(
            [array_ops.identity(input_tensor)]
        ):
            # 1. Transpose
            ...
            for i in range(self._group_size):
                ...
                split_tensors.append(
                    gather_padded_out_tensor[start_pos : start_pos + first_dims[i]]
                )
    ...
    def all_reduce_indexed_slices(
        self, input_slices, communication_hint="AUTO", timeout=0
    ):
    ...
    """
        ...
        with ops.device(self._device):
            ...
            all_lengths = self._all_gather(length, communication_hint, timeout=timeout)
            ...
            def all_gather_with_padding(input_tensor, communication_hint, timeout):
                ...
                for i in range(self._group_size):
                    ...
                    split_tensors.append(
                        all_padded_tensors[start_pos : start_pos + all_lengths[i]]
                    )
                return array_ops.concat(split_tensors, 0)

            return control_flow_ops.cond(
                math_ops.equal(
                    math_ops.reduce_max(all_lengths), math_ops.reduce_min(all_lengths)
                ),
...
def divide_by_n_tensors_or_indexed_slices(value, n):
    if isinstance(value, ops.IndexedSlices):
        ...
        return ops.IndexedSlices(value.values / n, value.indices, value.dense_shape)
    else:
        return value / n
