...
class MetricsV1Test(test.TestCase, parameterized.TestCase):
    ...
    def testMeanPerClassAccuracy(self, distribution):
        ...
        def _expected_fn(num_batches):
            ...
            return [
                ...
                mean([0.5, 0.5, 0.5, 0.0, 0.0]),
                mean([1.0 / 3, 1.0 / 3, 0.5, 0.0, 0.0]),
                mean([0.5, 1.0 / 3, 1.0 / 3, 0.0, 0.0]),
            ][num_batches - 1]
    ...
    def testMeanIOU(self, distribution):
        ...
        def _expected_fn(num_batches):
            mean = lambda x: sum(x) / len(x)
            return [
                mean([1.0 / 2, 1.0 / 1, 1.0 / 1, 0.0]),  # no class 4 in first batch
                mean([1.0 / 4, 1.0 / 4, 1.0 / 3, 0.0, 0.0]),
                mean([1.0 / 6, 1.0 / 6, 1.0 / 5, 0.0, 0.0]),
                mean([2.0 / 8, 1.0 / 7, 1.0 / 7, 0.0, 0.0]),
            ][num_batches - 1]
