...
class DistributedVariableTest(test.TestCase, parameterized.TestCase):
    def testExtendsVariable(self, distribution, synchronization, aggregation):
        with distribution.scope():
            v = variables_lib.Variable(
                1.0, synchronization=synchronization, aggregation=aggregation
            )
    ...
    def testCheckpointing(self, distribution, synchronization, aggregation, mode):
        ...
        with distribution.scope():
            v = variables_lib.Variable(
                constant_op.constant([1.0, 2.0, 3.0, 4]),
        ...
        self.evaluate(v.assign(constant_op.constant([4.0, 3.0, 2.0, 1.0])))
    ...
    def testIsTensorLike(self, distribution, synchronization, aggregation):
        if (
            isinstance(distribution.extended, tpu_strategy.TPUExtended)
        ...
        with distribution.scope():
            v = variables_lib.Variable(
                0.0, synchronization=synchronization, aggregation=aggregation
            )
    ...
    def testAssignReturnValueIsTensorLike(
        self, distribution, synchronization, aggregation
    ):
    ...
    def testStrategyExtendedUpdate(self, distribution, synchronization, aggregation):
        ...
        read_assign_fn = lambda var, value: var.assign_add(
            var.value() + var.read_value()
        )
    ...
    def testSaveNonDistributed(self, distribution, synchronization, aggregation):
        ...
        if isinstance(
            ...
            self.skipTest(
                "b/148689177: AggregatingVariable doesn't "
        ...
        _test(
            lambda: check_ops.assert_equal_v2(
                v.get_shape(), tensor_shape.TensorShape(())
            ),
            v,
        )
        ...
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_add(_make_index_slices(values=[1.0, 2.0], indices=[0, 2])),
                [1.0, 0.0, 2.0],
            ),
            w,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_div(_make_index_slices(values=[4.0, 2.0], indices=[0, 2])),
                [0.25, 0.0, 1.0],
            ),
            w,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_max(_make_index_slices(values=[1.0, 0.5], indices=[1, 2])),
                [0.25, 1.0, 1.0],
            ),
            w,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_min(_make_index_slices(values=[1.0, 0.5], indices=[0, 1])),
                [0.25, 0.5, 1.0],
            ),
            w,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_mul(_make_index_slices(values=[2.0, 0.5], indices=[0, 1])),
                [0.5, 0.25, 1.0],
            ),
            w,
        )
        ...
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_sub(_make_index_slices(values=[2.0, 0.5], indices=[0, 1])),
                [-1.5, -0.25, 1.0],
            ),
            w,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                w.scatter_update(_make_index_slices(values=[2.0, 0.5], indices=[0, 1])),
                [2.0, 0.5, 1.0],
            ),
            w,
        )
        ...
        _test(
            lambda: check_ops.assert_equal_v2(
                math_ops.cast(v / 2.0, dtypes.float32), 3.5
            ),
            v,
        )
        _test(
            lambda: check_ops.assert_equal_v2(
                math_ops.cast(14.0 / v, dtypes.float32), 2.0
            ),
            v,
        )
        ...
        if isinstance(distribution.extended, tpu_strategy.TPUExtended):
            ...
            _test(
                lambda: check_ops.assert_equal_v2(w[0].assign(1.0), [1.0, 0.5, 1.0]), w
            )
    ...
    def testUnsaveable(self, distribution, synchronization, aggregation, mode):
        ...
        with distribution.scope():
            v = variables_lib.Variable(
                [1.0, 1.0], synchronization=synchronization, aggregation=aggregation
            )
        ...
        if synchronization == variables_lib.VariableSynchronization.ON_READ and (
            ...
            or (not distribution.extended._use_merge_call())
            or (
                isinstance(
                    distribution.extended,
