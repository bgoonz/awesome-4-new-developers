...
def regroup(values, wrap_class=values_lib.PerReplica, always_wrap=False):
  ...
  """
    ...
    if isinstance(v0, abc.Mapping):
        ...
        return type(v0)(
            {
                key: regroup(tuple(v[key] for v in values), wrap_class, always_wrap)
                for key in v0keys
            }
        )
    ...
    if hasattr(v0, "_distributed_container"):
        ...
        assert not isinstance(
            v0, values_lib.MirroredVariable
...
def select_replica(replica_id, structured):
    ...
    def _get(x):
        ...
        if isinstance(x, values_lib.DistributedVariable) or not isinstance(
            x, values_lib.DistributedValues
        ):
            return x
        else:
...
def update_regroup(extended, updates, group):
    ...
    if not group:
        ...
        return nest.map_structure(
            extended._local_results, regrouped
        )  # pylint: disable=protected-access
...
def _validate_synchronization(kwargs):
    ...
    if synchronization not in (
        vs.VariableSynchronization.ON_READ,
...
def create_mirrored_variable(
    strategy, real_mirrored_creator, class_mapping, policy_mapping, **kwargs
):
