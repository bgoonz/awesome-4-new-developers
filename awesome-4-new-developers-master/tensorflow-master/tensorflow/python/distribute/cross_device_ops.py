...
class AllReduceCrossDeviceOps(CrossDeviceOps):
  ...
  """
    ...
    def _batch_all_reduce(self, reduce_op, per_replica_values):
        ...
        dense_values, dense_indices, sparse_values, sparse_indices = cross_device_utils.split_by_sparsity(
            per_replica_values
        )
        if dense_values:
...
class CollectiveAllReduce(CrossDeviceOps):
  ...
  """
    ...
    def __init__(
        self, devices, group_size, collective_keys=None, canonicalize_devices=True
    ):
        """Initializes the object.
    ...
    def _all_reduce(self, reduce_op, value, replica_id, options):
        ...
        if (
            self._limited_nccl
            and options.implementation == CommunicationImplementation.NCCL
        ...
        if dense_values:
            ...
            dense_results = launcher.batch_all_reduce(
        ...
        if sparse_values:
            ...
            for indexed_slice in sparse_values:
                sparse_results.append(
                    launcher.all_reduce_indexed_slices(
                        indexed_slice, implementation, options.timeout_seconds
                    )
                )
    ...
    def _batch_all_gather(self, per_replica_values, axis, options):
        ...
        # is NCCL.
        if (
            options.implementation == CommunicationImplementation.NCCL
            and batch_size == 1
        ):
            implementation = CommunicationImplementation.AUTO.value
        ...
        logging.log_first_n(
            ...
            % (batch_size, len(self._devices), self._group_size, implementation),
            10,
        )
    ...
    def __deepcopy__(self, memo):
        ...
        return CollectiveAllReduce(
            self._devices, self._group_size, collective_keys, self._canonicalize_devices
        )
