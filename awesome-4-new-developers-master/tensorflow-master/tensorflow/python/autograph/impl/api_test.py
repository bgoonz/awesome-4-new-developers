...
class ApiTest(test.TestCase):
    ...
    def assertNoMemoryLeaks(self, f):
        ...
        self.assertEmpty(tuple(o for o in objects_after if isinstance(o, TestResource)))
    ...
    def test_do_not_convert_argspec(self):
        ...
        # arg spec.
        self.assertEqual((), tuple(function_utils.fn_args(tc.test_method_allowlisted)))
    ...
    def test_converted_call_functools_partial_kwarg_mutation(self):
        ...
        x = api.converted_call(
            partial_fn,
            args=(),
            kwargs={"y": constant_op.constant(-2)},
            options=DEFAULT_RECURSIVE,
        )
        self.assertEqual((1, 2, 3), self.evaluate(x))
    ...
    def test_converted_call_already_converted(self):
        ...
        x = api.converted_call(
            converted_f, (constant_op.constant(0),), None, options=DEFAULT_RECURSIVE
        )
        self.assertTrue(self.evaluate(x))
    ...
    def test_converted_call_no_user_code(self):
        ...
        x = api.converted_call(len, (constant_op.constant([0]),), None, options=opts)
