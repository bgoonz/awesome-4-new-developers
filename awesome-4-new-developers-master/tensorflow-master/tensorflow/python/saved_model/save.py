...
_CapturedTensor = collections.namedtuple(
    "_CapturedTensor", ["name", "concrete_function"]
)
  ...
  """
    ...
    def _add_function_to_graph(self, function):
        ...
        if not concrete_functions:
            self._untraced_functions.append(
                function._name
            )  # pylint: disable=protected-access
    ...
    def fill_object_graph_proto(self, proto):
        ...
        for node_id, node in enumerate(self.nodes):
            ...
            for local_name, (save_fn, restore_fn) in self._saveable_objects_map[
                node
            ].items():
    ...
    """
        ...
        for node_id, obj in enumerate(self.nodes):
            if isinstance(obj, tracking.Asset):
                ...
                node_object_map, node_resource_map = obj._map_resources(
                    self._options
                )  # pylint: disable=protected-access
        ...
        for concrete_function in self.concrete_functions:
            ...
            for capture in concrete_function.captured_inputs:
                if (
                    tensor_util.is_tf_type(capture)
                    ...
                    if hasattr(capture, "_cached_variable"):
                        if concrete_function not in self._wrapped_functions:
                            wrapped = self._wrapped_functions[
                                concrete_function
...
def _generate_signatures(signature_functions, resource_map):
  ...
  """
    ...
    for signature_key, function in sorted(signature_functions.items()):
        ...
        mapped_inputs, exterior_argument_placeholders = _map_function_arguments_to_created_inputs(
...
def _iterate_op_types(fn):
    ...
    if isinstance(
        fn, framework_fn._DefinedFunction
    ):  # pylint: disable=protected-access
        ...
        for op_type, op in _iterate_op_types(fn):
            ...
            for capture in grad_fn.captured_inputs:
                ...
                if outer_fn is None or isinstance(outer_capture, ops.EagerTensor):
                    ...
                    saveable_view.captured_tensor_node_ids[
                        capture
                    ...
                    if isinstance(
                        outer_fn, defun._EagerDefinedFunction
                    ):  # pylint:disable=protected-access
                        try:
...
def _write_object_proto(obj, proto, asset_file_def_index, function_name_map):
    ...
    if registered_name:
        ...
        proto.serialized_user_proto.Pack(
            obj._serialize_to_proto()
        )  # pylint: disable=protected-access
...
def save_and_return_nodes(
    obj, export_dir, signatures=None, options=None, experimental_skip_checkpoint=False
):
  ...
  """
    ...
    _, exported_graph, object_saver, asset_info, saved_nodes, node_paths = _build_meta_graph(
...
def _build_meta_graph_impl(obj, signatures, options, meta_graph_def=None):
    ...
    saveable_view = _SaveableView(checkpoint_graph_view, options, wrapped_functions)
