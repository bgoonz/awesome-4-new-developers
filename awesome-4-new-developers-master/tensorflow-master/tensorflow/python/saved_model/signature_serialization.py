...
def _get_signature(function):
    if (
        ...
        function = (
            function._get_concrete_function_garbage_collected()
...
def canonicalize_signatures(signatures):
    ...
    for signature_key, function in signatures.items():
        ...
        wrapped_functions[
            original_function
        ] = signature_function = wrapped_functions.get(
            original_function
        ) or function_serialization.wrap_cached_variables(
            original_function
        )
        ...
        for keyword, inp in zip(
            signature_function._arg_keywords, inputs  # pylint: disable=protected-access
        ):
...
def validate_saveable_view(saveable_view):
    ...
    for name, dep in saveable_view.list_dependencies(saveable_view.root):
