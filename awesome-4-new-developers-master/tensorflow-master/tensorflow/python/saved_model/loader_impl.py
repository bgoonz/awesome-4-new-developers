...
def parse_saved_model_with_debug_info(export_dir):
  ...
  """
    ...
    debug_info_path = os.path.join(
        saved_model_utils.get_debug_dir(export_dir), constants.DEBUG_INFO_FILENAME_PB
    )
  ...
  """
    ...
    for asset_proto in asset_protos:
        ...
        asset_tensor_dict[tensor_name] = os.path.join(
            compat.as_bytes(assets_directory), compat.as_bytes(asset_proto.filename)
        )
...
def _get_op_from_signature_def(meta_graph_def, op_signature_key, import_scope):
    ...
    if op_signature_key in meta_graph_def.signature_def:
        return signature_def_utils.load_op_from_signature_def(
            meta_graph_def.signature_def[op_signature_key],
            op_signature_key,
            import_scope,
        )
    else:
...
def get_init_op(meta_graph_def, import_scope=None):
    return (
        _get_op_from_signature_def(
            meta_graph_def, constants.INIT_OP_SIGNATURE_KEY, import_scope
        )
        or _get_op_from_collection(meta_graph_def, constants.MAIN_OP_KEY)
        or _get_op_from_collection(meta_graph_def, constants.LEGACY_INIT_OP_KEY)
    )
...
class SavedModelLoader(object):
    ...
    """
        with sess.graph.as_default():
            if saver is None and not variables._all_saveable_objects(
                scope=import_scope
