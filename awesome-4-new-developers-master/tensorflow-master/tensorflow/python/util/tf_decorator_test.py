...
class TfDecoratorTest(test.TestCase):
    def testInitCapturesTarget(self):
        self.assertIs(
            test_function, tf_decorator.TFDecorator("", test_function).decorated_target
        )

    def testInitCapturesDecoratorName(self):
        self.assertEqual(
            "decorator name",
            tf_decorator.TFDecorator("decorator name", test_function).decorator_name,
        )

    def testInitCapturesDecoratorDoc(self):
        self.assertEqual(
            "decorator doc",
            tf_decorator.TFDecorator("", test_function, "decorator doc").decorator_doc,
        )
    ...
    def testInitCapturesNonNoneArgspec(self):
        ...
        self.assertIs(
            argspec,
            tf_decorator.TFDecorator("", test_function, "", argspec).decorator_argspec,
        )

    def testInitSetsDecoratorNameToTargetName(self):
        self.assertEqual(
            "test_function", tf_decorator.TFDecorator("", test_function).__name__
        )
    ...
    def testDocstringOnBoundProperty(self):
        self.assertEqual(
            "Return parameters.", TestDecoratedClass().return_params.__doc__
        )
...
class TfMakeDecoratorTest(test.TestCase):
    ...
    def testSetsTFDecoratorNameToDecoratorNameArg(self):
        decorated = tf_decorator.make_decorator(
            test_function, test_wrapper, "test decorator name"
        )
        decorator = getattr(decorated, "_tf_decorator")
    ...
    def testSetsTFDecoratorArgSpec(self):
        ...
        decorated = tf_decorator.make_decorator(
            test_function, test_wrapper, "", "", argspec
        )
        decorator = getattr(decorated, "_tf_decorator")
...
class TfDecoratorUnwrapTest(test.TestCase):
    ...
    def testUnwrapReturnsDecoratorListFromOutermostToInnermost(self):
        ...
        self.assertEqual(
            "test_decorator_increment_first_int_arg", decorators[1].decorator_name
        )
    ...
    def testUnwrapBoundMethods(self):
        ...
        self.assertEqual(
            "test_decorator_increment_first_int_arg", decorators[0].decorator_name
        )
