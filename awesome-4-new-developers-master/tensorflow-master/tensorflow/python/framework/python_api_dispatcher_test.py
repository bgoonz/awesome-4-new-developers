...
class PythonAPIDispatcherTest(test_util.TensorFlowTestCase):
    ...
    def testMultipleDispatchers(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Multiple dispatch targets .*" r"match the arguments to tf\.foo"
        ):
            dispatcher.Dispatch((rt, rt), None)

    def testListAndUnionDispatch(self):
        dispatcher = dispatch.PythonAPIDispatcher(
            "tf.foo", ["x", "ys", "name"], (None,)
        )
        ...
        self.assertEqual(
            dispatcher.Dispatch((), {"x": rt, "ys": [t], "name": "x"}), "f1"
        )
