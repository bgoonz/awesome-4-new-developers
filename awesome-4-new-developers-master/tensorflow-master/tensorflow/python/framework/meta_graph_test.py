...
class SimpleMetaGraphTest(test.TestCase):
    ...
    def testStrippedOpListNestedFunctions(self):
        with self.cached_session():
            ...
            op_list = meta_graph.stripped_op_list_for_graph(
                ops.get_default_graph().as_graph_def()
            )
    ...
    def testDefaultAttrStrippingUnregisteredOps(self):
        ...
        with self.cached_session():
            ...
            node_def = test_util.get_node_def_from_graph(
                "node_with_unreg_op", meta_graph_def.graph_def
            )
...
class ScopedMetaGraphTest(test.TestCase):
    def _testScopedExport(self, test_dir, exported_filenames):
        ...
        with graph.as_default():
            ...
            self.assertEqual(
                ["hidden1/biases:0", "hidden1/weights:0"], sorted(var_names)
            )
    ...
    def _testScopedImport(self, test_dir, exported_filenames):
        ...
        self.assertEqual(
            ["new_hidden1/biases:0", "new_hidden1/weights:0"], sorted(new_var_names)
        )
        ...
        new_var_names = [v.name for _, v in var_list.items()]
        self.assertEqual(
            ["new_hidden2/biases:0", "new_hidden2/weights:0"], sorted(new_var_names)
        )
        ...
        new_meta_graph3, var_list = meta_graph.export_scoped_meta_graph(
            graph=graph, export_scope="new_softmax_linear", unbound_inputs_col_name=None
        )
        self.assertEqual(["biases:0", "weights:0"], sorted(var_list.keys()))
    ...
    def testImportWhileLoopInWhileLoop(self):
        ...
        with ops.Graph().as_default():
            ...
            _, x = control_flow_ops.while_loop(
                lambda i, x: i < 2, body, [0, 0.0], name=""
            )
            with session.Session() as sess:
    ...
    def testScopedImportUnderNameScope(self):
        ...
        with graph.as_default():
            with ops.name_scope("foo"):
                ...
                self.assertEqual(
                    list(imported_variables.values())[0].name, "foo/bar/myvar:0"
                )
    ...
    def testImportsUsingSameScopeName(self):
        ...
        with ops.Graph().as_default():
            for suffix in ["", "_1"]:
                ...
                self.assertEqual(
                    list(imported_variables.values())[0].name, "s" + suffix + "/v:0"
                )
    ...
    def testScopedImportWithSelectedCollections(self):
        ...
        def _test_import(include_collection_keys, omit_collection_keys):
            ...
            def _restore_collections_predicate(collection_key):
                return (
                    collection_key in include_collection_keys
                    and collection_key not in omit_collection_keys
                )
    ...
    def _testScopedImportWithQueue(
        self, test_dir, exported_filename, new_exported_filename
    ):
        graph = ops.Graph()
        meta_graph.import_scoped_meta_graph(
    ...
    def doTestExportNestedNames(self, use_resource=False):
        ...
        var_names = [v.name for _, v in var_list.items()]
        self.assertEqual(
            ["hidden3/biases:0", "hidden3/weights:0"], sorted(var_list.keys())
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ["hidden3/biases:0", "hidden3/weights:0"], sorted(new_var_list.keys())
        )
    ...
    def testPotentialCycle(self):
        ...
        with graph2.as_default():
            ...
            meta_graph.import_scoped_meta_graph(
                ...
                import_scope="new_hidden1",
                input_map={
                    "$unbound_inputs_MatMul": constant_op.constant(4.0, shape=[2, 2])
                },
            )
        ...
        with graph1.as_default():
            with ops.device("/device:CPU:0"):
                a = variables.Variable(
                    constant_op.constant(1.0, shape=[2, 2]), name="a"
                )
            with ops.device("/job:ps/replica:0/task:0/device:GPU:0"):
                b = variables.Variable(
                    constant_op.constant(2.0, shape=[2, 2]), name="b"
                )
...
class ExportImportAcrossScopesTest(test.TestCase):
    ...
    """
        ...
        exported_meta_graph_def = meta_graph.export_scoped_meta_graph(
            graph=original_graph, export_scope="dropA/dropB"
        )[0]

        with ops.Graph().as_default() as imported_graph:
            meta_graph.import_scoped_meta_graph(
                exported_meta_graph_def, import_scope="importA"
            )
