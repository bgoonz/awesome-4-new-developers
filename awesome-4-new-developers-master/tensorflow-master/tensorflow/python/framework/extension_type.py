  ...
  """
    ...
    def __setattr__(self, name, value):
        if name in _MUTABLE_KERAS_PROPERTIES or (
            hasattr(self, _IN_CONSTRUCTOR) and self._tf_extension_type_has_field(name)
        ):
    ...
    def _type_spec(self):  # CompositeTensor API.
        ...
        if self._tf_extension_type_cached_type_spec is None:
            ...
            self.__dict__["_tf_extension_type_cached_type_spec"] = self.Spec.from_value(
                self
            )
...
def pack(value):
  ...
  """
    ...
    spec = value._type_spec._tf_extension_type_with_packed(
        True
...
def is_packed(value):
    ...
    if not isinstance(value, ExtensionType):
        raise ValueError(
            f"Expected `value` to be an object of type ExtensionType,"
...
class ExtensionTypeSpec(type_spec.TypeSpec):
    ...
    def _to_components(self, value):  # TypeSpec API.
        if self._tf_extension_type_is_packed:
            return (
                value._tf_extension_type_packed_variant
            )  # pylint: disable=protected-access
        ...
        return tuple(
            x for x in nest.flatten(value_tuple) if isinstance(x, tensor_or_composite)
        )
    ...
    def _tf_extension_type_fields(cls):
        return (
            cls.value_type._tf_extension_type_fields()
        )  # pylint: disable=protected-access
...
def _deserialize_for_reduce(value_type, serialization):
    return value_type.Spec._deserialize(
        serialization
    )  # pylint: disable=protected-access
...
def _change_nested_mappings_to(value, new_type):
    ...
    if isinstance(value, (dict, immutable_dict.ImmutableDict)):
        return new_type(
            [(k, _change_nested_mappings_to(v, new_type)) for (k, v) in value.items()]
        )
...
def _check_field_annotations(cls):
    ...
    for name in cls.__dict__:
        if extension_type_field.ExtensionTypeField.is_reserved_name(name):
            raise ValueError(
                f"The field annotations for {cls.__name__} are invalid. "
...
def _convert_anonymous_fields(value, for_spec=False):
    ...
    raise ValueError(
        f"Cannot convert anonymous fields from "
