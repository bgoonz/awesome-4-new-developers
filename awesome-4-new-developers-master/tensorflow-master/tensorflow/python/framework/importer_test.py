...
class ImportGraphDefTest(test.TestCase):
    ...
    """
        ...
        with ops.Graph().as_default():
            ...
            a, b = importer.import_graph_def(
                graph_def, return_elements=["A", "B"], name=""
            )
            ...
            a3, b3 = importer.import_graph_def(
                graph_def, return_elements=["A", "B"], name="A"
            )
          ...
          """
            ...
            outer, inner, new_c, outer_inner, outer_inner_c = importer.import_graph_def(
                self._MakeGraphDef(
                ...
                return_elements=["outer", "inner", "c", "outer/inner", "outer/inner/c"],
                name="",
            )
    ...
    def testResources(self):
        ...
        with ops.Graph().as_default():
            ...
            imported_var, imported_assign, imported_shape, imported_init = importer.import_graph_def(
                graph_def,
                return_elements=[var.name, var_assign.name, var_shape.name, init.name],
            )
    ...
    def testImportWhileLoopInCond(self):
        ...
        with ops.Graph().as_default():
            ...
            out = control_flow_ops.cond(pred, ImportFn, lambda: constant_op.constant(1))
            with self.cached_session() as sess:
    ...
    def testTypeMismatchInGraphDef(self):
        ...
        error_msg = (
            "Input 0 of node import/B was passed int32 from import/A:0 "
    ...
    def testMissingInputOpInGraphDef(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegex(
                ValueError, "Node 'B': Unknown input node 'A:0'"
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        """
    ...
    def testMissingControlInputInGraphDef(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegex(
                ValueError, r"Node 'B': Unknown input node '\^A'"
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        """
    ...
    def testInvalidTensorNameOutputIndexInGraphDef(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegex(
                ValueError, "Node 'B': Unknown input node 'A:B'"
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        """
    ...
    def testInvalidTensorNameInGraphDef(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegex(
                ValueError, "Node 'B': Unknown input node 'A:B:0'"
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        """
    ...
    def testMissingReturnTensor(self):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegex(
                ValueError, "Cannot convert 'A:B:0' to a tensor name."
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        """
    ...
    def testInputMapTypeMismatch(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegex(
                ValueError,
                "Input 0 of node import/B was passed float from Const:0 "
    ...
    def testNamePrefixColocationAttrs(self):
        ...
        with ops.Graph().as_default():
            ...
            self.assertProtoEquals(
                "list { s: 'loc:@imported_graph/A' }", b.node_def.attr["_class"]
            )
    ...
    def testMultipleColocationWithDeviceFn(self):
        ...
        with ops.Graph().as_default():
            with ops.device(CustomDeviceFn):
                a, b, c = importer.import_graph_def(
                    original_graph_def,
    ...
    def testInvalidInputForInputMap(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                "input_map must be a dictionary mapping strings to " "Tensor objects.",
                str(e.exception),
            )
        graph_def = self._MakeGraphDef(
            """
        ...
        with ops.Graph().as_default():
            with self.assertRaises(ValueError) as e:
                importer.import_graph_def(
                    graph_def, input_map={"a:0": variables.Variable(5.0)}, name=""
                )
    ...
    def testInvalidInputForReturnOperations(self):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegex(
                ValueError, "Cannot convert 'a:b:c' to a tensor name."
            ):
                importer.import_graph_def(
        ...
        for producer in v0, v1, v2:
            for min_consumer in v0, v1, v2:
                with ops.Graph().as_default():
                    ...
                    self.assertEqual(
                        a.graph.graph_def_versions.min_consumer, min_consumer
                    )
    ...
    def testVersionAppliesToOpConstruction(self):
        ...
        with ops.Graph().as_default():
            with self.assertRaisesWithPredicateMatch(
                ValueError, "Wrong graph version.*"
            ):
                importer.import_graph_def(
                    self._MakeGraphDef(
                        "node { name: 'A' op: 'RequiresOlderGraphVersion' }",
    ...
    @test_util.run_v1_only(
        "import inside defun not supported when eager " "execution is enabled."
    )
    ...
    def testImportGraphWithFunctionTwice(self):
        ...
        with ops.name_scope("first"):
            z1 = importer.import_graph_def(
                gdef, return_elements=["z:0"], input_map=input_map
            )[0]

        with ops.name_scope("second"):
            z2 = importer.import_graph_def(
                gdef, return_elements=["z:0"], input_map=input_map
            )[0]
