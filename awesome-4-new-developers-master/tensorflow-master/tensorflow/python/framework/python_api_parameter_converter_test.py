...
from tensorflow.python.framework._pywrap_python_tensor_converter import (
...
class PythonAPIWrapperTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testConvertAttribute(self, attr_type, attr_val, expected):
        api_info = self.makeApiInfoFromParamSpecs(
            "ConvertAttributes", ["x"], {}, {"x": attr_type}
        )
    ...
    def testConvertMultipleAttributes(self):
        ...
        api_info = self.makeApiInfoFromParamSpecs(
            "ConvertAttributes", ["x", "y", "z"], {}, attr_specs
        )
        tensor_converter = self.makeTensorConverter()
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                ...
                outputs=lambda: [[Const(1), Const(2), Const(3)], Const(4)],
            ...
            dict(
                ...
                outputs=lambda: [[Const(1.0), Const(2.0), Const(3.0)], Const(4.0)],
            ...
            dict(
                ...
                outputs=lambda: [
                    Const([[1, 2, 3], [4, 5, 6]]),
                    [Const([1, 2]), Const([3, 4, 5]), Const([6])],
                    [1, 2, 3],
                    "Foo",
                    [Const([1, 2]), Const([["three"]]), Const([4]), Const("five")],
                    [Const(1), Const("two"), Const([[3, 4], [5, 6]]), Const([["7"]])],
                ],
                inferred=dict(
                    N=3,
                    X=dtypes.int32,
    ...
    def testConvert(
        self, param_names, input_specs, attr_specs, inputs, outputs, inferred
    ):
        api_info = self.makeApiInfoFromParamSpecs(
            "TestFunc", param_names, input_specs, attr_specs
        )
        tensor_converter = self.makeTensorConverter()
        param_values = inputs()
    ...
    def testConvertError(
        self, param_names, input_specs, attr_specs, inputs, message, exception=TypeError
    ):
        api_info = self.makeApiInfoFromParamSpecs(
            "TestFunc", param_names, input_specs, attr_specs
        )
