    ...
    def testTrue(self):
        with ops.Graph().as_default():
            with session.Session():
                ...
                z = smart_cond.smart_cond(
                    True,
                    lambda: math_ops.multiply(x, 16),
    ...
    def testEval(self):
        with ops.Graph().as_default():
            with session.Session():
                ...
                z = smart_cond.smart_cond(
                    x * y > 0, lambda: constant_op.constant(1), raise_exception
                )
...
class SmartCaseTest(test_util.TensorFlowTestCase):
    ...
    def testTrue(self):
        ...
        conditions = [
            (True, lambda: constant_op.constant(1)),
            (x == 0, raise_exception),
        ]
    ...
    def testFalse(self):
        ...
        y = smart_cond.smart_case(
            conditions, default=lambda: constant_op.constant(1), exclusive=False
        )
        z = smart_cond.smart_case(
            conditions, default=lambda: constant_op.constant(1), exclusive=True
        )
        with session.Session() as sess:
            self.assertEqual(self.evaluate(y), 1)
            self.assertEqual(self.evaluate(z), 1)
    ...
    def testMix(self):
        ...
        conditions = [
            (x > 1, lambda: constant_op.constant(1)),
            (y < 1, raise_exception),
    ...
    def testCond(self):
        with ops.Graph().as_default():
            ...
            x = control_flow_ops.cond(
                pred, lambda: constant_op.constant(1), lambda: constant_op.constant(2)
            )
