...
class _GraphMerger(object):
    ...
    def merge_node_lists(repeated_nodes1, repeated_nodes2):
        ...
        return _GraphMerger.merge_lists(
            repeated_nodes1,
            repeated_nodes2,
    ...
    def merge_functions(fn1, fn2):
        ...
        merged.signature.input_arg.extend(
            _GraphMerger.merge_lists(
                fn1.signature.input_arg[:],
                fn2.signature.input_arg[:],
        ...
        merged.signature.output_arg.extend(
            _GraphMerger.merge_lists(
                fn1.signature.output_arg[:],
                fn2.signature.output_arg[:],
                op_def_pb2.OpDef.ArgDef,
                lambda a: a.name,
                lambda x, y: _GraphMerger.merge_any(x, y, op_def_pb2.OpDef.ArgDef),
            )
...
class VariablesToConstantsTest(test.TestCase):
    ...
    def testIf(self):
        ...
        input_data = {
            "x": constant_op.constant([1.0, 2.0], shape=[1, 2]),
    ...
    def testWhile(self):
        ...
        @def_function.function(
            input_signature=[tensor_spec.TensorSpec(shape=[2, 2], dtype=dtypes.float32)]
        )
        def model(x):
...
class ConvertVariablesToConstantsV2SessionTest(test.TestCase):
    ...
    def _testConvertedFunction(
        self, sess, obj, func, converted_concrete_func, input_data
    ):
        # Ensure the converted graph has no variables and no function calls.
        constant_graph_def = converted_concrete_func.graph.as_graph_def()
        self.assertEqual(0, get_num_variables(constant_graph_def))
        ...
        with ops.Graph().as_default():
            with session_lib.Session() as sess:
                ...
                self._testConvertedFunction(
                    sess, root, root.add, output_func, input_data
                )
    ...
    def testWhile(self):
        ...
        with ops.Graph().as_default():
            with session_lib.Session() as sess:
                input_data = {
                    "x": constant_op.constant([1.0, 2.0, 3.0, 4.0], shape=[2, 2])
                }
        ...
        def normalize_uids(msg):
            ...
            for name, indices in names.items():
                for new_index, old_index in enumerate(sorted(list(indices))):
                    msg = re.sub(
                        r"\b" + name + "_" + str(old_index) + r"\b",
                        name + "_" + str(new_index),
                        msg,
                    )
            return msg
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=use_resource):
                ...
                with session_lib.Session() as sess:
                    ...
                    constant_graph_def = convert_to_constants.convert_variables_to_constants_from_session_graph(
                        session=sess,
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=True):
                ...
                with session_lib.Session() as sess:
                    ...
                    constant_graph_def_with_denylist = convert_to_constants.convert_variables_to_constants_from_session_graph(
                        session=sess,
                        graph_def=variable_graph_def,
    ...
    def testGraphWithSwitch(self):
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=True):
                ...
                cond_node = control_flow_ops.case(
                    [(gen_math_ops.less(x, y), f1)], default=f2
                )
    ...
    def testConvertOneVariableOfTwo(self):
        ...
        with ops.Graph().as_default():
            ...
            with session_lib.Session() as sess:
                ...
                variable_graph_def = sess.graph.as_graph_def()
                constant_graph_def = convert_to_constants.convert_variables_to_constants_from_session_graph(
                    sess, variable_graph_def, ["out"], variable_names_denylist=["y"]
                )
                self._assertGraphContains(
                    constant_graph_def,
                    """
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=False):
                ...
                _ = control_flow_ops.case(
                    [(gen_math_ops.less(x, y), lambda: x)], default=lambda: y
                )
            with session_lib.Session() as sess:
                sess.run(variables.global_variables_initializer())
                variable_graph_def = sess.graph.as_graph_def()
    ...
    def testConvertV2Case(self):
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=False):
                ...
                y = variable_scope.get_variable("y", initializer=2.0)
                _ = control_flow_ops.case(
                    [(gen_math_ops.less(x, y), lambda: a)], default=lambda: y
                )
                control_flow_v2_toggles.disable_control_flow_v2()
            with session_lib.Session() as sess:
                sess.run(variables.global_variables_initializer())
