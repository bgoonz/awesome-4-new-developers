...
class TypesTest(test_util.TensorFlowTestCase):
    def testAllTypesConstructible(self):
        for datatype_enum in types_pb2.DataType.values():
            if datatype_enum == types_pb2.DT_INVALID:
                continue
            self.assertEqual(
                datatype_enum, dtypes.DType(datatype_enum).as_datatype_enum
            )
    ...
    def testNumpyConversion(self):
        ...
        self.assertIs(dtypes.string, dtypes.as_dtype(np.array(["foo", "bar"]).dtype))
    ...
    def testMinMax(self):
        ...
        for datatype_enum in types_pb2.DataType.values():
            ...
            if (
                dtype.is_quantized
                or dtype.base_dtype == dtypes.bool
                or dtype.base_dtype == dtypes.string
                or dtype.base_dtype == dtypes.complex64
                or dtype.base_dtype == dtypes.complex128
            ):
                continue
