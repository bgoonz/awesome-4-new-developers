...
_FRAMEWORK_PATH_PREFIXES = [
    ...
    os.path.join(os.path.dirname(_FRAMEWORK_COMMON_PREFIX), "py", "keras") + os.sep,
]

# Patterns of filename patterns that should be considered internal to
# the TensorFlow framework.
_FRAMEWORK_FILENAME_PATTERNS = [re.compile(r"<embedded")]
...
def _compute_colocation_summary_from_dict(name, colocation_dict, prefix=""):
  ...
  """
    ...
    str_list = []
    str_list.append(
        "%sNode-device colocations active during op '%s' creation:" % (prefix, name)
    )
...
def create_graph_debug_info_def(func_named_operations):
  ...
  """
    ...
    file_to_index = dict([(y, x) for x, y in enumerate(graph_debug_info_def.files)])
    ...
    for node_name, frames in node_to_trace.items():
        ...
        for frame in reversed(frames):
            trace_def.file_line_cols.add(
                file_index=file_to_index[frame.filename], line=frame.lineno
            )
...
def interpolate(error_message, graph):
  ...
  """
    ...
    return "".join(itertools.chain(*six.moves.zip_longest(seps, subs, fillvalue="")))
