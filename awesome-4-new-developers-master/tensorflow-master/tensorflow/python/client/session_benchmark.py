...
class SessionBenchmark(test.Benchmark):
    ...
    def benchmarkGrpcSession(self):
        ...
        self._benchmarkFeed(
            "benchmark_session_feed_grpc_4MB", server.target, 1 << 20, 25000
        )
        session.Session.reset(server.target)
        ...
        session.Session.reset(server.target)
        self._benchmarkFetchPrebuilt(
            "benchmark_session_fetchprebuilt_grpc_4B", server.target, 1, 50000
        )
        session.Session.reset(server.target)
        self._benchmarkFetchPrebuilt(
            "benchmark_session_fetchprebuilt_grpc_4MB", server.target, 1 << 20, 50000
        )
        session.Session.reset(server.target)
        self._benchmarkRunOp("benchmark_session_runop_grpc", server.target, 50000)
        session.Session.reset(server.target)
        self._benchmarkRunOpPrebuilt(
            "benchmark_session_runopprebuilt_grpc", server.target, 100000
        )
        session.Session.reset(server.target)
    ...
    def benchmarkDirectSession(self):
        ...
        self._benchmarkFetchPrebuilt(
            "benchmark_session_fetchprebuilt_direct_4B", "", 1, 200000
        )
        self._benchmarkFetchPrebuilt(
            "benchmark_session_fetchprebuilt_direct_4MB", "", 1 << 20, 200000
        )
        self._benchmarkRunOp("benchmark_session_runop_direct", "", 200000)
        self._benchmarkRunOpPrebuilt(
            "benchmark_session_runopprebuilt_direct", "", 200000
        )
