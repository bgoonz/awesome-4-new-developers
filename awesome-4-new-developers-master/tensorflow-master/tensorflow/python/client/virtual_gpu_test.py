...
class VirtualGpuTestUtil(object):
    ...
    def _GetSessionConfig(self):
        virtual_device_gpu_options = config_pb2.GPUOptions(
            ...
            experimental=config_pb2.GPUOptions.Experimental(
                virtual_devices=[
                    config_pb2.GPUOptions.Experimental.VirtualDevices(memory_limit_mb=i)
                    for i in self._mem_limits_mb
                ]
            ),
        )
    ...
    def _TestRandomGraphWithDevices(
        self, sess, seed, op_placement, devices, debug_mode=False
    ):
        data = []
        ...
        for i in range(len(devices)):
            with ops.device(devices[i]):
                ...
                feed_dict[var] = np.random.uniform(low=0, high=0.1, size=shape).astype(
                    np.float32
                )
                data.append(var)
        ...
        for i in range(self._dim):
            for j in range(self._dim):
                if result_vgd[i][j] != result_cpu[i][j]:
                    logging.error(
                        "Result mismatch at row %d column %d: expected %f, actual %f",
                        i,
                        j,
                        result_cpu[i][j],
                        result_vgd[i][j],
