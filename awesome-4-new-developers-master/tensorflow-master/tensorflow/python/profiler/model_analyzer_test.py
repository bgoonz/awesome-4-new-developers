...
class PrintModelAnalysisTest(test.TestCase):
    ...
    def testSelectEverythingDetail(self):
        ...
        with profile_context.ProfileContext(
            test.get_temp_dir(), trace_steps=[], dump_steps=[]
        ) as pctx:
            with session.Session(
                config=self._no_rewrite_session_config()
            ) as sess, ops.device(dev):
    ...
    def testComplexCodeView(self):
        ...
        opts = (
            ...
            .with_file_output(outfile)
            .with_accounted_types([".*"])
            .with_node_names(show_name_regexes=[".*model_analyzer_testlib.py.*"])
            .account_displayed_op_only(False)
            .select(["params", "float_ops"])
            .build()
        )
    ...
    def testCodeViewLeafGraphNode(self):
        ...
        opts = (
            ...
            .with_accounted_types([".*"])
            .account_displayed_op_only(False)
            .select(["bytes", "params", "float_ops", "device"])
            .build()
        )
    ...
    def testMinOption(self):
        ...
        with session.Session(config=self._no_rewrite_session_config()) as sess:
            ...
            opts = (
                builder(builder.time_and_memory(min_micros=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_accelerator_micros=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_cpu_micros=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_bytes=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_peak_bytes=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_residual_bytes=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
            ...
            opts = (
                builder(builder.time_and_memory(min_output_bytes=min_val))
                .with_empty_output()
                .build()
            )
            tfprof_node = model_analyzer.profile(
    ...
    def testSelectOption(self):
        ...
        with session.Session(config=self._no_rewrite_session_config()) as sess:
            ...
            check_selection(
                ["total execution time", "accelerator execution time"], ["bytes"]
            )
            ...
            _ = model_analyzer.profile(sess.graph, run_meta=run_meta, options=opts)
            check_selection(
                ["requested bytes"], ["peak bytes", "residual bytes", "output bytes"]
            )

            opts = (
                builder(builder.time_and_memory())
                .with_file_output(outfile)
                .select(["peak_bytes", "residual_bytes", "output_bytes"])
                .build()
            )
            _ = model_analyzer.profile(sess.graph, run_meta=run_meta, options=opts)
            check_selection(
        ...
        with session.Session(config=self._no_rewrite_session_config()) as sess:
            ...
            for i in range(1, train_steps + 1):
                ...
                if i in time_step:
                    ...
                    self.assertTrue(
                        gfile.Open(os.path.join(time_dir, ret[0]), "r")
                        .read()
                        .find("execution time")
                        > 0
                    )
                ...
                if i in memory_step:
                    ...
                    self.assertEqual(len(ret), 1)
                    self.assertTrue(
                        gfile.Open(os.path.join(memory_dir, ret[0]), "r")
                        .read()
                        .find("requested bytes")
                        > 0
                    )
    ...
    def testAutoProfiling(self):
        ...
        x = lib.BuildSmallModel()
        with profile_context.ProfileContext(
            profile_dir, trace_steps=[1, 2, 3], dump_steps=[3, 4]
        ) as pctx:
    ...
    def testOOM(self):
        ...
        try:
            ...
            self.assertTrue(
                "OOM when allocating tensor with shape[30000,10000,20000]"
                in exception_str
            )
            mat = re.search(
                "(.*)GiB from test_random2/RandomStandardNormal", exception_str
            )
            self.assertGreater(float(mat.group(1)), 0.0)
            mat = re.search(
                "(.*)MiB from test_random1/RandomStandardNormal", exception_str
            )
            self.assertGreater(float(mat.group(1)), 0.0)
    ...
    def testTraceLoopBytes(self):
        ...
        with ops.device("/gpu:0"):
            ...
            _, y = control_flow_ops.while_loop(
                lambda i, x: i < n, loop_body, [array_ops.constant(0), x]
            )
