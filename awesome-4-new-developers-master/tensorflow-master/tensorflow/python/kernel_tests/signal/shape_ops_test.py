...
class FrameTest(test.TestCase):
        ...
        if not context.executing_eagerly():
            ...
            shape_ops.frame(
                array_ops.placeholder_with_default(
                    1, shape=tensor_shape.TensorShape(None)
                ),
                1,
                1,
            )
    ...
    def test_shape_inference(self):
        ...
        frame_step = 1
        result = shape_ops.frame(
            signal, frame_length, frame_step, pad_end=True, pad_value=99, axis=1
        )
        self.assertEqual([1, 2, None, 3, 4], result.shape.as_list())

        result = shape_ops.frame(
            signal, frame_length, frame_step, pad_end=False, axis=1
        )
        ...
        frame_step = 3
        result = shape_ops.frame(
            signal, frame_length, frame_step, pad_end=True, pad_value=99, axis=3
        )
        self.assertEqual([None, 5, None, 7, 4, 5, 3], result.shape.as_list())

        result = shape_ops.frame(
            signal, frame_length, frame_step, pad_end=False, axis=3
        )
        ...
        for signal_length in range(2):
            ...
            for frame_length in range(2):
                for pad_end in [False, True]:
                    op = shape_ops.frame(
                        signal, frame_length, frame_step, pad_end=pad_end, pad_value=99
                    )
    ...
    def test_basic_mono(self):
        ...
        for rank in range(5):
            ...
            result = shape_ops.frame(
                nd_signal, frame_length, frame_step, pad_end=True, pad_value=99
            )
    ...
    def test_axis(self):
        ...
        result = shape_ops.frame(
            signal, frame_length=2, frame_step=2, pad_end=True, axis=1
        )
        expected = np.reshape(np.arange(16), (2, 2, 2, 2))
        self.assertAllEqual(expected, self.evaluate(result))

        result = shape_ops.frame(
            signal, frame_length=2, frame_step=1, pad_end=True, axis=1
        )
        expected = [
            [[[0, 1], [2, 3]], [[2, 3], [4, 5]], [[4, 5], [6, 7]], [[6, 7], [0, 0]]],
            [
                [[8, 9], [10, 11]],
                [[10, 11], [12, 13]],
                [[12, 13], [14, 15]],
        ...
        result = shape_ops.frame(
            signal, frame_length=3, frame_step=1, pad_end=True, axis=1
        )
        expected = [
            [
                [[0, 1], [2, 3], [4, 5]],
    ...
    def test_window_larger_than_signal(self):
        ...
        self.assertAllClose(
            [[[1, 2, 99, 99], [2, 99, 99, 99]], [[11, 12, 99, 99], [12, 99, 99, 99]]],
            result,
        )
    ...
    def test_dynamic_tensor(self):
        ...
        self.assertAllEqual(
            [[[0, 1], [2, 3]], [[10, 11], [12, 13]], [[20, 21], [22, 23]]], result
        )
