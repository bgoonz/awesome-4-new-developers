...
class WindowOpsTest(test.TestCase, parameterized.TestCase):
    ...
    def _compare_window_fns(
        self, np_window_fn, tf_window_fn, window_length, periodic, tf_dtype_tol
    ):
    ...
    @parameterized.parameters(
        itertools.product(_WINDOW_LENGTHS, (4.0, 8.0, 10.0, 12.0), _TF_DTYPE_TOLERANCE)
    )
    ...
    @parameterized.parameters(
        itertools.product(
            (
                window_ops.hann_window,
                window_ops.hamming_window,
                window_ops.kaiser_window,
                window_ops.kaiser_bessel_derived_window,
    ...
    @parameterized.parameters(
        itertools.product(_MDCT_WINDOW_LENGTHS, _TF_DTYPE_TOLERANCE)
    )
    def test_vorbis_window(self, window_length, tf_dtype_tol):
        """Check if vorbis windows satisfy MDCT window conditions."""
        self._check_mdct_window(
            window_ops.vorbis_window(window_length, dtype=tf_dtype_tol[0]),
            tol=tf_dtype_tol[1],
        )
    ...
    def test_kaiser_bessel_derived_window(self, window_length, beta, tf_dtype_tol):
        ...
        self._check_mdct_window(
            window_ops.kaiser_bessel_derived_window(
                window_length, beta=beta, dtype=tf_dtype_tol[0]
            ),
            tol=tf_dtype_tol[1],
        )
