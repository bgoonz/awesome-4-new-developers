...
class SegmentReductionHelper(test.TestCase):
    def _input(self, input_shape, dtype=dtypes_lib.int32):
        ...
        return (
            constant_op.constant(np_values, shape=input_shape, dtype=dtype),
            np_values,
        )

    def _segmentReduce(
        self, indices, x, op1, op2=None, num_segments=None, initial_value=0
    ):
        if not x.size:
            return np.array([])
...
class SegmentReductionOpTest(SegmentReductionHelper):
    def testValues(self):
        ...
        ops_list = [
            (np.add, None, math_ops.segment_sum),
            (self._mean_cum_op, self._mean_reduce_op, math_ops.segment_mean),
...
class UnsortedSegmentTest(SegmentReductionHelper):
    def __init__(self, methodName="runTest"):
        ...
        self.ops_list = [
            (np.add, None, math_ops.unsorted_segment_sum, lambda t: 0),
            (
                self._mean_cum_op,
            ...
            (
                self._mean_cum_op,
                self._sqrt_n_reduce_op,
                math_ops.unsorted_segment_sqrt_n,
                lambda t: 0,
            ),
            (np.ndarray.__mul__, None, math_ops.unsorted_segment_prod, lambda t: 1),
            (np.minimum, None, math_ops.unsorted_segment_min, lambda t: t.max),
            (np.maximum, None, math_ops.unsorted_segment_max, lambda t: t.min),
        ]

        # A subset of ops has been enabled for complex numbers
        self.complex_ops_list = [
            (np.add, None, math_ops.unsorted_segment_sum, lambda t: 0),
            (np.ndarray.__mul__, None, math_ops.unsorted_segment_prod, lambda t: 1),
        ]
        self.differentiable_dtypes = [
            dtypes_lib.float16,
            dtypes_lib.float32,
            dtypes_lib.float64,
        ]
                ...
                for use_gpu in [True, False]:
                    with self.cached_session():
                        for np_op1, np_op2, tf_op, init_op in ops_list:
                            ...
                            s = tf_op(
                                tf_x, segment_ids=indices, num_segments=num_segments
                            )
                            tf_ans = self.evaluate(s)
                    ...
                    with self.cached_session(use_gpu=use_gpu):
                        for _, _, tf_op, _ in ops_list:
                            ...
                            self.assertAllCloseAccordingToType(
                                jacob_t, jacob_n, half_atol=1e-2
                            )
        ...
        for use_gpu in [False, True]:
            with self.cached_session(use_gpu=use_gpu):
                for ind, grad_gt in [
                    ...
                    s = math_ops.unsorted_segment_prod(
                        values_tf, constant_op.constant(ind), 3
                    )
                    jacob_t, jacob_n = gradient_checker.compute_gradient(
...
class SparseSegmentReductionHelper(SegmentReductionHelper):
    def _sparse_input(self, input_shape, num_indices, dtype=dtypes_lib.int32):
        ...
        return (constant_op.constant(indices, dtype=dtypes_lib.int32), indices, a, b)

    def _sparseSegmentReduce(
        self, x, indices, segment_indices, op1, op2=None, num_segments=None
    ):
        return self._segmentReduce(
            segment_indices, x[indices], op1, op2, num_segments=num_segments
        )

    def _sparseSegmentReduceGrad(self, ygrad, indices, segment_ids, output_dim0, mode):
        assert mode in ("sum", "mean", "sqrtn")
        if mode != "sum":
            weights = np.zeros(ygrad.shape[0], ygrad.dtype)
            for segment in segment_ids:
                weights[segment] += 1
            weights = 1.0 / weights if mode == "mean" else 1.0 / np.sqrt(weights)
...
class SparseSegmentReductionOpTest(SparseSegmentReductionHelper):
    def testValues(self):
        ...
        # Each item is np_op1, np_op2, tf_op
        ops_list = [
            (np.add, None, math_ops.sparse_segment_sum),
                    ...
                    for segment_ids_dtype in segment_ids_dtypes:
                        with self.cached_session():
                            ...
                            for np_op1, np_op2, tf_op in ops_list:
                                if (
                                    tf_op == math_ops.sparse_segment_mean
                                    and dtype not in mean_dtypes
                                ):
                                    continue
                                np_ans = self._sparseSegmentReduce(
                                    np_x, np_indices, segment_indices, np_op1, np_op2
                                )
                                s = tf_op(
                                    data=tf_x,
                                    indices=math_ops.cast(tf_indices, index_dtype),
                                    segment_ids=math_ops.cast(
                                        segment_indices, segment_ids_dtype
                                    ),
                                )
                                tf_ans = self.evaluate(s)
                                self.assertAllClose(np_ans, tf_ans)
    ...
    def testSegmentIdsHole(self):
        ...
        with self.session():
            for np_op1, np_op2, tf_op in ops_list:
                np_ans = self._sparseSegmentReduce(
                    np_x, tf_indices, segment_indices, np_op1, np_op2
                )
    ...
    def testWithNumSegments(self):
        tf_x, np_x = self._input([10, 4], dtype=dtypes_lib.float32)
        ops_list = [
            (np.add, None, math_ops.sparse_segment_sum_with_num_segments),
            (
                self._mean_cum_op,
                self._mean_reduce_op,
    ...
    def testGradientWithEmptySegmentsAtEnd(self):
        ...
        for tf_op in [
            ...
            with self.cached_session():
                ...
                jacob_t, jacob_n = gradient_checker.compute_gradient(
                    tf_x, shape, s, [5, 4], x_init_value=np_x.astype(np.double), delta=1
                )
            self.assertAllClose(jacob_t, jacob_n)
    ...
    def testGradientSegmentsInvalid1(self):
        tf_x, _ = self._input([3, 4], dtype=dtypes_lib.float32)  # expecting 3 segments
        ops_list = [
            math_ops.sparse_segment_sum_grad,
            math_ops.sparse_segment_mean_grad,
            math_ops.sparse_segment_sqrt_n_grad,
        ...
        with session.Session() as sess:
            ...
            r = self.run_op_benchmark(
                ...
                name="_".join(
                    map(
                        str,
                        [name, outer_dim, ratio, inner_dim, self._npTypeToStr(dtype)],
                    )
                ),
            )
