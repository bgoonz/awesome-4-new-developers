...
def random_gamma_with_alpha_beta(shape):  # pylint: disable=invalid-name
    return random_ops.random_gamma(
        shape, alpha=[[1.0], [3.0], [5.0], [6.0]], beta=[[3.0, 4.0]]
    )
...
class WhileV2Test(test.TestCase, parameterized.TestCase):
    ...
    def testSingleLoopVar(self):
        ...
        ret = while_loop_v2(
            lambda v: v < 8.0, lambda v: v * v, [x], return_same_structure=False
        )
    ...
    def testCustomGradient(self):
        ...
        ret = while_loop_v2(
            lambda v: v < 8.0, body_fn, [x], return_same_structure=False
        )
        grad = gradients_impl.gradients(ret, [x])
        with self.cached_session():
            self.assertEqual(self.evaluate(ret), 16.0)
    ...
    def testReturnSameStructureTrue(self):
        x = constant_op.constant(2.0)
        ret = while_loop_v2(
            lambda v: v < 8.0, lambda v: v * v, [x], return_same_structure=True
        )
        grad = gradients_impl.gradients(ret, [x])
            ...
            def fnWithLoop():  # pylint: disable=invalid-name
                with backprop.GradientTape() as tape:
                    _, x = while_loop_v2(
                        lambda i, _: i < 2,
                        lambda i, x: (i + 1, x * v),
                        [0, constant_op.constant(2.0, dtype=dtype)],
                    )
    ...
    def testIteratedGradients(self):
        def _Func(x):
            _, z = while_loop_v2(
                lambda i, _: i < 2, lambda i, y: (i + 1, math_ops.cos(y)), [0, x]
            )
            return z
    ...
    def testIteratedGradientsWithList(self):
        def _Func(x):
            ...
            return z + math_ops.reduce_sum(
                list_ops.tensor_list_stack(results, dtypes.float32)
            )
    ...
    def testGradWhileGradWhileWithVariable(self):
        with context.eager_mode():
            ...
            def _Func(x):
                ...
                _, z = while_loop_v2(
                    lambda i, _: i < 2, lambda i, y: (i + 1, _Inner(y)), [0, x]
                )
                return z
            ...
            self.assertAllClose(array_ops.reshape(numerical, []), dx, rtol=1e-3)
    ...
    def testGradientTape(self):
        with backprop.GradientTape() as t:
            ...
            t.watch(x)
            ret = while_loop_v2(
                lambda v: v < 4.0, lambda v: v * v, [x], return_same_structure=False
            )  # x**2
    ...
    def testMultipleWhileLoops(self):
        ...
        ret2 = while_loop_v2(
            lambda v: v < 16.0, lambda v: v * v, [ret1], return_same_structure=False
        )  # x**4
    ...
    def testDoubleDerivative(self):
        x = constant_op.constant(2.0)
        ret = while_loop_v2(
            lambda v: v < 8.0, lambda v: v ** 2, [x], return_same_structure=False
        )  # x**4
    ...
    def testPruningNested(self):
        ...
        def Body(x, tl):
            ...
            return control_flow_ops.while_loop(InnerCond, InnerBody, [inner_x, x, tl])[
                1:
            ]
    ...
    def testPruningNested3(self):
        ...
        def CreateWhileLoop():
            r = control_flow_ops.while_loop(
                lambda _: True,
                lambda x: math_ops.multiply(v, x, name="my_mul"),
                [1.0],
                maximum_iterations=5,
                name="outer",
            )
    ...
    def _assertNotAccumulated(self, while_op, index):
        ...
        self.assertNotIn(
            "TensorListPushBack", [op.type for op in placeholder.consumers()]
        )
    ...
    def testCaptureExternalTensorInCond(self):
        ...
        ret = while_loop_v2(
            lambda v: v + y < 9.0, lambda v: v * 3.0, [x], return_same_structure=False
        )
        grad = gradients_impl.gradients(ret, [x])
        with self.cached_session():
    ...
    def testCaptureExternalTensorInBody(self):
        ...
        y = constant_op.constant(3.0)
        ret = while_loop_v2(
            lambda v: v < 8.0, lambda v: v * y, [x], return_same_structure=False
        )
        grad = gradients_impl.gradients(ret, [x])
        with self.cached_session():
            self.assertEqual(self.evaluate(ret), 18.0)
    ...
    def testAccumulatorElementShape(self, shape):
        ...
        _, val = list_ops.tensor_list_pop_back(
            grad_output, element_dtype=dtypes.float32
        )
        MatchShape(val.shape)
    ...
    def testNestedWhile(self):
        ...
        result = while_loop_v2(
            lambda i, _: i >= 0, Body, [m, sum_of_powers], return_same_structure=False
        )[1]
    ...
    def testRandomOpsShape(self, random_fn, expected_shape):
        ...
        _, _ = while_loop_v2(
            cond=lambda i, _: i < 3,
            body=Body,
            loop_vars=[0, array_ops.zeros(expected_shape, dtype=dtypes.float32)],
        )
    ...
    def testReshapeShape(self):
        ...
        _, _ = while_loop_v2(
            cond=lambda i, _: i < 3,
            body=Body,
            loop_vars=[0, array_ops.zeros([5, 3, 4], dtype=dtypes.float32)],
        )

    @parameterized.named_parameters(
        ("Zeros", array_ops.zeros), ("Ones", array_ops.ones), ("Fill", fill)
    )
    @test_util.run_deprecated_v1
                ...
                def Body(i, x):
                    ...
                    def IdentityWithZeroGrad(x):
                        def Grad(
                            ...
                            assert size.graph is forward_graph
                            zeros = gen_list_ops.tensor_list_reserve(
                                shape, size, dtypes.float32
                            )
                            assert zeros.graph is gradient_graph
                            return zeros
