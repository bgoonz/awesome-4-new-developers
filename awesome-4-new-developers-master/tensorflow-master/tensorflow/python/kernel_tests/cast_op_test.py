...
class CastOpTest(test.TestCase):
    ...
    def _testTypes(self, x, use_gpu=False):
        ...
        if use_gpu:
            type_list = [np.float32, np.float64, np.int64, np.complex64, np.complex128]
        else:
            type_list = [
                np.float32,
                np.float64,
    ...
    def testSmallValues(self):
        ...
        self._testAll(
            np.array(
                [0, -1, 1, -f4.resolution, f4.resolution, f8.resolution, -f8.resolution]
            )
        )
    ...
    def testBfloat16(self):
        ...
        with self.cached_session(use_gpu=False):
            ...
            self.assertAllClose(a, self.evaluate(b), rtol=1 / 128.0)
    ...
    def _compare(self, x, dst_dtype, expected, use_gpu=False):
        np.testing.assert_equal(
            self._cast(x, dst_dtype, use_gpu=use_gpu), dst_dtype(expected)
        )
...
class SparseTensorCastTest(test.TestCase):
    def testCast(self):
        ...
        self.assertAllEqual(st_cast.values, np.array([1, 2, 3], np.float32))
