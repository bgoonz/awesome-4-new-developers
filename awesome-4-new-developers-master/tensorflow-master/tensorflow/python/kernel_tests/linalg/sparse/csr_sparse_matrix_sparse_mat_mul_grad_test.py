...
class CSRSparseMatrixGradTest(test.TestCase):
    ...
    def _testLargeBatchSparseMatrixSparseMatMulGrad(
        self, datatype, transpose_a, transpose_b, adjoint_a, adjoint_b
    ):
        ...
        a_mats_val = sparsify(
            np.random.randn(3, 5, 11) + 1.0j * np.random.randn(3, 5, 11)
        ).astype(datatype)
        ...
        b_mats_val = sparsify(
            np.random.randn(3, 11, 13) + 1.0j * np.random.randn(3, 11, 13)
        ).astype(datatype)
