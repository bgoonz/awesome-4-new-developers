...
class UseOperatorOrProvidedHintUnlessContradictingTest(
    ...
    def test_computes_an_or_if_non_contradicting(
        self, operator_hint_value, provided_hint_value, expected_result
    ):
        self.assertEqual(
            expected_result,
    ...
    @parameterized.named_parameters(
        ("true_false", True, False), ("false_true", False, True)
    )
...
class BlockwiseTest(test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ("split_dim_1", [3, 3, 4], -1), ("split_dim_2", [2, 5], -2)
    )
    def test_blockwise_input(self, op_dimension_values, split_dim):
        ...
        if split_dim == -1:
            blockwise_arrays = [
                np.zeros(batch_shape + [arg_dim, d]) for d in op_dimension_values
            ]
        else:
            blockwise_arrays = [
                np.zeros(batch_shape + [d, arg_dim]) for d in op_dimension_values
            ]
    ...
    def test_non_blockwise_input(self):
        ...
        for inputs in [x, x_tensor, x_placeholder, x_list]:
            self.assertFalse(
                linear_operator_util.arg_is_blockwise(op_dimensions, inputs, -1)
            )
        ...
        for inputs in [x, x_tensor, x_placeholder, x_list]:
            self.assertFalse(
                linear_operator_util.arg_is_blockwise(unknown_op_dimensions, inputs, -1)
            )
