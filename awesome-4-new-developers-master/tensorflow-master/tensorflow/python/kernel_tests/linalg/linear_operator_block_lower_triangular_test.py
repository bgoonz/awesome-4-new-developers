...
from tensorflow.python.ops.linalg import (
    linear_operator_block_lower_triangular as block_lower_triangular,
)
...
class SquareLinearOperatorBlockLowerTriangularTest(
    ...
    def operator_shapes_infos():
        ...
        return [
            ...
            shape_info((2, 4, 6, 6), blocks=[[(2, 1, 2, 2)], [(1, 4, 2), (4, 4, 4)]]),
        ]
    ...
    def operator_and_matrix(
        ...
        broadcasted_matrices = linear_operator_util.broadcast_matrix_batch_dims(
            [op for row in matrices for op in row]
        )  # pylint: disable=g-complex-comprehension
        matrices = [
            broadcasted_matrices[i * (i + 1) // 2 : (i + 1) * (i + 2) // 2]
    ...
    def test_block_lower_triangular_inverse_type(self):
        ...
        self.assertIsInstance(
            inverse, block_lower_triangular.LinearOperatorBlockLowerTriangular
        )
    ...
    def test_tape_safe(self):
        operator_1 = linalg.LinearOperatorFullMatrix(
            variables_module.Variable([[1.0, 0.0], [0.0, 1.0]]),
            is_self_adjoint=True,
            is_positive_definite=True,
        )
        operator_2 = linalg.LinearOperatorFullMatrix(
            variables_module.Variable([[2.0, 0.0], [1.0, 0.0]])
        )
        operator_3 = linalg.LinearOperatorFullMatrix(
            variables_module.Variable([[3.0, 1.0], [1.0, 3.0]]),
            is_self_adjoint=True,
            is_positive_definite=True,
        )
        operator = block_lower_triangular.LinearOperatorBlockLowerTriangular(
    ...
    def test_is_non_singular_auto_set(self):
        ...
        operator_4 = linalg.LinearOperatorFullMatrix(
            [[1.0, 0.0], [2.0, 0.0]], is_non_singular=False
        )
        ...
        with self.assertRaisesRegex(ValueError, "must contain `i` blocks"):
            block_lower_triangular.LinearOperatorBlockLowerTriangular(
                [
                    ...
                    [linalg.LinearOperatorFullMatrix(rng.rand(2, 2)) for _ in range(3)],
                ]
            )
