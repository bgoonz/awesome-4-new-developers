...
class SessionOpsTest(test.TestCase):
    ...
    def testMultiDevices(self):
        with self.cached_session() as sess:
            ...
            c_handle = sess.run(
                ...
                feed_dict={a_p: a_handle.handle, b_p: b_handle.handle},
            )
    ...
    def testHandleGC(self):
        with self.cached_session() as sess:
            ...
            with ops.device(test.gpu_device_name()):
                add_h1, add_t1 = session_ops.get_session_tensor(
                    one_handle.handle, dtypes.float32
                )
                add_h2, add_t2 = session_ops.get_session_tensor(
                    x_handle.handle, dtypes.float32
                )
            ...
            for _ in range(20):
                x_handle = sess.run(
                    add_output,
                    feed_dict={add_h1: one_handle.handle, add_h2: x_handle.handle},
                )
