...
class SparseTensorDenseMatMulTest(test.TestCase):
    def _testMatmul(
        self, x, y, adjoint_a=False, adjoint_b=False, indices_dtype=np.int64
    ):
        x_mat = np.array(x)
        if adjoint_a:
            x_mat = x_mat.T.conj()
    ...
    def testShapeInference(self):
        ...
        with ops.Graph().as_default():
            ...
            x_st_shape_unknown = sparse_tensor.SparseTensor(
                x_indices, x_values, x_shape_unknown
            )
    ...
    def testInvalidIndicesForSparseTensorDenseMatmul(self):
        ...
        with self.assertRaisesOpError("m .10. from index.0,1. out of bounds .>=2."):
            self.evaluate(
                sparse_ops.sparse_tensor_dense_matmul(sparse_t, dense_t, adjoint_a=True)
            )
...
def main(_):
    ...
    print("")
    print(
        "% nnz \t n \t gpu \t m \t k \t dt(dense) \t dt(sparse) "
        "\t dt(sparse)/dt(dense)"
    )
