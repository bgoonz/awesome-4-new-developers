...
class BinaryOpTest(test.TestCase):
    ...
    def testFloatBasic(self):
        x = (
            np.linspace(-5, 20, 15).reshape(1, 3, 5).astype(np.float32)
        )  # pylint: disable=too-many-function-args
        y = (
            np.linspace(20, -5, 15).reshape(1, 3, 5).astype(np.float32)
        )  # pylint: disable=too-many-function-args
        ...
        try:
            ...
            a_pos_small = (
                np.linspace(0.1, 2, 15).reshape(1, 3, 5).astype(np.float32)
            )  # pylint: disable=too-many-function-args
            x_pos_small = (
                np.linspace(0.1, 10, 15).reshape(1, 3, 5).astype(np.float32)
            )  # pylint: disable=too-many-function-args
            self._compareBoth(
    ...
    def testDoubleBasic(self):
        x = (
            np.linspace(-5, 20, 15).reshape(1, 3, 5).astype(np.float64)
        )  # pylint: disable=too-many-function-args
        y = (
            np.linspace(20, -5, 15).reshape(1, 3, 5).astype(np.float64)
        )  # pylint: disable=too-many-function-args
    ...
    def testComplex64Basic(self):
        x = (1 + 1j) * np.linspace(-10, 10, 6).reshape(
            1, 3, 2
        ).astype(  # pylint: disable=too-many-function-args
            np.complex64
        )
        y = (1 + 1j) * np.linspace(20, -20, 6).reshape(
            1, 3, 2
        ).astype(  # pylint: disable=too-many-function-args
            np.complex64
        )
