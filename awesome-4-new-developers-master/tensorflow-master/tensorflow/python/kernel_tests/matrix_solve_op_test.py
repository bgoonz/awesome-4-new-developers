...
class MatrixSolveOpTest(test.TestCase):
    ...
    def _generateMatrix(self, m, n):
        ...
        matrix.imag = (
            np.random.normal(-5, 5, m * n).astype(np.complex128).reshape([m, n])
        )
        return matrix
    ...
    def testNonSquareMatrix(self):
        ...
        with self.assertRaises((ValueError, errors_impl.InvalidArgumentError)):
            matrix = constant_op.constant([[1.0, 2.0, 3.0], [3.0, 4.0, 5.0]])
    ...
    def testConcurrent(self):
        ...
        for adjoint_ in False, True:
            lhs1 = stateless_random_ops.stateless_random_normal(matrix_shape, seed=seed)
            lhs2 = stateless_random_ops.stateless_random_normal(matrix_shape, seed=seed)
            rhs1 = stateless_random_ops.stateless_random_normal(matrix_shape, seed=seed)
            rhs2 = stateless_random_ops.stateless_random_normal(matrix_shape, seed=seed)
...
class MatrixSolveBenchmark(test.Benchmark):
    ...
    def _GenerateTestData(self, matrix_shape, num_rhs):
        ...
        n = matrix_shape[0]
        matrix = np.ones(matrix_shape).astype(np.float32) / (2.0 * n) + np.diag(
