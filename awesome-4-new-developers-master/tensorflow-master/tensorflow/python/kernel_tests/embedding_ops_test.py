...
def _EmbeddingParamsAsPartitionedVariable(
    num_shards, vocab_size, dtype=dtypes.float32, shape=None, use_resource=False
):
...
def _EmbeddingResult(
    params, id_vals, num_shards, vocab_size, partition_strategy="mod", weight_vals=None
):
    ...
    for ids, wts in zip(id_vals, weight_vals):
        ...
        for i, weight_value in zip(ids, wts):
            if partition_strategy == "mod":
                val = (
                    np.copy(params[_PName(i % num_shards) + ":0"][i // num_shards, :])
                    * weight_value
                )
                ...
                val = (
                    np.copy(params[_PName(partition) + ":0"][offset, :]) * weight_value
                )
            else:
                assert False
        ...
        with self.cached_session():
            for params_shape in (12,), (6, 3), (6, 2, 3):
                ...
                for ids_shape in (), (3), (4, 3), (2, 3, 4):
                    ids = np.random.randint(
                        params.shape[0], size=np.prod(ids_shape, dtype=np.int64)
                    ).reshape(ids_shape)
                    # Compare nonsharded to gather
    ...
    def testTransform(self):
        ...
        with self.cached_session():
            ...
            params_norm = (
                l2_norm
                * params
                / np.sqrt(np.sum(params * params, axis=1, keepdims=True))
            )
...
class EmbeddingLookupSparseTest(test.TestCase):
    ...
    def testEmbeddingLookupSparse(self):
        ...
        sp_ids, sp_weights, ids, weights, vals_per_batch_entry = self._RandomIdsAndWeights(
        ...
        for num_shards, combiner, dtype, ignore_weights in itertools.product(
            ...
            with self.cached_session():
                ...
                embedding_sum = embedding_ops.embedding_lookup_sparse(
                    p, sp_ids, None if ignore_weights else sp_weights, combiner=combiner
                )
        ...
        for num_shards, combiner, dtype, ignore_weights in itertools.product(
            ...
            with self.cached_session():
                ...
                y = embedding_ops.embedding_lookup_sparse(
                    x, sp_ids, None if ignore_weights else sp_weights, combiner=combiner
                )
                x_name = [_PName(i) for i in range(num_shards)]
                x_init_value = [params[x_n + ":0"] for x_n in x_name]
                x_shape = [i.shape for i in x_init_value]
...
class SafeEmbeddingLookupSparseTest(test.TestCase):
    ...
    def test_safe_embedding_lookup_sparse_partitioned_inconsistent_weights(self):
        with self.cached_session():
            ...
            embedding_weights = [
                constant_op.constant(w, dtype=dtypes.float64) for w in embedding_weights
            ]
            self.assertRaises(
                ValueError,
                embedding_ops.safe_embedding_lookup_sparse,
