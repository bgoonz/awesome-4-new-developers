...
class FIFOQueueTest(test.TestCase):
    ...
    def testMultiQueueConstructor(self):
        with ops.Graph().as_default():
            q = data_flow_ops.FIFOQueue(
                5, (dtypes_lib.int32, dtypes_lib.float32), shared_name="foo", name="Q"
            )
        self.assertTrue(isinstance(q.queue_ref, ops.Tensor))
        self.assertProtoEquals(
            """
    ...
    def testEnqueueManyWithShape(self):
        ...
        self.evaluate(q.enqueue_many([[1, 2, 3, 4], [[1, 1], [2, 2], [3, 3], [4, 4]]]))
    ...
    def testQueueInFunction(self):
        class _M(module.Module):
            ...
            def uses_queues(self, x):
                if self.q2 is None:
                    self.q2 = data_flow_ops.FIFOQueue(
                        10, [dtypes_lib.int32], shapes=[()]
                    )
                self.q2.enqueue(x)
                self.q2.enqueue(x + 3)
        ...
        if context.executing_eagerly():
            ...
            with self.assertRaisesRegex(
                errors_impl.NotFoundError, r"Resource .* does not exist."
            ):
    ...
    def testEnqueueWrongShape(self):
        q = data_flow_ops.FIFOQueue(10, (dtypes_lib.int32, dtypes_lib.int32), ((), (2)))
...
class GPUCompatibleFIFOQueueTests(test.TestCase):
    ...
    def testEnqueueDequeue(self):
        with test_util.use_gpu():
            ...
            elems = [array_ops.identity(constant_op.constant(x)) for x in elems_numpy]
...
class UnconvertedFIFOQueueTests(test.TestCase):
    ...
    def testEnqueueDequeueOneComponent(self):
        with self.cached_session() as sess:
            q = data_flow_ops.FIFOQueue(10, dtypes_lib.float32, shapes=((),), names="f")
    ...
    def testEnqueueDequeueMultipleComponent(self):
        with self.cached_session() as sess:
            q = data_flow_ops.FIFOQueue(
                10,
                (dtypes_lib.float32, dtypes_lib.int32, dtypes_lib.string),
                shapes=((), (), ()),
                names=("f", "i", "s"),
            )
            # Verify that enqueue() checks that when using names we must enqueue a
            ...
            with self.assertRaisesRegex(ValueError, "match names of Queue"):
                enqueue_op4 = q.enqueue_many(
                    {"f": [40.0, 50.0], "i": [126, 127], "s": ["dd", "ee"], "x": [1, 2]}
                )
            enqueue_op4 = q.enqueue_many(
                {"f": [40.0, 50.0], "i": [126, 127], "s": ["dd", "ee"]}
            )
            dequeue = q.dequeue()
            dequeue_2 = q.dequeue_many(2)
            self.evaluate(enqueue_op)
    ...
    def testEnqueueWrongShapeAtRuntime(self):
        with self.cached_session() as sess:
            q = data_flow_ops.FIFOQueue(
                10, (dtypes_lib.int32, dtypes_lib.int32), ((2, 2), (3, 3))
            )
            ...
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, r"Expected \[3,3\], got \[3,4\]"
            ):
                sess.run(
                    [enqueue_op],
    ...
    def testDequeueEnqueueFail(self):
        with self.cached_session() as session:
            ...
            with self.assertRaisesWithPredicateMatch(
                errors_impl.InvalidArgumentError, lambda e: "Before enqueue" in str(e)
            ):
                session.run([a, c])
...
class FIFOQueueWithTimeoutTest(test.TestCase):
    def testDequeueWithTimeout(self):
        with self.session(
            config=config_pb2.ConfigProto(operation_timeout_in_ms=20)
        ) as sess:
            q = data_flow_ops.FIFOQueue(10, dtypes_lib.float32)
            self.assertEqual(compat.as_bytes(""), q.queue_ref.op.get_attr("container"))
            dequeued_t = q.dequeue()

            # Intentionally do not run any enqueue_ops so that dequeue will block
            # until operation_timeout_in_ms.
            with self.assertRaisesRegex(
                errors_impl.DeadlineExceededError, "Timed out waiting for notification"
            ):
                self.evaluate(dequeued_t)
    ...
    def testContainer(self):
        with ops.Graph().as_default():
            with ops.container("test"):
                q = data_flow_ops.FIFOQueue(10, dtypes_lib.float32)
        self.assertEqual(compat.as_bytes("test"), q.queue_ref.op.get_attr("container"))
