...
class AtrousConvolutionTest(test.TestCase):
    ...
    def _test_atrous_convolution(
        self, add_check, input_shape, filter_shape, dilation_rate, **kwargs
    ):
        filters = np.arange(np.prod(filter_shape), dtype=np.float32).reshape(
            filter_shape
        )
        ...
        with self._delay_checks() as add_check:
            for padding in ["SAME", "VALID"]:
                for depth, height, width in [[9, 9, 10], [9, 10, 9]]:
                    for kernel_depth, kernel_height, kernel_width in [
                        [3, 3, 3],
                        [3, 2, 2],
                        [2, 1, 3],
                    ]:
                        for dilation_rate in [
                            [1, 1, 1],
                            [3, 3, 3],
                            [3, 2, 3],
                            [3, 1, 2],
                ...
                for height in range(15, 17):
                    for width in range(15, 17):
                        ...
                        for kernel in kernel_sizes:
                            ...
                            f1 = 1e-2 * np.random.random_sample(f_shape).astype(
                                np.float32
                            )
                            f2 = 1e-2 * np.random.random_sample(f_shape).astype(
                                np.float32
                            )

                            def combined_op(
                                converted_input, num_spatial_dims, padding_arg
                            ...
                            for rate_height in range(2, 4):
                                for rate_width in range(2, 4):
                                    ...
                                    def check(y1_eval, y2_eval):
                                        self.assertAllClose(
                                            y1_eval, y2_eval, rtol=1e-2, atol=1e-2
                                        )
