...
class PadOpTest(test.TestCase):
    ...
    def testNpPad(self):
        self.assertAllEqual(
            ...
            self._npPad(
                np.array([[3, 3], [4, 4], [5, 5]]), [[1, 2], [1, 3]], mode="constant"
            ),
        )
        ...
        self.assertAllEqual(
            ...
            self._npPad(
                np.array([[0, 1, 2], [3, 4, 9]]), [[1, 1], [1, 2]], mode="reflect"
            ),
        )
        ...
        self.assertAllEqual(
            ...
            self._npPad(
                np.array([[0, 1, 2], [3, 4, 9]]), [[1, 1], [1, 2]], mode="symmetric"
            ),
        )
    ...
    def testPaddingsDim2(self):
        with test_util.use_gpu():
            with self.assertRaisesRegex(
                ...
                array_ops.pad(
                    array_ops.reshape([1, 2], shape=[1, 2]),
                    array_ops.reshape([1, 2], shape=[2, 1]),
                )
    ...
    def testPaddingsNonNegative(self):
        with test_util.use_gpu():
            with self.assertRaisesRegex(
                ...
                array_ops.pad(
                    constant_op.constant([1], shape=[1]),
        ...
        with test_util.use_gpu():
            with self.assertRaises(Exception):
                array_ops.pad(
                    ...
                    constant_op.constant([2, 0], shape=[1, 2]),
                    mode="REFLECT",
                ).eval()
            with self.assertRaises(Exception):
                array_ops.pad(
                    constant_op.constant([1], shape=[2]),
                    constant_op.constant([0, 3], shape=[1, 2]),
                    mode="SYMMETRIC",
                ).eval()
    ...
    def testString(self):
        ...
        constant = array_ops.pad(
            x, [[1, 0], [0, 1]], mode="CONSTANT", constant_values="PAD"
        )
        reflect = array_ops.pad(
            x, [[1, 0], [0, 1]], mode="REFLECT", constant_values="PAD"
        )
        symmetric = array_ops.pad(
            x, [[1, 0], [0, 1]], mode="SYMMETRIC", constant_values="PAD"
        )
        with test_util.use_gpu():
            self.assertAllEqual(
                [
            ...
            self.assertAllEqual(
                [
                    ...
                    [b"Hello", b"World", b"World"],
                    [b"Goodnight", b"Moon", b"Moon"],
                ],
                self.evaluate(symmetric),
            )
    ...
    def testCollapseAdjacentNonPaddedDimensions(self):
        ...
        for paddings_value in paddings_values:
            for dtype in [dtypes.float32, dtypes.int32]:
                ...
                middle = array_ops.slice(
                    padded,
                    [row[0] for row in paddings_value],
                    [dim.value for dim in inp.shape.dims],
                )
                left = array_ops.slice(
                    padded, [0, 0, 0, 0], [row[0] for row in paddings_value]
                )
                ...
                with self.cached_session():
                    ...
                    self.assertAllEqual(
                        np.zeros([row[0] for row in paddings_value]),
                        self.evaluate(left),
                    )
                    self.assertAllEqual(
