...
class UnsortedSegmentJoinOpTest(UnicodeTestCase, parameterized.TestCase):
    ...
    def test_multiple_cases_with_different_dims(
        self, inputs, segment_ids, num_segments, separator, output_array
    ):
        res = self.evaluate(
            string_ops.unsorted_segment_join(
                inputs=inputs,
    ...
    @parameterized.parameters(
        [
            {"separator": "", "output_array": ["thisisatest"]},
            {"separator": ":", "output_array": ["this:is:a:test"]},
            {"separator": "UNK", "output_array": ["thisUNKisUNKaUNKtest"]},
        ]
    )
