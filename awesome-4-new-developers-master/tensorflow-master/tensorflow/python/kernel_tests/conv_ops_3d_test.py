...
class Conv3DTest(test.TestCase):
    ...
    def _SetupValuesForDevice(
        ...
        with self.cached_session(use_gpu=use_gpu):
            ...
            conv = nn_ops.conv3d(
                t1, t2, strides, padding=padding, data_format=data_format
            )
            if data_format == "NCDHW":
    ...
    def _VerifyDilatedConvValues(
        self, tensor_in_sizes, filter_in_sizes, stride, padding, dilations
    ):
        expected_results = []
        computed_results = []
        default_dilations = (
    ...
    def _CreateNumpyTensor(self, sizes):
        return np.asarray(
            [f * 1.0 for f in range(1, np.prod(sizes) + 1)], dtype=np.float32
        ).reshape(sizes)
    ...
    def ConstructAndTestGradient(self, **kwargs):
        for data_format, use_gpu in GetTestConfigs():
            self._ConstructAndTestGradientForConfig(
                data_format=data_format, use_gpu=use_gpu, **kwargs
            )
        ...
        if default_dilations or use_gpu:
            with self.cached_session(use_gpu=use_gpu) as sess:
                ...
                actual_grad = gradients_impl.gradients(
                    actual, t1 if mode == "input" else t2
                )[0]
                expected_grad = gradients_impl.gradients(
                    expected, t1 if mode == "input" else t2
                )[0]
