...
class RandomGammaTest(test.TestCase):
    ...
    def testShape(self):
        ...
        rnd = random_ops.random_gamma(
            [123], array_ops.placeholder(dtypes.float32, shape=(2,))
        )
        self.assertEqual([123, 2], rnd.get_shape().as_list())
        # Partially known shape.
        rnd = random_ops.random_gamma(
            array_ops.placeholder(dtypes.int32, shape=(1,)), array_ops.ones([7, 3])
        )
        self.assertEqual([None, 7, 3], rnd.get_shape().as_list())
        rnd = random_ops.random_gamma(
            array_ops.placeholder(dtypes.int32, shape=(3,)), array_ops.ones([9, 6])
        )
        self.assertEqual([None, None, None, 9, 6], rnd.get_shape().as_list())
        # Unknown shape.
        rnd = random_ops.random_gamma(
            array_ops.placeholder(dtypes.int32), array_ops.placeholder(dtypes.float32)
        )
        ...
        for dt in [dtypes.float16, dtypes.float32, dtypes.float64]:
            with self.cached_session():
                ...
                self.assertEqual(
                    0,
                    math_ops.reduce_sum(
                        math_ops.cast(math_ops.less_equal(x, 0.0), dtype=dtypes.int64)
                    ).eval(),
                )
