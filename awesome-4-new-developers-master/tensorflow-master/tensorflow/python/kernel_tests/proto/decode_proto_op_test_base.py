...
class DecodeProtoOpTestBase(test_base.ProtoOpTestBase, parameterized.TestCase):
    ...
    def __init__(
        self, decode_module, methodName="runTest"
    ...
    def _compareValues(self, fd, vs, evs):
        ...
        if len(vs) != len(evs):
            self.fail(
                "Field %s decoded %d outputs, expected %d"
    ...
    """
        ...
        for field in fields:
            ...
            tf_type_to_primitive_value_field = {
                dtypes.bool: "bool_value",
                dtypes.float32: "float_value",
                dtypes.float64: "double_value",
                dtypes.int8: "int8_value",
                dtypes.int32: "int32_value",
                dtypes.int64: "int64_value",
                dtypes.string: "string_value",
                dtypes.uint8: "uint8_value",
                dtypes.uint32: "uint32_value",
                dtypes.uint64: "uint64_value",
            }
    ...
    def testCorruptProtobuf(self, sanitize):
        ...
        with self.assertRaisesRegexp(
            ...
            "Unable to parse binary protobuf" "|Failed to consume entire buffer",
        ):
            self.evaluate(
    ...
    def testOutOfOrderRepeated(self):
        fragments = [
            ...
            test_example_pb2.TestValue(
                message_value=[test_example_pb2.PrimitiveValue(string_value="abc")]
            ).SerializeToString(),
            test_example_pb2.TestValue(
                message_value=[test_example_pb2.PrimitiveValue(string_value="def")]
            ).SerializeToString(),
        ]
            ...
            for num_fields_to_parse in range(len(all_fields_to_parse)):
                for comb in itertools.combinations(
                    ...
                    for field_name, parsed in zip(comb, parsed_values):
                        self.assertAllEqual(
                            parsed,
                            expected_field_values[field_name],
