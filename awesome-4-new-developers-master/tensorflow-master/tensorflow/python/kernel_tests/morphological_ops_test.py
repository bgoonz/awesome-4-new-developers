...
class DilationTest(test.TestCase):
    ...
    def _testDilationSamePaddingDepth(self, use_gpu):
        # [1, 2, 2, 3]
        image = [
            [[[0.1, 0.2, 0.0], [0.2, 0.3, 0.1]], [[0.3, 0.4, 0.2], [0.4, 0.5, 0.3]]]
        ]
        # [2, 2, 3]
        kernel = [
    ...
    def _ConstructAndTestGradient(
        self, image_shape, kernel_shape, strides, rates, padding, use_gpu
    ):
        ...
        if use_gpu and test.is_gpu_available(cuda_only=True):
            try:
                ...
                with self.assertRaisesRegexp(
                    ...
                    "Determinism is not yet supported " "for Dilation2DBackpropInput.",
                ):
                    self._ConstructAndTestGradient(
                        image_shape=[1, 3, 3, 1],
                        kernel_shape=[1, 1, 1],
