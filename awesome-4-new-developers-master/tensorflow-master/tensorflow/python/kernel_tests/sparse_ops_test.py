...
class SparseMergeTest(test_util.TensorFlowTestCase):
    ...
    def _AssertResultsNotSorted(self, output, vocab_size):
        self.assertAllEqual(
            output.indices, [[0, 0], [1, 13], [1, 10], [2, 33], [2, 32], [1, 14]]
        )
    ...
    def testShouldSetLastDimensionInDynamicShape(self):
        with ops.Graph().as_default():
            ...
            ids = sparse_tensor.SparseTensor(
                indices=[[0, 0], [0, 1]], values=[1, 3], dense_shape=dynamic_shape
            )
            values = sparse_tensor.SparseTensor(
                indices=[[0, 0], [0, 1]], values=[0.4, 0.7], dense_shape=dynamic_shape
            )
            merged = sparse_ops.sparse_merge(sp_ids=ids, sp_values=values, vocab_size=5)
...
class SparseMergeHighDimTest(test_util.TensorFlowTestCase):
    def _SparseTensor_3x50(self, indices_dtype, values_dtype):
        ...
        indices0 = sparse_tensor.SparseTensorValue(
            np.array(ind, np.int64),
            np.array(indices0, indices_dtype),
            np.array(shape, np.int64),
        )
        indices1 = sparse_tensor.SparseTensorValue(
            np.array(ind, np.int64),
            np.array(indices1, indices_dtype),
            np.array(shape, np.int64),
        )
        values = sparse_tensor.SparseTensorValue(
            np.array(ind, np.int64),
        ...
        return (
            [
                sparse_tensor.SparseTensor.from_value(indices0),
    ...
    def _AssertResultsSorted(self, output, vocab_size):
        self.assertAllEqual(
            output.indices,
            [[0, 0, 12], [1, 10, 0], [1, 13, 4], [1, 14, 30], [2, 32, 1], [2, 33, 0]],
        )
        self.assertAllEqual(output.values, [-3, 1, 4, 1, 5, 9])
        self.assertAllEqual(output.dense_shape, [3] + vocab_size)
...
class SparseRetainTest(test_util.TensorFlowTestCase):
    def _SparseTensorValue_5x6(self):
        ...
        return sparse_tensor.SparseTensorValue(
            np.array(ind, np.int64), np.array(val, np.int32), np.array(shape, np.int64)
        )
...
class SparseResetShapeTest(test_util.TensorFlowTestCase):
    ...
    def _SparseTensor_2x5x6_Empty(self):
        return sparse_tensor.SparseTensor(
            constant_op.constant(np.empty(shape=[0, 3], dtype=np.int64), dtypes.int64),
    ...
    def _SparseTensorValue_2x5x6(self):
        return sparse_tensor.SparseTensorValue(
            self._IND_2_5_6, self._VAL_2_5_6, self._SHP_2_5_6
        )
    ...
    def testFeedInputUnavailableInGraphConstructionOk(self):
        with self.session(use_gpu=False) as sess:
            ...
            output = sess.run(
                sp_output, feed_dict={sp_input: self._SparseTensorValue_2x5x6()}
            )
...
class SparseFillEmptyRowsTest(test_util.TensorFlowTestCase):
    def _SparseTensorValue_5x6(self, dtype=np.int32):
        ...
        shape = np.array([5, 6])
        return sparse_tensor.SparseTensorValue(
            np.array(ind, np.int64), np.array(val, dtype), np.array(shape, np.int64)
        )
    ...
    def testFillNumber(self):
        with test_util.use_gpu():
            for sp_input in (self._SparseTensorValue_5x6(), self._SparseTensor_5x6()):
                ...
                self.assertAllEqual(output.dense_shape, [5, 6])
                self.assertAllEqual(
                    empty_row_indicator_out, np.array([0, 0, 1, 0, 1]).astype(np.bool_)
                )
    ...
    def testFillString(self):
        with test_util.force_cpu():
            ...
            self.assertAllEqual(
                output.values, [b"a", b"b", b"c", b"d", b"", b"e", b"f", b""]
            )
            self.assertAllEqual(output.dense_shape, [5, 6])
            self.assertAllEqual(
                empty_row_indicator_out, np.array([0, 0, 1, 0, 1]).astype(np.bool_)
    ...
    def testUnordered(self):
        with test_util.use_gpu():
            ...
            self.assertAllEqual(
                output.indices, [[0, 1], [0, 3], [1, 0], [2, 3], [2, 2]]
            )
    ...
    def testInvalidIndices(self):
        with test_util.use_gpu():
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, r"indices\(2, 0\) is invalid"
            ):
...
class SparseReduceTest(test_util.TensorFlowTestCase):
    ...
    def _compare(self, sp_t, reduction_axes, ndims, keep_dims, do_sum):
        ...
        with self.cached_session():
            if do_sum:
                tf_dense_ans = sparse_ops.sparse_reduce_sum(
                    sp_t, reduction_axes, keep_dims
                )
            else:
                tf_dense_ans = sparse_ops.sparse_reduce_max(
                    sp_t, reduction_axes, keep_dims
                )
            out_dense = self.evaluate(tf_dense_ans)

            if do_sum:
                tf_sparse_ans = sparse_ops.sparse_reduce_sum_sparse(
                    sp_t, reduction_axes, keep_dims
                )
            else:
                tf_sparse_ans = sparse_ops.sparse_reduce_max_sparse(
                    sp_t, reduction_axes, keep_dims
                )
            ...
            for dims in test_dims:
                ...
                for d in range(1, len(dims) + 1):
                    ...
                    err = gradient_checker.compute_gradient_error(
                        sp_t.values, (nnz,), reduced, self.evaluate(reduced).shape
                    )
    ...
    def _testSparseReduceShape(self, sp_t, reduction_axes, ndims, keep_dims, do_sum):
        densified = self.evaluate(sparse_ops.sparse_tensor_to_dense(sp_t))
...
class SparseMathOpsTest(test_util.TensorFlowTestCase):
    def _check(self, result_tensor, result_np, input_sp_t):
        ...
        self.assertAllCloseAccordingToType(input_sp_t.indices, result_tensor.indices)
        self.assertAllCloseAccordingToType(
            input_sp_t.dense_shape, result_tensor.dense_shape
        )
        ...
        with self.session(use_gpu=False):
            for dtype in [np.float32, np.float64]:
                for sp_shape, dense_shape in zip(sp_shapes, dense_shapes):
                    ...
                    cdiv = sp_t / dense_t
                    err = gradient_checker.compute_gradient_error(
                        sp_t.values, (nnz,), cdiv.values, (nnz,)
                    )
                    self.assertLess(err, 1e-4)
                    err = gradient_checker.compute_gradient_error(
                        dense_t,
...
class SparseSoftmaxTest(test_util.TensorFlowTestCase):
    ...
    def testEquivalentToDensified(self):
        ...
        for dtype in [np.float32, np.float64]:
            ...
            batched_sp_t, unused_nnz1 = _sparsify(
                sp_vals_np.reshape((1, n, m)), thresh=0.0
            )  # No masking.
            ...
            with test_util.force_cpu():
                ...
                sp_result = self.evaluate(
                    sparse_ops.sparse_softmax(batched_sp_t)
                ).values.reshape((n, m))
    ...
    def testHigherRanks(self):
        ...
        for shape in shapes:
            values = np.asarray([0.0, np.e, 1.0, 0.0, np.e, 0.0, np.e, np.e]).reshape(
                shape
            )
            sp_t, unused_nnz = _sparsify(values, thresh=1e-2)
            expected_values = [1.0, 1.0, 1.0, 0.5, 0.5]
    ...
    def testGradient(self):
        ...
        with self.cached_session(use_gpu=False):
            for dtype in [np.float32, np.float64]:
                ...
                err = gradient_checker.compute_gradient_error(
                    x_tf.values, (nnz,), y_tf.values, (nnz,)
                )
                self.assertLess(err, 1e-4)
