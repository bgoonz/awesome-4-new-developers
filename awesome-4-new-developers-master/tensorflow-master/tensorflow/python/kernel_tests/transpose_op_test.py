...
class TransposeTest(test.TestCase):
    ...
    def testRank1(self):
        self._compareCpu(np.arange(0.0, 2), [0])
        ...
        for datatype in datatypes:
            for input_shape, perm in zip(large_shapes, perms):
                with self.subTest(
                    ...
                    inp = np.arange(1, total_size + 1, dtype=datatype).reshape(
                        input_shape
                    )
    ...
    def test4DGPU(self):
        ...
        for input_shape, perm in zip(large_shapes, perms):
            with self.subTest(input_shape=input_shape, perm=perm):
                total_size = np.prod(input_shape)
                inp = np.arange(1, total_size + 1, dtype=np.float32).reshape(
                    input_shape
                )
                np_ans = self._np_transpose(inp, perm)
                with self.cached_session():
    ...
    def test3DGPU(self):
        ...
        perms = [[0, 2, 1]] * 6 + [[2, 1, 0]] * 6 + [[1, 2, 0]] * 3 + [[2, 0, 1]] * 3
        for datatype in datatypes:
            for input_shape, perm in zip(large_shapes, perms):
                with self.subTest(
                    datatype=datatype, input_shape=input_shape, perm=perm
    ...
    def testRandomizedSmallDimLargeSizeGPU(self):
        ...
        for dim_zero_size in dim_zero_sizes:
            ...
            large_dim_size = (
                int(total_size / dim_zero_size / small_dim_size) + small_dim_limit
            )
            input_shapes += (
                [[dim_zero_size, small_dim_size, large_dim_size]]
                if np.random.randint(2)
                else [[dim_zero_size, large_dim_size, small_dim_size]]
            )
    ...
    def testComplex64(self):
        ...
        self._testBoth((1 + 2j) * np.arange(0, 21).reshape([3, 7]).astype(np.complex64))
        self._testBoth(
    ...
    def testComplex128(self):
        ...
        self._testBoth(
            (1 + 2j) * np.arange(0, 210).reshape([2, 3, 5, 7]).astype(np.complex128)
        )
        self._testBoth(
            (1 + 2j)
    ...
    def testTransposeShapes(self):
        self.assertEqual(
            [],
            array_ops.transpose(constant_op.constant(1, dtype=dtypes.int32, shape=[]))
            .get_shape()
            .dims,
        )
        self.assertEqual(
            [100],
            array_ops.transpose(
                constant_op.constant(1, dtype=dtypes.int32, shape=[100])
            )
            .get_shape()
            .dims,
        )
        self.assertEqual(
            [37, 100],
            array_ops.transpose(
                constant_op.constant(1, dtype=dtypes.int32, shape=[100, 37])
            )
            .get_shape()
            .dims,
        )
        self.assertEqual(
            [100, 37],
            array_ops.transpose(
                constant_op.constant(1, dtype=dtypes.int32, shape=[100, 37]), [0, 1]
            )
            .get_shape()
            .dims,
        )
    ...
    def testTransposeDynamicShapes(self):
        ...
        expected_transpose = constant_op.constant(
            [[1, 4], [2, 5], [3, 6]]
        )  # Shape (3, 2)
    ...
    def testError(self):
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), r"must be rank 1"
        ):
            array_ops.transpose(np.arange(0.0, 30).reshape([2, 3, 5]), [[0, 1], [2, 3]])
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), r"3 is out of range"
        ):
            array_ops.transpose(np.arange(0.0, 30).reshape([2, 3, 5]), [0, 1, 3])
        self._testError(
            np.arange(0.0, 30).reshape([2, 3, 5]), [0, 1, 1], "2 is missing"
        )
