...
class SparseSplitOpTest(test.TestCase):
    ...
    def testSplitMatrixUnevenCols(self):
        for axis in (1, -1):
            ...
            self.assertAllEqual(
                sp_tensors_3[2].indices, [[0, 0], [1, 1], [2, 0], [3, 0], [4, 1]]
            )
    ...
    def testSplitMatrixUnevenRows(self):
        for axis in (0, -2):
            ...
            self.assertAllEqual(
                sp_tensors_2[0].values, [0, 2, 4, 5, 11, 13, 14, 16, 20, 23, 25]
            )
    ...
    def testInvalidAxis(self):
        for axis in (-3, 2):
            with self.assertRaisesRegexp(
                errors.InvalidArgumentError, r"axis should be in range \[-2, 2\)"
            ):
                self.evaluate(
                    sparse_ops.sparse_split(
                        sp_input=self._SparseTensor_4x6(), num_split=3, axis=axis
