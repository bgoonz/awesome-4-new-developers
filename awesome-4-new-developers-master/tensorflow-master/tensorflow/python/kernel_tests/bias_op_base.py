...
class BiasAddTestBase(test.TestCase):
    ...
    def testNpBias(self):
        self.assertAllClose(
            ...
            self._npBias(np.array([[10, 20, 30], [40, 50, 60]]), np.array([1, 2, 3])),
        )
    ...
    def test4DFloatTypes(self):
        for t in [np.float16, np.float32, np.float64]:
            ...
            self._testAll(
                np.random.rand(2048, 4, 4, 4).astype(t), np.random.rand(4).astype(t)
            )
            self._testAll(
                np.random.rand(4, 4, 4, 2048).astype(t), np.random.rand(2048).astype(t)
            )
    ...
    def _computeGradient(self, np_input, bias, dtype, data_format):
        ...
        return (
            (input_jacob_a, bias_jacob_a, grad_jacob_a),
    ...
    def _testGradient(self, np_input, bias, dtype, data_format, use_gpu):
        with self.cached_session(use_gpu=use_gpu):
            ...
            if dtype == np.float16:
                ...
                _, jacob_n = self._computeGradient(
                    np_input, bias, np.float32, data_format
                )
                input_jacob_n, bias_jacob_n, grad_jacob_n = jacob_n
    ...
    def testGradientTensor4D(self):
        for (data_format, use_gpu) in [("NHWC", False)]:
            for dtype in (dtypes.float16, dtypes.float32, dtypes.float64):
                np_input = (
                    np.arange(1.0, 49.0, dtype=dtype.as_numpy_dtype)
                    .reshape([2, 3, 4, 2])
                    .astype(np.float32)
                )
                bias = np.array([1.3, 2.4], dtype=dtype.as_numpy_dtype)
                self._testGradient(np_input, bias, dtype, data_format, use_gpu)
                np_input = (
                    np.arange(1.0, 513.0, dtype=dtype.as_numpy_dtype)
                    .reshape([64, 2, 2, 2])
                    .astype(np.float32)
                )
                self._testGradient(np_input, bias, dtype, data_format, use_gpu)
