...
class SigmoidCrossEntropyLossTest(test.TestCase):
    ...
    def testLossWithSingleDimPlaceholderForLogitsAndWeights1(self):
        ...
        with self.cached_session() as sess:
            loss = sess.run(
                loss, feed_dict={logits: np.ones((32, 1)), labels: np.ones((32, 1))}
            )
    ...
    def testLossWithSingleDimPlaceholderForLogitsAndWeights2(self):
        ...
        with self.cached_session() as sess:
            loss = sess.run(
                loss, feed_dict={logits: np.ones((32, 2)), labels: np.ones((32, 2))}
            )
            self.assertAlmostEqual(0.313, loss, 3)
...
class MeanPairwiseSquaredErrorTest(test.TestCase):
    ...
    def test3d(self):
        labels = np.array(
            [[[1, 9, 2], [12, 11, 10], [9, 8, 7]], [[-5, -5, 7], [6, 5, 4], [3, 2, 1]]]
        )
        predictions = np.array(
            [[[4, 8, 12], [1, 2, 3], [4, 5, 6]], [[8, 1, 3], [7, 8, 9], [10, 11, 12]]]
        )
...
class ComputeWeightedLossTest(test.TestCase):
    ...
    def testInvalid3x2Weight(self):
        self._test_invalid_weights(((17.0, 3.0), (5.0, 31.0), (2.0, 7.0)))
    ...
    def test1x2x4Weight(self):
        self._test_valid_weights((((17.0, 0.0, 2.0, 5.0), (3.0, 13.0, 11.0, 2.0)),))
