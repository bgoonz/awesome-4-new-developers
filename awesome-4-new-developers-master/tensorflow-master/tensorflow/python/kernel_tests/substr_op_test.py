...
class SubstrOpTest(test.TestCase, parameterized.TestCase):
    ...
    def testScalarString(self, dtype, pos, unit):
        ...
        expected_value = {"BYTE": b"ell", "UTF8_CHAR": u"e\xc3\xc3".encode("utf-8")}[
            unit
        ]
    ...
    def testVectorStrings(self, dtype, pos, unit):
        test_string = {
            ...
            "UTF8_CHAR": [x.encode("utf-8") for x in [u"H\xc3llo", u"W\U0001f604rld"]],
        }[unit]
        expected_value = {
            "BYTE": [b"ell", b"orl"],
    ...
    def testMatrixStrings(self, dtype, unit):
        test_string = {
            ...
            "UTF8_CHAR": [
                ...
                [
                    x.encode("utf-8")
                    for x in [u"He\xc3\xc3o", u"W\U0001f604rld", u"d\xfcd\xea"]
                ],
            ],
        }[unit]
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [
                    x.encode("utf-8")
                    for x in [u"\U0001d227n", u"\u053c\u025bv\u025b", u"w\u0c1dlv"]
                ],
                [
                    x.encode("utf-8")
                    for x in [u"e\xc3\xc3o", u"\U0001f604rld", u"\xfcd\xea"]
                ],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [
                    x.encode("utf-8")
                    for x in [u"\U0001d229\U0001d227", u"v\u025b", u"lv"]
                ],
                [x.encode("utf-8") for x in [u"\xc3\xc3", u"rl", u"\xfcd"]],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
    ...
    def testElementWisePosLen(self, dtype, unit):
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in [u"\U0001d227n", u"\u025bv", u"lv\u025b"]],
                [x.encode("utf-8") for x in [u"e\xc3\xc3o", u"rld", u"d\xfc"]],
                [
                    x.encode("utf-8")
                    for x in [u"xt\xea\xean", u"\U00010299ente", u"h\x86een"]
                ],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
    ...
    def testBroadcast(self, dtype, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                ...
                [x.encode("utf-8") for x in [u"nineteen", u"twenty", u"twentyone"]],
            ],
        }[unit]
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in [u"\U0001d227", u"\u025bv", u"lv\u025b"]],
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in [u"h\xcdr", u"\xean", u"t"]],
                [x.encode("utf-8") for x in [u"\xea", u"ur", u"\xcd\ua09ct"]],
                [x.encode("utf-8") for x in [u"\xea\xea", u"\xea\xea", u"\ua09ct"]],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
    ...
    def testOutOfRangeError_VectorScalar(self, dtype, pos, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                x.encode("utf-8") for x in [u"g\xc3\xc3d", u"b\xc3d", u"g\xc3\xc3d"]
            ],
        }[unit]
        position = np.array(pos, dtype)
    ...
    def testOutOfRangeError_MatrixMatrix(self, dtype, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                ...
                [x.encode("utf-8") for x in [u"g\xc3\xc3d", u"g\xc3\xc3d", u"b\xc3d"]],
                [
                    x.encode("utf-8")
                    for x in [u"g\xc3\xc3d", u"g\xc3\xc3d", u"g\xc3\xc3d"]
                ],
