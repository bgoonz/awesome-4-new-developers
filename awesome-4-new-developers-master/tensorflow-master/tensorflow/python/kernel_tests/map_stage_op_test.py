...
class MapStageTest(test.TestCase):
    ...
    def testCapacity(self):
        ...
        with ops.Graph().as_default() as G:
            ...
            with ops.device(test.gpu_device_name()):
                stager = data_flow_ops.MapStagingArea(
                    [dtypes.int32], capacity=capacity, shapes=[[]]
                )
        ...
        with self.session(graph=G) as sess:
            ...
            if not i == capacity:
                self.fail(
                    "Expected to timeout on iteration '{}' "
                    "but instead timed out on iteration '{}' "
                    "Staging Area size is '{}' and configured "
                    "capacity is '{}'.".format(capacity, i, sess.run(size), capacity)
                )
    ...
    def testOrdering(self):
        ...
        with ops.Graph().as_default() as G:
            ...
            with ops.device(test.gpu_device_name()):
                stager = data_flow_ops.MapStagingArea(
                    [dtypes.int32], shapes=[[]], ordered=True
                )
                stage = stager.put(pi, [x], [0])
                get = stager.get()
                size = stager.size()
    ...
    def testPartialDictInsert(self):
        ...
        with self.session(graph=G) as sess:
            ...
            self.assertTrue(
                sess.run([key, ret], feed_dict={gi: 0}) == [0, {"x": 1, "f": 2, "v": 1}]
            )
            ...
            self.assertTrue(
                sess.run([key, ret], feed_dict={gi: 1}) == [1, {"x": 1, "f": 2, "v": 3}]
            )
    ...
    def testPartialDictGetsAndPeeks(self):
        ...
        with self.session(graph=G) as sess:
            ...
            self.assertTrue(
                sess.run([key_xf, get_xf], feed_dict={gi: 0}) == [0, {"x": 1, "f": 2}]
            )
            ...
            self.assertTrue(
                sess.run([key_v, get_v], feed_dict={gi: 0}) == [0, {"v": 1}]
            )
            # 0 complete and 1 incomplete entry
            self.assertTrue(sess.run([size, isize]) == [0, 1])
            ...
            self.assertTrue(
                sess.run([pop_key_v, pop_v], feed_dict={pi: 1}) == [1, {"v": 1}]
            )
            # Nothing is left
            self.assertTrue(sess.run([size, isize]) == [0, 0])
