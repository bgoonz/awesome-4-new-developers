...
class NormalTest(test.TestCase):
    ...
    def testNormalLogPDF(self):
        ...
        self.assertAllEqual(
            self.evaluate(normal.batch_shape_tensor()), self.evaluate(log_pdf).shape
        )
        ...
        self.assertAllEqual(
            self.evaluate(normal.batch_shape_tensor()), self.evaluate(pdf).shape
        )
    ...
    def testNormalCDF(self):
        ...
        self.assertAllEqual(
            self.evaluate(normal.batch_shape_tensor()), self.evaluate(cdf).shape
        )
    ...
    def testNormalSurvivalFunction(self):
        ...
        self.assertAllEqual(
            self.evaluate(normal.batch_shape_tensor()), self.evaluate(sf).shape
        )
    ...
    def testNormalQuantile(self):
        ...
        self.assertAllEqual(
            self.evaluate(normal.batch_shape_tensor()), self.evaluate(x).shape
        )
    ...
    def testNormalSample(self):
        ...
        expected_samples_shape = tensor_shape.TensorShape(
            ...
            tensor_shape.TensorShape(self.evaluate(normal.batch_shape_tensor()))
        )
        ...
        expected_samples_shape = tensor_shape.TensorShape(
            [self.evaluate(n)]
        ).concatenate(normal.batch_shape)
    ...
    def testNegativeSigmaFails(self):
        with self.assertRaisesOpError("Condition x > 0 did not hold"):
            normal = normal_lib.Normal(
                loc=[1.0], scale=[-5.0], validate_args=True, name="G"
            )
    ...
    def testNormalShapeWithPlaceholders(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertAllEqual(
                sess.run(
                    normal.batch_shape_tensor(), feed_dict={mu: 5.0, sigma: [1.0, 2.0]}
                ),
                [2],
            )
