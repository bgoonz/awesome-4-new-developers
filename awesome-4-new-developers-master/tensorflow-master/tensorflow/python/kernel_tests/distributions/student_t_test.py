...
class StudentTTest(test.TestCase):
    ...
    def testStudentCDFAndLogCDF(self):
        ...
        self.assertAllClose(expected_log_cdf, log_cdf_values, atol=0.0, rtol=1e-5)
        self.assertAllClose(np.log(expected_cdf), log_cdf_values, atol=0.0, rtol=1e-5)
        self.assertAllClose(expected_cdf, cdf_values, atol=0.0, rtol=1e-5)
        self.assertAllClose(np.exp(expected_log_cdf), cdf_values, atol=0.0, rtol=1e-5)

    def testStudentEntropy(self):
        df_v = np.array([[2.0, 3.0, 7.0]])  # 1x3
        mu_v = np.array([[1.0, -1, 0]])  # 1x3
        sigma_v = np.array([[1.0, -2.0, 3.0]]).T  # transposed => 3x1
    ...
    def _checkKLApprox(self, df, mu, sigma, samples):
        ...
        hist_min1 = hist + 1.0  # put at least one item in each bucket
        hist_norm = hist_min1 / hist_min1.sum()
        hist_scipy_min1 = hist_scipy + 1.0  # put at least one item in each bucket
    ...
    def testBroadcastingPdfArgs(self):
        ...
        def _check2d_rows(student):
            ...
            xs = np.array([2.0, 3.0, 4.0], dtype=np.float32)  # (3,)
            _assert_shape(student, xs, (3, 3))
            xs = np.array([xs])  # (1,3)
            _assert_shape(student, xs, (3, 3))
            xs = xs.T  # (3,1)
    ...
    def testMeanAllowNanStatsIsFalseRaisesWhenBatchMemberIsUndefined(self):
        mu = [1.0, 3.3, 4.4]
        student = student_t.StudentT(
            df=[0.5, 5.0, 7.0], loc=mu, scale=[3.0, 2.0, 1.0], allow_nan_stats=False
        )
    ...
    def testMeanAllowNanStatsIsTrueReturnsNaNForUndefinedBatchMembers(self):
        ...
        student = student_t.StudentT(
            df=[0.5, 1.0, 3.0, 5.0, 7.0], loc=mu, scale=sigma, allow_nan_stats=True
        )
        mean = self.evaluate(student.mean())
    ...
    def testPdfOfSample(self):
        ...
        if not stats:
            return
        self.assertNear(stats.t.pdf(np.pi, 3.0, loc=np.pi), mean_pdf_val, err=1e-6)
    ...
    def testPdfOfSampleMultiDims(self):
        ...
        if not stats:
            return
        self.assertNear(
            stats.t.var(7.0, loc=0.0, scale=3.0),  # loc d.n. effect var
            np.var(sample_vals[:, :, 0]),
            err=1.0,
        )
        self.assertNear(
            stats.t.var(11.0, loc=0.0, scale=3.0),  # loc d.n. effect var
    ...
    def testNegativeDofFails(self):
        with self.assertRaisesOpError(r"Condition x > 0 did not hold"):
            student = student_t.StudentT(
                df=[2, -5.0], loc=0.0, scale=1.0, validate_args=True, name="S"
            )
            self.evaluate(student.mean())
