...
class MultinomialTest(test.TestCase):
    ...
    def testSampleUnbiasedNonScalarBatch(self):
        with self.cached_session() as sess:
            dist = multinomial.Multinomial(
                total_count=[7.0, 6.0, 5.0],
                logits=math_ops.log(2.0 * self._rng.rand(4, 3, 2).astype(np.float32)),
            )
            n = int(3e4)
            ...
            [
                ...
                [sample_mean, sample_covariance, dist.mean(), dist.covariance()]
            )
            self.assertAllEqual([4, 3, 2], sample_mean.get_shape())
            self.assertAllClose(actual_mean_, sample_mean_, atol=0.0, rtol=0.10)
    ...
    def testSampleUnbiasedScalarBatch(self):
        with self.cached_session() as sess:
            dist = multinomial.Multinomial(
                total_count=5.0,
                logits=math_ops.log(2.0 * self._rng.rand(4).astype(np.float32)),
            )
            n = int(5e3)
            x = dist.sample(n, seed=0)
            sample_mean = math_ops.reduce_mean(x, 0)
