...
def _make_grid(dtype, grid_spec):
    ...
    grid += (
        0.1 * grid_spacing * rng.randn(*grid.shape)
...
class NdtriTest(test.TestCase):
    ...
    def testNdtriDynamicShape(self):
        ...
        with self.cached_session() as sess:
            ...
            p_ = np.linspace(0.0, 1.0, 50).astype(np.float32)

            x = special_math.ndtri(p)
            x_ = sess.run(x, feed_dict={p: p_})

            expected_x_ = special.ndtri(p_)
            self.assertAllClose(expected_x_, x_, atol=0.0)
    ...
    def _baseNdtriFiniteGradientTest(self, dtype):
        ...
        _, grads = _value_and_gradient(
            lambda x: special_math.ndtri(x), p
...
class NdtrTest(test.TestCase):
    ...
    _grid32 = GridSpec(min=-12.9, max=5.0, shape=[100])
    _grid64 = GridSpec(min=-37.5, max=8.0, shape=[100])
    _error32 = ErrorSpec(rtol=1e-4, atol=0.0)
    _error64 = ErrorSpec(rtol=1e-6, atol=0.0)
...
class LogNdtrTestLower(NdtrTest):
    _use_log = True
    _grid32 = GridSpec(min=-100.0, max=sm.LOGNDTR_FLOAT32_LOWER, shape=[100])
    _grid64 = GridSpec(min=-100.0, max=sm.LOGNDTR_FLOAT64_LOWER, shape=[100])
    _error32 = ErrorSpec(rtol=1e-4, atol=0.0)
    _error64 = ErrorSpec(rtol=1e-4, atol=0.0)
...
class LogNdtrTestUpper(NdtrTest):
    ...
    _grid32 = GridSpec(
        ...
        max=12.0,  # Beyond this, log_cdf(x) may be zero.
        shape=[100],
    )
    _grid64 = GridSpec(
        min=sm.LOGNDTR_FLOAT64_UPPER,
        max=35.0,  # Beyond this, log_cdf(x) may be zero.
        shape=[100],
    )
...
class NdtrGradientTest(test.TestCase):
    _use_log = False
    _grid = GridSpec(min=-100.0, max=100.0, shape=[1, 2, 3, 8])
    ...
    def _test_grad_finite(self, dtype):
        x = constant_op.constant([-100.0, 0.0, 100.0], dtype=dtype)
    ...
    def _test_grad_accuracy(self, dtype, grid_spec, error_spec):
        ...
        with self.cached_session():
            ...
            # function delta.
            grad_eval, _ = gradient_checker.compute_gradient(
            ...
            self.assert_all_true(grad_eval > 0.0)
            # isfinite checks for NaN and Inf.
            ...
            if self._use_log:
                ...
                self.assert_all_true(g[: int(half)] > 0.0)
                self.assert_all_true(g[int(half) :] >= 0.0)
            else:
...
class ErfInvTest(test.TestCase):
    def testErfInvValues(self):
        with self.cached_session():
            ...
            x = np.linspace(0.0, 1.0, 50).astype(np.float64)
...
class LogCDFLaplaceTest(test.TestCase):
    ...
    CUTOFF_FLOAT64_UPPER = np.log(1.0 / (2.0 * np.finfo(np.float64).eps)) - 1.0
    CUTOFF_FLOAT32_UPPER = np.log(1.0 / (2.0 * np.finfo(np.float32).eps)) - 1.0
    ...
    def _test_grid_log(self, dtype, scipy_dtype, grid_spec, error_spec):
        with self.cached_session():
            ...
            scipy_dist = stats.laplace(loc=0.0, scale=1.0)
    ...
    def test_float32_extreme_values_result_and_gradient_finite_and_nonzero(self):
        with self.cached_session() as sess:
            ...
            grid = _make_grid(np.float32, GridSpec(min=-200, max=80, shape=[20, 100]))
    ...
    def test_float64_extreme_values_result_and_gradient_finite_and_nonzero(self):
        with self.cached_session() as sess:
            # On the lower branch, log_cdf_laplace(x) = x, so we know this will be
            # fine, but test to -200 anyways.
            grid = _make_grid(np.float64, GridSpec(min=-200, max=700, shape=[20, 100]))
            grid = ops.convert_to_tensor(grid)
