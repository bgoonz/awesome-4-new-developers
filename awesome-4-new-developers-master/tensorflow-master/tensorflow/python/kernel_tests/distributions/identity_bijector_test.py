...
class IdentityBijectorTest(test.TestCase):
    ...
    def testBijector(self):
        ...
        self.assertAllEqual(
            0.0, self.evaluate(bijector.inverse_log_det_jacobian(x, event_ndims=3))
        )
        self.assertAllEqual(
            0.0, self.evaluate(bijector.forward_log_det_jacobian(x, event_ndims=3))
        )
    ...
    def testScalarCongruency(self):
        with self.cached_session():
            ...
            bijector_test_util.assert_scalar_congruency(
                bijector, lower_x=-2.0, upper_x=2.0
            )
