...
class TemplateTest(test.TestCase):
    ...
    """
        ...
        test_loss = math_ops.reduce_mean(math_ops.square(test_prediction - test_output))
    ...
    def test_same_unique_name_raise_error(self):
        tmpl1 = template.make_template("_", variable_scoped_function, unique_name_="s1")
        tmpl1()
        tmpl2 = template.make_template("_", variable_scoped_function, unique_name_="s1")
    ...
    def test_unique_name_raise_error_in_eager(self):
        with context.eager_mode():
            with self.assertRaisesRegex(
                ...
                template.make_template("_", variable_scoped_function, unique_name_="s1")
    ...
    def test_enforces_no_extra_trainable_variables_eager(self):
        tmpl = template.make_template("s", function_with_side_create, trainable=True)
    ...
    def test_permits_extra_non_trainable_variables_eager(self):
        with context.eager_mode():
            tmpl = template.make_template(
                "s", function_with_side_create, trainable=False
            )
    ...
    def test_immediate_scope_creation(self):
        ...
        with variable_scope.variable_scope("ctor_scope"):
            # Create scope here:
            tmpl_immed = template.make_template("a", variable_scoped_function, True)
            # default: create scope at __call__
            tmpl_defer = template.make_template("b", variable_scoped_function, False)
    ...
    def test_fails_gracefully(self):
        for create_scope_now in [True, False]:
            ...
            templatized_function = template.make_template(
                "f1", module_function_with_one_arg, create_scope_now_=create_scope_now
            )
    ...
    def test_name_scopes_for_variable_scopes(self):
        ...
        def linear_module(x, output_size):
            ...
            b = variable_scope.get_variable(
                "b", shape=[output_size], initializer=init_ops.zeros_initializer()
            )
            return (math_ops.matmul(x, w) + b), w

        def make_linear_module(output_size, name):
            return template.make_template(
                name, linear_module, output_size=output_size, create_scope_now_=True
            )
    ...
    def test_non_trainable_variables(self):
        ...
        with variable_scope.variable_scope("foo2"):
            ta = template.make_template("a", variable_scoped_function, trainable=True)
            tb = template.make_template("b", variable_scoped_function, trainable=False)
        # Initially there are not variables created.
    ...
    def test_local_variables(self):
        ...
        with variable_scope.variable_scope("foo3"):
            ...
            ta = template.make_template("bar", variable_scoped_function, True)
            tb = template.make_template(
                "bar", variable_scoped_function_with_local_variable
            )
