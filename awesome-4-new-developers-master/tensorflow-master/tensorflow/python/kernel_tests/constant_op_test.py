...
class ConstantTest(test.TestCase):
    ...
    def testBFloat16(self):
        ...
        self._testAll(np.random.normal(size=30).reshape([2, 3, 5]).astype(bfloat16))
        self._testAll(np.empty((2, 0, 5)).astype(bfloat16))

    @test_util.run_deprecated_v1
    def testHalf(self):
        self._testAll(np.arange(-15, 15).reshape([2, 3, 5]).astype(np.float16))
        self._testAll(np.random.normal(size=30).reshape([2, 3, 5]).astype(np.float16))
    ...
    def testComplex64(self):
        ...
        self._testAll(
            (1 + 2j) * np.random.normal(size=30).reshape([2, 3, 5]).astype(np.complex64)
        )
    ...
    def testVariant(self):
        ...
        with self.session(use_gpu=False):
            ...
            logging_const_op = logging_ops.Print(
                const, [const], message="Variant storing an int, decoded const value:"
            ).op
    ...
    def testAsTensorForShapeInput(self):
        with self.cached_session():
            ...
            x = ops.convert_to_tensor(
                tensor_shape.TensorShape([2 ** 31 - 1, 2, 3]), dtype=dtypes_lib.int32
            )
            self.assertEqual(dtypes_lib.int32, x.dtype)
            self.assertAllEqual([2 ** 31 - 1, 2, 3], self.evaluate(x))
            ...
            x = ops.convert_to_tensor(
                tensor_shape.TensorShape([2 ** 31, 2, 3]), dtype=dtypes_lib.int64
            )
            self.assertEqual(dtypes_lib.int64, x.dtype)
            self.assertAllEqual([2 ** 31, 2, 3], self.evaluate(x))
            ...
            with self.assertRaisesRegex(
                ...
                x = ops.convert_to_tensor(
                    tensor_shape.TensorShape([2 ** 31, 2, 3]), dtype=dtypes_lib.int32
                )
    ...
    def testZerosLikeVariant(self):
        ...
        with self.session(use_gpu=False):
            ...
            zeros_like_op = logging_ops.Print(
                zeros_like,
                [const_variant, zeros_like],
                message="Variant storing an int, input and output of zeros_like:",
            ).op
...
class FillTest(test.TestCase):
    ...
    def testShapeFunctionEdgeCases(self):
        ...
        f = array_ops.fill([array_ops.placeholder(dtypes_lib.int32, shape=()), 17], 1.0)
