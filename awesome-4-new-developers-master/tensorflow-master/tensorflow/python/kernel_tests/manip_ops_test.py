...
class RollTest(test_util.TensorFlowTestCase):
    ...
    def testNegativeAxis(self):
        ...
        with self.cached_session():
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "is out of range"
            ):
                manip_ops.roll(
                    np.random.randint(-100, 100, (4, 4)).astype(np.int32), 3, -10
                ).eval()
    ...
    def testRollInputMustVectorHigherRaises(self):
        ...
        with self.cached_session():
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "input must be 1-D or higher"
            ):
