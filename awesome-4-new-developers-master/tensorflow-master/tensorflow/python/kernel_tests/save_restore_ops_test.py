...
class SaveTest(test.TestCase):
    ...
    def testRelativePath(self):
        ...
        self.evaluate(
            io_ops.save_v2("ckpt", ["x"], [""], [constant_op.constant(100.0)])
        )
        self.assertAllEqual(
            [100.0],
            self.evaluate(io_ops.restore_v2("ckpt", ["x"], [""], [dtypes.float32])),
        )
...
class ShardedFileOpsTest(test.TestCase):
    def testShardedFileName(self):
        with session.Session(
            ...
            self.assertEqual(
                gen_io_ops.sharded_filename("foo", 4, 100).eval(), b"foo-00004-of-00100"
            )
            self.assertEqual(
