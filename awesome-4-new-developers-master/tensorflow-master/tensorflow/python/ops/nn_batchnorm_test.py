...
class BatchNormalizationTest(test.TestCase):
    ...
    def _tfBatchNormV1BW(
        self, x, m, v, beta, gamma, epsilon, scale_after_normalization
    ):
    ...
    def _tfBatchNormV2(
        ...
        return nn_impl.batch_normalization(
            ...
            m,
            v,
            beta if shift_after_normalization else None,
    ...
    def _testBatchNormGradient(
        ...
        print(
            "Batch normalization v%d %s gradient %s scale and %s shift err = "
            % (
                version,
                tag,
                "with" if scale_after_normalization else "without",
    ...
    def testBatchNormVarianceGradient(self):
        self._testBatchNormGradientInAllNeedConfigs(2, "variance", err_tolerance=1e-03)
    ...
    def testBatchNormBetaGradient(self):
        ...
        for scale_after_normalization in [True, False]:
            for v in [1, 2]:
                self._testBatchNormGradient(
                    3, "beta", scale_after_normalization, True, v
                )
    ...
    def _testBatchNormArbitraryShapes(
        self,
        x_shape,
        param_shape,
        atol=0.0001,
    ...
    def testBatchNormMixedPrecision(self):
        self._testBatchNormArbitraryShapes(
            (3, 3), (1, 3), dtype=dtypes.float16, param_dtype=dtypes.float32, atol=0.001
        )
        ...
        for use_gpu in [True, False]:
            with self.cached_session(use_gpu=use_gpu) as sess:
                if has_shape:
                    x = constant_op.constant(x_val, name="x")
                    x.set_shape(x_shape)
                    op_c, op_m, op_v, op_s = self._opSuffStats(
                        x, axes, shift, keep_dims
                    )
                    if shift:
                    ...
                    if shift:
                        tf_c, tf_m, tf_v, tf_s = sess.run(
                            [op_c, op_m, op_v, op_s], feed_dict={x: x_val}
                        )
                    else:
                        tf_c, tf_m, tf_v = sess.run(
                            [op_c, op_m, op_v], feed_dict={x: x_val}
                        )
        ...
        for use_gpu in [True, False]:
            with self.cached_session(use_gpu=use_gpu) as sess:
                ...
                if shift:
                    ...
                    tf_shift_v = None
                opm, opv = self._opNormalizeMoments(
                    tf_counts, tf_mean_ss, tf_variance_ss, tf_shift_v
                )
...
class MomentsTest(test.TestCase):
    ...
    def RunMomentTestWithDynamicShape(self, shape, axes, keep_dims, dtype):
        with self.cached_session():
            ...
            expected_x_squared = (
                np.sum(np.multiply(x_numpy, x_numpy), axis=ax, keepdims=keep_dims)
                / num_elements
            )
    ...
    def testGlobalNormalization(self):
        for keep_dims in [False, True]:
            for dtype in [dtypes.float32, dtypes.float16]:
                self.RunMomentTest(
                    shape=[2, 3, 5, 4], axes=[0, 1, 2], keep_dims=keep_dims, dtype=dtype
                )
                self.RunMomentTestWithDynamicShape(
                    shape=[2, 3, 5, 4], axes=[0, 1, 2], keep_dims=keep_dims, dtype=dtype
                )
    ...
    def testAxes(self):
        for keep_dims in [False, True]:
            for dtype in [dtypes.float32, dtypes.float16]:
                self.RunMomentTest(
                    shape=[2, 3, 5, 4], axes=[1, 2, 3], keep_dims=keep_dims, dtype=dtype
                )
                self.RunMomentTestWithDynamicShape(
                    shape=[2, 3, 5, 4], axes=[1, 2, 3], keep_dims=keep_dims, dtype=dtype
                )
    ...
    def _testGlobalGradient(self, from_y="mean"):
        with self.cached_session():
            ...
            for (i, v) in enumerate(inputs_to_compute_gradients_for):
                err = gradient_checker.compute_gradient_error(
                    v, v.get_shape().as_list(), y, y_shape
                )
