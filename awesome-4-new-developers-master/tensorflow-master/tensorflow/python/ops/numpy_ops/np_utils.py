...
def _has_docstring(f):
    return f and hasattr(f, "__doc__") and isinstance(f.__doc__, str) and f.__doc__
...
def generate_link(flag, np_fun_name):
  ...
  """
    ...
    if flag == "dev":
        ...
        template = (
            "https://numpy.org/doc/" + flag + "/reference/generated/numpy.%s.html"
        )
    else:
        return None
...
_is_sig_mismatch_an_error = os.getenv("TF_NP_SIG_MISMATCH_IS_ERROR", "False") in (
    "True",
    "true",
    "1",
)
...
def np_doc(np_fun_name, np_fun=None, export=True, unsupported_params=None, link=None):
...
def _result_type_binary(t1, t2):  # pylint: disable=missing-function-docstring
    ...
    try:
        return np_dtypes._result_type(
            _maybe_get_dtype(t1),  # pylint: disable=protected-access
            _maybe_get_dtype(t2),
        )  # pylint: disable=protected-access
    except ValueError:
