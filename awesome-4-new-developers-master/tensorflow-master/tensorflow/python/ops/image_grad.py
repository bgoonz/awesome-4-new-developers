...
def _RGBToHSVGrad(op, grad):
  ...
  """
    ...
    # Max values
    red_biggest = math_ops.cast((reds >= blues) & (reds >= greens), dtypes.float32)
    green_biggest = math_ops.cast((greens > reds) & (greens >= blues), dtypes.float32)
    blue_biggest = math_ops.cast((blues > reds) & (blues > greens), dtypes.float32)
    # Min values
    red_smallest = math_ops.cast((reds < blues) & (reds < greens), dtypes.float32)
    green_smallest = math_ops.cast((greens <= reds) & (greens < blues), dtypes.float32)
    blue_smallest = math_ops.cast((blues <= reds) & (blues <= greens), dtypes.float32)
    ...
    ds_dr = math_ops.cast(reds > 0, dtypes.float32) * math_ops.add(
        red_biggest
    ...
    ds_dg = math_ops.cast(greens > 0, dtypes.float32) * math_ops.add(
        green_biggest
    ...
    ds_db = math_ops.cast(blues > 0, dtypes.float32) * math_ops.add(
        blue_biggest
    ...
    dh_dr_1 = 60 * (
        math_ops.cast(reds > 0, dtypes.float32)
        * red_biggest
        * -1
        * (greens - blues)
    ...
    dh_dr_2 = 60 * (
        math_ops.cast(greens > 0, dtypes.float32)
        * green_biggest
        * red_smallest
        * (blues - greens)
    ...
    dh_dr_4 = 60 * (
        math_ops.cast(blues > 0, dtypes.float32)
        * blue_biggest
        * red_smallest
        * (blues - greens)
    ...
    dv_drgb = array_ops.stack(
        [grad[..., 2] * dv_dr, grad[..., 2] * dv_dg, grad[..., 2] * dv_db], axis=-1
    )
    ds_drgb = array_ops.stack(
        [grad[..., 1] * ds_dr, grad[..., 1] * ds_dg, grad[..., 1] * ds_db], axis=-1
    )
    dh_drgb = array_ops.stack(
        [grad[..., 0] * dh_dr, grad[..., 0] * dh_dg, grad[..., 0] * dh_db], axis=-1
    )
