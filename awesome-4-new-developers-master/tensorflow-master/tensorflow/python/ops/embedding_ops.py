...
def _embedding_lookup_and_transform(
    params, ids, partition_strategy="mod", name=None, max_norm=None, transform_fn=None
):
  ...
  """
    ...
    with ops.name_scope(name, "embedding_lookup", params + [ids]) as name:
        ...
        if not any(
            isinstance(p, resource_variable_ops.BaseResourceVariable) for p in params
        ):
        ...
        if np == 1 and (not transform_fn or ids.get_shape().ndims == 1):
            ...
            pindices = data_flow_ops.dynamic_partition(
                original_indices, p_assignments, np
            )
  ...
  """
    ...
    if not ignore_weights:
        if not isinstance(sp_weights, sparse_tensor.SparseTensor):
            raise TypeError(
                f"sp_weights must be either None or SparseTensor,"
...
def safe_embedding_lookup_sparse(
  ...
  """
    ...
    with ops.name_scope(
        name, "embedding_lookup", embedding_weights + [sparse_ids, sparse_weights]
    ) as scope:
...
def embedding_lookup_ragged(
    embedding_weights, ragged_ids, partition_strategy="mod", max_norm=None, name=None
):
