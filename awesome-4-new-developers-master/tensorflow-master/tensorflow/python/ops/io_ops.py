...
def _save(filename, tensor_names, tensors, tensor_slices=None, name="save"):
  ...
  """
    if tensor_slices is None:
        ...
        return gen_io_ops.save_slices(
            filename, tensor_names, tensor_slices, tensors, name=name
        )
...
class ReaderBase(object):
  ...
  """
    ...
    def read_up_to(self, queue, num_records, name=None):  # pylint: disable=invalid-name
    ...
    """
        if isinstance(queue, ops.Tensor):
            ...
            queue_ref = queue.queue_ref
        if self._reader_ref.dtype == dtypes.resource:
            return gen_io_ops.reader_read_up_to_v2(
                self._reader_ref, queue_ref, num_records, name=name
            )
        else:
            # For compatibility with pre-resource queues, create a ref(string) tensor
            # which can be looked up as the same queue by a resource manager.
            old_queue_op = gen_data_flow_ops.fake_queue(queue_ref)
            return gen_io_ops.reader_read_up_to(
                self._reader_ref, old_queue_op, num_records, name=name
            )
    ...
    """
        if self._reader_ref.dtype == dtypes.resource:
            return gen_io_ops.reader_num_records_produced_v2(
                self._reader_ref, name=name
            )
        else:
    ...
    """
        if self._reader_ref.dtype == dtypes.resource:
            return gen_io_ops.reader_num_work_units_completed_v2(
                self._reader_ref, name=name
            )
        else:
            return gen_io_ops.reader_num_work_units_completed(
                self._reader_ref, name=name
            )
...
class TextLineReader(ReaderBase):
  ...
  """
    ...
    Args:
      ...
      name: A name for the operation (optional).
    """
        rr = gen_io_ops.text_line_reader_v2(
            skip_header_lines=skip_header_lines, name=name
        )
