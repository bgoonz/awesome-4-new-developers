...
def while_loop(
    ...
    loop_vars = nest.map_structure(
        ops.internal_convert_to_tensor_or_indexed_slices,
        loop_vars,
    ...
    with ops.name_scope(name) as scope:
      ...
      """
            ...
            _copy_handle_data(
                nest.flatten(loop_vars[2:], expand_composites=True),
...
def _WhileGrad(op, *grads):  # pylint: disable=invalid-name
    ...
    if "skip_input_indices" in op.__dict__ and op.skip_input_indices is not None:
        captures_start_index = len(body_graph.inputs) - len(
            body_graph.internal_captures
        )
    ...
    body_grad_graph, args = _create_grad_func(
        ...
        cond_graph,
        body_graph,
        util.unique_grad_fn_name(body_graph.name),
        op,
        maximum_iterations,
...
def _build_while_op(
    ...
    def _make_op(inputs):
        while_op, tensors = util.get_op_and_outputs(
            op_fn(
                inputs,
        ...
        while_op._set_attr(
            "_num_original_outputs", attr_value_pb2.AttrValue(i=num_original_outputs)
        )
        while_op._set_attr(
            "_stateful_parallelism", attr_value_pb2.AttrValue(b=stateful_parallelism)
        )
    ...
    for op in func_graph.get_operations():
        ...
        for o in op.outputs:
            if (
                ...
                and o.dtype != dtypes.resource  # Loop counter.
                and _get_accumulator(o) is None  # Do not accumulate resource tensors.
                and o.ref() not in reverse_captures  # Has existing accumulator.
...
def _create_grad_func(
  ...
  """
    ...
    grad_func_graph = func_graph_module.func_graph_from_py_func(
        ...
        args,
        {},
        func_graph=_WhileBodyGradFuncGraph(
            name,
            cond_graph,
            body_graph,
            maximum_iterations,
    ...
    for external_capture, internal_capture in grad_func_graph.captures:
        if (
            ...
            new_output = grad_func_graph.internal_capture_to_output[
                ops.tensor_id(internal_capture)
            ]
        else:
            raise ValueError(
...
def _get_accumulator(tensor):
  ...
  """
    ...
    def get_func_graph_output(t):
        ...
        if identity_op.type == "Identity" and any(
            identity_op.outputs[0] is t for t in tensor.graph.outputs
        ):
...
class _WhileBodyGradFuncGraph(util.WhileBodyFuncGraph):
  ...
  """
    ...
    def _create_op_internal(
        ...
        if (
            op_type in optimized_reduction_ops
            and not util.output_all_intermediates()
            ...
            and not util.graph_wrapped_for_higher_order_tape_gradients(
                self._forward_graph
            )
        ):
    ...
    def _get_optimized_reduction_ops_cache_key(
        ...
        return OptimizedReductionOpsCacheKey(
            op_type, inputs, dtypes, input_types, name, attrs, op_def, compute_device
        )
    ...
    def _capture_helper(self, tensor, name):
        ...
        if _is_loop_invariant(
            tensor, self._forward_graph.inputs, self._forward_graph.outputs
        ):
            captured_tensor = super(_WhileBodyGradFuncGraph, self)._capture_helper(
                tensor, name
            )
        ...
        self._indirect_captures[ops.tensor_id(tensor)] = captured_tensor
        self.internal_capture_to_output[
            ops.tensor_id(captured_accumulator)
        ] = new_tensor_list
        return captured_tensor
    ...
    """
        ...
        if index != util.resource_input_index(
            self._forward_graph.outputs[index].name,
            forward_graph_input_names,
            forward_graph_name_to_opdef,
            self._forward_graph._functions,
        ):
...
def _duplicate_body_captures_in_cond(cond_graph, body_graph_captures):
  ...
  """
    ...
    placeholder_ops = [
        _OperationWithOutputs(ph.oper, cond_graph) for ph in placeholders
    ]
