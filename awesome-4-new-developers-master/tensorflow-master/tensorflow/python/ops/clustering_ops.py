...
class KMeans(object):
    ...
    """
        ...
        if (
            isinstance(initial_clusters, str)
            and initial_clusters not in initialization_algorithms
        ):
            raise ValueError(
        ...
        for inp in inputs:
            with ops.colocate_with(inp, ignore_existing=True):
                ...
                squared_distance = (
                    ...
                    + array_ops.transpose(
                        math_ops.reduce_sum(math_ops.square(clusters), 1, keepdims=True)
                    )
                )
    ...
    """
        ...
        output = []
        if (
            self._distance_metric == COSINE_DISTANCE
    ...
    def _clusters_l2_normalized(self):
        ...
        return self._distance_metric == COSINE_DISTANCE and (
            not self._use_mini_batch or self._mini_batch_steps_per_iteration > 1
        )
    ...
    """
        ...
        (
            ...
            cluster_centers_updated,
            update_in_steps,
        ) = self._create_variables(num_clusters)
    ...
    def _mini_batch_sync_updates_op(
        self,
        update_in_steps,
        cluster_centers_var,
        cluster_centers_updated,
        ...
        for inp, cluster_idx in zip(inputs, cluster_idx_list):
            with ops.colocate_with(inp, ignore_existing=True):
                ...
                cluster_center_updates -= (
                    math_ops.cast(
                        array_ops.reshape(count_updates, broadcast_shape), inp.dtype
                    )
                    * old_cluster_centers
                )
            ...
            update_counts = state_ops.scatter_add(
                total_counts, unique_ids, count_updates
            )
    ...
    def _full_batch_training_op(
        self, inputs, num_clusters, cluster_idx_list, cluster_centers
    ):
...
class _InitializeClustersOpFactory(object):
  ...
  """
    ...
    """
        ...
        def _body(i, _):
            ...
            new_centers = control_flow_ops.cond(
                math_ops.equal(self._num_selected, 0),
                _sample_random,
                _sample_kmc2_chain,
            )
    ...
    def _greedy_batch_sampler(self, sampler):
        ...
        # sampler.
        return control_flow_ops.cond(
            self._num_data <= self._num_remaining,
            ...
            with ops.control_dependencies(
                [
                    check_ops.assert_equal(
                        self._num_remaining, array_ops.shape(self._initial_clusters)[0]
                    )
                ]
            ):
