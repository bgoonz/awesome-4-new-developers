...
class RaggedTensorValue(object):
  ...
  """
    ...
    """
        if not (
            isinstance(row_splits, (np.ndarray, np.generic))
            and row_splits.dtype in (np.int64, np.int32)
            and row_splits.ndim == 1
        ):
        ...
        if (
            isinstance(values, RaggedTensorValue)
            and row_splits.dtype != values.row_splits.dtype
        ):
            raise ValueError(
                "row_splits and values.row_splits must have " "the same dtype"
            )
        self._values = values
    ...
    def __repr__(self):
        return "tf.RaggedTensorValue(values=%r, row_splits=%r)" % (
            self._values,
            self._row_splits,
        )
