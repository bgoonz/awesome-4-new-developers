  ...
  """
    with ops.name_scope(name, "RaggedBatchGatherWithDefault"):
        ...
        row_splits_dtype, (
            ...
            indices,
            default_value,
        ) = ragged_tensor.match_row_splits_dtypes(
            params, indices, default_value, return_dtype=True
        )
        ...
        with ops.control_dependencies(
            [
                check_ops.assert_greater_equal(
                    array_ops.rank(params), array_ops.rank(indices)
                )
            ]
        ):
            ...
            adjusted_indices = ragged_where_op.where(
                is_out_of_bounds, x=array_ops.zeros_like(indices), y=shifted_indices
            )
...
def _get_pad_shape(params, indices, row_splits_dtype):
    ...
    if params.shape.ndims == indices.shape.ndims:
        ...
        if num_batch_dimensions == 0:
            ...
            ) * (params_shape.num_partitioned_dimensions - num_batch_dimensions - 1)
        else:
            ...
            pad_dims = batch_dimensions + (array_ops.ones_like(gather_dimension),) * (
