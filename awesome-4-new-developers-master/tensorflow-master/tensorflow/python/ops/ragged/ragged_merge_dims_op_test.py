...
class RaggedMergeDimsOpTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testRaggedMergeDims(
        self, rt, outer_axis, inner_axis, expected, ragged_ranks=(None,)
    ):
        for ragged_rank in ragged_ranks:
            ...
            if outer_axis >= 0 and inner_axis >= 0:
                self.assertEqual(
                    actual.shape.rank, x.shape.rank - (inner_axis - outer_axis)
                )

            # Check behavior with negative axis.
            if outer_axis >= 0 and inner_axis >= 0:
                actual_with_neg_axis = x.merge_dims(
                    outer_axis - x.shape.rank, inner_axis - x.shape.rank
                )
    ...
    def testRaggedMergeDimsError(
        self, rt, outer_axis, inner_axis, exception, message=None, ragged_rank=None
    ):
        x = ragged_factory_ops.constant(rt, ragged_rank=ragged_rank)
