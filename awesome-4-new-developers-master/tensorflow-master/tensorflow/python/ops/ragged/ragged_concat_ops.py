...
def _ragged_stack_concat_helper(rt_inputs, axis, stack_values):
  ...
  """
    ...
    rt_inputs = [
        ragged_tensor.convert_to_tensor_or_ragged_tensor(rt_input, name="rt_input")
    ...
    if stack_values and ndims == 1 and axis == 0:
        return ragged_tensor.RaggedTensor.from_row_lengths(
            ...
            row_lengths=array_ops.concat(
                [array_ops.shape(r) for r in rt_inputs], axis=0
            ),
        )
    ...
    rt_inputs = [
        _increase_ragged_rank_to(rt, ragged_rank, row_splits_dtype) for rt in rt_inputs
    ]
        ...
        if rt_input.ragged_rank < ragged_rank:
            rt_input = rt_input.with_values(
                _increase_ragged_rank_to(
                    rt_input.values, ragged_rank - 1, row_splits_dtype
                )
            )
    return rt_input
