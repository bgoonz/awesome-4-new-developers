...
class WrappedTensorOpDispatcher(dispatch.GlobalOpDispatcher):
    ...
    OPS_THAT_RETURN_UNTRACED_RESULTS = (
        array_ops.shape,
        array_ops.shape_v2,
    ...
    def handle(self, op, args, kwargs):
        ...
        if not (
            any(self.is_wrapped_tensor_arg(x) for x in args)
            or any(self.is_wrapped_tensor_arg(x) for x in kwargs.values())
        ):
...
class RaggedTensorSupportedValuesTest(
    ...
    @parameterized.parameters(
        ...
        # Test each unary op.
        # =========================================================================
        [{"x": ([[-2.0, 3.0], [-3.0]]), "op": op} for op in test_ops.UNARY_FLOAT_OPS]
        + [{"x": ([[True, False], [True]]), "op": op} for op in test_ops.UNARY_BOOL_OPS]
        + [
            {"x": [[18, 512], [12412]], "x_dtype": dtypes.int32, "op": op}
            for op in test_ops.UNARY_INT_OPS
        ]
        + [
            {"x": ([["abcd", "efgh"], ["aabbccdd"]]), "op": op}
        ...
        + [
            ...
            {"op": math_ops.cast, "x": ([[-2.0, 3.0], [-3.0]]), "dtype": dtypes.int32},
            {
            ...
            {"op": string_ops.string_to_number, "x": ([["-2.0", "3.0"], ["-3.0"]])},
            {
            ...
            {"op": array_ops.expand_dims_v2, "x": ([[-2.0, 3.0], [-3.0]]), "axis": -1},
        ]
    )  # pyformat: disable
    ...
    @parameterized.parameters(
        ...
        # =========================================================================
        [
            {"x": [[-2.0, 3.0], [-3.0]], "y": [[5.0, 1.0], [12.0]], "op": op}
            for op in test_ops.BINARY_FLOAT_OPS
        ]
        + [
            {"x": [[-2, 3], [-3]], "y": [[5, 1], [12]], "op": op}
            for op in test_ops.BINARY_INT_OPS
        ]
        + [
            {"x": [[True, True], [False]], "y": [[False, True], [False]], "op": op}
...
@test_util.run_all_in_graph_and_eager_modes
class RaggedTensorSpecSupportedValuesTest(
    test_util.TensorFlowTestCase, parameterized.TestCase
):
    def assertAllTensorsEqual(self, list1, list2):
        self.assertLen(list1, len(list2))
        ...
        [
            ...
            (
                RaggedTensorSpec(
                    ...
                    ragged_rank=1,
                    dtype=dtypes.float64,
                    flat_values_spec=tensor_spec.TensorSpec((5,), dtypes.float64),
                ),
                (
    ...
    def testToList(self):
        with context.eager_mode():
            ...
            with self.subTest("Raise on unsupported"):
                with self.assertRaisesRegex(
                    ValueError, "values must be convertible to a list"
                ):
                    _ = rt.to_list()
