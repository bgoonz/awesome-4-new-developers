...
class StructuredTensorSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        [
            ...
            ([], {"x": 0}, r"field_specs must be a dictionary with TypeSpec values\."),
        ]
    )
    ...
    @parameterized.parameters(
        [
            {"shape": [], "fields": dict(x=[[1.0, 2.0]]), "field_specs": dict(x=T_1_2)},
            {
                "shape": [2],
                "fields": dict(
        ...
        [
            {
                ...
                "batch_size": 2,
                "batched": lambda: StructuredTensor.from_fields(
                    shape=[2], fields={"a": [1, 2], "b": [[5, 6], [7, 8]]}
                ),
            },
            {
                "unbatched": lambda: [
                    StructuredTensor.from_fields(
                        shape=[3],
                        fields={"a": [1, 2, 3], "b": [[5, 6], [6, 7], [7, 8]]},
                    ),
                    StructuredTensor.from_fields(
                        shape=[3],
                        fields={"a": [2, 3, 4], "b": [[2, 2], [3, 3], [4, 4]]},
                    ),
                ],
                "batch_size": 2,
                "batched": lambda: StructuredTensor.from_fields(
            ...
            },
            {
                "unbatched": lambda: [
                    StructuredTensor.from_fields(
                        shape=[],
                        fields={"a": 1, "b": StructuredTensor.from_fields({"x": [5]})},
                    ),
                    StructuredTensor.from_fields(
                        shape=[],
                        fields={"a": 2, "b": StructuredTensor.from_fields({"x": [6]})},
                    ),
                ],
                "batch_size": 2,
                "batched": lambda: StructuredTensor.from_fields(
    ...
    def testBatchUnbatchValues(
        self, unbatched, batch_size, batched, use_only_batched_spec=False
    ):
        ...
        batched_tensor_list = [
            array_ops.stack(tensors) for tensors in zip(*unbatched_tensor_lists)
        ]
