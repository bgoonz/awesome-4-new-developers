...
class StructuredArrayOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    def assertAllEqual(self, a, b, msg=None):
        ...
        a_tensors = [
            x
            for x in nest.flatten(a, expand_composites=True)
            if isinstance(x, ops.Tensor)
        ]
        b_tensors = [
            x
            for x in nest.flatten(b, expand_composites=True)
            if isinstance(x, ops.Tensor)
        ]
    ...
    @parameterized.named_parameters(
        [
            dict(testcase_name="0D_0", st={"x": 1}, axis=0, expected=[{"x": 1}]),
            dict(testcase_name="0D_minus_1", st={"x": 1}, axis=-1, expected=[{"x": 1}]),
            dict(
                testcase_name="1D_0",
                st=[{"x": [1, 3]}, {"x": [2, 7, 9]}],
                axis=0,
    ...
    def testExpandDimsAxisTooBig(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "axis=4 out of bounds: expected -4<=axis<4"
        ):
    ...
    def testExpandDimsAxisTooSmall(self):
        ...
        st = StructuredTensor.from_pyval(st)
        with self.assertRaisesRegex(
            ValueError, "axis=-5 out of bounds: expected -4<=axis<4"
        ):
    ...
    def testSizeObject(self, row_partitions, shape, dtype, expected):
        ...
        st = StructuredTensor.from_fields(
            {}, shape=shape, row_partitions=row_partitions
        )
    ...
    @parameterized.named_parameters(
        [
            dict(testcase_name="scalar", row_partitions=None, shape=(), expected=0),
            dict(testcase_name="list_0", row_partitions=None, shape=(0,), expected=1),
            dict(
                testcase_name="list_0_0", row_partitions=None, shape=(0, 0), expected=2
            ),
            dict(testcase_name="list_7", row_partitions=None, shape=(7,), expected=1),
            dict(
    ...
    @parameterized.named_parameters(
        [
            dict(testcase_name="list_empty_2_1", values=[[{}, {}], [{}]], expected=2),
            dict(testcase_name="list_empty_2", values=[{}, {}], expected=1),
            dict(testcase_name="list_empty_1", values=[{}], expected=1),
            dict(
                testcase_name="list_example_1",
                values=[{"x": [3]}, {"x": [4, 5]}],
                expected=1,
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                ...
                values=[[{"a": {}, "b": [3, 4]}], [{"a": {}, "b": [5, 6]}]],
                axis=0,
                expected=[{"a": {}, "b": [3, 4]}, {"a": {}, "b": [5, 6]}],
            ),
            dict(
    ...
    def testConcatTuple(self):
        values = (
            StructuredTensor.from_pyval([{"a": 3}]),
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                testcase_name="rank_submessage_change",
                values=[[{"a": [{"b": [[3]]}]}], [{"a": [[{"b": [3]}]]}]],
                axis=0,
                error_type=ValueError,
            ...
            dict(
                testcase_name="rank_message_change",
                values=[[{"a": [3]}], [[{"a": 3}]]],
                axis=0,
                error_type=ValueError,
                error_regex="Ranks of sub-message do not match",
            ...
            dict(
                testcase_name="concat_axis_large",
                values=[[{"a": [3]}], [{"a": [4]}]],
                axis=1,
                error_type=ValueError,
            ...
            dict(
                testcase_name="concat_axis_large_neg",
                values=[[{"a": [3]}], [{"a": [4]}]],
                axis=-2,
                error_type=ValueError,
            ...
            dict(
                testcase_name="concat_deep_rank_wrong",
                values=[[{"a": [3]}], [{"a": [[4]]}]],
                axis=0,
                error_type=ValueError,
    ...
    def testStructuredTensorArrayLikeNoRank(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Can't build StructuredTensor w/ unknown rank"
        ):
    ...
    def testGatherError(
        self, params, indices, axis, batch_dims, error_type, error_regex
    ):
        params = StructuredTensor.from_pyval(params)
        with self.assertRaisesRegex(error_type, error_regex):
