...
class StructuredTensorSliceTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    def assertAllEqual(self, a, b, msg=None):
        ...
        elif isinstance(a, structured_tensor.StructuredTensor) and isinstance(
            b, structured_tensor.StructuredTensor
        ):
    ...
    def testGetitemFromVectorStruct(
        self, slice_spec, expected, test_requires_typespec=False
    ):
        # By default, lists are converted to RaggedTensors.
    ...
    @parameterized.parameters(
        [
            (
                SLICE_BUILDER[:2],
                r"Key for indexing a StructuredTensor must be "
    ...
    @parameterized.parameters(
        [(SLICE_BUILDER[:, 1], r"Key for indexing a StructuredTensor must be a string")]
    )
