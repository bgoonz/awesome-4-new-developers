...
def clip_by_value(t, clip_value_min, clip_value_max, name=None):
  ...
  """
    with ops.name_scope(
        name, "clip_by_value", [t, clip_value_min, clip_value_max]
    ) as name:
...
def global_norm(t_list, name=None):
  ...
  """
    if not isinstance(t_list, collections_abc.Sequence) or isinstance(
    ...
    with ops.name_scope(name, "global_norm", t_list) as name:
        values = [
            ops.convert_to_tensor(
                t.values if isinstance(t, ops.IndexedSlices) else t, name="t_%d" % i
            )
            if t is not None
            else t
