...
class InitializersTest(test.TestCase):
    ...
    def test_Orthogonal(self):
        ...
        with self.cached_session():
            for tensor_shape in [shape, tensor_shape_lib.TensorShape(shape)]:
                self._runner(
                    init_ops.Orthogonal(seed=123), tensor_shape, target_mean=0.0
                )
            ...
            for tensor_shape in [shape, tensor_shape_lib.TensorShape(shape)]:
                with self.assertRaises(ValueError):
                    self._runner(
                        init_ops.Identity(),
                        tensor_shape,
                        target_mean=1.0 / int(tensor_shape[0]),
                        target_max=1.0,
                    )
    ...
    def test_Zeros(self):
        ...
        with self.cached_session():
            for tensor_shape in [shape, tensor_shape_lib.TensorShape(shape)]:
                self._runner(
                    init_ops.Zeros(), tensor_shape, target_mean=0.0, target_max=0.0
                )
    ...
    def test_Ones(self):
        ...
        with self.cached_session():
            for tensor_shape in [shape, tensor_shape_lib.TensorShape(shape)]:
                self._runner(
                    init_ops.Ones(), tensor_shape, target_mean=1.0, target_max=1.0
                )
