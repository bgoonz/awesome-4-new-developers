...
def add_leading_unit_dimensions(x, num_dimensions):  # pylint: disable=invalid-name
    new_shape = array_ops.concat(
        [array_ops.ones([num_dimensions], dtype=dtypes.int32), array_ops.shape(x)],
        axis=0,
    )
...
def _Ndtr(x):
    ...
    y = array_ops.where(
        z < half_sqrt_2,
        1.0 + math_ops.erf(w),
        array_ops.where(w > 0.0, 2.0 - math_ops.erfc(z), math_ops.erfc(z)),
    )
    return 0.5 * y


@ops.RegisterGradient("StatelessParameterizedTruncatedNormal")
def _StatelessParameterizedTruncatedNormalGrad(
    op, grad
  ...
  """
    ...
    with ops.control_dependencies([grad]):
        ...
        dmaxval = math_ops.exp(
            0.5 * (sample_std ** 2 - maxval_std ** 2) + math_ops.log(cdf_sample)
        )
        dminval = math_ops.exp(
            0.5 * (sample_std ** 2 - minval_std ** 2) + math_ops.log1p(-cdf_sample)
        )
        ...
        grad_stddev = array_ops.reshape(
            math_ops.reduce_sum(grad_stddev, axis=rstddev, keepdims=True), stddev_shape
        )

        grad_minval = array_ops.reshape(
            math_ops.reduce_sum(grad_minval, axis=rminval, keepdims=True), minval_shape
        )

        grad_maxval = array_ops.reshape(
            math_ops.reduce_sum(grad_maxval, axis=rmaxval, keepdims=True), maxval_shape
        )
