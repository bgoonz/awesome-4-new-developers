  ...
  """
    ...
    def _batch_shape(self):
        return array_ops.broadcast_static_shape(
            self.low.get_shape(), self.high.get_shape()
        )
    ...
    def _sample_n(self, n, seed=None):
        ...
        samples = random_ops.random_uniform(shape=shape, dtype=self.dtype, seed=seed)
        return self.low + self.range() * samples

    def _prob(self, x):
        broadcasted_x = x * array_ops.ones(self.batch_shape_tensor(), dtype=x.dtype)
    ...
    def _variance(self):
        return math_ops.square(self.range()) / 12.0

    def _stddev(self):
        return self.range() / math.sqrt(12.0)
