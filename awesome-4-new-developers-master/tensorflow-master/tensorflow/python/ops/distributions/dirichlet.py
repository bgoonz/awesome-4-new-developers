  ...
  * `Gamma` is the [gamma function](
  ...
  """
    ...
    def _log_unnormalized_prob(self, x):
        ...
        return math_ops.reduce_sum(math_ops.xlogy(self.concentration - 1.0, x), -1)
    ...
    def _entropy(self):
        ...
        return (
            ...
            - math_ops.reduce_sum(
                (self.concentration - 1.0) * math_ops.digamma(self.concentration),
                axis=-1,
            )
        )
    ...
    def _variance_scale_term(self):
        ...
        return math_ops.rsqrt(1.0 + self.total_concentration[..., array_ops.newaxis])
    ...
    def _mode(self):
        ...
        if self.allow_nan_stats:
            ...
            return array_ops.where_v2(
                math_ops.reduce_all(self.concentration > 1.0, axis=-1), mode, nan
            )
...
def _kl_dirichlet_dirichlet(d1, d2, name=None):
  ...
  """
    with ops.name_scope(
        name, "kl_dirichlet_dirichlet", values=[d1.concentration, d2.concentration]
    ):
        ...
        return (
            math_ops.reduce_sum(concentration_diff * digamma_diff, axis=-1)
