...
def embed_check_nonnegative_integer_form(
    ...
    with ops.name_scope(name, values=[x]):
        ...
        if not x.dtype.is_integer:
            assertions += [
                assert_integer_form(
                    x, message="'{}' cannot contain fractional components.".format(x)
                )
            ]
...
def _is_known_unsigned_by_dtype(dt):
    ...
    return {dtypes.bool: True, dtypes.uint8: True, dtypes.uint16: True}.get(
        dt.base_dtype, False
    )
...
def embed_check_integer_casting_closed(
    x, target_dtype, assert_nonnegative=True, name="embed_check_casting_closed"
):
  ...
  """
    ...
    with ops.name_scope(name, values=[x]):
        ...
        if not _is_integer_like_by_dtype(x.dtype) and not x.dtype.is_floating:
            raise TypeError(
                "{}.dtype must be floating- or " "integer-type.".format(x.dtype.name)
            )
        if not _is_integer_like_by_dtype(target_dtype) and not target_dtype.is_floating:
            raise TypeError(
                "target_dtype ({}) must be floating- or "
                "integer-type.".format(target_dtype.name)
            )
        if not _is_integer_like_by_dtype(x.dtype) and not _is_integer_like_by_dtype(
            target_dtype
        ):
            raise TypeError(
                "At least one of {}.dtype ({}) and target_dtype ({}) "
                "must be integer-type.".format(x, x.dtype.name, target_dtype.name)
            )
        ...
        if x.dtype.is_floating:
            ...
            assertions += [
                assert_integer_form(
                    ...
                    message="Elements must be {}-equivalent.".format(target_dtype.name),
                )
            ]
        else:
            if _largest_integer_by_dtype(x.dtype) > _largest_integer_by_dtype(
                target_dtype
            ):
            ...
            if not assert_nonnegative and (
                ...
                assertions += [
                    check_ops.assert_greater_equal(
                        ...
                        _smallest_integer_by_dtype(target_dtype),
                        message=(
                            "Elements cannot be smaller than {}.".format(
                                _smallest_integer_by_dtype(target_dtype)
                            )
                        ),
                    )
  ...
  """
    with ops.name_scope(name, values=(cond, true_vector, false_vector)):
        ...
        if cond.dtype != dtypes.bool:
            raise TypeError(
                "%s.dtype=%s which is not %s" % (cond, cond.dtype, dtypes.bool)
            )
  ...
  """
    with ops.name_scope(name, values=[shape1, shape2]):
        ...
        def get_shape_tensor(s):
            ...
            raise ValueError(
                "Cannot broadcast from partially " "defined `TensorShape`."
            )
...
def fill_triangular_inverse(x, upper=False, name=None):
  ...
  """
    ...
    with ops.name_scope(name, "fill_triangular_inverse", values=[x]):
        x = ops.convert_to_tensor(x, name="x")
        if tensor_shape.dimension_value(x.shape.with_rank_at_least(2)[-1]) is not None:
            ...
            m = (n * (n + 1)) // 2
            static_final_shape = x.shape.with_rank_at_least(2)[:-2].concatenate([None])
        ndims = prefer_static_rank(x)
        if upper:
        ...
        rotated_triangular_portion = array_ops.reverse(
            array_ops.reverse(triangular_portion, axis=[ndims - 1]), axis=[ndims - 2]
        )
...
def reduce_weighted_logsumexp(
  ...
  """
    with ops.name_scope(name, "reduce_weighted_logsumexp", [logx, w]):
        ...
        wx_over_max_absw_x = math_ops.sign(w) * math_ops.exp(
            log_absw_x - max_log_absw_x
        )
  ...
  """
    with ops.name_scope(name, "softplus_inverse", values=[x]):
        ...
        x = array_ops.where_v2(
            math_ops.logical_or(is_too_small, is_too_large), array_ops.ones_like(x), x
        )
...
def process_quadrature_grid_and_probs(
  ...
  """
    with ops.name_scope(
        name, "process_quadrature_grid_and_probs", [quadrature_grid_and_probs]
    ):
  ...
  """
    with ops.name_scope(name, "pad", [x, value, count]):
        ...
        if not count.dtype.is_integer:
            raise TypeError(
                "`count.dtype` (`{}`) must be `int`-like.".format(count.dtype.name)
            )
            ...
            if axis_ >= 0 or x.shape.ndims is not None:
                ...
                middle = tensor_shape.TensorShape(
                    ...
                    if count_ is None
                        ...
                        tensor_shape.dimension_at_index(x.shape, axis)
                        + count_ * (front + back)
                    )
                )
                tail = x.shape[axis + 1 :]
        ...
        x = array_ops.pad(
            ...
            paddings=array_ops.one_hot(
                indices=array_ops.stack([axis if front else -1, axis if back else -1]),
                depth=ndims,
                axis=0,
                on_value=count,
                dtype=dtypes.int32,
...
def parent_frame_arguments():
  ...
  """
    ...
    arg_names, variable_arg_name, keyword_arg_name, local_vars = tf_inspect._inspect.getargvalues(  # pylint: disable=protected-access
