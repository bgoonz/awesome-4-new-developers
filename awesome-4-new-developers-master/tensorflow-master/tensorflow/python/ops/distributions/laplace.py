...
class Laplace(distribution.Distribution):
  ...
  """
    ...
    def __init__(
        self, loc, scale, validate_args=False, allow_nan_stats=True, name="Laplace"
    ):
    ...
    def _sample_n(self, n, seed=None):
        ...
        uniform_samples = random_ops.random_uniform(
            shape=shape,
            minval=np.nextafter(
                self.dtype.as_numpy_dtype(-1.0), self.dtype.as_numpy_dtype(0.0)
            ),
            maxval=1.0,
            dtype=self.dtype,
            seed=seed,
        )
        return self.loc - self.scale * math_ops.sign(uniform_samples) * math_ops.log1p(
            -math_ops.abs(uniform_samples)
        )
    ...
    def _cdf(self, x):
        z = self._z(x)
        return 0.5 + 0.5 * math_ops.sign(z) * (1.0 - math_ops.exp(-math_ops.abs(z)))

    def _log_unnormalized_prob(self, x):
        return -math_ops.abs(self._z(x))

    def _log_normalization(self):
        return math.log(2.0) + math_ops.log(self.scale)

    def _entropy(self):
        # Use broadcasting rules to calculate the full broadcast scale.
        scale = self.scale + array_ops.zeros_like(self.loc)
        return math.log(2.0) + 1.0 + math_ops.log(scale)
