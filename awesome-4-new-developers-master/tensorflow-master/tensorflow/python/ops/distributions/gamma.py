  ...
  """
    ...
    def _batch_shape_tensor(self):
        return array_ops.broadcast_dynamic_shape(
            array_ops.shape(self.concentration), array_ops.shape(self.rate)
        )

    def _batch_shape(self):
        return array_ops.broadcast_static_shape(
            self.concentration.get_shape(), self.rate.get_shape()
        )
    ...
    def _log_normalization(self):
        return math_ops.lgamma(self.concentration) - self.concentration * math_ops.log(
            self.rate
        )
    ...
    def _entropy(self):
        return (
            ...
            + ((1.0 - self.concentration) * math_ops.digamma(self.concentration))
        )
    ...
    def __init__(
        ...
        with ops.name_scope(name, values=[concentration, rate]) as name:
            super(GammaWithSoftplusConcentrationRate, self).__init__(
                concentration=nn.softplus(concentration, name="softplus_concentration"),
...
def _kl_gamma_gamma(g0, g1, name=None):
  ...
  """
    with ops.name_scope(
        ...
        return (
            ((g0.concentration - g1.concentration) * math_ops.digamma(g0.concentration))
            ...
            + g0.concentration * (g1.rate / g0.rate - 1.0)
        )
