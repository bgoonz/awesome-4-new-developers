...
class MatMulTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testMatMul2D(self):
        for dtype in self.SUPPORTED_DTYPES:
            ...
            c_np = constant_op.constant(
                [[58, 64], [139, 154]], shape=(2, 2), dtype=dtype
            )
    ...
    def testUnsupportedtypeMatmul(self):
        a = constant_op.constant(np.arange(1, 13), shape=[2, 2, 3], dtype=dtypes.int8)
        b = constant_op.constant(np.arange(13, 25), shape=[2, 3, 2], dtype=dtypes.int8)
        with self.assertRaisesRegex(
            (TypeError, errors.InvalidArgumentError), "list of allowed values:"
        ):
            math_ops.matmul(a, b)
        ...
        for dtype in [dtypes.int8, dtypes.bfloat16]:
            ...
            if context.executing_eagerly():
                if context.is_tfrt_enabled():
                    with self.assertRaisesRegex(
                        errors.InvalidArgumentError, "NodeDef expected inputs"
                    ):
                        math_ops.matmul(a, b, output_type=dtypes.float32)
                else:
                    with self.assertRaisesRegex(
                        errors.NotFoundError, "Could not find device for node:"
                    ):
                        math_ops.matmul(a, b, output_type=dtypes.float32)
            else:
                with self.assertRaisesRegex(
...
class AddNTest(test_util.TensorFlowTestCase):
    ...
    def testFloat(self):
        ...
        for num_inputs in range(1, 10):
            ...
            with test_util.use_gpu():
                ...
                self.assertAllClose(
                    x[0] * num_inputs, math_ops.add_n([tf_x[0]] * num_inputs)
                )
    ...
    def testInt(self):
        ...
        for num_inputs in range(1, 10):
            x = [
                np.random.randint(-128, 128, (5, 4, 3, 2, 1)) for _ in range(num_inputs)
            ]
            tf_x = ops.convert_n_to_tensor(x)
            with test_util.use_gpu():
    ...
    def testIndexedSlices(self):
        slc = ops.IndexedSlices(
            array_ops.constant([1, 2], shape=[1, 2]),
...
class DivAndModTest(test_util.TensorFlowTestCase):
    ...
    def floatTestData(self):
        ...
        divs = np.arange(-3, 0, 0.25).reshape(1, 12)
        return nums, divs
    ...
    def testFloorModBfloat16(self):
        ...
        tf_result = math_ops.floormod(
            math_ops.cast(nums, dtypes.bfloat16), math_ops.cast(divs, dtypes.bfloat16)
        )
        np_result = nums % divs
        self.assertAllEqual(tf_result, np_result)
    ...
    def testComplexDiv(self):
        ...
        _ = math_ops.divide(foo, 1.0)
        _ = math_ops.div(foo, 2.0)
    ...
    def testFloorDivGrad(self):
        ...
        self.assertAllEqual([self.evaluate(x) for x in c_grad0], [0.25, -0.125])
        self.assertAllEqual([self.evaluate(x) for x in c_grad1], [0.25, -0.125])
        self.assertAllEqual(
            [None if x is None else self.evaluate(x) for x in c_grad2], [None, None]
        )
    ...
    def testConsistent(self):
        ...
        tf3_result = math_ops.truncatediv(nums, divs) * divs + math_ops.truncatemod(
            nums, divs
        )
...
class XlogyTest(test_util.TensorFlowTestCase):
    def testXlogyNoZero(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            x = constant_op.constant([[0.1, 0.2, 3.5], [-2.0, -5.0, 30.0]], dtype=dtype)
            y = constant_op.constant([[0.1, 0.2, 3.5], [3.1, 4.0, 2.0]], dtype=dtype)
            with test_util.use_gpu():
    ...
    def testXlogyWithZero(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            ...
            y = constant_op.constant([[0.1, 0.2, 3.5], [0.0, 1.0, 2.0]], dtype=dtype)
            with test_util.use_gpu():
    ...
    def testXlogyWithZeroBroadcast(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            x = constant_op.constant([[0.0], [1.0]], dtype=dtype)
            y = constant_op.constant([[0.1, 0.2, 3.5], [0.0, 1.0, 2.0]], dtype=dtype)
            with test_util.use_gpu():
                xlogy_tf_np = self.evaluate(math_ops.xlogy(x, y))
...
class Xlog1pyTest(test_util.TensorFlowTestCase):
    def testXlog1pyNoNeg1(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            x = constant_op.constant([[0.1, 0.2, 3.5], [-2.0, -5.0, 30.0]], dtype=dtype)
            y = constant_op.constant([[-0.1, -0.2, 3.5], [3.1, -0.9, 2.0]], dtype=dtype)
            with test_util.use_gpu():
    ...
    def testXlog1pyWithNegOne(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            x = constant_op.constant(np.zeros((2, 3)), dtype=dtype)
            y = constant_op.constant([[0.1, 0.2, 3.5], [-1.0, 1.0, 2.0]], dtype=dtype)
            with test_util.use_gpu():
    ...
    def testXlog1pyWithZeroBroadcast(self):
        for dtype in [dtypes.float16, dtypes.float32, dtypes.float64]:
            x = constant_op.constant([[0.0], [1.0]], dtype=dtype)
            y = constant_op.constant([[-0.1, -0.2, -1.0], [0.0, 1.0, 2.0]], dtype=dtype)
            with test_util.use_gpu():
                xlog1py_tf_np = self.evaluate(math_ops.xlog1py(x, y))
                zeros_np = self.evaluate(array_ops.zeros_like(y[0]))
...
class NextAfterTest(test_util.TensorFlowTestCase):
    ...
    def testBasic(self):
        ...
        for dtype in [dtypes.float32, dtypes.float64]:
            ...
            eps = constant_op.constant(
                [np.finfo(dtype.as_numpy_dtype).eps], dtype=dtype
            )
