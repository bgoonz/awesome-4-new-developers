  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  """
    ...
    """
        ...
        self._use_cholesky = (
            base_operator.is_positive_definite
    ...
    def _check_shapes(self):
        ...
        tensor_shape.Dimension(
            self.base_operator.domain_dimension
        ).assert_is_compatible_with(uv_shape[-2])
    ...
    def _shape(self):
        batch_shape = array_ops.broadcast_static_shape(
            self.base_operator.batch_shape, self.diag_operator.batch_shape
        )
    ...
    def _shape_tensor(self):
        ...
        batch_shape = array_ops.broadcast_dynamic_shape(
            batch_shape, array_ops.shape(self.u)[:-2]
        )
        batch_shape = array_ops.broadcast_dynamic_shape(
            batch_shape, array_ops.shape(self.v)[:-2]
        )
