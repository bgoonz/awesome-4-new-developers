  ...
  """
    ...
    def _check_row_col(self, row, col):
        ...
        for name, tensor in [["row", row], ["col", col]]:
            if tensor.shape.ndims is not None and tensor.shape.ndims < 1:
                raise ValueError(
                    "Argument {} must have at least 1 dimension.  "
    ...
    def _assert_self_adjoint(self):
        return check_ops.assert_equal(
            ...
            self.col,
            message=(
                "row and col are not the same, and "
    ...
    def _matmul(self, x, adjoint=False, adjoint_arg=False):
        ...
        circulant = linear_operator_circulant.LinearOperatorCirculant(
            fft_ops.fft(_to_complex(circulant_col)), input_output_dtype=row.dtype
        )
    ...
    def _trace(self):
        return (
            math_ops.cast(self.domain_dimension_tensor(), dtype=self.dtype)
            * self.col[..., 0]
        )
