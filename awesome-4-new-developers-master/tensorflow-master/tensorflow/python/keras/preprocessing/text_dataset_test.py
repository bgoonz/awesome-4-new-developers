...
class TextDatasetFromDirectoryTest(keras_parameterized.TestCase):
    ...
    def test_text_dataset_from_directory_validation_split(self):
        ...
        dataset = text_dataset.text_dataset_from_directory(
            directory, batch_size=10, validation_split=0.2, subset="training", seed=1337
        )
        batch = next(iter(dataset))
        self.assertLen(batch, 2)
        self.assertEqual(batch[0].shape, (8,))
    ...
    def test_text_dataset_from_directory_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "`labels` argument should be"):
            _ = text_dataset.text_dataset_from_directory(directory, labels="other")

        with self.assertRaisesRegex(ValueError, "`label_mode` argument must be"):
            _ = text_dataset.text_dataset_from_directory(directory, label_mode="other")
        ...
        with self.assertRaisesRegex(
            ...
            _ = text_dataset.text_dataset_from_directory(directory, labels=[0, 0, 1, 1])
        ...
        with self.assertRaisesRegex(ValueError, "there must exactly 2 classes"):
            _ = text_dataset.text_dataset_from_directory(directory, label_mode="binary")

        with self.assertRaisesRegex(
            ValueError, "`validation_split` must be between 0 and 1"
        ):
            _ = text_dataset.text_dataset_from_directory(directory, validation_split=2)
