...
class TestSequence(test.TestCase):
    def test_pad_sequences(self):
        ...
        b = preprocessing_sequence.pad_sequences(a, maxlen=2, truncating="post")
    ...
    def test_TimeseriesGenerator(self):
        ...
        self.assertAllClose(batch[1], np.array([[r]]))
        ...
        self.assertAllClose(
            data_gen[0][0],
            np.array([[[10], [12], [14], [16], [18]], [[11], [13], [15], [17], [19]]]),
        )
        ...
        self.assertEqual(len(data_gen), 6)
        self.assertAllClose(
            data_gen[0][0], np.array([np.array(data[10:19:2]), np.array(data[11:20:2])])
        )
    ...
    def test_TimeSeriesGenerator_doesnt_miss_any_sample(self):
        ...
        for stride, length, batch_size, shuffle in zip(
            ...
            if shuffle:
                ...
                expected_sequences = (
                    ceil((23 - length) / float(batch_size * stride)) * batch_size
                )
            else:
