        ...
        class Foo(keras.Model):
            def __init__(self):
                ...
                self.notdep_var = data_structures.NoDependency(
                    variables_lib.Variable(1.0, name="notdep_var")
                )
    ...
    def test_extra_variable(self):
        class ExtraVar(keras.Model):
            def __init__(self):
                ...
                self.var = variables_lib.Variable(1.0)
                self.not_trainable_var = variables_lib.Variable(2.0, trainable=False)
...
class CustomCallSignatureTests(test.TestCase, parameterized.TestCase):
    ...
    def test_positional_arg_in_call(self):
        ...
        with self.assertRaisesRegex(ValueError, r"Models passed to `train_on_batch`"):
            m.train_on_batch(x, y)
        with self.assertRaisesRegex(ValueError, r"Models passed to `test_on_batch`"):
            m.test_on_batch(x, y)
        with self.assertRaisesRegex(ValueError, r"Models passed to `predict_on_batch`"):
