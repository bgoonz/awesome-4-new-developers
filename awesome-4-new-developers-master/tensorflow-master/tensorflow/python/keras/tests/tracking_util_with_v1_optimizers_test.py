...
class CheckpointingTests(keras_parameterized.TestCase):
    ...
    def testNamingWithOptimizer(self):
        ...
        self.assertEqual(
            "global_step", named_variables["optimizer_step" + suffix].full_name
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "beta1_power", named_variables["optimizer/beta1_power" + suffix].full_name
        )
        self.assertEqual(
            "beta2_power", named_variables["optimizer/beta2_power" + suffix].full_name
        )
    ...
    def testSaveRestore(self):
        with self.test_session():
            ...
            self.assertAllEqual(
                optimizer_variables[2:], self.evaluate(on_create_optimizer.variables())
            )
            dummy_var = variables.Variable([1.0])
...
class CheckpointCompatibilityTests(keras_parameterized.TestCase):
    def _initialized_model(self):
        ...
        self.evaluate(
            optimizer.get_slot(var=model._named_dense.bias, name="m").assign([2.0])
        )
    ...
    def testLoadFromNameBasedSaver(self):
        ...
        with testing_utils.device(should_use_gpu=True):
            with self.test_session():
                ...
                # checkpoint.
                root.not_in_name_checkpoint = variables.Variable([1.0])
                status = object_saver.restore(save_path)
                with self.assertRaises(AssertionError):
                    status.assert_existing_objects_matched()
