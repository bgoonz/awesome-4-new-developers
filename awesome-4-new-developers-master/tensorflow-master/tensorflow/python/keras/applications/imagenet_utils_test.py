...
class TestImageNetUtils(keras_parameterized.TestCase):
    ...
    def test_preprocess_input_symbolic(self, mode):
        ...
        outputs = keras.layers.Lambda(
            lambda x: utils.preprocess_input(x, mode=mode), output_shape=x.shape[1:]
        )(inputs)
    ...
    @parameterized.named_parameters(
        [
            {"testcase_name": "channels_last_format", "data_format": "channels_last"},
            {"testcase_name": "channels_first_format", "data_format": "channels_first"},
        ]
    )
