...
from tensorflow.python.ops.parallel_for import (
    control_flow_ops as parallel_control_flow_ops,
)
...
def result_wrapper(result_fn):
  ...
  """
    ...
    def decorated(metric_obj, *args):
        ...
        if (
            not has_strategy
            or replica_context is None
            or not distribution_strategy_context.get_strategy().extended._use_merge_call()
        ):
            with distribution_strategy_context.variable_sync_on_read_context():
                ...
                if isinstance(
                    raw_result, (ops.Tensor, variables_module.Variable, float, int)
                ):
  ...
  """
    ...
    if multi_label:
        ...
        fp_bucket_v = math_ops.unsorted_segment_sum(
            data=false_labels, segment_ids=bucket_indices, num_segments=num_thresholds
        )
