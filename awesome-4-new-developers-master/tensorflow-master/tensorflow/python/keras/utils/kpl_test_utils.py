...
class DistributeKplTestUtils(test.TestCase):
    ...
    def define_reverse_lookup_layer(self):
        ...
        label_inverse_lookup_layer = string_lookup.StringLookup(
            num_oov_indices=0, mask_token=None, vocabulary=self.LABEL_VOCAB, invert=True
        )
    ...
    def create_serving_signature(
        ...
        return serve_fn.get_concrete_function(
            tensor_spec.TensorSpec(shape=(3), dtype=dtypes.string, name="example")
        )
    ...
    def test_save_load_serving_model(
        ...
        loaded_serving_fn = keras.saving.save.load_model(saved_model_dir).signatures[
            "serving_default"
        ]

        # check the result w/ and w/o avenger.
        prediction0 = loaded_serving_fn(
            constant_op.constant(["avenger", "ironman", "avenger"])
        )["output_0"]
        self.assertIn(prediction0.numpy().decode("UTF-8"), ("yes", "no"))

        prediction1 = loaded_serving_fn(
            constant_op.constant(["ironman", "ironman", "unkonwn"])
        )["output_0"]
