...
def serialize_keras_object(instance):
  ...
  """
    ...
    supports_masking = getattr(instance, "supports_masking", False) or (
        hasattr(instance, "compute_mask") and not is_default(instance.compute_mask)
    )
...
def make_batches(size, batch_size):
  ...
  """
    ...
    return [
        (i * batch_size, min(size, (i + 1) * batch_size)) for i in range(0, num_batches)
    ]
...
def check_for_unexpected_keys(name, input_dict, expected_values):
    ...
    if unknown:
        raise ValueError(
            "Unknown entries in {} dictionary: {}. Only expected "
            "following keys: {}".format(name, list(unknown), expected_values)
        )
