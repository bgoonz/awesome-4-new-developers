...
def squeeze_or_expand_dimensions(y_pred, y_true=None, sample_weight=None):
  ...
  """
    ...
    def _maybe_adjust_weights():
        return control_flow_ops.cond(
            math_ops.equal(rank_diff, 1), maybe_squeeze_weights, _maybe_expand_weights
        )
...
def compute_weighted_loss(
    losses, sample_weight=None, reduction=ReductionV2.SUM_OVER_BATCH_SIZE, name=None
):
...
def scale_loss_for_distribution(loss_value):
    ...
    num_replicas = distribution_strategy_context.get_strategy().num_replicas_in_sync
    if num_replicas > 1:
