...
class MinimizeLossStepTest(test.TestCase, parameterized.TestCase):
    ...
    @ds_combinations.generate(
        ...
        + combinations.times(
            ...
            combinations.combine(mode=["graph", "eager"], use_callable_loss=[True]),
        )
    ...
    def testMeanVsSum(
        ...
        with distribution.scope():
            ...
            def dataset_fn():
                features = dataset_ops.Dataset.from_tensors([[2.0], [7.0]])
                labels = dataset_ops.Dataset.from_tensors([[6.0], [21.0]])
            ...
            if loss_reduction == losses_impl.Reduction.SUM:
                ...
                self.assertNear(
                    weight, 2 + 0.106 * distribution.num_replicas_in_sync, 0.0001
                )
            else:
    ...
    def _verify_loss_output(self, initial_loss, loss_output, reduced, distribution):
        if not reduced:
            self.assertLen(
            ...
            loss_tensor = distribution.reduce(
                reduce_util.ReduceOp.MEAN, loss_output, axis=None
            )
        else:
    ...
    @ds_combinations.generate(optimizer_combinations.distributions_and_v2_optimizers())
