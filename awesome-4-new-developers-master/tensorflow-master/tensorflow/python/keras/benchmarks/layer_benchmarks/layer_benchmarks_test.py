...
from tensorflow.python.keras.benchmarks.layer_benchmarks import (
...
def _get_metadata(name):
    return {"model_name": "ideal_layers", "parameters": name[1] + "_shape"}
...
def _get_input_data(inputs):
    if "input_shape" in inputs:
        ...
        raise ValueError(
            "Please specificy either `input_shape` or `input`" "for the benchmark test"
        )
...
CORE_LAYERS = [
    ...
    ("Flatten_small_shape", tf.keras.layers.Flatten, {}, {"input_shape": (1, 1)}, 100),
]
...
class KerasLayerBenchmarks(  # pylint: disable=undefined-variable
    ...
    def benchmark_layer_call_with_xla(self, layer_cls, layer_args, inputs, num_iters):
        name = benchmark_util.get_benchmark_name(self._get_name())
        # TODO(b/173461426)
    ...
    def benchmark_layer_call_backward(self, layer_cls, layer_args, inputs, num_iters):
        layer = layer_cls(**_get_layer_args(layer_cls, layer_args))
        x = _get_input_data(inputs)
