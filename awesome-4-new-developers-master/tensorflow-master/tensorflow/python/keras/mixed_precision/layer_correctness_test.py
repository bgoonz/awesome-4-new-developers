...
class LayerCorrectnessTest(keras_parameterized.TestCase):
    def setUp(self):
        ...
        config_module.set_logical_device_configuration(
            cpus[0],
            [
    ...
    @parameterized.named_parameters(
        ...
        ("Conv2DTranspose", lambda: convolutional.Conv2DTranspose(2, 2), (2, 2, 2, 2)),
        ("SeparableConv2D", lambda: convolutional.SeparableConv2D(2, 2), (2, 2, 2, 1)),
        ("DepthwiseConv2D", lambda: convolutional.DepthwiseConv2D(2, 2), (2, 2, 2, 1)),
        ...
        ("Attention", dense_attention.Attention, [(2, 2, 3), (2, 3, 3), (2, 3, 3)]),
        (
            "AdditiveAttention",
            dense_attention.AdditiveAttention,
            [(2, 2, 3), (2, 3, 3), (2, 3, 3)],
        ),
        (
            "Embedding",
            lambda: embeddings.Embedding(4, 4),
            (2, 4),
            2e-3,
            2e-3,
        ...
        ("LocallyConnected2D", lambda: local.LocallyConnected2D(2, 2), (2, 2, 2, 1)),
        ...
        ("SimpleRNN", lambda: recurrent.SimpleRNN(units=4), (4, 4, 4), 1e-2, 1e-2),
        ...
        ("TimeDistributed", lambda: wrappers.TimeDistributed(core.Dense(2)), (2, 2, 2)),
        (
            "Bidirectional",
        ...
        ("NormalizationNoAdapt", _create_normalization_layer_without_adapt, (4, 4)),
        ("Resizing", lambda: image_preprocessing.Resizing(3, 3), (2, 5, 5, 1)),
        ("Rescaling", lambda: image_preprocessing.Rescaling(2.0, 1.0), (6, 6)),
        ("CenterCrop", lambda: image_preprocessing.CenterCrop(3, 3), (2, 5, 5, 1)),
    )
    def test_layer(
    ...
    """
        ...
        self.assertAllClose(
            distributed_mp_model.predict(input_data), f32_output, rtol=rtol, atol=atol
        )
        ...
        f32_weights = f32_model.get_weights()
        self.assertAllClose(mp_model.get_weights(), f32_weights, rtol=rtol, atol=atol)
        self.assertAllClose(
