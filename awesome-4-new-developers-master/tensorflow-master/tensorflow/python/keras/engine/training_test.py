...
class TrainingTest(keras_parameterized.TestCase):
    ...
    def test_fit_and_validate_learning_phase(self):
        ...
        model.compile("sgd", loss="mae", run_eagerly=testing_utils.should_run_eagerly())
    ...
    def test_target_dtype_matches_output(self):
        ...
        model.compile(
            "sgd", loss=loss_fn, run_eagerly=testing_utils.should_run_eagerly()
        )
    ...
    def test_fit_on_arrays(self):
        ...
        model.fit(
            [input_a_np, input_b_np],
            [output_d_np, output_e_np],
            validation_data=([input_a_np, input_b_np], [output_d_np, output_e_np]),
            epochs=1,
            batch_size=5,
            verbose=0,
        )
        ...
        if testing_utils.get_model_type() == "functional":
            ...
            metrics = {"dense": "mse", "dropout": metrics_module.CategoricalAccuracy()}
            model.compile(
                optimizer,
                loss,
                metrics=metrics,
        ...
        model.compile(
            optimizer, loss="mse", run_eagerly=testing_utils.should_run_eagerly()
        )
        # This will work
    ...
    def test_evaluate_predict_on_arrays(self):
        ...
        self.assertEqual(len(out), 7)
        out = model.test_on_batch([input_a_np, input_b_np], [output_d_np, output_e_np])
        self.assertEqual(len(out), 7)
    ...
    @keras_parameterized.run_with_all_model_types
    @parameterized.named_parameters(
        ("dataset", "dataset"), ("generator", "generator"), ("sequence", "sequence")
    )
    def test_sequence_input_types(self, input_type):
        ...
        model.compile(
            loss="mse", optimizer="sgd", run_eagerly=testing_utils.should_run_eagerly()
        )
    ...
    def test_training_on_sparse_data_with_dense_placeholders_v1(self):
        with ops.Graph().as_default():
            ...
            optimizer = "rmsprop"
            model.compile(
                optimizer, "mse", metrics=["mae", metrics_module.CategoricalAccuracy()]
            )
            model.fit(
                test_inputs, test_outputs, epochs=1, batch_size=2, validation_split=0.5
            )
    ...
    def test_weight_shared_across_layers(self):
        ...
        class LayerWithWeightSharedLayers(layers_module.Layer):
            def __init__(self):
                ...
                shared_trainable_var = variables_module.Variable(1.0)
    ...
    def test_mismatched_output_shape_and_target_shape(self):
        model = sequential.Sequential(
            [layers_module.Dense(2, input_shape=(3, 4)), layers_module.Dense(5)]
        )
        model.compile(
    ...
    def test_losses_in_defun(self):
        ...
        self.assertAllEqual(self.evaluate(layer.losses), self.evaluate(get_losses()))
    ...
    def test_training_with_loss_instance(self):
        ...
        model.fit(
            [input_a_np, input_b_np], [output_d_np, output_e_np], epochs=1, batch_size=5
        )
    ...
    def test_compatible_batch_size_functional_model(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "specified batch sizes of the Input Layers"
        ):
    ...
    def test_calling_subclass_model_on_different_datasets(self):
        ...
        self.assertAllEqual([[6], [8], [10], [12]], model.predict(dataset_two, steps=2))
    ...
    def test_training_on_sparse_categorical_crossentropy_loss_with_softmax(self):
        ...
        reference_model = testing_utils.get_small_sequential_mlp(16, 2, input_dim=4)
    ...
    def test_training_on_categorical_crossentropy_loss_with_softmax(self):
        np.random.seed(1337)
        train_x = np.ones((100, 4))
        train_y = np_utils.to_categorical(np.random.randint(0, 1, size=(100, 1)), 2)
    ...
    def test_training_on_binary_crossentropy_loss(self):
        ...
        reference_model = testing_utils.get_small_sequential_mlp(16, 1, input_dim=4)
        reference_model.compile(
            loss="binary_crossentropy",
            optimizer=RMSPropOptimizer(learning_rate=0.001),
            run_eagerly=True,
    ...
    def test_losses_of_different_dtypes(self):
        ...
        out_1 = layers_module.Dense(2, dtype="float32", kernel_regularizer="l2")(inp)
        out_2 = layers_module.Dense(2, dtype="float16", kernel_regularizer="l2")(inp)
        ...
        model.compile(
            "sgd", ["mse", "mse"], run_eagerly=testing_utils.should_run_eagerly()
        )
    ...
    def test_regularizer_of_different_dtype(self):
        ...
        out = layers_module.Dense(2, dtype="float32", kernel_regularizer=regularizer)(
            inp
        )
    ...
    def test_calling_aggregate_gradient(self):
        ...
        model.compile(
            mock_optimizer, "mse", run_eagerly=testing_utils.should_run_eagerly()
        )
      ...
      """
            ...
            def apply_gradients(
                ...
                return super(_OptimizerOverrideApplyGradients, self).apply_gradients(
                    grads_and_vars, name
                )
    ...
    def test_trainable_state_setting(self):
        class UpdateLayer(layers_module.Layer):
            def __init__(self):
                ...
                self.v = variables_module.Variable(0.0, trainable=False)
    ...
    def test_evaluate_with_custom_test_step(
        ...
        if not use_compiled_metrics and use_custom_metrics:
            ...
            self.assertEqual(results_list, [results_dict["mean"], results_dict["sum"]])
...
class TestExceptionsAndWarnings(keras_parameterized.TestCase):
    ...
    def test_compile_warning_for_loss_missing_output(self):
        with self.cached_session():
            ...
            out_2 = layers_module.Dense(3, activation="softmax", name="dense_2")(out_1)
    ...
    def test_on_batch_error_inconsistent_batch_size(self):
        ...
        with self.assertRaisesRegex(ValueError, "Data cardinality is ambiguous"):
            model.train_on_batch([np.ones((10, 5)), np.ones((10, 5))], np.ones((11, 4)))

        with self.assertRaisesRegex(ValueError, "Data cardinality is ambiguous"):
            model.test_on_batch([np.ones((10, 5)), np.ones((10, 5))], np.ones((11, 4)))
...
class LossWeightingTest(keras_parameterized.TestCase):
        ...
        with self.cached_session():
            ...
            model.add(
                layers_module.TimeDistributed(
                    layers_module.Dense(num_classes), input_shape=(timesteps, input_dim)
                )
            )
    ...
    def test_fit_with_incorrect_weights(self):
        ...
        model = testing_utils.get_multi_io_model(branch_a, branch_b)
        model.compile(
            optimizer="adam", loss="mse", run_eagerly=testing_utils.should_run_eagerly()
        )
    ...
    def test_sample_weight_tensor(self):
        ...
        with ops.get_default_graph().as_default():
            ...
            sample_weights = array_ops.constant([[0, 0.4, 1, 1], [2, 0.4, 0.3, 1]])
            ...
            with self.cached_session() as sess:
                self.assertAllClose(
                    (0.4 + 1 + 1) / 4, sess.run(model.total_loss, feed_dict=feeds)
                )
                self.assertAllClose(
                    (2 + 0.4 + 0.3 + 1) / 4, sess.run(model.total_loss, feed_dict=feeds)
                )
...
class TestTrainingWithDataTensors(keras_parameterized.TestCase):
    def test_training_and_eval_methods_on_symbolic_tensors_single_io(self):
        with ops.Graph().as_default():
            ...
            loss = "mse"
            model.compile(
                optimizer, loss, metrics=["mae", metrics_module.CategoricalAccuracy()]
            )
    ...
    def test_training_and_eval_methods_on_symbolic_tensors_multi_io(self):
        ...
        model.evaluate(
            [input_a_tf, input_b_tf], [output_d_tf, output_e_tf], steps=2, verbose=0
        )
    ...
    """
        with ops.Graph().as_default(), self.cached_session():
            ...
            # test evaluate
            out = model.evaluate(
                {"input_b": input_b_np}, [output_a_np, output_b_np], batch_size=10
            )
    ...
    def test_target_tensors(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            model.compile(
                optimizer="rmsprop", loss="mse", target_tensors={"dense": target}
            )
            model.train_on_batch(input_val, None)
            ...
            with self.assertRaises(ValueError):
                model.compile(
                    optimizer="rmsprop", loss="mse", target_tensors=[target, target]
                )
            with self.assertRaises(ValueError):
                model.compile(
                    optimizer="rmsprop", loss="mse", target_tensors={"dense2": None}
                )
            with self.assertRaises(ValueError):
                model.compile(optimizer="rmsprop", loss="mse", target_tensors=[target])
            ...
            model.compile(
                optimizer="rmsprop", loss="mse", target_tensors=[target_a, target_b]
            )
            model.train_on_batch(input_val, None)
            ...
            model.train_on_batch(
                input_val, None, sample_weight={"dense_a": np.random.random((10,))}
            )
    ...
    def test_model_custom_target_tensors(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            model.compile(
                optimizer="rmsprop", loss="mse", target_tensors={"dense_1": pl_target_a}
            )
...
class TestTrainingWithMetrics(keras_parameterized.TestCase):
    ...
    def test_add_metric_with_tensor_on_model(self):
        ...
        model = training_module.Model(x, y)
        model.add_metric(math_ops.reduce_sum(y), name="metric_1", aggregation="mean")
        ...
        model.compile("sgd", loss="mse", run_eagerly=testing_utils.should_run_eagerly())

        inputs = np.ones(shape=(10, 1))
        targets = np.ones(shape=(10, 1))
        history = model.fit(
            inputs, targets, epochs=2, batch_size=5, validation_data=(inputs, targets)
        )
    ...
    def test_model_metrics_list(self):
        ...
        model = training_module.Model(x, y)
        model.add_metric(math_ops.reduce_sum(y), name="metric_3", aggregation="mean")
    ...
    def test_model_metrics_list_in_call(self):
        ...
        self.assertEqual([m.name for m in model.metrics], ["loss", "acc", "metric_1"])
    ...
    def test_multiple_add_metric_calls(self):
        ...
        model = TestModel()
        self.assertListEqual([m.name for m in model.metrics], ["metric_1", "metric_2"])
        model.compile(
            loss="mse",
            optimizer=RMSPropOptimizer(0.01),
            run_eagerly=testing_utils.should_run_eagerly(),
    ...
    def test_multiple_add_metric_calls_layer(self):
        ...
        layer(np.ones((10, 10)))
        self.assertListEqual(
            [m.name for m in layer.metrics], ["m_1", "m_2", "m_3", "m_4", "m_5", "m_6"]
        )
    ...
    def DISABLED_test_add_metric_invalid_aggregation(self):
        ...
        with self.assertRaisesRegex(
            ...
            model.add_metric(math_ops.reduce_sum(y), name="metric_1", aggregation="sum")

        with self.assertRaisesRegex(
            ValueError, "only `mean` sample-wise metric aggregation"
        ):
            model.add_metric(math_ops.reduce_sum(y), name="metric_1", aggregation=None)
    ...
    def test_calling_evaluate_in_callback_during_fit(self):
        ...
        train_ds = dataset_ops.Dataset.from_tensor_slices((ones, ones)).batch(5)
        val_ds_1 = dataset_ops.Dataset.from_tensor_slices((ones, ones)).batch(5)
        val_ds_2 = dataset_ops.Dataset.from_tensor_slices((zeros, zeros)).batch(5)
        model.compile("sgd", "mse", run_eagerly=testing_utils.should_run_eagerly())
    ...
    def test_model_with_nested_compiled_model(self):
        ...
        inner_model = training_module.Model(x, y)
        inner_model.add_metric(math_ops.reduce_sum(y), name="mean1", aggregation="mean")
        ...
        self.assertEqual(
            [m.name for m in inner_model.metrics], ["loss", "acc", "mean", "mean1"]
        )

        x = layers_module.Input(shape=[1])
        y = inner_model(x)
        outer_model = training_module.Model(x, y)
        outer_model.add_metric(math_ops.reduce_sum(y), name="mean2", aggregation="mean")
    ...
    def test_model_with_metric_class_that_returns_dict(self):
        ...
        class DictMetric(metrics_module.Metric):
            def __init__(self):
                ...
                self.l2_sum = variables_module.Variable(0.0)
        ...
        self.assertEqual(
            list(dict_train_on_batch_res.keys()), ["loss", "mae", "my_mse", "my_rmse"]
        )
        list_test_on_batch_res = model.test_on_batch(np.ones((10, 2)), np.ones((10, 3)))
        self.assertEqual(len(list_test_on_batch_res), 4)
        dict_test_on_batch_res = model.test_on_batch(
            np.ones((10, 2)), np.ones((10, 3)), return_dict=True
        )
        self.assertEqual(
            list(dict_test_on_batch_res.keys()), ["loss", "mae", "my_mse", "my_rmse"]
        )
...
class BareUpdateLayer(layers_module.Layer):
    def build(self, input_shape):
        self.counter = self.add_weight(
            "counter", dtype="int32", shape=(), initializer="zeros", trainable=False
        )
