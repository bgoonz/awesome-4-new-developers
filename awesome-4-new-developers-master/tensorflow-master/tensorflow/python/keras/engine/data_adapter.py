    ...
    def can_handle(x, y=None):
        ...
        def _is_composite(v):
            ...
            if (
                ...
                and not isinstance(
                    v, (dataset_ops.DatasetV2, iterator_ops.IteratorBase)
                )
...
class DataHandler(object):
    ...
    def _truncate_execution_to_epoch(self):
        ...
        should_truncate = (
            ...
            and self._steps_per_execution_value > self._inferred_steps
        )
    ...
    def steps(self):
        ...
        while self._inferred_steps is None or self._current_step < self._inferred_steps:
            ...
            can_run_full_execution = (
                ...
                or self._inferred_steps - self._current_step
...
class _ClusterCoordinatorDataHandler(DataHandler):
    ...
    def _configure_dataset_and_inferred_steps(
        ...
        if not isinstance(x, dataset_creator.DatasetCreator):
            raise TypeError(
                "When using `ParameterServerStrategy`, `x` must be a "
                "`DatasetCreator`."
            )
