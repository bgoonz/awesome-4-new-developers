...
class BaseLayerTest(keras_parameterized.TestCase):
    ...
    def test_dynamic_layer_error_running_in_graph_mode(self):
        with ops.get_default_graph().as_default():
            ...
            with self.assertRaisesRegex(ValueError, "You must enable eager execution"):
                model.compile(rmsprop.RMSprop(0.001), loss="mse")
    ...
    def test_add_weight_by_getter(self):
        ...
        added = layer.add_weight(dtype=dtypes.string, getter=lambda *_, **__: variable)
        ...
        class CompositeVariableLayer(base_layer.Layer):
            def __init__(self):
                ...
                self.composite_var = CompositeVariable(
                    [variables.Variable(1.0), variables.Variable(2.0)]
                )
    ...
    def test_layer_names(self):
        ...
        model = training_lib.Model(inputs=[inputs], outputs=[add1, add2, add3, add4])
    ...
    def test_tf_module_tracking(self):
        class MyModule(module.Module):
            def __init__(self):
                ...
                self.v1 = variables.Variable(1.0, trainable=True, name="v1")
                self.v2 = variables.Variable(2.0, trainable=False, name="v2")
...
class NestedTrackingTest(test.TestCase):
    ...
    def test_attribute_reassignment(self):
        ...
        l.a = []
        l.a = variables.Variable(1.0)
        l.a = base_layer.Layer()
        last_assignment = base_layer.Layer()
        l.a = last_assignment
        l.b = variables.Variable(1.0)
        del l.b
    ...
    def test_layer_call_fn_args(self):
        ...
        self.assertEqual(
            nondefun_layer._call_fn_args, ["inputs", "a", "mask", "b", "training"]
        )
        defun_layer = DefunLayer()
        self.assertEqual(defun_layer._call_fn_args, ["x", "mask", "a", "training", "b"])
...
class NameScopingTest(keras_parameterized.TestCase):
    ...
    def test_name_scope_tf_tensor(self):
        ...
        layer = layers.Dense(10, activation=layers.ReLU(name="MyAct"), name="MyName3")
        layer(x)
...
class AutographControlFlowTest(keras_parameterized.TestCase):
    def test_disabling_in_context_is_matched(self):
        ...
        def test_fn():
            return MyLayer()(constant_op.constant([[1.0, 2.0, 3.0]]))
    ...
    def test_if_training_pattern_metric(self):
        ...
        model.compile("sgd", "mse", run_eagerly=testing_utils.should_run_eagerly())
        for _ in range(3):
            _, train_metric = model.train_on_batch(np.ones((2, 3)), np.ones((2, 3)))

            self.assertEqual(train_metric, 2 * 3)
            _, test_metric = model.test_on_batch(np.ones((2, 3)), np.ones((2, 3)))
    ...
    def test_conditional_callable_losses(self):
        ...
        assert_graph(get_losses(constant_op.constant(2.0)))
    ...
    def test_conditional_metrics_in_call(self):
        class MyLayer(base_layer.Layer):
            ...
            def call(self, inputs, training=None):
                if training:
                    self.add_metric(
                        math_ops.reduce_sum(inputs), name="sum", aggregation="mean"
                    )
                return inputs
...
class DTypeTest(keras_parameterized.TestCase):
    ...
    def test_extra_args_and_kwargs_not_casted(self):
        ...
        x, y, z = layer(
            self._const("float16"), self._const("float16"), kwarg=self._const("float16")
        )
        self.assertEqual(x.dtype, "float64")
    ...
    def test_composite_tensors_input_casting(self):
        sparse = sparse_tensor.SparseTensor(
            ...
            values=array_ops.constant([0.0, 1.0], dtype="float32"),
            dense_shape=array_ops.constant([4, 4], dtype="int64"),
        )
        ragged = ragged_tensor.RaggedTensor.from_row_splits(
            values=array_ops.constant([1.0, 2.0, 3.0], dtype="float32"),
