  ...
  """
    ...
    if check_all_flat:
        nest.assert_same_structure(
            list_to_tuple(sample_weights), list_to_tuple(nest.flatten(sample_weights))
        )
        nest.assert_same_structure(
            list_to_tuple(outputs), list_to_tuple(nest.flatten(outputs))
        )
    ...
    for i, sw in enumerate(sample_weights):
        if sw is None:
            ...
            is_temporal = (
                sample_weight_modes is not None and sample_weight_modes[i] == "temporal"
            )
            sw_shape = (
