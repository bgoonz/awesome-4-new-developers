...
class KerasCallbacksTest(keras_parameterized.TestCase):
    ...
    def test_progbar_logging_with_stateful_metrics(self):
        ...
        with self.captureWritesToStream(sys.stdout) as printed:
            model = self._get_model(input_shape=(8,), additional_metrics=[AddAllOnes()])
    ...
    def test_callback_warning(self):
        ...
        warning_msg = (
            "Callback method `on_train_batch_end` is slow compared " "to the batch time"
        )
    ...
    def test_progbar_logging_training_validation(self):
        ...
        validation = dataset_ops.Dataset.from_generator(
            generator=generator,
            output_types=("float64", "float64"),
            output_shapes=([2], []),
        ).batch(2)
    ...
    def test_progbar_logging_with_dataset_and_partial_batch(self):
        ...
        training = dataset_ops.Dataset.from_generator(
            generator=generator,
            output_types=("float64", "float64"),
            output_shapes=([2], []),
        ).batch(2)
    ...
    def test_ModelCheckpoint_subclass_save_weights_false(self):
        ...
        cbks = [keras.callbacks.ModelCheckpoint(filepath, save_weights_only=False)]
    ...
    def test_fit_with_ModelCheckpoint_with_dir_as_h5_filepath(self):
        ...
        with self.assertRaisesRegex(
            IOError, "Please specify a non-directory " "filepath for ModelCheckpoint."
        ):
            model.fit(train_ds, epochs=1, callbacks=[callback])
    ...
    def test_ModelCheckpoint_nonblocking(self):
        ...
        tensor = ops.convert_to_tensor_v2_with_dispatch(1.0)
    ...
    def test_LearningRateScheduler(self):
        with self.cached_session():
            ...
            cbks = [keras.callbacks.LearningRateScheduler(lambda x: 1.0 / (1.0 + x))]
            model.fit(
                x_train,
                y_train,
                batch_size=BATCH_SIZE,
            ...
            cbks = [
                keras.callbacks.LearningRateScheduler(
                    lambda epoch, _: learning_rate_schedule.CosineDecay(0.01, 2)(epoch)
                )
            ]
            model.compile(
                loss="categorical_crossentropy", optimizer="sgd", metrics=["accuracy"]
    ...
    def test_ReduceLROnPlateau(self):
        with self.cached_session():
            ...
            cbks = [
                keras.callbacks.ReduceLROnPlateau(
                    monitor="val_loss", factor=0.1, min_delta=10, patience=1, cooldown=5
                )
            ]
            model.fit(
                x_train,
    ...
    @unittest.skipIf(
        os.name == "nt", "use_multiprocessing=True does not work on windows properly."
    )
...
class TestTensorBoardV2(keras_parameterized.TestCase):
    ...
    def test_TensorBoard_default_logdir(self):
        ...
        model.fit(
            x, y, batch_size=2, epochs=2, validation_data=(x, y), callbacks=[tb_cbk]
        )
    ...
    def test_TensorBoard_basic(self):
        ...
        self.assertEqual(
            summary_file.scalars,
            {
                ...
                _ObservedSummary(
                    logdir=self.validation_dir, tag="evaluation_loss_vs_iterations"
                ),
            },
        )
    ...
    def test_TensorBoard_global_step(self):
        ...
        self.assertEqual(
            summary_file.scalars,
            {
                ...
                _ObservedSummary(logdir=self.train_dir, tag="batch_loss"),
                _ObservedSummary(logdir=self.train_dir, tag="epoch_learning_rate"),
                _ObservedSummary(logdir=self.train_dir, tag="epoch_steps_per_second"),
                _ObservedSummary(logdir=self.train_dir, tag="batch_steps_per_second"),
            },
        )
    ...
    def test_TensorBoard_weight_images(self):
        ...
        self.assertEqual(
            self._strip_layer_names(summary_file.images, model_type), expected
        )
    ...
    def test_custom_summary(self):
        ...
        self.assertEqual(
            summary_file.scalars,
            {
                ...
                _ObservedSummary(logdir=self.train_dir, tag="batch_loss"),
                _ObservedSummary(
                    logdir=self.train_dir, tag="model/layer_with_summary/custom_summary"
                ),
                _ObservedSummary(
...
class TestTensorBoardV2NonParameterizedTest(keras_parameterized.TestCase):
    ...
    def fitModelAndAssertKerasModelWritten(self, model):
        ...
        model.fit(
            x, y, batch_size=2, epochs=3, validation_data=(x, y), callbacks=[tb_cbk]
        )
        summary_file = list_summaries(self.logdir)
        self.assertEqual(
            summary_file.tensors, {_ObservedSummary(logdir=self.train_dir, tag="keras")}
        )
    ...
    def test_TensorBoard_autoTrace_profileBatchRangeSingle(self):
        ...
        model.fit(
            x, y, batch_size=3, epochs=2, validation_data=(x, y), callbacks=[tb_cbk]
        )
        summary_file = list_summaries(self.logdir)
    ...
    def test_TensorBoard_autoTrace_profileBatchRangeTwice(self):
        ...
        model.fit(
            x, y, batch_size=3, epochs=10, validation_data=(x, y), callbacks=[tb_cbk]
        )
    ...
    def test_TensorBoard_autoTrace_profileBatchRange(self):
        ...
        model.fit(
            x, y, batch_size=4, epochs=2, validation_data=(x, y), callbacks=[tb_cbk]
        )
        summary_file = list_summaries(self.logdir)
    ...
    def test_TensorBoard_autoTrace_profileInvalidBatchRange(self):
        ...
        with self.assertRaises(ValueError):
            keras.callbacks.TensorBoard(
                self.logdir, histogram_freq=1, profile_batch="1,None", write_graph=False
            )
...
class MostRecentlyModifiedFileMatchingPatternTest(test.TestCase):
    ...
    def test_using_checkpoint_management_latest_checkpoint(self):
        ...
        with open(ckpt_file_path, "w") as f:
            f.write("dummy ckpt")
        checkpoint_management.update_checkpoint_state_internal(test_dir, ckpt_file_path)
...
class SummaryOpsTest(test.TestCase):
    ...
    def testKerasModel_subclass(self):
        ...
        with test.mock.patch.object(logging, "warning") as mock_log:
            self.assertFalse(
                keras.callbacks.keras_model_summary(name="my_name", data=model, step=1)
            )
            self.assertRegex(
