...
class PreprocessingLayerTest(test.TestCase):
    ...
    def validate_accumulator_computation(self, combiner, data, expected):
        ...
        self.compare_accumulators(
            all_merge,
            unordered_all_merge,
            msg="The order of merge arguments should not change the data " "output.",
        )

        hierarchical_merge = combiner.merge(
            [
                combiner.compute(data_1),
                combiner.merge([combiner.compute(data_2), combiner.compute(data_0)]),
            ]
        )
        self.compare_accumulators(
            all_merge,
        ...
        self.compare_accumulators(
            all_merge,
            mixed_compute,
            msg="Mixing merge and compute calls should not change the data " "output.",
        )
