...
class Normalization(base_preprocessing_layer.PreprocessingLayer):
  ...
  """
    ...
    def update_state(self, data):
        ...
        batch_mean, batch_variance = nn_impl.moments_v2(data, axes=self._reduce_axis)
        ...
        batch_weight = math_ops.cast(batch_count, dtype=self.dtype) / math_ops.cast(
            total_count, dtype=self.dtype
        )
    ...
    def merge_state(self, layers):
        ...
        for l in layers:
            if l.input_mean is not None:
                raise ValueError(
                    ...
                    "`mean` and `variance`, you passed `mean={}` and `variance={}`.".format(
                        l.name, l.input_mean, l.input_variance
                    )
                )
            if not l.built:
                raise ValueError(
        ...
        layer_weightings = math_ops.cast(layer_counts, self.dtype) / math_ops.cast(
            total_count, self.dtype
        )
        layer_weightings = array_ops.reshape(
            layer_weightings, shape=[len(layers)] + [1] * self.adapt_mean.shape.rank
        )
    ...
    def finalize_state(self):
        ...
        self.variance = array_ops.reshape(self.adapt_variance, self._broadcast_shape)
    ...
    def call(self, inputs):
        ...
        return (inputs - self.mean) / math_ops.maximum(
            math_ops.sqrt(self.variance), backend.epsilon()
        )
