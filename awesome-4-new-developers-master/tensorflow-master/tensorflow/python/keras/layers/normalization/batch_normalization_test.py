...
class BatchNormalizationTest(keras_parameterized.TestCase):
    ...
    def test_batchnorm_mixed_precision(self):
        norm = keras.layers.BatchNormalization(
            axis=-1, input_shape=(4, 4, 3), momentum=0.8, dtype="mixed_float16"
        )
    ...
    @combinations.generate(
        combinations.combine(mode=["graph", "eager"], fused=[True, False])
    )
    @testing_utils.enable_v2_dtype_behavior
    ...
    def test_bessels_correction(self):
        ...
        x = constant_op.constant([0.0, 2.0], shape=[2, 1, 1, 1])
        ...
        x = constant_op.constant([0.0, 2.0], shape=[2, 1, 1, 1, 1])
        layer = batch_normalization.BatchNormalization(
            momentum=0.5, moving_variance_initializer="zeros"
        )
        layer(x, training=True)
...
def _run_batchnorm_correctness_test(layer, dtype="float32", fused=False):
    ...
    # centered on 5.0, variance 10.0
    x = np.random.normal(loc=5.0, scale=10.0, size=(1000, 2, 2, 2)).astype(dtype)
    model.fit(x, x, epochs=4, verbose=0)
    out = model.predict(x)
...
@parameterized.parameters(
    [batch_normalization_v1.BatchNormalization, batch_normalization.BatchNormalization]
)
class NormalizationLayersGraphModeOnlyTest(test.TestCase, parameterized.TestCase):
