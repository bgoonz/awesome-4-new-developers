  ...
  """
    ...
    def build(self, input_shape):
        ...
        if self.virtual_batch_size is not None:
            ...
            if self.adjustment is not None:
                raise ValueError(
                    "When using virtual_batch_size, adjustment cannot " "be specified"
                )
            ...
            if self._USE_V2_BEHAVIOR:
                if self.fused is None:
                    ...
                    raise ValueError(
                        ...
                        "Received tensor with shape: %s" % (tuple(input_shape),)
                    )
            else:
        ...
        try:
            ...
            if self.renorm:
                ...
                def _renorm_variable(
                    name, shape, initializer=init_ops.zeros_initializer()
                ):
    ...
    def _assign_moving_average(self, variable, value, momentum, inputs_size):
        def calculate_update_delta():
            decay = ops.convert_to_tensor_v2_with_dispatch(1.0 - momentum, name="decay")
    ...
    def _renorm_correction_and_moments(self, mean, variance, training, inputs_size):
        ...
        renorm_stddev = math_ops.maximum(
            self.renorm_stddev, math_ops.sqrt(self.epsilon)
        )
        ...
        r = control_flow_util.smart_cond(
            training, lambda: r, lambda: array_ops.ones_like(r)
        )
        d = control_flow_util.smart_cond(
            training, lambda: d, lambda: array_ops.zeros_like(d)
        )
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        ...
        if self._support_zero_size_input():
            input_batch_size = array_ops.shape(inputs)[0]
            mean = array_ops.where(input_batch_size > 0, mean, backend.zeros_like(mean))
    ...
    def call(self, inputs, training=None):
        ...
        outputs = nn.batch_normalization(
            inputs, _broadcast(mean), _broadcast(variance), offset, scale, self.epsilon
        )
        if inputs_dtype in (dtypes.float16, dtypes.bfloat16):
    ...
    def _calculate_mean_and_var(self, x, axes, keep_dims):
        ...
        with backend.name_scope("moments"):
            ...
            if replica_ctx:
                ...
                axes_vals = [
                    (array_ops.shape_v2(y))[axes[i]] for i in range(1, len(axes))
                ]
            ...
            if x.dtype == dtypes.float16:
                return (
                    math_ops.cast(mean, dtypes.float16),
