...
def build_all_signature_defs(
  ...
  """
    ...
    if export_outputs is None or not isinstance(export_outputs, dict):
        raise ValueError(
            "export_outputs must be a dict and not" "{}".format(type(export_outputs))
        )
  ...
  """
    if mode not in SIGNATURE_KEY_MAP:
        raise ValueError(
            ...
            "this function. Please ensure that you are using the new ModeKeys.".format(
                mode, SIGNATURE_KEY_MAP.keys()
            )
        )
