...
class SparseDense(keras.layers.Dense):
    def call(self, inputs):
        input_shape = array_ops.stack(
            (math_ops.reduce_prod(array_ops.shape(inputs)[:-1]), self.kernel.shape[0])
        )
...
class ReviveTestBase(keras_parameterized.TestCase):
    ...
    def _assert_revived_correctness(self, model, revived):
        ...
        input_arr = constant_op.constant(np.random.random((2, 2, 3)).astype(np.float32))
        if isinstance(revived._saved_model_inputs_spec, sparse_tensor.SparseTensorSpec):
