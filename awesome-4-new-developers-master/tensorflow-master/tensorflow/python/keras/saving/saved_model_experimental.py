...
sequential = LazyLoader(
    "sequential", globals(), "tensorflow.python.keras.engine.sequential"
)
...
def _save_v1_format(model, path, custom_objects, as_text, input_signature):
    ...
    builder = saved_model_builder._SavedModelBuilder(
        path
    ...
    if model.optimizer:
        if isinstance(
            model.optimizer, (optimizer_v1.TFOptimizer, optimizer_v2.OptimizerV2)
        ):
