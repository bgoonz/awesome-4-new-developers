...
class Topology(object):
  ...
  """
        ...
        if serialized:
            ...
            if len(
                ...
                raise ValueError(
                    ...
                    "got device_coordinates={} len(device_coordinates)={} device_coordinates.shape[2]={} mesh_shape={}, len(mesh_shape)={}".format(
                        self._device_coordinates.shape,
    ...
    def _parse_topology(self, serialized):
        ...
        if any(coords < 0):
            raise ValueError(
                "All values in `device_coordinates` must be >= 0, got {}".format(coords)
            )
