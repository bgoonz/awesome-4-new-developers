  ...
  """
    if isinstance(categorical_column, _DENYLISTED_CATEGORICAL_COLUMNS_V2):
        raise TypeError(
            ...
            " embedding_column was denylisted type %s" % type(categorical_column)
        )
    if not isinstance(categorical_column, _SUPPORTED_CATEGORICAL_COLUMNS):
        raise TypeError(
            ...
            % (
                " or ".join([cc.__name__ for cc in _SUPPORTED_CATEGORICAL_COLUMNS]),
...
class _TPUBaseEmbeddingColumn(object):
    ...
    def __init__(
        self, categorical_column, max_sequence_length=0, learning_rate_fn=None
    ):
        ...
        if self.is_sequence_column() and max_sequence_length < 1:
            raise ValueError(
                "max_sequence_length must be greater than 0 for "
                "sequence columns. Got max_sequence_length={} for "
                "sequence column {}.".format(
                    max_sequence_length, categorical_column.name
                )
            )
        if not self.is_sequence_column() and max_sequence_length != 0:
            raise ValueError(
                "Non zero max_seq_length={} specified for non "
                "sequence column {}.".format(
                    max_sequence_length, categorical_column.name
                )
            )
...
class _TPUSharedEmbeddingColumn(_TPUBaseEmbeddingColumn, fc._SharedEmbeddingColumn):
    ...
    def _get_dense_tensor(self, inputs, weight_collections=None, trainable=None):
        ...
        # Add to collection for _create_tpu_embedding_variables_and_ops
        _record_variable_scope_and_name(
            self.get_embedding_var_name(), "embedding_weights", is_shared_embedding=True
        )
        return tensor
...
def _record_variable_scope_and_name(
    ...
    if embedding_var_name in var_def_dict:
        if (
            var_def_dict[embedding_var_name][0] != captured_scope_name
            and not is_shared_embedding
            and not bypass_scope_validation
        ):
            raise ValueError(
        ...
        var_def_dict[embedding_var_name] = (
            captured_scope_name,
            embedding_var_name_in_fc,
        )
