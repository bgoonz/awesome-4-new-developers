...
class TableConfig(
    ...
    """
        if not isinstance(vocabulary_size, int) or vocabulary_size < 1:
            raise ValueError(
                f"vocabulary_size must >= 1. " f"Received: {vocabulary_size}."
            )
        ...
        if optimization_parameters is not None:
            if not isinstance(optimization_parameters, _OptimizationParameters):
                raise ValueError(
                    f"`optimization_parameters` must inherit from "
...
AdagradSlotVariableNames = collections.namedtuple(
    "AdagradSlotVariableNames", ["accumulator"]
)

MomentumSlotVariableNames = collections.namedtuple(
    "MomentumSlotVariableNames", ["momenta"]
)
...
RMSPropSlotVariableNames = collections.namedtuple(
    "RMSPropSlotVariableNames", ["ms", "mom"]
)
...
FtrlSlotVariableNames = collections.namedtuple(
    "FtrlSlotVariableNames", ["accumulator", "linear"]
)
...
class _OptimizationParameters(object):
    ...
    def __init__(
        ...
        if not use_gradient_accumulation and (
            clip_gradient_min is not None or clip_gradient_max is not None
        ):
            raise ValueError(
                "When using gradient clipping limits, gradient  "
...
class TPUEmbedding(object):
  ...
  """
    ...
    """
        ...
        self._table_to_features_dict, self._table_to_num_features_dict = _create_table_to_features_and_num_features_dicts(
        ...
        if mode == TRAINING:
            _validate_optimization_parameters(
                optimization_parameters, self._table_to_config_dict
            )
            self._optimization_parameters = optimization_parameters
        elif mode == INFERENCE:
            if optimization_parameters is not None:
                raise ValueError(
                    f"`optimization_parameters` should be `None` "
    ...
    def _create_config_proto(self):
        ...
        for table in self._table_to_config_dict:
            ...
            optimization_parameters = self._optimizer_handler_dict[
                table
            ...
            parameters.gradient_accumulation_status = (
                ...
                if optimization_parameters.use_gradient_accumulation
            ...
            if optimization_parameters.weight_decay_factor:
                ...
                if (
                    optimization_parameters.multiply_weight_decay_factor_by_learning_rate
                ):
    ...
    """
        ...
        for i, table in enumerate(self._table_to_config_dict):
            ...
            if slot_variable_names_by_table:
                ...
                optimizer_handler = self._optimizer_handler_dict[table]
                slot_variable_names = optimizer_handler.get_default_slot_variable_names(
                    table
                )
            ...
            with ops.device(device):
                ...
                slot_variables_for_table, load_ops_fn, retrieve_ops_fn = self._optimizer_handler_dict[
                    table
                ].create_variables_and_ops(
                    table,
                    slot_variable_names,
                    self._num_hosts,
                    self._table_to_config_dict[table],
                    table_variables,
                    config,
                )
    ...
    def _validate_generate_enqueue_ops_enqueue_datas_list(self, enqueue_datas_list):
        ...
        def _check_agreement(data, name, feature, enqueue_data):
            ...
            if (
                data is not None
                and data.device != enqueue_data.embedding_indices.device
            ):
                raise ValueError(
        ...
        for i, enqueue_datas in enumerate(enqueue_datas_list):
            ...
            for feature, enqueue_data in six.iteritems(enqueue_datas):
                ...
                if device is None:
                    ...
                    if device != enqueue_data.embedding_indices.device:
                        raise ValueError(
                            "Devices are different between features in "
                            "`enqueue_datas_list[{}]`; "
            ...
            if i % self._num_cores_per_host:
                if device != contiguous_device:
                    raise ValueError(
                        "We expect the `enqueue_datas` which are on the "
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, accumulator_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_accumulator = tpu_ops.retrieve_tpu_embedding_adagrad_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
...
class _AdagradMomentumHandler(_OptimizerHandler):
  ...
  """
    ...
    def create_variables_and_ops(
        ...
        slot_variables = AdagradMomentumSlotVariables(
            accumulator_variables, momenta_variables
        )
      ...
      """
            ...
            for host_id, table_variable, accumulator_variable, momenta_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    load_parameters_op = tpu_ops.load_tpu_embedding_adagrad_momentum_parameters(
                        parameters=table_variable,
                        accumulators=accumulator_variable,
      ...
      """
            ...
            for host_id, table_variable, accumulator_variable, momenta_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_accumulator, retrieved_momenta = tpu_ops.retrieve_tpu_embedding_adagrad_momentum_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
...
class _ProximalAdagradHandler(_OptimizerHandler):
    ...
    def get_default_slot_variable_names(self, table):
        return ProximalAdagradSlotVariableNames(
            "{}/{}".format(table, "ProximalAdagrad")
        )
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, accumulator_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_accumulator = tpu_ops.retrieve_tpu_embedding_proximal_adagrad_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, m_variable, v_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_m, retrieved_v = tpu_ops.retrieve_tpu_embedding_adam_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, accumulator_variable, linear_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_accumulator, retrieved_linear = tpu_ops.retrieve_tpu_embedding_ftrl_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, v_variable, m_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_v, retrieved_m = tpu_ops.retrieve_tpu_embedding_proximal_yogi_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, momenta_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_momenta = tpu_ops.retrieve_tpu_embedding_momentum_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
...
class _RMSPropHandler(_OptimizerHandler):
    ...
    def get_default_slot_variable_names(self, table):
        return RMSPropSlotVariableNames(
            "{}/{}/ms".format(table, "RMSProp"), "{}/{}/mom".format(table, "RMSProp")
        )
    ...
    def create_variables_and_ops(
      ...
      """
            ...
            for host_id, table_variable, ms_variable, mom_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_ms, retrieved_mom = tpu_ops.retrieve_tpu_embedding_rms_prop_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
...
class _FrequencyEstimatorHandler(_OptimizerHandler):
    ...
    def create_variables_and_ops(
        ...
        if table_config.dimension != 1:
            raise ValueError(
                "FrequencyEstimator tables should only have a dimension "
                "of 1. Received dimension {}".format(table_config.dimension)
            )
      ...
      """
            ...
            for host_id, table_variable, last_hit_step_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    load_parameters_op = tpu_ops.load_tpu_embedding_frequency_estimator_parameters(
                        parameters=table_variable,
      ...
      """
            ...
            for host_id, table_variable, last_hit_step_variable in zip(
                ...
                with ops.colocate_with(table_variable):
                    retrieved_table, retrieved_last_hit_step = tpu_ops.retrieve_tpu_embedding_frequency_estimator_parameters(
                        table_name=table,
                        num_shards=num_hosts,
                        shard_id=host_id,
                        config=config,
                    ...
                    retrieve_parameters_op = control_flow_ops.group(
                        state_ops.assign(table_variable, retrieved_table),
                        state_ops.assign(
                            last_hit_step_variable, retrieved_last_hit_step
                        ),
                    )
                config = None
                retrieve_op_list.append(retrieve_parameters_op)
