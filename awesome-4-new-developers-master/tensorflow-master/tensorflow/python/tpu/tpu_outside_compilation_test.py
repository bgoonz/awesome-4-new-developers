...
class TpuOutsideCompilationTest(test.TestCase, parameterized.TestCase):
    ...
    def testHostNoInput(self):
        ...
        self.assertAllEqual(
            ...
            constant_op.constant(35.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testHostInputOutput(self):
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(36.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testHostMultipleInputs(self):
        ...
        def train_step():
            def tpu_fn(x, y):
                ...
                return (
                    math_ops.reduce_max(c)
                    + math_ops.reduce_min(d)
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(213.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testMultipleClusters(self):
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(21.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testOutsideCompilationControlFlowIf(self, take_true_branch):
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(output_value, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testOutsideCompilationControlFlowWhile(self):
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(58.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testSummary(self):
        ...
        with summary_writer.as_default(), summary.always_record_summaries():
            self.assertAllEqual(
                ...
                constant_op.constant(7.0, shape=(strategy.num_replicas_in_sync)),
            )
    ...
    def testSummaryInWhile(self):
        ...
        with summary_writer.as_default(), summary.always_record_summaries():
            self.assertAllEqual(
                strategy.experimental_local_results(step()),
                constant_op.constant(31.0, shape=(strategy.num_replicas_in_sync)),
            )
    ...
    def testOutsideCompilationAtHeadAndTail(self):
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(15.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testGradientAcrossOutsideCompilation(self):
        ...
        def train_step():
            def computation(x, y):
                ...
                d = gradients_impl.gradients(
                    [c], [x], colocate_gradients_with_ops=True
                )[0]
                return d
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(8748.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testGradientOfGradientAcrossOutsideCompilation(self):
        ...
        def train_step():
            def computation(x, y):
                ...
                e = gradients_impl.gradients(
                    [d], [x], colocate_gradients_with_ops=True
                )[0]
                return e
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(2916.0, shape=(strategy.num_replicas_in_sync)),
        )
    ...
    def testColocateGradientWithOutsideCompiledOp(self):
        ...
        def train_step():
            ...
            def tpu_fn(x):
                ...
                grad = gradients_impl.gradients(
                    [x1], [x], colocate_gradients_with_ops=True
                )[0]
                sqrt = [
                    op
                ...
                assert (
                    sqrt_grad.get_attr(tpu._OUTSIDE_COMPILATION_ATTR)
                    == b"0.gradients/uid"
                )
                return grad
        ...
        self.assertAllEqual(
            strategy.experimental_local_results(train_step()),
            constant_op.constant(0.1, shape=(strategy.num_replicas_in_sync)),
        )
...
class OutsideCompilationOnUnsupportedOpTest(test.TestCase, parameterized.TestCase):
    ...
    def testImageSummary(self):
        ...
        decoded_image = image_ops.decode_png(
            events[1].summary.value[0].tensor.string_val[2]
        ).numpy()
        # Ensure that non-zero values were written to the image summary.
        self.assertNotAllEqual(
            array_ops.zeros((3072,), dtype=dtypes.float32), list(decoded_image.flat)
        )
