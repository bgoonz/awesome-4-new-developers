...
flags.DEFINE_string(
    "service_addr",
    None,
    "Address of TPU profiler service e.g. "
...
flags.DEFINE_string(
    "logdir",
    None,
    "Path of TensorBoard log directory e.g. /tmp/tb_log, " "gs://tb_bucket",
)
...
flags.DEFINE_integer(
    "host_tracer_level",
    2,
    "Adjust host tracer level to control the verbosity "
...
flags.DEFINE_integer(
    "monitoring_level",
    0,
    "Choose a monitoring level between "
...
def main(unused_argv=None):
    ...
    if FLAGS.service_addr:
        if FLAGS.tpu:
            logging.warn(
                "Both --service_addr and --tpu are set. Ignoring "
    ...
    if FLAGS.monitoring_level > 0:
        print(
            "Since monitoring level is provided, profile",
            service_addr,
            " for ",
            FLAGS.duration_ms,
            " ms and show metrics for ",
            FLAGS.num_queries,
            " time(s).",
        )
        monitoring_helper(
            service_addr, duration_ms, FLAGS.monitoring_level, FLAGS.num_queries
        )
    else:
        if not FLAGS.logdir:
