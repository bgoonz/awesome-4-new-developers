...
class EmbeddingColumnTestV2(test.TestCase, parameterized.TestCase):
    ...
    def test_feature_layer_cpu(self, use_safe_embedding_lookup):
        ...
        sequence_categorical_column = fc_lib.sequence_categorical_column_with_identity(
        ...
        with _initialized_session():
            ...
            self.assertAllEqual(
                expected_lookups_sequence, sequence_embedding_lookup[0].eval()
            )
            ...
            if use_safe_embedding_lookup:
                self.assertEqual(
                    2,
                    [x.type for x in ops.get_default_graph().get_operations()].count(
                        "SparseFillEmptyRows"
                    ),
                )
            else:
    ...
    def test_deepcopy(self):
        ...
        embedding_column = tpu_fc.embedding_column_v2(categorical_column, dimension=2)
        embedding_column_copy = copy.deepcopy(embedding_column)
        self.assertEqual(embedding_column.dimension, embedding_column_copy.dimension)
        self.assertEqual(
            embedding_column._max_sequence_length,
    ...
    def test_with_scope_validation(self):
        ...
        embedding_dimension = 2
        initializer = init_ops.truncated_normal_initializer(mean=0.0, stddev=0.5)
...
class SharedEmbeddingColumnTestV2(test.TestCase, parameterized.TestCase):
    ...
    def test_defaults(self):
        ...
        self.assertEqual(
            "aaa_bbb_shared_embedding", embedding_column_a.get_embedding_var_name()
        )
        self.assertEqual(
            "aaa_bbb_shared_embedding", embedding_column_b.get_embedding_var_name()
        )
    ...
    def test_all_constructor_args(self):
        ...
        self.assertEqual("var_scope_name", embedding_column_a.get_embedding_var_name())
        self.assertEqual("var_scope_name", embedding_column_b.get_embedding_var_name())
        self.assertEqual("aaa_shared_embedding", embedding_column_a.name)
        self.assertEqual("bbb_shared_embedding", embedding_column_b.name)
        self.assertEqual((embedding_dimension,), embedding_column_a.variable_shape)
        self.assertEqual((embedding_dimension,), embedding_column_b.variable_shape)
    ...
    def test_feature_layer_cpu(self, use_safe_embedding_lookup):
        ...
        expected_lookups_b = (
            # example 0:
            ((7.0, 11.0), (0.0, 0.0)),  # ids [2], embedding = [[7, 11], [0, 0]]
            # example 1:
            ((1.0, 2.0), (3.0, 5.0)),  # ids [0, 1], embedding = [[1, 2], [3, 5]]
            # example 2:
            ((0.0, 0.0), (0.0, 0.0)),  # ids [], embedding = [[0, 0], [0, 0]]
        )
        ...
        with _initialized_session():
            ...
            self.assertAllEqual(expected_lookups_b, embedding_lookup_b[0].eval())
            # The graph will still have SparseFillEmptyRows due to sequence being
            # a Rank3 embedding lookup.
            if use_safe_embedding_lookup:
                self.assertEqual(
...
class DeviceSpecificEmbeddingColumnTestV2(test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        {"testcase_name": "invalid_shared", "shared": True},
        {"testcase_name": "invalid_not_shared", "shared": False},
    )
    @test_util.deprecated_graph_mode_only
    def test_invalid_cases(self, shared):
        ...
        dense_features = fc_lib.DenseFeatures(embedding_column)
        with self.assertRaisesRegex(
            ValueError, r".*embedding_lookup_device=\"cpu\" during training is not"
        ):
    ...
    @parameterized.named_parameters(
        {"testcase_name": "combiner_mean_shared", "shared": True, "combiner": "mean"},
        {"testcase_name": "combiner_sum_shared", "shared": True, "combiner": "sum"},
        {"testcase_name": "combiner_sqrtn_shared", "shared": True, "combiner": "sqrtn"},
        {
        ...
        with tpu_function.tpu_shard_context(1):
            ...
            if combiner == "mean":
                expected_lookups = (
                    ...
                    (7.0, 11.0),  # ids [2], embedding = [7, 11]
                    # example 1:
                    (2.0, 3.5),  # ids [0, 1], embedding = mean([1, 2] + [3, 5]) =
                    # [2, 3.5]
                )
            elif combiner == "sum":
                expected_lookups = (
                    ...
                    (7.0, 11.0),  # ids [2], embedding = [7, 11]
                    # example 1:
                    (4.0, 7),  # ids [0, 1], embedding = sum([1, 2] + [3, 5]) = [4, 7]
                )
            ...
            if shared:
                self.assertCountEqual(
                    ("inp_shared_embedding:0",), tuple([v.name for v in global_vars])
                )
            else:
                self.assertCountEqual(
    ...
    def test_error_dense_shape_invalid(self):
        ...
        with self.assertRaisesRegex(ValueError, "tensor_core_shape must be size 2"):
            tpu_fc.shared_embedding_columns_v2(
                [categorical_column_input],
                dimension=20,
