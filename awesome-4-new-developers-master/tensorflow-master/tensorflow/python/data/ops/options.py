...
class AutoShardPolicy(enum.IntEnum):
  ...
  """
    ...
    def _to_proto(cls, obj):
        ...
        raise ValueError(
            "%s._to_proto() is called with undefined enum %s."
    ...
    def _from_proto(cls, pb):
        ...
        if pb == dataset_options_pb2.AutoShardPolicy.HINT:
            return cls.HINT
        raise ValueError(
            "%s._from_proto() is called with undefined enum %s." % (cls.__name__, pb)
        )
...
class OptimizationOptions(options_lib.OptionsBase):
  ...
  """
    ...
    apply_default_optimizations = options_lib.create_option(
        ...
        ty=bool,
        docstring="Whether to apply default graph optimizations. If False, only graph "
    ...
    filter_fusion = options_lib.create_option(
        name="filter_fusion",
        ty=bool,
        docstring="Whether to fuse filter transformations. If None, defaults to False.",
    )

    map_and_batch_fusion = options_lib.create_option(
        name="map_and_batch_fusion",
        ty=bool,
        docstring="Whether to fuse map and batch transformations. If None, defaults to "
        "True.",
    )
    ...
    map_and_filter_fusion = options_lib.create_option(
        ...
        ty=bool,
        docstring="Whether to fuse map and filter transformations. If None, defaults to "
        "False.",
    )
    ...
    map_fusion = options_lib.create_option(
        name="map_fusion",
        ty=bool,
        docstring="Whether to fuse map transformations. If None, defaults to " "False.",
    )

    map_parallelization = options_lib.create_option(
        name="map_parallelization",
        ty=bool,
        docstring="Whether to parallelize stateless map transformations. If None, defaults "
        "to True.",
    )
    ...
    noop_elimination = options_lib.create_option(
        ...
        ty=bool,
        docstring="Whether to eliminate no-op transformations. If None, defaults to True.",
    )
...
class ThreadingOptions(options_lib.OptionsBase):
  ...
  """
    ...
    max_intra_op_parallelism = options_lib.create_option(
        ...
        ty=int,
        docstring="If set, it overrides the maximum degree of intra-op parallelism.",
    )

    private_threadpool_size = options_lib.create_option(
        name="private_threadpool_size",
        ty=int,
        docstring="If set, the dataset will use a private threadpool of the given size. "
...
class Options(options_lib.OptionsBase):
  ...
  """
    ...
    deterministic = options_lib.create_option(
        name="deterministic",
        ty=bool,
        docstring="Whether the outputs need to be produced in deterministic order. If None,"
        " defaults to True.",
    )
    ...
    experimental_distribute = options_lib.create_option(
        ...
        docstring="The distribution strategy options associated with the dataset. See "
    ...
    def __getattribute__(self, name):
        if name == "experimental_threading":
            logging.warning(
                "options.experimental_threading is deprecated. "
    ...
    def _to_proto(self):
        ...
        pb.autotune_options.CopyFrom(
            self.autotune._to_proto()
        )  # pylint: disable=protected-access
        pb.distribute_options.CopyFrom(
            self.experimental_distribute._to_proto()
        )  # pylint: disable=protected-access
        if self.experimental_external_state_policy is not None:
            pb.external_state_policy = ExternalStatePolicy._to_proto(  # pylint: disable=protected-access
                self.experimental_external_state_policy
            )
        pb.optimization_options.CopyFrom(
            self.experimental_optimization._to_proto()
        )  # pylint: disable=protected-access
        if self.experimental_slack is not None:
            pb.slack = self.experimental_slack
        pb.threading_options.CopyFrom(
            self.threading._to_proto()
        )  # pylint: disable=protected-access
        return pb
    ...
    def _from_proto(self, pb):
        ...
        self.autotune._from_proto(
            pb.autotune_options
        )  # pylint: disable=protected-access
        self.experimental_distribute._from_proto(
            pb.distribute_options
        )  # pylint: disable=protected-access
        if pb.WhichOneof("optional_external_state_policy") is not None:
            self.experimental_external_state_policy = ExternalStatePolicy._from_proto(  # pylint: disable=protected-access
                pb.external_state_policy
            )
        self.experimental_optimization._from_proto(
            pb.optimization_options
        )  # pylint: disable=protected-access
        if pb.WhichOneof("optional_slack") is not None:
            self.experimental_slack = pb.slack
        self.threading._from_proto(
            pb.threading_options
        )  # pylint: disable=protected-access
