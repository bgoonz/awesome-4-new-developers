...
def default_test_combinations():
    ...
    return (
        verify_unused_iterator_combination
...
class CheckpointTestBase(test.TestCase):
    ...
    def verify_unused_iterator(
        self, ds_fn, num_outputs, sparse_tensors=False, verify_exhausted=True
    ):
    ...
    """
        self.gen_outputs(
            ds_fn, [], num_outputs, verify_exhausted=True, sparse_tensors=sparse_tensors
        )
    ...
    def verify_error_on_save(
        self, ds_fn, num_outputs, error, break_point=None, sparse_tensors=False
    ):
    ...
    def _add_iterator_ops_to_collection(
        self, init_op, get_next, ds_fn, sparse_tensors=False
    ):
    ...
    def _get_iterator_ops_from_collection(self, ds_fn, sparse_tensors=False):
        ...
        return (
            all_ops[0],
            nest.pack_sequence_as(self._get_output_types(ds_fn), get_next_list),
        )
