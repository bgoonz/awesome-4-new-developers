...
def _optional_spec_test_combinations():
    ...
    cases = [
        ...
        (
            "Optional",
            lambda: optional_ops.Optional.from_value(37.0),
...
def _get_next_as_optional_test_combinations():
    ...
    cases = [
        (
            ...
            lambda: constant_op.constant([4, 5, 6], dtype=dtypes.int32),
            True,
        ),
        (
            "Sparse",
        ...
        (
            ...
            lambda: {
                "a": constant_op.constant([4, 5, 6], dtype=dtypes.int32),
...
class OptionalTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testFromStructuredValue(self):
        ...
        self.assertTrue(self.evaluate(opt.has_value()))
        self.assertEqual(
            {"a": 37.0, "b": ([b"Foo"], b"Bar")}, self.evaluate(opt.get_value())
        )
        ...
        for device in devices:
            with ops.device(device):
                ...
                inner_add_opt = optional_ops._OptionalImpl(
                    add_opt.get_value()[1], opt1.element_spec
                )
        ...
        for device in devices:
            with ops.device(device):
                ...
                zeros_opt = optional_ops._OptionalImpl(
                    zeros_tensor, opt_none.element_spec
                )
        ...
        for device in devices:
            with ops.device(device):
                ...
                inner_zeros_opt = optional_ops._OptionalImpl(
                    zeros_opt.get_value(), opt1.element_spec
                )
    ...
    def testNestedCopyToGPU(self):
        ...
        inner_none = optional_ops._OptionalImpl(
            gpu_nested_optional_values[1], optional_none.element_spec
        )

        self.assertEqual(
            (37.0, b"Foo", 42), self.evaluate(inner_with_value.get_value())
        )
    ...
    def testOptionalSpec(self, tf_value_fn, expected_value_structure):
        ...
        self.assertTrue(
            structure.are_compatible(
                opt_structure._element_spec, expected_value_structure
            )
        )
        self.assertEqual(
            [dtypes.variant], structure.get_flat_tensor_types(opt_structure)
        )
        self.assertEqual(
