...
class ListFilesTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testEmptyDirectory(self):
        with self.assertRaisesWithPredicateMatch(
            errors.InvalidArgumentError, "No files matched"
        ):
    ...
    def testFixedSeedResultsInRepeatableOrder(self):
        ...
        expected_filenames = [
            compat.as_bytes(path.join(self.tmp_dir, filename)) for filename in filenames
        ]
    ...
    def tesEmptyDirectoryInitializer(self):
        ...
        self.assertDatasetProduces(
            dataset_fn(),
            expected_error=(errors.InvalidArgumentError, "No files matched pattern"),
    ...
    def testNoShuffle(self):
        ...
        self.assertEqual(
            actual_filenames[: len(filenames)], actual_filenames[len(filenames) :]
        )
