...
class FeaturesTestBase(test_base.DatasetTestBase):
        ...
        example = example_pb2.Example(
            features=feature_pb2.Features(
                feature={
                    ...
                    "record": feature_pb2.Feature(
                        int64_list=feature_pb2.Int64List(value=[r])
                    ),
                    "keywords": feature_pb2.Feature(
                        bytes_list=feature_pb2.BytesList(value=self._get_keywords(f, r))
                    ),
                    "label": feature_pb2.Feature(
                        bytes_list=feature_pb2.BytesList(value=[compat.as_bytes(l)])
                    ),
                }
            )
        )
    ...
    def _next_expected_batch(
        self, file_indices, batch_size, num_epochs, cycle_length=1
    ):
        ...
        def _next_record_interleaved(file_indices, cycle_length):
            return self._interleave(
                [_next_record([i]) for i in file_indices], cycle_length
            )
            ...
            for record in next_records:
                ...
                if len(file_batch) == batch_size:
                    yield [
                        ...
                        keywords_batch_indices,
                        keywords_batch_values,
                        [batch_size, keywords_batch_max_len],
                        record_batch,
                        label_batch,
                    ]
                    file_batch = []
        ...
        if file_batch:
            yield [
                ...
                keywords_batch_indices,
                keywords_batch_values,
                [len(file_batch), keywords_batch_max_len],
                record_batch,
                label_batch,
            ]
    ...
    def _verify_records(
        ...
        for expected_batch in self._next_expected_batch(
            file_indices, batch_size, num_epochs, cycle_length=interleave_cycle_length
        ):
