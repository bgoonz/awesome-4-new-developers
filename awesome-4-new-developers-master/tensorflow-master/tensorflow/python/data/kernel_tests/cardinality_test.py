...
def _test_combinations():
    v1_only_cases = [
        (
            "Map1",
            lambda: dataset_ops.Dataset.range(5).map(lambda x: x),
            dataset_ops.UNKNOWN,
        ),
        (
            "Map2",
            lambda: dataset_ops.Dataset.range(5).map(lambda x: x, num_parallel_calls=1),
            dataset_ops.UNKNOWN,
        ),
    ]
    v2_only_cases = [
    ...
    v1_and_v2_cases = [
        (
            "Batch1",
            lambda: dataset_ops.Dataset.range(5).batch(2, drop_remainder=True),
            2,
        ),
        (
            "Batch2",
            lambda: dataset_ops.Dataset.range(5).batch(2, drop_remainder=False),
            3,
        ),
        (
            "Batch3",
        ...
        (
            "Batch4",
            lambda: dataset_ops.Dataset.range(5).repeat().batch(2),
        ...
        (
            "Concatenate1",
            lambda: dataset_ops.Dataset.range(5).concatenate(
        ...
        (
            "Concatenate2",
            lambda: dataset_ops.Dataset.range(5)
        ...
        (
            ...
            lambda: dataset_ops.Dataset.range(5)
            .filter(lambda _: True)
            .concatenate(dataset_ops.Dataset.range(5).repeat()),
            dataset_ops.INFINITE,
        ),
        (
            "Concatenate9",
        ...
        (
            "FlatMap",
            lambda: dataset_ops.Dataset.range(5).flat_map(
        ...
        (
            "Interleave1",
            lambda: dataset_ops.Dataset.range(5).interleave(
        ...
        ("Prefetch", lambda: dataset_ops.Dataset.range(5).prefetch(buffer_size=1), 5),
        ...
        (
            "Repeat6",
            lambda: dataset_ops.Dataset.range(1).repeat(),
            dataset_ops.INFINITE,
        ),
        ("Shuffle", lambda: dataset_ops.Dataset.range(5).shuffle(buffer_size=1), 5),
        ...
        (
            "Shard4",
            lambda: dataset_ops.Dataset.range(5).repeat().shard(2, 0),
            dataset_ops.INFINITE,
        ),
        ...
        (
            "Skip4",
            lambda: dataset_ops.Dataset.range(5).repeat().skip(2),
            dataset_ops.INFINITE,
        ),
        ...
        (
            "Window1",
            lambda: dataset_ops.Dataset.range(5).window(
                size=2, shift=2, drop_remainder=True
            ),
            2,
        ),
        (
            "Window2",
            lambda: dataset_ops.Dataset.range(5).window(
                size=2, shift=2, drop_remainder=False
            ),
            3,
        ),
        ("Zip1", lambda: dataset_ops.Dataset.zip(dataset_ops.Dataset.range(5)), 5),
        (
            "Zip2",
            lambda: dataset_ops.Dataset.zip(
                (dataset_ops.Dataset.range(5), dataset_ops.Dataset.range(3))
            ),
            3,
        ),
        (
            "Zip3",
            lambda: dataset_ops.Dataset.zip(
                (dataset_ops.Dataset.range(5), dataset_ops.Dataset.range(3).repeat())
            ),
            5,
        ),
        (
