...
class WindowTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            ...
            + combinations.combine(
                count=[0, 1], size=10, shift=4, stride=1, drop_remainder=[True, False]
            ),
        )
    )
    def testWindowDataset(self, count, size, shift, stride, drop_remainder):
        ...
        for i in range(num_full_batches):
            ...
            for component, result_component in zip(components, result):
                for j in range(size):
                    self.assertAllEqual(
                        component[(i * shift + j * stride) % 7] ** 2,
                        result_component[j],
                    )
        if not drop_remainder:
            num_partial_batches = (
                (count * 7) // shift + ((count * 7) % shift > 0) - num_full_batches
            )
    ...
    def testWindowDatasetInvalid(self, count, size, shift, stride):
        with self.assertRaises(errors.InvalidArgumentError):
            ds = (
                ...
                .map(lambda x: x)
                .repeat(count)
                .window(size=size, shift=shift, stride=stride)
                .flat_map(lambda x: x.batch(batch_size=size))
            )
    ...
    def testWindowSparse(self):
        ...
        dataset = (
            dataset_ops.Dataset.range(10)
            .map(_sparse)
            .window(size=5, shift=3, drop_remainder=True)
            .flat_map(lambda x: x.batch(batch_size=5))
        )
    ...
    def testWindowSparseWithDifferentDenseShapes(self):
        def _sparse(i):
            return sparse_tensor.SparseTensorValue(
                indices=array_ops.expand_dims(math_ops.range(i, dtype=dtypes.int64), 1),
    ...
    def testNestedWindowSparse(self):
        ...
        dataset = (
            dataset_ops.Dataset.range(10)
            .map(_sparse)
            .window(size=4, shift=2, drop_remainder=True)
            .flat_map(lambda x: x.batch(batch_size=4))
            .window(size=3, shift=1, drop_remainder=True)
            .flat_map(lambda x: x.batch(batch_size=3))
        )
    ...
    def testWindowShapeError(self):
        ...
        dataset = (
            dataset_ops.Dataset.from_generator(
                generator, dtypes.float32, output_shapes=[None]
            )
            .window(size=3, shift=1)
            .flat_map(lambda x: x.batch(batch_size=3))
        )
    ...
    def testWindowIgnoreErrors(self):
        ...
        dataset = (
            ...
            .window(size=2, shift=2, stride=2, drop_remainder=True)
            .flat_map(lambda x: x.batch(batch_size=2))
        )
        self.assertDatasetProduces(
            dataset, expected_output=[np.float32([1.0, 2.0]), np.float32([2.0, 3.0])]
        )
...
class WindowCheckpointTest(
    checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    def _build_dataset(self):
        dataset = (
            dataset_ops.Dataset.range(42)
            .window(6)
            .interleave(lambda x: x, cycle_length=2, num_parallel_calls=2)
        )
        return dataset
