...
class NestTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testFlattenAndPackWithDicts(self):
        ...
        mess = (
            "z",
            named_tuple(3, 4),
            {"c": (1, collections.OrderedDict([("b", 3), ("a", 2)])), "b": 5},
            17,
        )
        ...
        structure_of_mess = (
            14,
            named_tuple("a", True),
            {"c": (0, collections.OrderedDict([("b", 9), ("a", 8)])), "b": 3},
            "hi everybody",
        )
    ...
    def testAssertSameStructure(self):
        ...
        self.assertRaises(
            TypeError, nest.assert_same_structure, (0, 1), named_type_0("a", "b")
        )
        ...
        with self.assertRaisesRegex(ValueError, "don't have the same set of keys"):
            nest.assert_same_structure(
                structure_dictionary, structure_dictionary_diff_nested
            )
        nest.assert_same_structure(
            structure_dictionary, structure_dictionary_diff_nested, check_types=False
        )
        nest.assert_same_structure(structure1_list, structure2_list, check_types=False)
