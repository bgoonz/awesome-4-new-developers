...
def _test_any_sparse_combinations():
    ...
    cases = [
        ...
        (
            "TestCase_5",
            lambda: (sparse_tensor.SparseTensor, sparse_tensor.SparseTensor),
            True,
        ),
...
def _test_as_dense_shapes_combinations():
    ...
    cases = [
        ...
        (
            "TestCase_9",
            lambda: (tensor_shape.TensorShape([]), (), tensor_shape.TensorShape([])),
        ...
        (
            ...
            lambda: (tensor_shape.TensorShape([]), (), tensor_shape.TensorShape([])),
            lambda: (sparse_tensor.SparseTensor, (), sparse_tensor.SparseTensor),
            lambda: (tensor_shape.unknown_shape(), (), tensor_shape.unknown_shape()),
        ),
        (
            "TestCase_11",
            lambda: ((), tensor_shape.TensorShape([]), ()),
...
def _test_as_dense_types_combinations():
    cases = [
        ("TestCase_0", lambda: (), lambda: (), lambda: ()),
        ("TestCase_1", lambda: dtypes.int32, lambda: ops.Tensor, lambda: dtypes.int32),
        (
            "TestCase_2",
            lambda: dtypes.int32,
            lambda: sparse_tensor.SparseTensor,
...
class SparseTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_any_sparse_combinations()
        )
    )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_as_dense_shapes_combinations()
        )
    )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_as_dense_types_combinations()
        )
    )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_get_classes_combinations()
        )
    )
    ...
    def testSerializeDeserialize(self, input_fn):
        ...
        shapes = nest.map_structure(lambda _: tensor_shape.TensorShape(None), classes)
        types = nest.map_structure(lambda _: dtypes.int32, classes)
        actual = sparse.deserialize_sparse_tensors(
            sparse.serialize_sparse_tensors(test_case),
            types,
            shapes,
    ...
    def testSerializeManyDeserialize(self, input_fn):
        ...
        types = nest.map_structure(lambda _: dtypes.int32, classes)
        actual = sparse.deserialize_sparse_tensors(
            sparse.serialize_many_sparse_tensors(test_case),
            types,
            shapes,
            sparse.get_classes(test_case),
        )
