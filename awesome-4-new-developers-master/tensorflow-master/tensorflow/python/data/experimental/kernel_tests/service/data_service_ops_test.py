...
class DataServiceOpsTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testDistributeBasic(self, work_dir, fault_tolerant_mode):
        cluster = data_service_test_base.TestCluster(
            num_workers=1, work_dir=work_dir, fault_tolerant_mode=fault_tolerant_mode
        )
        num_elements = 10
    ...
    def testDontGcUsedJob(self):
        ...
        num_elements = 10
        it1 = iter(
            self.make_distributed_range_dataset(num_elements, cluster, job_name="test1")
        )
        it2 = iter(
            self.make_distributed_range_dataset(num_elements, cluster, job_name="test2")
        )
    ...
    def testApplyDeterminismOption(self):
        ...
        self.checkDeterminism(
            dataset_fn=dataset_fn, expect_determinism=False, expected_elements=elements
        )
    ...
    def testZipDifferentProcessingModesDatasetsSharedJobName(self):
        ...
        ds = dataset_ops.Dataset.zip((ds1, ds2))
        with self.assertRaisesRegex(
            errors.FailedPreconditionError, "but there is already an existing job"
        ):
            self.getDatasetOutput(ds)
    ...
    def testFromDatasetId(self):
        ...
        from_dataset_id_ds = data_service_ops.from_dataset_id(
            "parallel_epochs", cluster.dispatcher_address(), dataset_id, ds.element_spec
        )
    ...
    def testFromDatasetIdWrongElementSpec(self):
        ...
        with self.assertRaisesRegex(
            errors.FailedPreconditionError, "Expected a tensor of type variant"
        ):
    ...
    def testFromDatasetIdNotRegistered(self):
        ...
        from_dataset_id_ds = data_service_ops.from_dataset_id(
            "parallel_epochs", cluster.dispatcher_address(), dataset_id, element_spec
        )
