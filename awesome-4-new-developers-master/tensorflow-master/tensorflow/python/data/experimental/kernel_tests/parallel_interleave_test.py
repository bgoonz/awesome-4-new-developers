...
class ParallelInterleaveTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(sloppy=[False, True], prefetch_input_elements=[0, 1]),
        )
    )
    def testSingleThreaded(self, sloppy, prefetch_input_elements):
        ...
        for expected_element in self._interleave(
            ...
            self.assertEqual(
                expected_element * expected_element, self.evaluate(next_element())
            )
    ...
    def testTwoThreadsNoContention(self, sloppy):
        ...
        for i, expected_element in enumerate(
            self._interleave([[4] * 4, [5] * 5, [6] * 6] * self.repeat_count, 2, 1)
        ):
            self.write_coordination_events[expected_element].set()
    ...
    def testTwoThreadsNoContentionBlockLength(self, sloppy):
        ...
        for i, expected_element in enumerate(
            self._interleave([[4] * 4, [5] * 5, [6] * 6] * self.repeat_count, 2, 2)
        ):
            self.write_coordination_events[expected_element].set()
            if done_first_event:  # First event starts the worker threads.
                self.read_coordination_events[expected_element].acquire()
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(sloppy=[False, True], prefetch_input_elements=[1, 0]),
        )
    )
    ...
    def testSparse(self):
        ...
        dataset = (
            ...
            .map(_map_fn)
            .apply(interleave_ops.parallel_interleave(_interleave_fn, cycle_length=1))
        )
        get_next = self.getNext(dataset)
...
class ParallelInterleaveCheckpointTest(
    ...
    def test(self, verify_fn, cycle_length, block_length):
        verify_fn(
            self, lambda: self._build_ds(cycle_length, block_length), self.num_outputs
        )
    ...
    def testSparse(self, verify_fn):
        ...
        def _build_dataset():
            return (
                dataset_ops.Dataset.range(10)
                .map(_map_fn)
                .apply(interleave_ops.parallel_interleave(_interleave_fn, 1))
            )
