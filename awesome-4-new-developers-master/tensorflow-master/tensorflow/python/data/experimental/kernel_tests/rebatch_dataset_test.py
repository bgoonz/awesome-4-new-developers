    ...
    def _test(
        ...
        for i in range(num_workers):
            batch_sizes_list.append(
                self.evaluate(
                    distribute.batch_sizes_for_worker(
                        global_batch_size, num_workers, num_replicas_per_worker, i
                    )
                )
            )
    ...
    def testBasic(self, is_batch_size_static):
        ...
        for worker_index in range(4):
            batch_sizes = distribute.batch_sizes_for_worker(
                global_batch_size, num_workers, num_replicas_per_worker, worker_index
            )
...
class RebatchDatasetTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testShapeInferenceNotAllBatchSizesEqual(self):
        ...
        rebatched_dataset = distribute._RebatchDataset(dataset, batch_sizes=[2, 1, 1])
    ...
    def testNestedDictionaryOutput(self):
        dataset = (
            dataset_ops.Dataset.range(8)
            .map(lambda x: {"a": x, "b": {"c": x + 1}})
            .batch(4, drop_remainder=True)
        )
        rebatched_dataset = distribute._RebatchDataset(dataset, [2, 2])
        self.assertEqual([[2], [2]], _flat_shapes(rebatched_dataset))
    ...
    def testRaggedDataset(self, drop_remainder):
        ...
        rebatched_dataset = distribute._RebatchDataset(dataset, batch_sizes=[2, 2])
...
class LegacyRebatchDatasetTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testFinalPartialBatch(self, drop_remainder):
        ...
        rebatched_dataset = distribute._LegacyRebatchDataset(dataset, num_replicas=2)
        self.assertEqual(
            [[2] if drop_remainder else [None]], _flat_shapes(rebatched_dataset)
        )
        if drop_remainder:
            expected_output = [[0, 1], [2, 3], [4, 5], [6, 7]]
        else:
