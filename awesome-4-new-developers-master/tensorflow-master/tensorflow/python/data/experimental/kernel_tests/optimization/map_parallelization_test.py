...
class MapParallelizationTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testMapParallelization(self, function, should_optimize):
        ...
        dataset = (
            ...
            .apply(testing.assert_next(next_nodes))
            .map(function)
        )
    ...
    def testCapturedConstant(self):
        ...
        dataset = (
            dataset_ops.Dataset.range(5)
            .apply(testing.assert_next(["ParallelMap"]))
            .map(fn)
        )
        options = options_lib.Options()
        options.experimental_optimization.apply_default_optimizations = False
    ...
    def testCapturedVariable(self):
        ...
        dataset = (
            dataset_ops.Dataset.range(5).apply(testing.assert_next(["Map"])).map(fn)
        )
        options = options_lib.Options()
        options.experimental_optimization.apply_default_optimizations = False
        options.experimental_optimization.map_parallelization = True
    ...
    def testAutotuneOption(self, apply_autotune):
        next_nodes = (
            ["ParallelMap"] if (apply_autotune is not False) else ["Map"]
