...
def _captured_refvar_test_combinations():
    ...
    def make_flat_map_dataset(var):
        return dataset_ops.Dataset.from_tensors(0).flat_map(
            lambda _: dataset_ops.Dataset.from_tensors(var)
        )
    ...
    def make_group_by_window_dataset(var):
        ...
        return (
            ...
            .repeat(10)
            .apply(grouping.group_by_window(lambda _: 0, reduce_fn, 10))
        )
...
class OptimizationTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testOptimizationStatefulFunction(self):
        dataset = (
            ...
            .map(lambda _: random_ops.random_uniform([]))
            .batch(10)
        )
    ...
    def testOptimizationMapParallelization(self, autotune, map_parallelization):
        dataset = dataset_ops.Dataset.range(5)
        if (
            autotune is not False and map_parallelization is not False
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(
                first_buffer_sizes=[(1, -1, -1, 4), (2, -1, 3, -1), (2, 1, -1, -1)]
            ),
            combinations.combine(
                second_buffer_sizes=[(1, -1, -1, 4), (2, -1, 3, -1), (2, 1, -1, -1)]
            ),
        )
    )
    def testOptimizationAutotuneBuffers(
        self, autotune, first_buffer_sizes, second_buffer_sizes
    ):
    ...
    @combinations.generate(
        combinations.times(
            test_base.graph_only_combinations(), _captured_refvar_test_combinations()
        )
    )
