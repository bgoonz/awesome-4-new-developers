...
class _DirectedInterleaveDataset(dataset_ops.DatasetV2):
    ...
    def __init__(self, selector_input, data_inputs, stop_on_empty_dataset=False):
        ...
        for i, data_input in enumerate(data_inputs[1:]):
            if (
                dataset_ops.get_legacy_output_types(data_input) != first_output_types
    ...
    if not isinstance(weights, dataset_ops.DatasetV2):
        if weights is None:
            ...
            if weights.dtype not in (dtypes.float32, dtypes.float64):
                raise TypeError(
                    "`weights` must be convertible to a tensor of "
...
def choose_from_datasets_v2(datasets, choice_dataset, stop_on_empty_dataset=False):
  ...
  """
    ...
    if choice_dataset is None or not structure.are_compatible(
        ...
        raise TypeError(
            "`choice_dataset` must be a dataset of scalar " "`tf.int64` tensors."
        )
    return _DirectedInterleaveDataset(choice_dataset, datasets, stop_on_empty_dataset)
