...
class DataFrame:
    ...
    def rows(self) -> Sequence[Sequence[Any]]:
        return (
            self._rows
            if self._rows
            else [[c[i] for c in self._columns] for i in range(len(self._columns[0]))]
        )
    ...
    def columns(self) -> Sequence[Sequence[Any]]:
        return (
            self._columns
            if self._columns
            else [[r[i] for r in self._rows] for i in range(len(self._rows[0]))]
        )

    def __add__(self, other: "DataFrame") -> "DataFrame":
        if not set(self.column_names).intersection(other.column_names) and len(
            self.rows
        ) == len(other.rows):
            return DataFrame(
                column_names=list(
    ...
    def __str__(self) -> str:
        return (
            ...
            + "\n"
            + "\n".join(
                ",".join("N/A" if v is None else str(v) for v in row)
...
def extract_test_info(test_results: model_handler.TestResultCollection) -> DataFrame:
    ...
    column_names = list(
        itertools.chain(
            model_handler.ModelConfig._fields,
...
def analyze_test_latency(
    test_results: model_handler.TestResultCollection, use_cpu_baseline: bool
) -> DataFrame:
    """Analyzes test latency."""
    base_result = (
        test_results.cpu_base_result
        if use_cpu_baseline
        else test_results.gpu_base_result
    )
