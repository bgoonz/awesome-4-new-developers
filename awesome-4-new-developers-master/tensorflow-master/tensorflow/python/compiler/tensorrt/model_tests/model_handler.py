...
def _get_concrete_tensor_shape(
    tensor_shape: tensor_shape_pb2.TensorShapeProto, batch_size: Optional[int] = None
) -> Sequence[int]:
    ...
    if shape[0] < 0:
        if batch_size is None or batch_size <= 0:
            raise ValueError(
                "Must provide a valid batch size "
...
def _generate_random_tensor_ops(
    shape: Sequence[int], dtype: tf_dtypes.DType, name: str
) -> framework_ops.Tensor:
...
def _generate_random_tensor_v1(
    tensor_info: meta_graph_pb2.TensorInfo, batch_size: Optional[int] = None
) -> np.ndarray:
    ...
    with framework_ops.Graph().as_default() as graph, session.Session(graph=graph):
        return _generate_random_tensor_ops(
            shape=shape,
            dtype=dtype,
            name=_remove_graph_sequence_number(tensor_info.name),
        ).eval()


def _generate_random_tensor_v2(
    tensor: framework_ops.Tensor, batch_size: Optional[int] = None
) -> framework_ops.Tensor:
    """Generates a random tensor based on the data type and tensor shape."""
...
@functools.lru_cache()
def load_meta_graph(
    saved_model_dir: str, saved_model_tags: str, saved_model_signature_key: str
    ...
    with framework_ops.Graph().as_default() as graph, session.Session(
        ...
        meta_graph = saved_model_loader.load(
            sess=sess, export_dir=saved_model_dir, tags=saved_model_tags
        )
        output_node_names = [
            _remove_graph_sequence_number(tensor.name)
            for tensor in meta_graph.signature_def[
...
def load_graph_func(
    ...
    imported = saved_model_load.load(export_dir=saved_model_dir, tags=saved_model_tags)
...
class ModelConfig(
    ...
    def __new__(
        ...
        return super(ModelConfig, cls).__new__(
            cls,
            saved_model_dir,
            saved_model_tags,
            saved_model_signature_key,
...
class TestResult(
    ...
    def __new__(
        ...
        return super(TestResult, cls).__new__(
            cls,
            model_config,
            enable_gpu,
            output_names,
...
class TestResultCollection(
    ...
    def __new__(
        ...
        return super(TestResultCollection, cls).__new__(
            cls, test_name, model_config, cpu_base_result, gpu_base_result, trt_results
        )
    ...
    def results(self) -> Iterable[TestResult]:
        return filter(
            ...
            itertools.chain(
                [self.cpu_base_result, self.gpu_base_result], self.trt_results
            ),
        )
            ...
            with session.Session(config=config_proto) as sess:
                ...
                try:
                    ...
                    for _ in range(benchmark_iterations):
                        before = time.time()
                        outputs = sess.run(
                            fetches=output_tensor_names, feed_dict=inputs
                        )
...
class _TrtModelHandlerBase(_ModelHandlerBase):
    ...
    def __init__(
        self, model_config: ModelConfig, trt_convert_params: trt.TrtConversionParams
    ):
    ...
    def __str__(self) -> str:
        ...
        return "{}, TrtConversionParams: {}".format(base, str(self._trt_convert_params))
    ...
    @abc.abstractmethod
    def convert(
        self, calibration_inputs: Optional[Mapping[str, np.ndarray]] = None, num_runs=1
    ) -> None:
    ...
    def save(
        self, output_saved_model_dir: Optional[str] = None, overwrite=True
    ) -> None:
...
class TrtModelHandlerV1(_TrtModelHandlerBase, ModelHandlerV1):
    ...
    def convert(
        ...
        if (
            self.trt_convert_params.precision_mode == trt.TrtPrecisionMode.INT8
            and self.trt_convert_params.use_calibration
        ):
            logging.info("Calibrating with TensorRT!")
            if not calibration_inputs:
                raise ValueError(
                    "Must provide calibration data " "when using TensorRT calibration!"
                )
            try:
                ...
                raise RuntimeError(
                    "Failed to calibrate! " "Model Information: {}".format(str(self))
                ) from exc
...
class TrtModelHandlerV2(_TrtModelHandlerBase, ModelHandlerV2):
    ...
    def convert(
        self,
        calibration_inputs: Optional[Sequence[framework_ops.Tensor]] = None,
        num_runs=1,
    ) -> None:
        logging.info("Converting with TensorRT!")
...
class _ModelHandlerManagerBase(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        trt_convert_params_updater: Callable[
    ...
    def run(
        self, inputs=None, warmup_iterations: int = 10, benchmark_iterations: int = 100
    ) -> TestResultCollection:
    ...
    """
        ...
        try:
            ...
            logging.info(
                "%s cannot run on CPU. Reason: %s.", self._ori_model.model_config, err
            )
