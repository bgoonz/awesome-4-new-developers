...
def build_graph(
    inp, dtype, num_filters, data_format, kernel_sizes, dilation_rates, padding="same"
):
    results = []
...
class Conv2DNCHWTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def GetParams(self):
        ...
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[13, 3, 7, 11]], [[13, 5, 7, 11]]
        )
...
class Conv2DNHWCTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def GetParams(self):
        # TODO(aaroey): test graph with different dtypes.
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[13, 7, 11, 3]], [[13, 7, 11, 5]]
        )
...
class Conv2DStridedNCHWTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def GetParams(self):
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[13, 3, 7, 11]], [[13, 5, 2, 3]]
        )
...
class Conv2DTranposeTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def GraphFn(self, inp):
        ...
        weights = constant_op.constant(np.random.randn(*weights_shape), dtype=dtype)
        output_shape = constant_op.constant(
            [n, num_filters, h * 2, w * 2], dtype=dtypes.int32
        )
    ...
    def GetParams(self):
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[13, 3, 7, 11]], [[13, 8, 14, 22]]
        )
