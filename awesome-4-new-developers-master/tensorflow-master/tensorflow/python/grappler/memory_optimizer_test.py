...
class MemoryOptimizerSwapTest(test.TestCase):
    ...
    def testSimpleSwap(self):
        ...
        with ops.device("/gpu:0"):
            ...
            self.assertTrue(
                set(node.name for node in graph.node)
                > set(["a", "b", "c", "d", "swap_in_d_0", "swap_out_d_0"])
            )
        ...
        with graph.as_default():
            ...
            for layer_number in range(10):
                with variable_scope.variable_scope("layer_{}".format(layer_number)):
                    after_conv = nn.conv2d(
                        current_activation, conv_filter, [1, 1, 1, 1], "SAME"
                    )
    ...
    def testRewritingDefaultGradientNames(self):
        ...
        self.assertEqual(
            ...
            len(
                [
                    node
                    for node in original_metagraph.graph_def.node
        ...
        self.assertEqual(
            ...
            len(
                [
                    node
                    for node in rewritten_graph_def.node
                    if "Recomputed/" in node.name
                ]
            ),
        )
