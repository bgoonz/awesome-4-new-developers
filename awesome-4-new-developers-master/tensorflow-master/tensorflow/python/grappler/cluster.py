...
class Cluster(object):
    ...
    """
        ...
        if devices is None:
            self._tf_cluster = tf_cluster.TF_NewCluster(
                allow_soft_placement, disable_detailed_stats
            )
        else:
    ...
    def ListDevices(self):
        ...
        if self._tf_cluster is None:
            return []
        return [
            device_properties_pb2.NamedDevice.FromString(device)
...
def Provision(
    ...
    cluster = Cluster(
        allow_soft_placement, disable_detailed_stats, disable_timeline, devices
    )
    yield cluster
    cluster.Shutdown()
