...
def _get_config(auto_mixed_precision_mode):
    ...
    if auto_mixed_precision_mode == "cuda":
        ...
        rewrite_config.auto_mixed_precision_mkl = rewriter_config_pb2.RewriterConfig.ON
    else:
...
class AutoMixedPrecisionTest(test.TestCase, parameterized.TestCase):
    ...
    def _assert_output_f16(self, mode, node_map, node_name, output_port=0):
        self.assertEqual(
            node_map[node_name].output_info[output_port].dtype,
        ...
        with ops.device(_get_device(mode)):
            ...
            for section in [inp, body, out]:
                ...
                for color in section:
                    if color.isupper():
                        expected_type = self._lower_precision_dtype(
                            mode
                        ).as_datatype_enum
                    else:
    ...
    def test_conv3d(self, mode):
        ...
        self._assert_output_f16(
            mode, node_map, "gradients/Conv3D_grad/Conv3DBackpropInputV2"
        )
        self._assert_output_f16(
            mode, node_map, "gradients/Conv3D_grad/Conv3DBackpropFilterV2"
        )
