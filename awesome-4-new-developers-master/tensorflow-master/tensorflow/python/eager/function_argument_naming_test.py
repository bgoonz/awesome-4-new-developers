...
class ArgumentNamingTests(test.TestCase, parameterized.TestCase):
    ...
    def testBasic(self, function_decorator):
        ...
        self.assertAllClose(
            [3.0, 2.0], fn_op(a=constant_op.constant(1.0), b=constant_op.constant(2.0))
        )
    ...
    def testDictReturned(self, function_decorator):
        ...
        fn_op2 = fn.get_concrete_function(
            z=(
                ...
                tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32, name="z_second"),
            ),
            y=tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32, name="custom"),
            x=4.0,
        )
        self.assertEqual(
            ["z_first", "z_second", "custom"], [inp.op.name for inp in fn_op2.inputs]
        )
        self.assertEqual(
        ...
        fn_op3 = fn.get_concrete_function(
            ...
            z=(
                tensor_spec.TensorSpec(shape=(None,), dtype=dtypes.float32, name="z1"),
    ...
    def testMethod(self, function_decorator):
        ...
        self.assertEqual(
            [b"x"], [inp.op.get_attr("_user_specified_name") for inp in class_op.inputs]
        )
        # Call the function to make def_function happy
