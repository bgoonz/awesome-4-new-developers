...
class ConcreteFunction(core.ConcreteFunction):
    ...
    def _set_function_spec(self, function_spec):
        ...
        if function_spec is not None and self.structured_input_signature is not None:
    ...
    """
        ...
        if input_signature is None:
            ...
            if not isinstance(input_signature, (tuple, list)):
                raise TypeError(
                    f"input_signature must be either a tuple or a "
...
class Function(object):
  ...
  """
    ...
    def _cache_key(
        self, args, kwargs, cache_key_context, include_tensor_ranks_only=False
    ):
    ...
    def _cache_key_context(self):
        ...
        if not executing_eagerly:
            ...
            if (
                xla_context is not None
                and xla_context.RequiresUniqueFunctionRetracing()
            ):
        ...
        uses_distribution_strategy = (
            strategy_stack
            and strategy_stack[-1].strategy.extended._retrace_functions_for_each_device
        )
        if executing_eagerly:
            colocation_stack = ()
        ...
        try:
            in_cross_replica_context = (
                strategy_stack[-1].replica_context is None
            )  # pylint: disable=protected-access
