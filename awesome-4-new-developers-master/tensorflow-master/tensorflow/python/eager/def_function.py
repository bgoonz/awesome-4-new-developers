...
class _FrequentTracingDetector(object):
    ...
    """
        ...
        while self._calls_per_tracings:
            if (
                self._call_count - self._calls_per_tracings[0]
  ...
  """
    ...
    """
        ...
        if initial_value is None:
            raise ValueError(
                "`initial_value` must be a Tensor or a Python "
        ...
        if constraint is not None and not callable(constraint):
            raise ValueError(
                f"`constraint` with type {type(constraint)} must be a " "callable."
            )
        ...
        with ops.name_scope(scope_name):
            if self._in_graph_mode:
                ...
                lifted_initializer = lift_to_graph.lift_to_graph(
                    [initial_value],
                    outer_graph,
                    disallowed_placeholders=placeholder_ops,
                )[initial_value]
                with ops.init_scope():
                    self._initial_value = lifted_initializer
                    with ops.name_scope("IsInitialized"):
                        self._is_initialized_op = resource_variable_ops.var_is_initialized_op(
                            self._handle
                        )
...
@deprecation.deprecated(
    None, "Use tf.config.functions_run_eagerly instead of the experimental version."
)
  ...
  """
    ...
    def _defun_with_scope(self, scope):
        ...
        def wrapped_fn(*args, **kwds):
            ...
            with default_graph._variable_creator_scope(
                scope, priority=50
            ):  # pylint: disable=protected-access
    ...
    """
        ...
        self._concrete_stateful_fn = self._stateful_fn._get_concrete_function_internal_garbage_collected(  # pylint: disable=protected-access
            *args, **kwds
        )
    ...
    def _clone(self, python_function):
        ...
        f = Function(
            python_function=(
                self._python_function if python_function is None else python_function
            ),
            name=self._name,
    ...
    def __call__(self, *args, **kwds):
        ...
        if self._created_variables is None:
            compiled = bool(
                self._jit_compile
                and not control_flow_util.GraphOrParentsInXlaContext(
                    ops.get_default_graph()
                )
            )
        ...
        with trace.Trace(self._name) as tm:
            ...
            tm.set_metadata(
                tf_function_call=execution_mode + "-" + compiler,
    ...
    def _call(self, *args, **kwds):
        ...
        if self._created_variables:
            ...
            _, _, _, filtered_flat_args = self._stateful_fn._function_spec.canonicalize_function_inputs(  # pylint: disable=protected-access
                *args, **kwds
            )
        ...
        def fn_with_cond(inner_args, inner_kwds, inner_filtered_flat_args):
            ...
            for wr in self._created_variables:
                ...
                condition = math_ops.logical_and(
                    condition,
                    resource_variable_ops.var_is_initialized_op(variable.handle),
                )
        ...
        canon_args, canon_kwds, _, filtered_flat_args = self._stateful_fn._function_spec.canonicalize_function_inputs(  # pylint: disable=protected-access
            *args, **kwds
        )
        return function_lib.defun(fn_with_cond)(
    ...
    def experimental_get_compiler_ir(self, *args, **kwargs):
        ...
        # pylint: disable=protected-access
        _, _, _, filtered_flat_args = concrete_fn._function_spec.canonicalize_function_inputs(
            *args, **kwargs
        )
    ...
    def _list_all_concrete_functions(self):
        ...
        if self._stateful_fn:
            concrete_functions.extend(self._stateful_fn._function_cache.all_values())
        if self._stateless_fn:
            concrete_functions.extend(self._stateless_fn._function_cache.all_values())
    ...
    """
        ...
        for concrete_function in concrete_functions:
            ...
            if any(
                isinstance(arg, func_graph_module.UnknownArgument) for arg in flattened
            ):
