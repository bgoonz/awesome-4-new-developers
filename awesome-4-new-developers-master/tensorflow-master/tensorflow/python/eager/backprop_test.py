...
class BackpropTest(test.TestCase, parameterized.TestCase):
    ...
    def testTapeNoOpGradient2By2(self):
        ...
        self.assertAllEqual(
            dy_dy.numpy(), constant_op.constant(1.0, shape=[2, 2]).numpy()
        )
    ...
    def testTapeNoOpGradientMultiTarget2By2(self):
        ...
        self.assertAllEqual(
            dy_dy.numpy(), constant_op.constant(2.0, shape=[2, 2]).numpy()
        )
    ...
    def testPythonMax(self):
        x = [
            ...
            resource_variable_ops.ResourceVariable(3.0),
            resource_variable_ops.ResourceVariable(5.0),
        ]
        with backprop.GradientTape() as t:
            f = max(x)
        grad = t.gradient(f, x)
    ...
    def testGradientTapeStructure(self):
        ...
        self.assertEqual(
            self.evaluate(g.gradient(y, [(x1, x2), (x2, x3)])), [(1.0, 2.0), (2.0, 3.0)]
        )
        self.assertEqual(
            self.evaluate(g.gradient(y, (x1, x2, [x1, x3]))), (1.0, 2.0, [1.0, 3.0])
        )
        self.assertEqual(
            self.evaluate(g.gradient(y, [x1, {"x2": x2, "x3": x3}])),
            [1.0, {"x2": 2.0, "x3": 3.0}],
        )
    ...
    def testBatchJacobianParallelIterations(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "persistent=True.*parallel_iterations"
        ):
            f(persistent=False)
    ...
    def testUnconnectedGradientsVariablesZeros(self):
        x = resource_variable_ops.ResourceVariable(
            constant_op.constant(1.0, shape=[2, 2])
        )
    ...
    def testMakeAttrType(self):
        self.assertEqual(
            dtypes.float32, backprop.make_attr(int(pywrap_tfe.TF_ATTR_TYPE), 1)
        )
    ...
    def testMakeAttrShape(self):
        for s in ([], None, [1, 2, 3], [None, None], [1, None, 3]):
            ...
            self.assertEqual(
                expected,
                actual,
                msg=("For shape %r, expected %r != %r actual" % (s, expected, actual)),
            )
    ...
    def testSameObjectForMultipleArguments(self):
        ...
        v = resource_variable_ops.ResourceVariable(
            initial_value=1.0, name="testSameObjectForMultipleArguments.Variable"
        )
    ...
    def testImplicitGradientsCustomGradientAndCachedVariableValue(self):
        ...
        x = resource_variable_ops.ResourceVariable(
            initial_value=3.0, name="X." + self.id()
        )
            ...
            def _call():
                ...
                def grad(dy, variables=None):  # pylint: disable=redefined-outer-name
                    with backprop.GradientTape(persistent=True) as g:
                        ...
                        y = c(x)
                    grad_vars = [
                        2 * math_ops.reduce_sum(dy * g.jacobian(y, v))
                        for v in variables
                    ]
                    del g
                    return (), grad_vars
    ...
    def testOverrideSecondOrderWithCustomGradient(self):
        ...
        self.assertAllClose(
            3.0 * 4.0 * f(c) ** 2.0 * -1.1 * -1.1 + 4.0 * f(c) ** 3.0 * -2.1,
            outer.gradient(dd, c),
        )
...
class JacobianTest(test.TestCase):
    ...
    def test_nested_batch_jacobian_foldl(self):
        ...
        def _func(x):
            return array_ops.reshape(
                functional_ops.foldl_v2(
                    lambda a, b: math_ops.cos(a + b), array_ops.transpose(x)
                ),
                [1, 1],
            )
        ...
        for _ in range(2):
            ...
            self.assertAllClose(expected_flat, array_ops.reshape(f(x), [-1]), rtol=1e-3)
            self.assertAllClose(
                expected_flat,
    ...
    def test_grad_jacobian_conv(self):
        def _inner(x):
            ...
            with backprop.GradientTape() as tape:
                tape.watch(x)
                y = nn_ops.conv2d(
                    x, kernel, strides=(1, 1), padding="SAME", data_format="NHWC"
                )
        ...
        self.assertAllClose(
            array_ops.reshape(numerical, [-1]),
            array_ops.reshape(_outer(), [-1]),
            rtol=1e-1,
        )
    ...
    def test_foldl_partial_function(self):
        ...
        self.assertAllClose(
            [1.0, 1.0, 1.0], tape.jacobian(result, x, experimental_use_pfor=True)
        )
        self.assertAllClose(
            [1.0, 1.0, 1.0], tape.jacobian(result, x, experimental_use_pfor=False)
        )
    ...
    def test_foldl_pure_function(self):
        ...
        self.assertAllClose(
            compute_jacobian(use_pfor=True), compute_jacobian(use_pfor=False)
        )

    def test_cond_func_grad_jacobian(self):
        @def_function.function
        def f(x):
            y = control_flow_ops.cond(x > 0.0, lambda: x ** 3.0, lambda: x ** 2.0)
            return y
...
class BatchJacobianTest(test.TestCase, parameterized.TestCase):
    def _batch_jacobian(self, experimental_use_pfor):
        ...
        answer = array_ops.stack(
            [array_ops.diag(2 * x[0] * y[0]), array_ops.diag(2 * x[1] * y[1])]
        )
    ...
    def test_zeros_type_correct(self, use_pfor):
        for dtype in [dtypes.float32, dtypes.float64]:
            ...
            jac = tape.batch_jacobian(
                y, x, unconnected_gradients="zero", experimental_use_pfor=use_pfor
            )
            self.assertEqual(dtype, jac.dtype)
            self.assertAllClose([[[0.0]]], jac)
...
class AggregateIndexedSlicesGradientsTest(test_util.TensorFlowTestCase):
    ...
    def testOneGradient(self):
        t = math_ops._as_indexed_slices(
            constant_op.constant([[1.0, 2.0], [0, 0], [3.0, 4.0]])
        )
    ...
    def testMultipleGradients(self):
        ...
        t1 = math_ops._as_indexed_slices(
            constant_op.constant([[0.0, 0.0], [5, 6], [7.0, 8.0]])
        )
