...
def _jvp(f, primals, tangents):
    ...
    return (
        primals_out,
        acc.jvp(primals_out, unconnected_gradients=UnconnectedGradients.ZERO),
    )
...
class ForwardpropTest(test.TestCase, parameterized.TestCase):
    ...
    def testJVPFunctionWithBatchOfTangents(self):
        ...
        jvp_flat = forwardprop._jvp_dispatch(
            ...
            tangents=(
                ...
                constant_op.constant([4.0, 5.0, 6.0]),
            ),
            use_batch=True,
        )
        ...
        self.assertAllClose(
            [constant_op.constant([1.0 + 4.0, 2.0 + 5.0, 3.0 + 6.0])], jvp_flat
        )
        ...
        jvp_flat = forwardprop._jvp_dispatch(
            ...
            outputs=mul_outputs,
            tangents=(
                constant_op.constant([[1.0], [0.0], [1.0]]),
                constant_op.constant([[0.0], [1.0], [1.0]]),
            ),
            use_batch=True,
        )
        self.assertAllClose(
            [constant_op.constant([[5.0], [4.0], [5.0 + 4.0]])], jvp_flat
        )
    ...
    def testJVPFunctionRaisesError(self):
        ...
        with self.assertRaisesRegex(ValueError, r".*was expected to be of shape*"):
            forwardprop._jvp_dispatch(
                op_name="Add",
                attr_tuple=(),
                inputs=(constant_op.constant(2.0), constant_op.constant(4.0)),
                outputs=sum_outputs,
                tangents=(
    ...
    def testExceptionCustomGradientRecomputeGradForward(self):
        ...
        with self.assertRaisesRegex(
            NotImplementedError, "recompute_grad tried to transpose"
        ):
    ...
    def testJVPPacking(self):
        ...
        with forwardprop.ForwardAccumulator(
            ...
            with forwardprop.ForwardAccumulator(primal_in, inner_jvp) as inner_acc:
                packed_input_indices, packed_input_tangents = forwardprop_util.pack_tangents(
                    [primal_in]
                )
                ...
                packed_output_indices, packed_output_tangents = forwardprop_util.pack_tangents(
                    [primal_out]
                )
...
class BatchTests(test.TestCase, parameterized.TestCase):
    @parameterized.parameters(
        [(math_ops.sin, (2, 3), 5), (math_ops.sin, (2, 3, 4), 10)]
    )
