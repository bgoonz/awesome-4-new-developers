...
class WrapFunctionTest(test.TestCase):
    ...
    def testVariableLifting(self):
        ...
        self.assertEqual(
            initializer.graph.external_captures, wrapped.graph.external_captures
        )
    ...
    def testCollectionsIsolation(self):
        ...
        self.assertIsNot(
            g_wrapped.graph.get_collection(ops.GraphKeys.LOSSES[0]),
    ...
    def testPruneOperations(self):
        ...
        f_wrapped = wrap_function.wrap_function(f, [])
        pruned = f_wrapped.prune(
            feeds=(), fetches=(f_wrapped.graph.get_operation_by_name("increment"),)
        )
        ...
        increments = f_wrapped.prune(
            ...
            fetches=(
                f_wrapped.graph.get_operation_by_name("increment"),
                f_wrapped.graph.get_tensor_by_name("other:0"),
            ),
        )
        first_output, second_output = increments(constant_op.constant(2))
        self.assertEqual(
            ["step:0", "increment/resource:0"], [t.name for t in increments.inputs]
        )
    ...
    def test_function_from_graph_def(self):
        ...
        revived_function = wrap_function.function_from_graph_def(
            graph_def,
            inputs=original_func_graph.inputs[0].name,
    ...
    def test_create_variables_with_same_name(self):
        ...
        self.assertDictEqual(
            ...
            {v.name: v.numpy() for v in f_wrapped._variable_holder.variables.values()},
        )
...
class WrappedGraphTest(test.TestCase):
    ...
    def testImportedFunctionsRegistered(self):
        ...
        with ops.Graph().as_default() as graph:
            ...
            ds = dataset_ops.from_variant(
                x, structure=(tensor_spec.TensorSpec([], dtypes.int32))
            )
