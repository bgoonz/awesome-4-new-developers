...
class LogicalDeviceConfiguration(
    collections.namedtuple(
        "LogicalDeviceConfiguration", ["memory_limit", "experimental_priority"]
    )
):
  ...
  """
    ...
    def __new__(cls, memory_limit=None, experimental_priority=None):
        return super(LogicalDeviceConfiguration, cls).__new__(
            cls, memory_limit, experimental_priority
        )
...
class Context(object):
    ...
    def ensure_initialized(self):
        ...
        with self._initialize_lock:
            ...
            try:
                ...
                if (
                    self._use_tfrt is not None
                    and self._use_tfrt_distributed_runtime is not None
                ):
        ...
        for dev in self.list_physical_devices("GPU"):
            ...
            if self._virtual_device_map:
                ...
                if priority and len(device_limits) != len(priority):
                    raise ValueError(
                        "priority must be specified for all virtual devices"
                    )
    ...
    def register_custom_device(self, device_capsule, device_name, device_info_capsule):
    ...
    """
        ...
        if "compute_capability" in details:
            try:
                ...
                raise RuntimeError(
                    "Device returned compute capability an in invalid "
    ...
    def get_compiler_ir(self, device_name, function_name, args, stage="hlo"):
        return pywrap_tfe.TF_GetCompilerIr(
            self._context_handle, function_name, stage, device_name, args
        )
