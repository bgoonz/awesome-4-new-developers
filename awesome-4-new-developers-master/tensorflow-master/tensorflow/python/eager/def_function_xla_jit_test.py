...
class DefFunctionTest(xla_test.XLATestCase):
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/155782411): MLIR bridge does not" "support stack traces"
    )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/181176476): Wrong stack trace for " "failed legalization in MLIR bridge"
    )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/162521846): MLIR bridge fails" " msan, function library not found"
    )
    ...
    def testNestedWhileLoopWithUnmodifiedCarriedShape(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            def g(x):
                ...
                return control_flow_ops.while_loop_v2(
                    lambda *_: True, outer, (x, shp), maximum_iterations=4
                )[0]
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            def g(x):
                ...
                x = control_flow_ops.while_loop_v2(
                    lambda *_: True, outer, (x, shp), maximum_iterations=5
                )[0]
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/162281863): MLIR bridge errors out" " lowering TensorListConcatV2"
    )
    ...
    def testUpdateVariableMemoryUsage(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            initial_usage = (
                context.context().get_memory_info(v.device)["current"] if on_gpu else 0
            )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/162381930): MLIR bridge renames " " functions"
    )
    ...
    def testReturnIdentity(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            on_gpu = "gpu" in self.device.lower()
            initial_usage = (
                context.context().get_memory_info(b.backing_device)["current"]
                if on_gpu
                else 0
            )
    ...
    def testGetCompilerIrConstants(self):
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            self.assertIn(
                "{1,2,0}", f.experimental_get_compiler_ir(a, b)(stage="optimized_hlo")
            )

    @test_util.disable_mlir_bridge(
        "TODO(b/168732524): MLIR bridge does not "
    ...
    def testGetCompilerIrResourceVars(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            self.assertIn(
                "input_output_alias={ {}: (2, {}, may-alias) }",
    ...
    def testGetCompilerIrKwargs(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            self.assertIn(
                "multiply", f.experimental_get_compiler_ir(b=a, a=b)(stage="hlo")
            )
    ...
    def testGetCompilerIrDot(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            self.assertIn(
                "label", f.experimental_get_compiler_ir(a, b)(stage="optimized_hlo_dot")
            )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/172845417): MLIR bridge does not "
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            def f(x):
                return array_ops.reshape(
                    x, [math_ops.cast(a, dtypes.int32), math_ops.cast(b, dtypes.int32)]
                )
    ...
    def testGetConstantOutOfResourceVariableAfterWrite(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "concrete values at compile time"
            ):
    ...
    def testTensorArrayErrorMessage(self):
        with ops.device("device:{}:0".format(self.device)):
            ...
            if test_util.is_mlir_bridge_enabled():
                with self.assertRaisesRegex(
                    errors.InvalidArgumentError, "EXPECTED_MESSAGE_NEW"
                ):
                    f()
            else:
                with self.assertRaisesRegex(
                    errors.InvalidArgumentError, "EXPECTED_MESSAGE_OLD"
                ):
                    f()
    ...
    def testCounter(self):
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            f(constant_op.constant(1.0))  # Calling again does not increment
            self.assertEqual(cell_nojit.value(), orig_nojit + 1)
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/162272821): MLIR bridge returns " " wrong status type"
    )
    def testResourceWrongDevice(self):
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "def_function_xla_jit_test.py"
            ):
                update_var(arg)
            ...
            with backprop.GradientTape():
                ...
                def f(x):
                    return control_flow_ops.cond(
                        math_ops.reduce_all(x > 1), lambda: 1.0 / x, lambda: x
                    )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/190444466): MLIR bridge seems to " "ignore resource assignments"
    )
    ...
    def testTfSummaryErrMsg(self):
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "defined @.+def_function_xla_jit_test"
            ):
                my_func_temp()
