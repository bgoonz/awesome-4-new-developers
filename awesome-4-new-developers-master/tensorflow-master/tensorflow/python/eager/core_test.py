...
def execute(op_name, num_outputs, inputs, attrs=None):
    return execute_lib.execute(op_name, num_outputs, inputs, attrs, context.context())
...
def current_device():
    return array_ops.identity(1.0).device
...
class TFETest(test_util.TensorFlowTestCase):
    ...
    def testContext(self):
        ...
        with ctx.device("GPU:0"):
            self.assertEqual(
                "/job:localhost/replica:0/task:0/device:GPU:0", ctx.device_name
            )
            self.assertEqual(ctx.device_name, ctx.device_spec.to_string())
            with ctx.device(None):
                ...
                with ctx.device("CPU:0"):
                    self.assertEqual(
                        "/job:localhost/replica:0/task:0/device:CPU:0", ctx.device_name
                    )
                    self.assertEqual(ctx.device_name, ctx.device_spec.to_string())
    ...
    def testContextConfig(self):
        ctx = context.Context(config=config_pb2.ConfigProto(device_count={"GPU": 0}))
    ...
    def testTensorPlacement(self):
        ...
        result = (
            execute(b"Add", 1, inputs=[x, y], attrs=("T", x.dtype.as_datatype_enum))[0]
            .cpu()
            .numpy()
        )
    ...
    def testPyFunctionNullContext(self):
        ...
        with ops.device("CPU:0"):
            test_var = variables.Variable([2.0, 3.0])
    ...
    def testPyFunctionAsync(self):
        ...
        with context.executor_scope(async_executor):
            test_var = variables.Variable(2.0)
    ...
    def testNumpyForceCPU(self):
        cpu = constant_op.constant([[1.0, 2.0], [3.0, 4.0]])
        c2g = cpu.gpu()
    ...
    def testExecuteBasic(self):
        ...
        product = execute(
            ...
            num_outputs=1,
            inputs=[three, five],
            attrs=("T", three.dtype.as_datatype_enum),
        )[0]
    ...
    @test_util.disable_tfrt(
        "TFRT asserts correct number of outputs instead of " "returning error status."
    )
    ...
    def testMatMulGPU(self):
        three = constant_op.constant([[3.0]]).gpu()
        five = constant_op.constant([[5.0]]).gpu()
        product = execute(
            b"MatMul",
            num_outputs=1,
            inputs=[three, five],
    ...
    def testExecuteStringAttr(self):
        checked_three = execute(
            ...
            attrs=("message", "just checking", "T", dtypes.float32.as_datatype_enum),
        )[0]
    ...
    def testExecuteBoolAttr(self):
        product = execute(
            b"MatMul",
            num_outputs=1,
            inputs=[constant_op.constant([[3.0]]), constant_op.constant([[5.0]])],
            attrs=(
                "transpose_a",
                True,
                "transpose_b",
    ...
    def testExecuteListFloatAttr(self):
        b = execute(
            ...
            attrs=("T", dtypes.float32.as_datatype_enum, "boundaries", [4.0, 6.0]),
        )[0]
    ...
    def testExecuteListIntAttr(self):
        b = execute(
            ...
            num_outputs=1,
            inputs=[constant_op.constant([[[3.0]]])],
            attrs=("T", dtypes.float32.as_datatype_enum, "squeeze_dims", [0, 2]),
        )[0]
    ...
    def testExecuteListTypeListShapeAttr(self):
        execute(
            ...
            attrs=(
                "component_types",
                [dtypes.float64.as_datatype_enum],
                "shapes",
                [[1, 2]],
                "capacity",
                -1,
    ...
    def testExecuteMultipleOutputs(self):
        ...
        x1, x2, x3 = execute(
            b"Split",
            num_outputs=3,
            inputs=[constant_op.constant(split_dim), constant_op.constant(value)],
    ...
    def testOperationWithNoInputsRunsOnDevice(self):
        ...
        execute(b"Add", 1, inputs=[x, y], attrs=("T", x.dtype.as_datatype_enum))
    ...
    def testConvertMixedEagerTensors(self):
        ...
        tensor = constant_op.constant(0.0, dtype=dtypes.float32)
    ...
    def testEmptyResourceReturned(self):
        ...
        self.assertEqual([0], empty_handle.shape.as_list())
...
class SendRecvTest(test_util.TensorFlowTestCase):
    ...
    def testBasic(self):
        ...
        self.assertAllEqual(self._recv(dtypes.float32, "t0", self.cpu_device), 1.0)
        self.assertAllEqual(self._recv(dtypes.float32, "t1", self.cpu_device), 2.0)
    ...
    def testLocalCrossDevice(self):
        ...
        with ops.device("GPU:0"):
            ...
            self._send(t0, "t0", self.cpu_device)
        with ops.device("cpu:0"):
            self.assertAllEqual(self._recv(dtypes.float32, "t0", gpu_device_name), 1.0)
