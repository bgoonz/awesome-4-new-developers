...
class FusedBatchNormTest(xla_test.XLATestCase, parameterized.TestCase):
    def _reference_training(
        ...
        if exponential_avg_factor != 1.0:
            mean = (
                1.0 - exponential_avg_factor
            ) * old_mean + exponential_avg_factor * mean
            corrected_var = (
                1.0 - exponential_avg_factor
    ...
    def _reference_grad(self, x, grad_y, scale, mean, var, epsilon, data_format):
        ...
        grad_x = (
            ...
            * (
                grad_y
                - np.mean(grad_y, axis=(0, 1, 2))
                - (x - mean)
                * np.mean(grad_y * (x - mean), axis=(0, 1, 2))
                / (var + epsilon)
            )
            / np.sqrt(var + epsilon)
        )
    ...
    def testInference(self, data_format):
        ...
        with self.session() as sess, self.test_scope():
            ...
            y_val, _, _ = sess.run(
                [y, mean, variance],
                {t_val: x_val_converted, scale: scale_val, offset: offset_val},
            )
