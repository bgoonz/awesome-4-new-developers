...
class CategoricalTest(xla_test.XLATestCase):
    ...
    """
        ...
        for i in range(batch_size):
            ...
            freqs = [
                (cnts[k] * 1.0 / num_samples if k in cnts else 0)
    ...
    def testCategoricalIsNotConstant(self):
        def rng(dtype, output_dtype):
            return random_ops.multinomial(
                np.array([[1.0, 1.0, 1.0]], dtype=dtype), 10, output_dtype=output_dtype
            )
    ...
    def testSamplingCorrectness(self):
        ...
        rand_probs2 = np.random.dirichlet([1.0, 4.0, 5.0], size=3)  # batched
    ...
    def testEmptyStateless(self):
        with self.session() as sess:
            with self.test_scope():
                seed_t = array_ops.placeholder(dtypes.int32, shape=[2])
                x = stateless_random_ops.stateless_multinomial(
                    array_ops.zeros([42, 40]), 0, seed=seed_t, output_dtype=dtypes.int32
                )
