...
class JitLaunchTest(test.TestCase):
    ...
    def _compare(self, fn, args, require_kernel_launch=True, name=None, noinline=None):
    ...
    def testSomeConstOutputs(self):
        ...
        def SomeConstOutputs(x):
            return (
                constant_op.constant([-2, 7]),
                array_ops.identity(x),
                constant_op.constant(3.5),
            )

        self._compare(
            SomeConstOutputs, [np.array([[1, 2, 3], [4, 5, 6]], dtype=np.float32)]
        )
...
class XlaCompilationTest(test.TestCase):
    ...
    def testReshape(self):
        ...
        with self.session(config=NoRewriteSessionConfig()) as sess:
            ...
            out = test_utils.RunWithWarmup(
                sess,
                z,
                {x: np.array([1, 2, 3, 4, 5, 6], np.float32), y: [-1, 3]},
                run_metadata=run_metadata,
                options=config_pb2.RunOptions(
                    trace_level=config_pb2.RunOptions.FULL_TRACE
                ),
    ...
    def testCond(self):
        ...
        with self.session(config=NoRewriteSessionConfig()) as session:
            ...
            result = test_utils.RunWithWarmup(
                session,
                t,
                {x: np.float32(2), y: np.float32(4), c: True},
                run_metadata=run_metadata,
                options=config_pb2.RunOptions(
                    trace_level=config_pb2.RunOptions.FULL_TRACE
                ),
    ...
    def testNestedFunction(self):
        ...
        with self.session(
            config=NoRewriteSessionConfig(), graph=g, use_gpu=True
        ) as sess:
            run_metadata = config_pb2.RunMetadata()
            val = sess.run(
                out,
                feed_dict={inp: [2.0, 10.0]},
                run_metadata=run_metadata,
                options=config_pb2.RunOptions(
                    trace_level=config_pb2.RunOptions.FULL_TRACE
                ),
            )
            self.assertAllClose(val, [20.0, 2100.0])
    ...
    def testLoopDeadlock(self):
        ...
        with self.session(config=NoRewriteSessionConfig()) as session:
            ...
            with jit_scope():
                ...
                _, _, w = control_flow_ops.while_loop(
                    c, b, (constant_op.constant(0), y, x)
                )
                u = w + y
...
class LazyCompilationTest(test.TestCase):
    def testLazyCompilation(self):
        ...
        with session_lib.Session(config=NoRewriteSessionConfig()) as sess:
            ...
            sess.run(
                y,
                feed_dict={x: [2.0, 10.0, 19.0, 77.0, 100.0]},
            ...
            self.assertTrue(
                InLabels(RunMetadataLabels(run_metadata_for_first_run), "_XlaCompile")
            )
            self.assertTrue(
            ...
            self.assertTrue(
                InLabels(RunMetadataLabels(run_metadata_before_warmup), "_XlaCompile")
            )
            self.assertFalse(
            ...
            self.assertTrue(
                InLabels(RunMetadataLabels(run_metadata_for_new_shape), "_XlaCompile")
            )
            self.assertFalse(
    ...
    def testIsMegamorphic(self):
        ...
        with session_lib.Session(config=NoRewriteSessionConfig()) as sess:
            ...
            for shape in range(10, 50):
                for _ in range(0, 49):
                    sess.run(y, feed_dict={x: [0.0] * shape})
