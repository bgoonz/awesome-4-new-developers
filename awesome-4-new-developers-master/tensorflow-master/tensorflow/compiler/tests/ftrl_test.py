    ...
    def testFtrlwithoutRegularization2(self):
        for dtype in self.float_types:
            with self.session(), self.test_scope():
                ...
                self.assertAllCloseAccordingToType(
                    np.array([-0.28232238, -0.56096673]),
                    self.evaluate(var1),
                    1e-5,
                    1e-5,
                )
    ...
    def testFtrlWithL1(self):
        for dtype in self.float_types:
            with self.session(), self.test_scope():
                ...
                self.assertAllCloseAccordingToType(
                    np.array([-0.93460727, -1.86147261]), self.evaluate(var1), rtol=1e-4
                )
    ...
    def testFtrlWithL1_L2(self):
        for dtype in self.float_types:
            with self.session(), self.test_scope():
                ...
                # Validate updated params
                self.assertAllCloseAccordingToType(
                    np.array([-0.24059935, -0.46829352]), self.evaluate(var0), rtol=1e-5
                )
                self.assertAllCloseAccordingToType(
                    np.array([-0.02406147, -0.04830509]), self.evaluate(var1), rtol=1e-5
                )
    ...
    """
        for dtype in self.float_types:
            with self.session(), self.test_scope():
                ...
                # Validate updated params
                self.assertAllCloseAccordingToType(
                    np.array([-0.22578996, -0.44345799]), self.evaluate(var0), rtol=1e-4
                )
                self.assertAllCloseAccordingToType(
                    np.array([-0.14378493, -0.13229476]), self.evaluate(var1), rtol=1e-4
                )
