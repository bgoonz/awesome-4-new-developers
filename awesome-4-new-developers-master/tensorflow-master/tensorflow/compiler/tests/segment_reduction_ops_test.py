...
class SegmentReductionOpsTest(xla_test.XLATestCase):
    ...
    def _unsortedSegmentSum(self, data, indices, num_segments):
        return self._segmentReduction(
            math_ops.unsorted_segment_sum, data, indices, num_segments
        )

    def _unsortedSegmentProd(self, data, indices, num_segments):
        return self._segmentReduction(
            math_ops.unsorted_segment_prod, data, indices, num_segments
        )

    def _unsortedSegmentMin(self, data, indices, num_segments):
        return self._segmentReduction(
            math_ops.unsorted_segment_min, data, indices, num_segments
        )

    def _unsortedSegmentMax(self, data, indices, num_segments):
        return self._segmentReduction(
            math_ops.unsorted_segment_max, data, indices, num_segments
        )
        ...
        for dtype in self.numeric_types:
            self.assertAllClose(
                ...
                self._unsortedSegmentSum(
                    ...
                    np.array([3, 0, 2, 1, 3, 3], dtype=np.int32),
                    4,
                ),
            )
        ...
        for dtype in self.numeric_types:
            self.assertAllClose(
                ...
                self._unsortedSegmentSum(
                    ...
                    np.array([3, -1, 0, 1, 0, -1, 3], dtype=np.int32),
                    4,
                ),
            )
