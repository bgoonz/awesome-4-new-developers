...
class FFTTest(xla_test.XLATestCase):
    def _VerifyFftMethod(
        self, inner_dims, complex_to_input, input_to_expected, tf_method
    ):
    ...
    def testContribSignalSTFT(self):
        ...
        expected = sps.stft(
            data, nperseg=ws, noverlap=ws - hs, boundary=None, window=window
        )[2]
    ...
    def testRFFT2D(self):
        ...
        self._VerifyFftMethod(
            INNER_DIMS_2D,
            np.real,
            lambda x: np.fft.rfft2(x, s=[x.shape[-2], x.shape[-1]]),
            _tf_fn,
        )
    ...
    def testRFFT3DMismatchedSize(self):
        def _to_expected(x):
            return np.fft.rfftn(
                x, axes=(-3, -2, -1), s=[x.shape[-3] // 2, x.shape[-2], x.shape[-1] * 2]
            )
    ...
    def testIRFFT(self):
        ...
        self._VerifyFftMethod(
            INNER_DIMS_1D,
            lambda x: np.fft.rfft(np.real(x), n=x.shape[-1]),
            lambda x: np.fft.irfft(x, n=2 * (x.shape[-1] - 1)),
            _tf_fn,
        )
    ...
    def testIRFFT3D(self):
        def _to_input(x):
            return np.fft.rfftn(
                np.real(x), axes=(-3, -2, -1), s=[x.shape[-3], x.shape[-2], x.shape[-1]]
            )
