...
def _composite_conv_add_relu(
    input_, filter_, bias, stride_w, stride_h, dilation_w, dilation_h, padding, act
):
...
def _conv_add_relu_grad(op, grad):
    ...
    updates_grad_reshaped = tf.reduce_sum(grad, axis=reduction_axes, keepdims=True)
...
def _composite_fully_connected(input_, filter_, bias, act):
    res = tf.raw_ops.MatMul(a=input_, b=filter_, transpose_a=False, transpose_b=True)
    res = tf.raw_ops.Add(x=res, y=bias)
    if act == "RELU":
        return tf.raw_ops.Relu(features=res)
    elif act == "RELU6":
...
def _composite_max_pool(
    input_, stride_w, stride_h, filter_width, filter_height, padding
):
