...
from tensorflow.compiler.mlir.tfr.examples.mnist import (
    ops_defs,
...
class FloatModel(tf.Module):
        ...
        self.weights = {
            "f1": tf.Variable(
                tf.random.truncated_normal(
                    [5, 5, num_channels, n_hidden_1], stddev=0.1, seed=seed
                )
            ),
            "f2": tf.Variable(
                tf.random.truncated_normal(
                    [5, 5, n_hidden_1, n_hidden_2], stddev=0.1, seed=seed
                )
            ),
            "f3": tf.Variable(
                tf.random.truncated_normal(
                    [n_hidden_3, flatten_size], stddev=0.1, seed=seed
                )
            ),
            "f4": tf.Variable(
                tf.random.truncated_normal(
                    [num_classes, n_hidden_3], stddev=0.1, seed=seed
                )
            ),
        }
    ...
    def __call__(self, data):
        ...
        conv1 = gen_mnist_ops.new_conv2d(
            x, self.weights["f1"], self.biases["b1"], 1, 1, 1, 1, "SAME", "RELU"
        )
        ...
        conv2 = gen_mnist_ops.new_conv2d(
            max_pool1, self.weights["f2"], self.biases["b2"], 1, 1, 1, 1, "SAME", "RELU"
        )
        ...
        # output shape: [-1, 1024]
        fc1 = gen_mnist_ops.new_fully_connected(
            reshape, self.weights["f3"], self.biases["b3"], "RELU"
        )
        # output shape: [-1, 10]
        return gen_mnist_ops.new_fully_connected(
            fc1, self.weights["f4"], self.biases["b4"]
        )
