...
class OpDefCache(object):
    ...
    def mlir_external_funcs(self):
        ...
        for _, (op_def, derived_attrs) in sorted(self._op_defs.items()):
            ...
            attrs_no_default = [
                attr for attr in non_derived_attrs if not attr.HasField("default_value")
            ]
            attrs_with_default = [
...
AG_MODULE = api._TRANSPILER.get_extra_locals()[
    "ag__"
...
class TFRTypeResolver(type_inference.Resolver):
    ...
    def res_call(self, ns, types_ns, node, f_type, args, keywords):
        ...
        if f_type == (TFRTypes.AG_BUILTIN_FUNC,):
            ...
            return (
                {tuple(_get_type_from_proto(arg) for arg in op_def.output_arg)},
                None,
            )
...
class TFRGen(transformer.CodeGenerator):
    ...
    """
        if loc is not None and loc.loc.filename:
            ...
            return 'loc("{}":{}:{})'.format(
                file_name, loc.loc.lineno, loc.loc.col_offset
            )
        else:
            return "loc(unknown)"
    ...
    def _get_inferred_type(self, node, default=None):
        ...
        if default is not None and type_ != default:
            print(
                "WARN: type annotation {}({}) does not match {}({})".format(
    ...
    def _index_to_I64(self, value, ty):
        if ty == TFRTypes.INDEX:
            ...
            self._emit_with_loc(
                "\n{} = index_cast {} : index to i64".format(casted, value)
            )
    ...
    def _i64_to_index(self, value, ty):
        if ty == TFRTypes.I64:
            casted = self._ssa_name("casted")
            self._emit_with_loc(
                "\n{} = index_cast {} : i64 to index".format(casted, value)
            )
            ...
            if len(targets) == len(values):
                ...
                if ty == TFRTypes.TENSOR_LIST:
                    ...
                    for idx, key in enumerate(targets):
                        ...
                        self._emit_with_loc(
                            " : (!tfr.tensor_list, index) -> !tfr.tensor", node
                        )
    ...
    def visit_Call(self, node):
        ...
        if func_type == TFRTypes.AG_BUILTIN_FUNC:
            if func_name == "if_stmt":
                ...
                return self._visit_if_stmt(
                    cond, body, orelse, get_state, out_symbols, node
                )
        ...
        if func_type == TFRTypes.PY_BUILTIN_FUNC:
            if func_name == "len":
                ...
                if ty == TFRTypes.TF_TENSOR_SHAPE_LIST:
                    ...
                    self._emit_with_loc(
                        "\n{} = tfr.get_length {} -> index".format(size_value, arg),
                        node,
                    )
                return (size_value, TFRTypes.INDEX)

        raise NotImplementedError(
            "call operator not recognized: {} {}".format(func_name, func_type)
        )
    ...
    def visit_FunctionDef(self, node):
        ...
        self.emit(
            "{}) -> ({}) {{".format(", ".join(arg_list), ", ".join(ret_type_list))
        )
    ...
    def _visit_for_stmt(self, range_, body_def, get_state, loop_carried, node):
        ...
        self.emit(
            "scf.for {} = {} to {} step {} ".format(it, range_[0], range_[1], range_[2])
        )
        if loop_carried:
            iter_args = []
    ...
    def _emit_default_constant_from_proto(self, attr_def):
        ...
        if cst_ty == TFRTypes.ATTR:
            ...
            self._emit_with_loc("\n{} = constant {} : {}".format(name, cst_val, cst_ty))
        return name, cst_ty
        ...
        for attr_def in ordered_args[len(args) :]:
            if attr_def.name in kw_args:
                ...
                if attr_def in input_args:
                    ...
                    if ty is TFRTypes.TENSOR_LIST and not _require_tensor_list(
                        attr_def
                    ):
...
def tfr_gen_from_module(
    source, method_prefix=None, op_libraries=None, op_defs=OpDefCache()
):
