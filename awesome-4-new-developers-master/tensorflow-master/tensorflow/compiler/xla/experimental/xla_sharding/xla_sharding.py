...
class Sharding(object):
  ...
  """
    ...
    def apply_to_tensor(
        self, tensor, assign_tuple_sharding=False, use_sharding_op=False
    ):
...
def tile(tensor, tile_assignment, assign_tuple_sharding=False, use_sharding_op=False):
...
def manual_to_auto_spmd_partition(tensor, manual_sharding, full_shape, single_dim=-1):
  ...
  """
    return tf2xla.spmd_shard_to_full_shape(
        tensor, manual_sharding=manual_sharding, full_shape=full_shape, dim=single_dim
    )
...
def mesh_split(tensor, device_mesh, tensor_split_dims_mapping, use_sharding_op=False):
