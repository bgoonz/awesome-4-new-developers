...
function StackShare({ name, description, stackshare }) {
  ...
  return (
    <Card description={description}>
        ...
        <div className="is-logo-link has-text-centered">
          <a href={stackshare.websiteUrl}>
            <img
              alt="Tool logo"
              className="is-logo"
              src={stackshare.imageUrl}
            ></img>
          </a>
        </div>
        <div className="has-margin-top-10 has-text-centered">
          <a
            className="has-text-info is-size-4 is-strong"
            href={stackshare.websiteUrl}
            title={stackshare.name}
            style={{
              whiteSpace: "nowrap",
              overflow: "hidden",
              display: "block",
      ...
      <div
        className="is-description has-text-centered"
        title={stackshare.description}
      >
      ...
      <div className="is-bottom has-margin-top-10">
        <div className="level is-mobile has-overflow-hidden">
          <div className="level-item has-text-left">
            <div>
              {starsMetric
                ? metricsLevelItem(
                    `star`,
                    `${stackshare.githubUrl}/stargazers`,
                    starsMetric.value
                  )
                : ``}
              {forksMetric
                ? metricsLevelItem(
                    `code-branch`,
                    `${stackshare.githubUrl}/network/members`,
                    forksMetric.value
                  )
                : ``}
              {stacksMetric
                ? metricsLevelItem(
                    `layer-group`,
                    `${stackshare.profileUrl}/in-stacks`,
                    stacksMetric.value
                  )
                : ``}
            </div>
          ...
          <div className="level-item has-text-right">
            <div>
              <div>
                <a className="tag" href={stackshare.group.url}>
                  {truncate(stackshare.group.name)}
                </a>
              </div>
              <div>
                <a className="tag" href={stackshare.category.url}>
                  {truncate(stackshare.category.name)}
                </a>
              </div>
            </div>
          </div>
  ...
  return (
    <div>
      <a className="has-text-grey" href={url}>
        ...
        <span>{shortenLargeNumber(value, 1)}</span>
      </a>
    </div>
  )
}
