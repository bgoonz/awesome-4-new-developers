...
class DiscordCommunityMetadata:
    ...
    def request(self):
        ...
        return s.get(
            f"{self.protocol}://discord.com/api/v{self.api_version}/invites/{self.invite_code}"
        )
    ...
    def parse_data(self):
        ...
        try:
            status = (
                data["code"] == self.invite_code
                or data["code"] == self.invite_code.lower()
            )
            if status:
                data_captured = {
            ...
            raise requests.exceptions.Timeout(
                f"Possible rate limit. Reply: {self.invite_code} : {data}"
            )
