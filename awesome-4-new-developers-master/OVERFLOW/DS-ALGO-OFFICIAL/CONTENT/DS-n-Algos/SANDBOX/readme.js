...
async function main() {
  const names = (await fs.readdir(path.join(__dirname, "../problems/"))).filter(
    (name) => /^\d\d\d\..*\.md$/.test(name)
  );
  ...
  const matters = await Promise.all(
    names.map(
      async (name) =>
        grayMatter(
          await fs.readFile(path.join(__dirname, "../problems/", name))
        ).data
    )
  );
...
function genDifficulty(names, matters) {
  ...
  return wrapDetails(
    ...
    ["Easy", "Medium", "Hard"]
      .map((level) =>
        wrapDetails(level, difficultyMap.get(level).join("\n"), 2)
      )
      .join("\n")
  );
}
...
function genTopicsAndQuestions(field, names, matters) {
  ...
  return wrapDetails(
    ...
    [...topicMap.keys()]
      .sort()
      .map((topic) => wrapDetails(topic, topicMap.get(topic).join("\n"), 2))
      .join("\n")
  );
}
